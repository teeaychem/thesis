...
\item\label{event-state-relation} If an agent ought not be in a state \(S\), then the agent ought not perform any action that would yield state \(S\).
\end{enumerate}
...
\item\label{event-state-relation-c} If it is not the case that an agent ought not perform all actions that would yield state \(S\), then it is not the case that the agent ought not be in state \(S\).
\end{enumerate}

Paraphrasing, if it is permissible for an agent to perform an action that would yield some state \(S\), then the agent is permitted to be in state \(S\).
...
\item\label{event-state-p2} It is not the case that the agent ought not to witness the event of judging that \(\chi\) holds on the basis of \(\Sigma\). \hfill (\ref{event-state-p1} \& \ref{goal}\(_{e}\))
\item Judging that \(\chi\) holds on the basis of \(\Sigma\) results in the agent being in the state of holding \(\chi\) on the basis of \(\Sigma\). \hfill (assumption)
...
\item It is not the case that the agent ought not be in state of holding \(\chi\) on the basis of \(\Sigma\). \hfill (\ref{event-state-relation-c})
\end{enumerate}
...

{
  \color{red}
  \begin{itemize}
  \item The problem here, and elsewhere, is that reasoning is an event, while holding is a state.
  \item And, it does not seem as though the same sense of can applies to both an event and a state.
  \item So, it seems that a single reading of can should be favoured, and furthermore, some event happens in order for the agent to be in the state of holding, and hence if the state is ruled out, then the relevant event should also be ruled out.
  \end{itemize}

  \begin{itemize}
  \item First pass is the following:
  \end{itemize}

  \begin{enumerate}[label=H\arabic*., ref=H\arabic*]
  \item\label{reason-hold-event} If an agent can reason from \(\Sigma\) to \(\chi\) then it is not the case that the agent can not judge that \(\chi\) holds on the basis of \(\Sigma\).
  \end{enumerate}

  \begin{itemize}
  \item The difficulty with \ref{reason-hold-event} is that it's hard to parse.
