...
\begin{itemize}
\item projection. controversial is that unordered.
\item shouldn't be too surprising.
\item recipe is not fully ordered.
\end{itemize}
...
\begin{itemize}
\item Fix scenario so that Morse's reasoning ensures that there's a relevant kind of public demonstration.
  \begin{itemize}
  \item That is, what it means for Morse to report amounts to there being a demonstration.
  \item This distinguishes from Lawrence.
  \end{itemize}
\item What the conclusion to be arrived at in the same way as with Morse.
\item Hence, want \(\chi = f\Sigma\) for some appropriate \(f\).
\item If Lewis' reasoning does not depend on \(f\), then Lewis may be confident, but Lewis' reasoning follows a different, and irrelevant kind of structure.
\end{itemize}

\hozlinedash

\begin{itemize}
\item Lewis needs to be able to show that \(\chi = f\Sigma\).
\item That is, Lewis needs to be able to demonstrate Woodthorpe's guilt on the basis that the case file constitutes evidence.
\item Demonstrating that there is a way to demonstrate Woodthorpe's guilt on the basis that the case file constitutes evidence is not sufficient.
\item To make things simpler, it's probably best to assume that Morse's testimony would not be sufficient to convince anyone other than Lewis.
  \begin{itemize}
  \item This is because Lewis could in principle add Morse's testimony to the case file, and then cite Morse as an expert witness.
  \item Though, this may not to too difficult.
    For, Lewis' can't claim themselves to be an expert, and Lewis and Morse are on par in this scenario.
    ({\color{red} If it helps, can think of Morse and Lewis as duplicates.})
  \end{itemize}
\item Why does this show that Lewis only makes the arrest on the basis that \(\chi = f\Sigma\)?
\item In part, because Morse' reasoning is irrelevant to the guilt of Woodthorpe.
\item The intuition is that Lewis arrests Woodthorpe on the basis of the evidence.
\item The problem is that Lewis has not reasoned from the evidence to the guilt of Woodthorpe.
\item This does not prevent Lewis from invoking the reasoning from the evidence to the guilt of Woodthorpe.
\item However, it does leave Lewis' reasoning incomplete.
\item Lewis is confident that they will be able to provide a completion.
\end{itemize}
