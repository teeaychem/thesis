...
\begin{itemize}
\item Active reading is the one of interest.
\item This is not to say that the active reading is required.
...
\item ``Brutus hugged Caesar'' is analysed with\dots
  ...
  Propositions take the role of `in' and `out', with \(\Sigma\) standing for the conjunction of propositions which are the premises of an agent's reasoning, \(\chi\) for the proposition which is the conclusion of the agent's reasoning.
  \[
...
\item Same with reasoning.
  ...
  \item Method may also be important, though for the most part this will not be of direct interest.
    \[
      \exists e(\everb{reason} \land \ein{\Sigma} \land \eout{\chi})
    \]
  \item If we want to attribute the reasoning to an agent
    \[
      \exists e(\everb{reason} \land \ein{\Sigma} \land \eout{\chi} \land \eagent{Brutus})
    \]
...
\item Brutus reasoned, at a coarse level of description, via the categorical imperative.
  \[
    \exists e(\everb{reason} \land \ein{\Sigma} \land \eout{\chi} \land \eagent{Brutus} \land \emethod{Categorical imperative})
  \]
