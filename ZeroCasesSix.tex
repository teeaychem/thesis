...
\item Hence, some event of reasoning from premises \(\Sigma\) to a conclusion \(\chi\) ensures the existence of a function defined for input \(\Sigma\) and output \(\chi\).
  \[
    ...
    \right)
  \]
\item For example, I may ask you what the median {\color{red} number of characters in a Shakespeare play?}
    \[
    \exists e
    \left(
      \begin{array}{l}
...
\item Understand function as capturing the fact.
  ...
  \item When we talk about the tickets, there are the physical tickets, but there are also the individuals who have purchased the tickets.
    ...
    (That is, the standard use mention distinction, and this is one way to understand the different uses that I'm attributing to functions.)
  \end{itemize}
\item {\color{blue}
    This is really important, and needs to be stressed.
    For, when Lewis reasons to the guilt of Woodthorpe, they focus on the abstract function, more-or-less.
    However, when Lewis promises reasoning, they're interested in providing a demonstration.
  }
\end{itemize}
...
\item ``the permissibility of the transition from p to q depends on the existence of a licence specifying that such a transition is indeed permissible''
\end{itemize}
