...
Our interest is in reasoning as a \emph{process} of drawing conclusions from premises.

`Reasoning' is a verbal noun.
...
\item It is true that Morse reasoned from the case file to the guilt of Woodthorpe happened earlier today. (Perfective aspect and past tense mix-up here.)
\end{enumerate}
...
\begin{itemize}
\item Brutus hugged Caesar.
  \[\exists e(\text{hug}(e) \land \text{agent}(e) = \text{Brutus} \land \text{patient}(e) = \text{Caesar})\]
\item Brutus did not only hug Caesar.
\item Brutus hugged and tickled Caesar.
  \[\exists e(\text{hug}(e) \land \text{tickle}(e) \land \text{agent}(e) = \text{Brutus} \land \text{patient}(e) = \text{Caesar})\]
...
\item And, not a separate event, the hugging was also a tickling, Brutus did not hug and also tickle.
  ...
  \[\exists e(\text{tickle-hug}(e) \land \text{agent}(e) = \text{Brutus} \land \text{patient}(e) = \text{Caesar})\]
\item Same with reasoning.
  \begin{itemize}
  \item Also have two roles: in and out.
  \item Often introduce a function.
  \item Method may also be important, though for the most part this will not be of direct interest.
    \[\exists e(\text{reason}(e) \land \text{in}(e) = \Sigma \land \text{out}(e) = \chi)\]
  \item If we want to attribute the reasoning to an agent
    \[\exists e(\text{reason}(e) \land \text{in}(e) = \Sigma \land \text{out}(e) = \chi \land \text{agent}(e) = \text{Brutus})\]
  \item Fill in some details\dots
    \[\Sigma = \{\exists e(\text{cry}(e) \land \text{agent}(e) = \text{Caesar})\}\]
    \[\chi = \text{Ought}(\exists e(\text{hug}(e) \land \text{agent}(e) = \text{Brutus} \land \text{patient}(e) = \text{Caesar}))\]
...
\item Brutus reasoned, at a coarse level of description, via the categorical imperative.
  \[\exists e(\text{reason}(e) \land \text{in}(e) = \Sigma \land \text{out}(e) = \chi \land \text{agent}(e) = \text{Brutus} \land \text{method}(e) = \text{Categorical imperative})\]
\item For the most part, ignore method.
\item Function, however, is useful.
  Abstracting away from the details, there's a function.
  \[\exists e(\text{reason}(e) \land \text{in}(e) = \Sigma \land \text{out}(e) = \chi \land \exists f(\text{func}(e) = f))\]
...
\[\exists e(\text{reason}(e) \land \text{in}(e) = \Sigma \land \text{out}(e) = \chi \land \text{func}(e) = \future{f})\]
...
\[\exists e(\text{reason}(e) \land \text{in}(e) = \Sigma \land \text{out}(e) = \chi \land \text{func}(e) = \underline{\future{f}})\]
...
\[\exists e(\text{reason}(e) \land \text{in}(e) = \Sigma \land \text{out}(e) = \chi \land \text{func}(e) = \future{f} \land \text{method}(e) = \text{speculate})\]

\subsection{Key inference}
\label{sec:key-inference}

\begin{note}[Name]
  Call this type of reasoning \emph{derivative}.

  There's a few ways in which this works
  \begin{itemize}
  \item Derivations, as these don't establish anything new.
  \item The reasoning doesn't do anything too important.
  \item Based on some relation that holds between the premises and the conclusion.
  \item Hence, based on some other source.
  \item The conclusion follows independently of the reasoning.
  \end{itemize}
\end{note}
