...
% \begin{note}
%   Rule \ruleCDiamondR\ can be seen as a special instance of a slightly more general rule.
%   \begin{prooftree}
%     \Axiom\(\Gamma, \phi \fCenter\ \Delta\)
%     \RightLabel{\ruleCDiamondRPP}
%     \UnaryInf\(\psi \CBox \Gamma, \psi \CDiamond \phi \fCenter\ \phi \CDiamond \Delta\)
%   \end{prooftree}
%   To obtain rule \ruleCDiamondR\ from \ruleCDiamondRPP\ take \(\psi\) to be \(\phi\), and note that \(\phi \CDiamond \phi\) is true just in case \(\top \CDiamond (\phi)\) is true.
%   And, as every \(\delta \in \Delta\) can be replaced with \(\phi \land \delta\), 
% \end{note}
