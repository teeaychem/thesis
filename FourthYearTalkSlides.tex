...
\begin{frame}
  ...
  \only<3->{
    \begin{enumerate}
    \item<2,4> It is \textcolor<2->{fuchsia}{\emph{not}} the case that the agent needs to \textcolor<2->{fuchsia}{\emph{reason via}} a means-end relation from an end they have supporting that action.
    \item<3,4> It \textcolor<3->{fuchsia}{\emph{is}} the case that the agent needs to \textcolor<3->{fuchsia}{\emph{takes there to be}} a means-end relation from an end they have supporting that action.
    \end{enumerate}
...
\begin{frame}
  ...
  \only<1-5>{
    ...
    \item \onslide<1,2>{A rational agent is permitted to settle on an action (in part) as a means \textcolor<2>{fuchsia}{only if}}
      \onslide<1,4->{
      \begin{enumerate}
      \item\tikz[na] \node[coordinate, xshift=-3em] (a2) {}; \textcolor<4>{fuchsia}{\emph{the agent takes there to be a means-end relation supporting the means and evaluates the end}}, and
      \item\tikz[na] \node[coordinate, xshift=-3em] (b2) {}; \textcolor<5>{fuchsia}{\emph{the evaluation of the end supports settling on the action as a means to the end}}.
      \end{enumerate}
      }
  ...
  \begin{tikzpicture}[overlay]
    \path[->]<4> (a1) edge [very thick, draw=fuchsia, bend right=75] (a2);
    \path[->]<5> (b1) edge [very thick, draw=fuchsia, bend right=75] (b2);
  \end{tikzpicture}
\end{frame}
...
\begin{frame}
  ...
  \onslide<3>{
    \begin{block}{Principle C: Dependence}
...
\begin{frame}[noframenumbering]
  ...
  {
    \tikz[remember picture,overlay]
    \draw (current page.south west) +(0, .5)
    node[anchor=south west,inner sep=0pt, opacity=0.5]{\def\svgwidth{1.2\linewidth}
