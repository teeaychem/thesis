...
% \begin{enumerate}
% \item A rational agent considers an action (in part) as a means only if the agent takes there to be a means-end relation from some end which supports taking the means. % This is part of the definition of a means, the only thing left is the idea that the end belongs to the agent.
...
In short, by taking the action as (in part) a means the agent takes the means to do some work in establishing the action as permissible.
This, in turn, commits the agent to the end.
...
\item A rational agent is permitted to settle on an action (in part) as a means only if the agent evaluates the action.
  ...
  \item As a means, some kind of evaluation is required, but only because the agent takes the action to be doing something.
  \end{itemize}
...
% \begin{principle}
%   Rationally settling on an action is explained by the action being evaluated both possible and worthwhile by the agent, perhaps in comparison to the same attributes to other actions.
% \end{principle}

% \begin{principle}[Settling]\label{principle:settling}
%   A rational agent settles on a action only if the agent evaluates the action as possible and no less worthwhile than any other alternative action.
% \end{principle}
