...
In addition, no account of why you hold claim \ref{chess:claim:focus-prop} is true follows from understanding the rules of chess and the game state alone.
...
Still, obtaining confidence that claim~\ref{chess:claim:focus-prop} appears straightforward.
...
\begin{note}[Example for alternative]
  \begin{itemize}
  \item White cannot prevent Black from occupying c4 on their second move \emph{and} you have the ability to demonstrate the former conjunct.
  \end{itemize}
\end{note}

Two alternative accounts.
First, anyone who has a moderate ability.
\begin{itemize}
...
\end{itemize}
Second, transformation to assertion.
...
Problem here is that this cuts out any reasoning with the agent's ability.
Perhaps these really are compelling alternatives.
However, it does seem puzzling that the agent's own ability would be sidestepped in all cases.

Obtaining the conclusion is not the goal.
...
First, cover a failure scenario.

\begin{note}[Failure scenarios]
  
\end{note}

\begin{note}[Winston and Julia]
  \begin{scenario}[Winston \& Julia]
    ...
    Julia replies ``No! But if that is so then you are able to conclude that George Orwell will be visiting us soon''.
    Winston reflects on Julia's reply \dots
  \end{scenario}

  Winston is confident that Julia reasoned from the propositions that Eric Blair is due to visit and that `Eric Blair' and `George Orwell' are the same person to the conclusion that George Orwell is due to visit.

  Still, something is amiss.
  Winston is unable to replicate Julia's reasoning.
\end{note}
...
\begin{note}[Alternative]
  ...
  The student is correct, but the reasoning is not identical.
  ...
  Allow student to make claims.
  However, mark down.
  And do so because the agent wasn't licensed.
  ...
  There is, in contrast to the factive inference, no particular \emph{problem} here.
  Rather, the agent took the risk, and it didn't work out for them.
\end{note}

Second, three success scenarios.
...
\begin{note}[Cases]
  ...
  \begin{enumerate}
  \item Shopping \(\leadsto\) positing a simple relation between agents desire and action.
  ...
  \item Winston \(\leadsto\) blocking the factive inference.
  \end{enumerate}
\end{note}
