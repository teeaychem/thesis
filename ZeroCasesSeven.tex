...
\subsection{Four Positive Cases}
...
\subsubsection{Voluntarism and Hobson's Choice}
...
\begin{note}[Link to belief]
  ...
  \begin{itemize}
  \item Belief(\(\phi\)) only if able-to-reason(\(\phi\)). [This is perhaps entailed by requirement of actual reasoning.]
  \item Able-to-reason(\(\phi\)) only if \emph{not}-able-to-reason(\(\lnot\phi\)).
  \end{itemize}
  Therefore:
  \begin{itemize}
  \item Not-able-to-reason(\(\phi\)) then no-belief(\(\phi\)).
  \end{itemize}
  These are really equivalent.
  In the cases \citeauthor{Ginet:2001aa} discusses, then, there's possibly a stronger requirement.

  If this is assumed, then \emph{doxastic} voluntarism seems difficult.
  For, it will not seem to the agent that there is an alternative.
  ...
  Hence, it's not so clear that there is voluntarism here.
  Instead, the kind of voluntarism that \textcite{Weatherson:2008uq} considers.
  \begin{quote}
