...
So, simple case with something being coloured.





E.g.\ informer worked from ability to \(\psi\), so \(\psi\), at least.

Rework of the above reasoning, attributing \(\psi\) to the informer, so to speak.

So get \(A(\phi) \rightarrow (\psi \land A(\psi)\).

Note that \(A(\phi) \rightarrow (\psi \land A(\psi))\) is still a conditional.
The agent is still required to obtain \(\psi\) from \(A(\phi)\).
And, note that this is that the agent has the ability, the informer has not made a claim about \(\psi\) following from the particular rules.
Intuitively, the informer doesn't need to make this claim, because they are focusing on general to specific with respect to agent's abilities.
This would not substantially differ from \(\psi\).
So, can keep focus on the antecedent.

And, to simply, focus on \(A(\phi) \rightarrow \psi\).
...
Problem with Moore external world reasoning, doesn't change if someone testifies.
Doesn't apply to this case, because assumption that the reasoning is sufficient.
...
In the latter, the general ability is supporting.
In the former, the application of the general ability leads to support.
...
Well, the problem roughly is that the ability is truth functionally independent.
This is clear from the contents of the antecedent and consequent.
\(A(\phi)\) does not impact whether \(\psi\).
\(A(\phi)\) does impact whether \(A(\psi)\).

If you able to do arithmetic, then \(7^{3} = 343\).

It is likely true that the agent is able to demonstrate that \(7^{3} = 343\), but that is not what the conditional states.
Rather, it states that the ability to do arithmetic is sufficient support for it being the case that \(7^{3} = 343\).

The antecedent does not contribute to the truth conditional consequent of the consequent.
Or, `truth-conditionally irrelevant'.
The conditional is true just in case \(7^{3} = 343\).

Similar to biscuit conditionals.
This part of the phenomena is recognised.

This is not reflected in the informer's claim.

\(A(\phi) \rightarrow A(\psi)\) is not true just in case \(A(\psi)\) is true.
It is true when both are false, in particular.
Reformulate with the presumed failure of the antecedent ability.

If you had the ability to do arithmetic, you would have had the ability to show that \(7^{3} = 343\).

This seems fine after the student complains about a test.

By contrast:

If you had the ability to do arithmetic, it would have been the case that \(7^{3} = 343\).

Remains true, but because \(7^{3} = 343\).

This is an important part of the informer's claim, and the resulting position of the agent.
...
So, the informers claim speaks of the application of a general ability.
The alternative highlights a relation.

The application of the general ability requires the agent to be confident of some fact.
However, the reason for this is the result of proposed application, not the presence of ability.

For, if the informer had appealed to the general ability, the agent would have reason (independent of ability) to hold the fact.
However, by requiring the application of ability, this option is withheld from the agent.

{
  \color{red}
  Hence, \(A(\phi) \rightarrow \psi\) doesn't make sense as a genuine conditional.
  So, very hard to see this working.

  Alternative is perhaps equally repelling.
  For, the agent is in a similar state.
  If the agent has the ability, then \(\psi\) holds independently.
  Still, there is a straightforward explanation as to why granting that the agent has the ability.
  Grant the agent too little than too much.
}

{
  \color{blue}
  So, if the agent tries and fails, the agent would reduce confidence that \(\psi\) is the case.
  Contrast, \(A(\phi) \rightarrow \psi\) does not require the same dynamic.
  May note that this is provided by the additional conjunct of \(A(\psi)\), earlier simplified away.
  But, once added, forces tensions.
  For now if antecedent confidence supported \(\psi\), and did not depend on witness, 
}
...
This is so wildly different I have no idea what to say.

The informer need not think this.
...
However, this faces the same problem.
