...
\begin{note}
  ...
  Hum.
  So, not able to show that \(\lnot\psi\).
  This step is fine, because it only relies on ability?
  However, by later argument, this doesn't establish \(\lnot\lnot\psi\).

  So, the \(\lnot\psi\) stuff is based, primarily, on potential.
  Hum, maybe due to the idea that with inquiry, the absence of reasons is enough to support suspension.
  Basically, I need this to hold without taking a stance on whether or not the agent has support for \(\psi\).
  This feels as though it's somewhat plausible.

  So, skepticism.
  Here, the skeptical scenario is always there.
  Hence, can't commit to the existence of the real world.
  But this isn't right.

  Difference is that in the skeptical case, there's no stability.
  While, in the cases I'm considering, it always ends up that \(\psi\).

  This isn't quite reflection either.
  For, the core principle here is something like dominance.
  The problem is that there's no path where \(\lnot\psi\) is preserved.
  One could potentially get reflection from this, but the principle is different, and doesn't require the same assumptions.
...
\begin{note}[Dynamics?]
  Attempting to require confidence that \(\psi\) by the agent's ability to demonstrate \(\psi\).
  Primarily from the idea that the agent won't be able to maintain \(\lnot\psi\).

  Roughly:
  \begin{itemize}
  \item If confident that \(\lnot\psi < \psi\) then confident that \(\psi\).
  \end{itemize}
  If confident that any process that shows \(\lnot\psi\) will eventually lead to \(\psi\), then be confident that \(\psi\).

  So, the antecedent here:
  \begin{itemize}
  \item If confident that \(\lnot\psi < \psi\) then confident that \(< \psi\).
  \end{itemize}

  This follows, but might not be too interesting.

  \begin{itemize}
  \item If confident that \(\psi\) then confident that \(< \psi\).
  \end{itemize}
  Seems more-or-less equivalent to:
  \begin{itemize}
  \item If confident that \(\psi\) then confident that \(\lnot\psi < \psi\).
  \end{itemize}

  \begin{itemize}
  \item If agent is confident that \(\psi\), then confident that \(\lnot\psi < \psi\).
  \end{itemize}
  However, this is somewhat weaker, as it leaves open the possibility of suspension, etc.
\end{note}
...
\begin{note}[Different with support for antecedent]
