...
\item Introduction
  \begin{itemize}
  \item Position arguing against.
    \begin{itemize}
    \item An agent may appeal to reasons in support of a conclusion only if the agent
used those reasons in some reasoning that yielded the conclusion.
    \end{itemize}
  \item Quick (plausible) argument for position arguing against
    \begin{enumerate}
    \item If conditional is false, then it is possible there are cases in which an agent is unaware of how the conclusion is supported by favoured reasons.
    \item If agent is unaware of how a conclusion is supported by reasons, then conclusion may not be supported by those reasons.
    \item If conclusion may not be supported, then the conclusion is not supported (from the agent's perspective).
    \end{enumerate}
  \item I primarily take issue with the second conditional.
  \item Short overview of ability claims, and a brief sketch of the main problem (to be added after fully working through the details of the main problem).
    Ability may provide sufficient guarantee that a conclusion is supported by reasons.
  \item To motivate the broader picture, and establish some connexions to the literature, link to the \citeauthor{Davidson:2001aa}ian observation/problem of there being many possible explanation/reasons for why an agent did something.
    For \citeauthor{Davidson:2001aa} this presents a problem for the theoretician, as we need to figure out the actual explanation.
    Reinterpreted, the agent observes that they have the ability to make various potential explanations/reason actual.
  \end{itemize}
\end{itemize}

\begin{itemize}
\item The following section works through the main problem in detail.
\item In broad outline, is how it is possible for an agent to be confident that certain kind of ability claims are true.
  \begin{itemize}
  \item In some cases, an agent is provided information that they have the ability to do something.
  \item Further, in some cases the details of the ability claim allow the agent to perform a `factive inference' --- the agent  has the ability to show that \(\phi\) is the case only if it is (independently) a fact that \(\phi\).
    \begin{itemize}
    \item I may need to rework this terminology, but the idea seems ok.
      If I have the ability to demonstrate that the butler watered the plants, then it must be the case that the butler did in fact water the plants.
      (On reflection, this might be narrowed to entailments from `demonstrate' rather than `demonstrate that \(\phi\)'.)
      In contrast, a factive inference cannot be made from the ability to have \$10 --- one might need to fill out a questionnaire first, etc.
    \end{itemize}
  \item The reasoning with the factive inference is simple:
    \begin{enumerate}
    \item I have the ability to (demonstrate, etc.\ that) \(\phi\)
    \item I only have the ability to \(\phi\) if it is (independently) a fact that \(\phi\)
    \item It is a fact that \(\phi\).
    \end{enumerate}
  \item However, this reasoning does not provide support for it being a fact that \(\phi\).
  \item For, the relevant fact is a necessary condition for possessing the ability, hence drawing out this necessary condition shows only that the agent is already committed to it being a fact that \(\phi\) by endorsing their ability to \(\phi\).
  \end{itemize}
\item So, given an ability claim that allows a factive inference, it seems an agent must be at least as confident that the fact holds as they are confident that they are able to demonstrate the fact.
\item As the fact needs to be the case in order for the agent to have the ability, that the agent has the ability does not provide support for the fact.
\item Hence, it seems the agent needs to have some independent support for the fact independent of their ability.
\item In many cases the agent will have some independent support for the fact.
  \begin{itemize}
  \item For example, I have the ability to demonstrate that DeMorgan's laws hold in propositional logic, but I have independent support that these laws holds from textbooks, tutors, my own previous proofs, etc.
  \end{itemize}
\item However, if there are cases in which an agent is \emph{only} informed of an ability claim, and it is reasonable for the agent to be confident that they have the ability, then there is a significant problem.
\item For, then we have a case in which the agent is confident that they have an ability, and thus requires some fact to be the case.
  However, the agent has no independent support for the fact, and the agent's confidence that they have the ability does not appropriately support the fact.
\item In these cases, I hope to claim, the agent cites those reasoning/that reasoning that they are able to perform.
\item For, the reasoning that the agent is able to do would provide sufficient support for the fact.
\item And, the circular reasoning is avoided because the agent does not need to assume the fact in order to do the reasoning.
\item In other words, the reasoning that the agent is able to do is the kind of independent support for the fact that is desired, with the problem that for the moment the reasoning is (merely) potential.
\item The agent's ability is therefore seen as a kind of placeholder, as the agent is confident they have the ability, they are confident that the fact obtains, but on the basis of reasons/reasoning that are (for the moment) potential.
\item Hence, this is a trade of between:
  \begin{enumerate}
  \item Preventing an agent for being confident that they have the ability to demonstrate something unless the agent has independent support for a fact.
  \item Allowing an agent to be confident that some fact obtains because to their confidence that they are able to demonstrate the fact.
  \item Allowing an agent to be confident that some fact obtains because their ability allows them to witness potential reasons which independently support the fact.
  \end{enumerate}
\end{itemize}

\begin{itemize}
\item So, if I can provide cases where the agent is only provided with the information that they have the ability to demonstrate something, then I am in a position to argue that the confidence of ability grants (independent) support of relevant fact by not yet witnessed reasoning/reasons.
\item And, given this idea is necessary in some cases, it is then easier to push for as a preferred explanation in cases where alternative explanations are available.
\item However, arguing for the existence of cases in which the agent is only provided with information that they have the ability is somewhat difficult.
\end{itemize}

\begin{itemize}
\item To simplify things, the argument for the existence of cases in which the agent is only provided with information that they have the ability fixes a particular case in which an informer informs the agent that they have the ability to demonstrate something, and is then broken down into two steps.
  \begin{enumerate}
  \item Argue that the informer does not provide the agent with obtaining independent support for the fact.
  \item Argue that the agent does not obtain (sufficient) support for the fact by the ability claim made by the informer.
  \end{enumerate}
\item The second step is a little more straightforward than the first.
  \begin{itemize}
  \item Roughly, the case is constructed so that the agent needs to (in part) endorse the ability claim made by the informer.
  \item For, example, the informer claims that if the agent has a basic grasp of the rules of chess, then the agent is able to demonstrate the existence of a particular strategy.
  \item Here, the informer may be confident of the claim, but the agent is an authority with regard to their grasp of the rules of chess, and therefore, as the ability claim entails the existence of some strategy, the agent endorsing their ability amounts to the agent assuming the existence of the particular strategy.
  \end{itemize}
\item The first step takes more work.
  \begin{itemize}
  \item There are a few ideas here.
  \item First, it seems as though the informer may want to only provide the agent with the ability claim.
    \begin{itemize}
    \item For example, as an instructor I would like to tell a student that they have the ability to show something without giving the student independent reason to think that the fact is true.
    \item However, here the student may reason that I may only provide them with the ability claim if I have some support for the ability, and therefore, as I am an authority and must also believe the fact, this is in turn sufficient evidence for the fact independent of their ability.
    \item So the argument from desirability is somewhat inconclusive.
    \end{itemize}
  \item Second, if the informer is required to provide (the option) of independent support, then the agent will not be in a position where they attempt to provide the demonstration, but end up in a position in which they are unsure whether they lack the ability or whether the fact does not obtain.
    For, if the informer provides some support, then the support provided by the agent will always tip slightly in favour of the agent lacking the ability.
    Hence, for the agent to be unsure, the support the agent has (ignoring the information from the informer) must tilt toward the claim being false.
    The hope is to argue that there are cases in which this is implausible.
  \end{itemize}
\end{itemize}

\begin{itemize}
\item The remainder of the paper then applies the core idea to a handful of interesting cases (most of which we've discussed before), and considers some objections.
\item However, it may be that these end up being moved to a second paper.
\end{itemize}
...
\begin{note}[Not \emph{just} the inference]
...
No surprise, factive inference so no implicature.
...
\item\label{supp:arg:supp} Hence, must be provided with support.
\end{enumerate}
...
However, present both conjuncts as true.
...
Straightforward to illustrate with what \citeauthor{Graham:2006vs} terms the `Liberal Fundamentalist' view of testimony.
...
\end{quote}
For \citeauthor{Owens:2006tw}, \emph{expressing} a belief `differs both from indicating to others than you have it and from giving them an assurance that it is true' --- expression is characterised by the option for the audience to `belief with the same justificational status'.
...
So, though for present purposes `claiming' is an artificial construction, it seems plausible that a robust account of how the agent acquires support for the existence of strategy is available, if the informer claims that a strategy exists.
...
Hence, the infelicity of \label{i:claim:exist}.
What matters is whether the agent obtains support \emph{from} the informer.
...
\begin{itemize}
\item\label{i:claim:no-supp} No, I was careful to not provide you with support for the existence of a strategy for Black, I claimed (and gave option of supporting) only that you have the ability to demonstrate (and hence independently support) the existence of a strategy.
\end{itemize}

If~\ref{i:claim:no-supp} is successful, then the informer does not provide the agent with (the option of) support for the existence of a strategy.
...
In order for \ref{i:claim:no-supp} to be successful the following must hold:

\begin{enumerate}
\item\label{keyPoint:poss-no-supp} It is possible to claiming an agent has the ability to demonstrate \(\phi\), and require \(\phi\) to be true, without providing the option of independent support for \(\phi\).
\end{enumerate}

Point~\ref{keyPoint:poss-no-supp} primarily concerns the informer.
The agent is involved, but only in the sense that the claim is made to the agent.
No problem with the informer making this claim about the agent to a third party --- it's whether making the claim to the agent the informer brings requires providing support.
Hence, if \ref{keyPoint:poss-no-supp} holds, there remains the possibility of the agent obtaining support for the fact via the ability claim itself.
This possibility will be considered in the following section (\S\ref{sec:agent-in-part}).

So, the goal is to establish \ref{keyPoint:poss-no-supp} by outlining the construction of scenarios in which the informer \emph{only} communicates to the agent their ability to \(\phi\).
