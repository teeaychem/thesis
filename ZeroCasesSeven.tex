...
\begin{enumerate}
\item\label{keyPoint:poss-no-supp} It is possible to claiming an agent has the ability to demonstrate \(\phi\), and require \(\phi\) to be true, without providing independent support for \(\phi\).
\end{enumerate}
...
In turn, scenarios in which \ref{i:claim:no-supp} would be successful in response to presumed support for \(\phi\) from the informer.

I assume default position is that a communicator communicates no more information than required --- anything else must be extracted from that message.\nolinebreak
\footnote{
...
So, the informer does not give enough information for the agent to have the option of obtaining support for the fact.
In part, because the informer does not have the option of sharing their support for the fact with the agent.
...
Less clear that support satisfies doxastic justification, given third clause.

This is tricky.
May be the case that agent only has propositional justification is they have the ability.
And, recognition of ability is sufficient for fixing the appropriate link.
Hence, no recognition without thereby acquiring doxastic justification.
...
Agent grasps the rules of the proof system, but has the tendency to make mistakes.
The problem here is not simply that the agent makes the mistakes, but also overlooks the mistakes when checking.
So, the agent only has a chance of proving the theorem.

Here, Goldman is interesting.
It seems the agent must have some kind of ability.

Here the case is read differently.
As the agent only has a \emph{chance} of proving the theorem, and therefore following Goldman the agent does not have propositional justification.

Puzzle here is whether it is possible to get to doxastic justification for propositional justification without thereby obtaining doxastic justification.



This does not push against the position in general --- only an attempt to apply the position to ensure the agent has the option of obtaining support.
