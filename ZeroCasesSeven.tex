...
\begin{note}[Big issue]
  The relation between an agent's reasons and explanations of the agent's action.
\end{note}
...
And, in particular, the rules of chess and the game state does not immediately justify the relevant proposition.
...
{
  \color{red}
  Intermediate steps of the kind sketched are required to justify.
}
...
\begin{figure}[h]
  ...
  \begin{subfigure}{.4\textwidth}
    ...
    \end{adjustbox}
    \caption{Fields White cannot prevent Black from occupying on Black's second move}
...
Still, even though you would be able to demonstrate claim~\ref{chess:claim:2} without my testimony, my claim remains important in order for you to be confident that you have the ability.
Even if you are confident that you understand the rules of chess and the game board, you would not be able to establish that White cannot prevent Black from occupying c4 on their second move without demonstrating a strategy for Black.
...
Colloquially, you may cite the reasoning that you are able to do (or the strategy that would result from the reasoning) as support for holding that claim~\ref{chess:claim:2} is true, and cite my testimony that claim~\ref{chess:claim:1} is true in support of your ability to do the reasoning.\nolinebreak
\footnote{
  ...
  So, alternative paths is not unique to ability attributions.
}
...

{
  \begin{note}[Important]
    What I need to make clear is that the reasoning the agent is able to do isn't the same as preservation of a designated value.
    This is more-or-less implicit in what I have written, but it must be explicit.
...
So, two different kinds of explanation.
One depends on the other.
The first type of explanation is a guarantee that the agent is able to generate the second kind of explanation.

The second type of explanation is, from the agent's point of view, why the agent holds the attitude.
It is the second type of explanation which verifies, vindicates, or supports the conclusion.
And, ability some same for attitude.
...
\begin{note}[Main issue]
  ...
  Interested from the agent's point of view, and we can't move so quickly.
  The reasoning explains how the agent came to hold the conclusion.
  The reasoning does not necessarily explain why the agent came to hold the conclusion.
...
\begin{note}[Achilles and the Tortoise]
  ...
  \end{itemize}
  The Tortoise may object to a step in this reasoning, but it is unclear whether they would.
  Achilles insists on convincing the Tortoise that Z must be true when A and B are true without working through the relation between the contents of A and B and the contents of Z.
  So, it is unclear whether the Tortoise does not accept that the two sides of the Triangle are equal to each other, or whether the Tortoise is seeking an explanation of why the two sides of the Triangle are equal to each other that follows from the contents of A, B, and Z, in contrast to the supposed preservation of truth.
