...
\begin{enumerate}
\item\label{cond:claim-to-opt-supp} If an informer claims that \(\phi\), then the agent has the option of obtaining support for \(\phi\) from the claim.
\end{enumerate}
...
And so the agent has the option.

For example, \citeauthor{Graham:2006vs} terms the `Liberal Fundamentalist' view of testimony.
...
Allowing presentation as-true to include requiring that true, then, option of comprehending, and therefore option of obtaining support.

Another option is \textcite{Owens:2006tw}.
...
If~\ref{i:claim:no-supp} is successful, then the agent as the agent did not provide the agent with support for the existence of a strategy, then \dots ???
...
\begin{itemize}
\item Possible to require \(\phi\) to be true without providing support for \(\phi\) independently of the ability claim.
\end{itemize}
