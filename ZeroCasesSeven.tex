...
\begin{note}[Achilles and the Tortoise]
  ...
  \item And, as both \(s_{1}\) and \(s_{2}\) are equal to \(s_{e}\), we see by A that \(s_{1}\) and \(s_{2}\) must be equal to each other.
  \item Therefore Z is true.
  \end{itemize}
  Perhaps the Tortoise would object to a step in this reasoning, but Achilles insists on convincing the Tortoise that Z must be true when A and B are true without touching on the relation between the contents of A and B and the contents of Z.

  There is no need for the Tortoise to accept C as brute, as these principles only of interest when the propositions they govern are interpreted. (or something.)
\end{note}
