...
\end{enumerate}
{
  ...
  \begin{enumerate}
  \item If you are confident that you have a good grasp on the rules of chess, then you may be confident that you can reason from the the game state (described in figure~\ref{fig:chess:board}) to the proposition that White cannot prevent Black from occupying c4 on their (Black's) second move.
  \end{enumerate}
}
...
\begin{note}[Moving to the argument]
  ...
  Parallel to, e.g.\ belief.
  Often this provides the option of support.
  However, not always, and if you're \emph{only} given a belief then you can focus on this.

  \emph{The Vertiable Principle}.
  Go for the minimum, basically.

  Or, better, some refusal to do additional work.
  Hence, break down with semantics and pragmatics.
  Semantics is required, pragmatics is optional, in a sense.
  Various things are available, but the agent isn't required to do the work.

  The relevance with respect to ability claims, is that even if the informer makes available some extra information, the agent won't need to pick up on it if they are able to adequately process the ability claim itself.

  Continue to use the chess example to illustrate.
\end{note}
...
This turns on the informers claim being a conditional.
This relates two/three propositions.

Nothing to special about noting that the informer's confidence about the relations that hold between these propositions is (largely) independent of the informer's confidence in any of the propositions.

So, the informer's claim is understood to be in terms of the relations between these propositions.
...
Problem with this type of argument is that it's really difficult.
...
The key here is that the claim is a pair of conditionals.

Clear that \(A \rightarrow B\) and \(B \rightarrow C\) without holding \(A\) nor \(C\) (nor \(B\)).
But this isn't interesting.
If the informer holds that \(A\) is false, then \(B\) may be arbitrary.

So, possible, but uninteresting.
Informer could claim that the agent doesn't have the ability.


The key here is confidence, and the structure of the agent's claim.
...
In the following section it will be argued that the agent doesn't get support via the ability statement.

\begin{itemize}
\item As highlighted, focus is on what is communicated.
\item This was sketched above, and developed in detail here.
\item Ease into this.
\item Start with opposing intuition.
\item This helps motivate why this is an important point to consider.
\item Point out some problems.
\item Motivate the restricted piece of communication.
\item Show how this works.
\end{itemize}

So, look.
In order to clarify the relevant issue, start by motivating the opposition.
Two purposes.
First, helps clarify the relevant issue.
Second, helps clarify where the distinction is.

Then move to the main argument.
And, some examples.

Stress again that presenting a strong argument is for a relatively weak claim.
The weak claim is that there are some (interesting) cases.
The strong argument is that there's no way for the agent to obtain support.
...
Perhaps semantics part and witness part.
The semantics establishes that this goes via ability.
Then, argument that the ability is there by the agent.
Then, witness.

So, technical, then scenarios.
...
\begin{enumerate}
\item\label{i:claim:exist} No, I was careful to not commit myself to the existence of a strategy for Black, I simply claimed with confidence that you have the ability to demonstrate the existence of a strategy, so long as you have some basic grasp on the rules of chess.
\end{enumerate}
...
So the informer goes on to add.

\begin{enumerate}
\item I have provided you with two pieces of information.
  \begin{enumerate}
  \item That there is some particular ability such confident that witnessing this ability will yield a demonstration of this strategy. And,
...
\begin{enumerate}
\item If you are confident that you are able to \(\phi\), then you may be confident that you are able to \(\psi\), and if you are able to \(\psi\), then I am confident that this allows you to establish \(\chi\).
\end{enumerate}

It may be that the original claim is ambiguous, but this is fairly straightforward.
...
\begin{note}[The factive content of the claim]
...
% \begin{tikzpicture}
%   \draw[pattern=north east lines, path fading=logLBRW] (0,0) rectangle (4,4);
%   \draw[pattern=north west lines, path fading=logLBRW] (2,2) rectangle (3,3);
% \end{tikzpicture}


% \begin{tikzpicture}[scale=.5]
%   \draw[path fading=myfading] (0,0)rectangle(10,10);
% \end{tikzpicture}
