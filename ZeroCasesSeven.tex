...
Argue against:
\emph{An agent may appeal to reasons in support of a conclusion only if the agent used those reasons in some reasoning that yielded the conclusion.}

Contrapositive seems like a platitude.
...
If false, then the agent may be unaware of how the conclusion is supported by those reasons.
If agent is unaware, then conclusion may not be supported by those reasons.
If conclusion may not be supported, then the conclusion is not supported.

So, the agent may do as they please, but they will fail to establish support.

Kind of \(\Diamond \lnot p \rightarrow \lnot p\), which is like \(p \rightarrow \Box p\).

\begin{quote}
  Necessarily, if S \(\phi\)'s because p then S knows that p
\end{quote}

This is perhaps a little strong, but something like this from the agent's viewpoint.
