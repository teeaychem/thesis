...
\begin{note}[Problem, again]
  ...
  If able to see animals that look like zebras, then able to see that they are not cleverly disguised mules.
  Able to see animals that look like zebras.
  Able to see that they are not cleverly disguised mules.
  Animals in the pen are not cleverly disguised mules.

  The key parallel is look like -> see.
  In the first case, this happens via an unstated premise.
  In the second, this is supplied.

  The problem is that there is a lift from `looks like' to `is'.
  In the ability case, this follows from general to specific ability.

  Intuitive problem with the former is that if the animals in the pen are cleverly disguised mules, then the agent doesn't go from appearance to seeing.
  Though, it may be the case that the agent has ground for looks like --- the mules are cleverly disguised.

  In extreme cases, the relevant failure is significant.
  Appearance of hands.
  Have hands.
  External world.

  Then it seems the conditional is not viable.

  Bracket these cases.
  Assume that some support flows from the antecedent to consequent.
  Key is the shift from non-factive to factive.

  Highlight a second difference between the first two examples.
  In the first, there's nothing the agent could be doing.
  In the second, the agent needs only witness their ability.

  It may be that the agent does in fact see zebras.
  It may be that the agent knows.

  In the second, it is clear the agent has not done the reasoning.
  ...
  With this, provide a more thoroughgoing sketch.
\end{note}

\begin{note}[Two different reasoning patterns]
  Difference between reasoning by witnessing ability and detaching from ability.
...
\begin{note}[Through sketch, details]
  ...
  We can simplify a little by distinguishing two different kinds of support.
  \begin{enumerate}
  \item Factive.
  \item Non-factive.
  \end{enumerate}
  The agent's specific ability to demonstrate the particular strategy is factive.
  The agent only has this ability if the strategy exists.
  Therefore, support for the strategy is sufficient.

  The agent's general ability to reason with the rules of chess is non-factive with respect to the existence of the strategy.
  At least, from the agent's point of view.
  For, possible for the agent to have the general ability and not the specific ability.
  ...
  In this sense, the agent can be confident in the general ability, without also being confident that \(\psi\).
  Whereas, the agent cannot be confident in ability to \(\psi\) without being confident that \(\psi\).

  Hence, mixing two different kinds of support.
  The link between abilities is non-factive.
  The link from ability to fact is factive.

  Intuitively, then, the non-factive doesn't do enough to license the factive.
  \(\psi\) is the conclusion of the agent's reasoning.
  Limited by confidence.
  However, the principle that is employed is distinct from confidence.
  The specific ability involves ruling out.

  The factive support is not confidence based.
  And, this support isn't polymorphic.
  Failure of transmission.
  Because,

  So, \(A(\psi) \rightarrow \psi\) is because ruling out.
  Observe that the agent is confident that \(\lnot\psi\) is unlikely.
  However, this shows that the agent has not witnessed \(A(\psi)\), as this would rule out \(\lnot\psi\).
  Hence, distinct kind of support (hypothesis).
  This doesn't unlock the support provided by witnessing \(A(\psi)\).
  However, agent is relying on the support that would be provided to be confident that \(\psi\).

  Hence, the agent is confident that \(\psi\) without witnessing the appropriate support.

  The agent needs to be in a stronger position than they are in to get those reasons.
  However, if the agent does not appeal to the availability of those reasons, then the agent should not be confident that \(\psi\).

  So, the reasoning is in some sense problematic because it requires stronger premises to support the factive reasoning.
  ...
  Key difference, is that there is no need to deny confidence.
  ...
  Shop is open.
  If shop is open, able to buy product.
  Product is being stocked.

  Well, lacking the required support for the factive inference.
  Avoid in this case.
  For, shop is open, likely product is in stock, able to buy product.

  Not necessarily.
  Signs of it being a coffee shop.
  Able to buy coffee.
  They have coffee in there.

  Here, there's appropriate support between the antecedent and consequent.
  However, factive inference requires reasoning from the shop serving coffee.
  Could reason in a different way, but this seems quite natural.
  Yet, not in a position to make the factive inference.
  For, if I were, then I would rule out not having coffee, which I am unable to do given my lack of certainty.
  Perhaps this is bad, though.
  The agent out to have done the alternative reasoning.
  The agent is excused, because there's not too much of a fuss to make, but the agent is in need to excuse.
\end{note}

\begin{note}[Through, summary]
...
\begin{note}[Quick argument/upshot]
...
\begin{note}[Financial audit]
  So, something like a financial audit.
  Worries about transmission wrt.\ skeptical cases look as though they might bankrupt.
  Here, minor.
  However, funds one didn't realise they have.
  What can be purchased with these?
\end{note}
...
\begin{note}[Sketch]
  ...
  The purpose of this section is to argue that there is no alternative reasoning available to the agent in order to obtain \(\psi\).
...
The issue here is that there's something else going on.
However, information provided is sufficiently mundane that it seems a safe assumption that the agent reasons with it.

So, the second point assumes that the agent is working with the information provided, and that it really is a pure conditional --- denies that there's something else which comes through this information.

Following this, minor observation that one would expect the agent to initially drop confidence in ability, even if going by conditional.
Hence, this should not be taken to indicate that the agent has greater support for consequent.

And, also observation that there's appropriate support.
...
So, novel information, but does not provide immediate support.

Here, perhaps, distinguish between propositional and doxastic.
For, it is likely that if the agent has the ability, then they have propositional support.
This may be denied, depending on the details.
...
The second leads into the third.
...
I.e., it seems plausible that the agent may be okay with the informer providing conditionals.
For example, \(n\)th in a sequence.
Observe that the informer is \(50/50\) on ability, but conditional always works.
Sometimes ability follows.
Other times no ability and doesn't extend.
Other times specific was true (didn't require understanding of a specific piece).

No guarantee that these will always be satisfied.
Failures of the last, by the agent's ability being a copy of another who certainly is competent, etc.



\begin{note}[Dynamics]
  Then, this is perhaps a good place to explain the relevant dynamics of the conditional.
\end{note}

\begin{note}[Support between ability]
  General to specific makes sense.
\end{note}
...
This section expands on sketch provided above.
...
\begin{note}[Goals]
  ...
  Goal is to highlight problem with detachment.
...
\begin{note}[Formal stuff]
  \begin{enumerate}
  \item \(\phi\)
  \item \((\phi \land \chi) \rightarrow \psi\)
  \item \(\psi \rightarrow \chi\)
  \end{enumerate}

  Observe that \([(\phi \land \chi) \rightarrow \psi] \rightarrow [\phi \rightarrow \psi]\).
  And, if \(\phi \rightarrow \psi\) then \(\phi \rightarrow \chi\).

  So, the idea here is that it's clear that \((\phi \land \chi) \rightarrow \psi\) would do the work.
  So, support for this conditional.
  However, if one assumes \(\phi \rightarrow \psi\), then the above conditional simplifies.
  Hence, it seems the assumption works in favour of itself.

  So, if \((A(\phi) \land \psi) \rightarrow A(\psi)\).
  This works, because ability would extend.
  Well, then, consider the support for \(A(\phi) \rightarrow A(\psi)\).
  Given the support for this, and then entailment from \(A(\psi) \rightarrow \psi\).

  Ugh.
  The idea is, if one assumes that \(A(\phi) \rightarrow A(\psi)\) without support, then it seems plausible to try to show that \((A(\phi) \land \psi) \rightarrow A(\psi)\) then reduces to \(A(\phi) \rightarrow A(\psi)\).
  Hence, support for \((A(\phi) \land \psi) \rightarrow A(\psi)\) just is support for \(A(\phi) \rightarrow A(\psi)\).

  The interesting thing here is that the reasoner gets reasoning from \(\phi\) to \(\psi\) that does not explicitly appeal to \(\phi \rightarrow \psi\) given the assumption of \(\phi \rightarrow \psi\).
  In particular, this goes via the stronger conditional.
  ...
  \item \((\phi \land \chi) \rightarrow \psi\).
    ...
    \item \(\phi \rightarrow \psi\)
      ...
      \end{itemize}
    \end{itemize}
    So, the mistake is to assume that one gets a demonstration of transmission from reasoning that is not (explicitly) premise circular if this reasoning requires the truth of something.

    Of course, the informer went from \(\phi\) to \(\chi\), so the informer does get what's inside the box (to be added).
    Hence, the way to view the problem is in terms of combining the support provided by the informer with what is available to the agent.

    Well, \(\chi\) is left in the background of the informer's conclusion.
    And, the informer had the assumption of \(A(\phi)\).
    So, the agent gets support that the conditional is true.
    And, for the agent \(A(\phi)\) is contrasted by not being an assumption.

    A different way to look at this is that the informer concludes with \(A(\psi)\).
    However, the agent must use \(A(\psi)\) in order to apply the conjunction.
    So, informer goes from \((A(\phi) \land \psi) \rightarrow A(\psi)\).
    However, this path is not available to the agent without \(A(\psi)\).
    The agent can't get to \(\psi\) without using \(A(\psi)\).
    But if \(\psi\) is not the case then \(A(\psi)\) is not the case.
    Hence, the agent can't go via the stronger conditional.

    \emph{Key point: The agent does not recreate the informer's reasoning with a missing premise.}
\end{note}
...
\begin{note}[Summary]
  ...
  Similar to literature, but doxastic.
