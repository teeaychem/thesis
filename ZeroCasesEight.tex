...
\begin{note}[\citeauthor{Wright:2011wn}'s template, related argument 1]
  Interesting thing about the attributive argument is that it does not rely on the agent witnessing their ability.

  Consider the following:

  \begin{enumerate}
  \item Support for ability.
  \item Therefore, support for able to generate witness.
  \item So, existence of strategy.
  \end{enumerate}

  Here, the premise is slightly stronger.
  Not merely that the agent possesses the ability, but is able to witness ability.

  Difficulty here is that this presupposes that the agent has the ability.
  We haven't provided an argument that the agent has support for having the ability.
  Still, may assume this premise.
  For, we're interested in whether this leads to trouble.
  ...
  How does the agent extend support?
  Well, (assumed) support for possessing ability.
  And, success of previous witnesses given possession.

  Need case in which agent possesses ability, but never produces a witness.
  So, for example, add an unsound rule of inference, so have the ability, but something is missing in terms of providing witnesses.
  This seems fine.
  Additions to an underlying ability sometimes lead to failures of witness.
  In chess, for example, focus on a particular piece.

  In this respect, there's no clear entailment from possession of ability to providing witnesses.
  ...
  Do, here the agent mixes support for specific ability with support for providing witnesses.

  Is there a problem?


  This is not an entailment, but that doesn't matter.
  The interest with entailment is in the final step.
