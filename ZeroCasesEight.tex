...
\begin{note}[A quick summary of the reasoning of interest]
  ...
  \begin{enumerate}
  \item\label{as:1} I possess the ability to reason with the rules of chess.
  \item\label{as:2} If I am able to reason with the rules of chess then you are able to demonstrate the existence of a (particular) strategy.
  \end{enumerate}
  As~\ref{as:1} is the antecedent of~\ref{as:2} and the consequent of~\ref{as:2} is novel information, \ref{as:1} and~\ref{as:2} combine to provide support for:
  \begin{enumerate}[resume]
  \item\label{as:4} I have the ability to demonstrate the existence of the (particular) strategy.
  \end{enumerate}
  Sub-conclusion~\ref{as:4}, is the antecedent of the following conditional:
  \begin{enumerate}[resume]
  ...
  Therefore:
  \begin{enumerate}[resume]
  \item\label{as:5} The (particular) strategy exists.
  \end{enumerate}

  In summary, the reasoning proposes to transmit support from the ability to demonstrate the existence of a (particular) strategy across a (known) entailment to obtain support for the existence of a (particular) strategy.

  Our interest in the reasoning is from~\ref{as:4} and~\ref{as:3} to~\ref{as:5}, and in particular~\ref{as:3}.
  For, the antecedent of~\ref{as:3} concerns the agent having the attribute of being able to demonstrate the existence of the (particular) strategy, and there allows the agent to obtain the existence of the (particular) strategy without appeal an event of demonstrating the existence of the (particular) strategy.
\end{note}
...
\begin{note}[Relation to transmission]
  The important step of the argument is~\ref{as:5} from~\ref{as:4} and~\ref{as:3}.

  The type of argument here is familiar.
  The agent's goal is to \emph{transmit} support for~\ref{as:4} to~\ref{as:5} \emph{via} the entailment noted in~\ref{as:3}.
\end{note}
...
\begin{note}[After observations]
  So, interest is in whether \ref{as:3} is suitable for transmission of support.
  The above observations link the attribute to the verb.

  Contrast with alternative \WR.
  \begin{enumerate}
...
\begin{note}[Still]
  The issue is with whether~\ref{as:3} is suitable for transmission.
  Suggest that if~\ref{as:3} is suitable, then plausible that support does transmit.

  Tension between transmission from ability and transmission from the relevant witnessing event.
  If~\ref{as:3} then support transmits to propositions for which transmission would fail given the relevant witnessing event.
...
\begin{note}[Mistakes and presuppositions, introduction]
  ...
  There are two parts to the basic premise:
  ...
  \item\label{Att:Prem:Presupp}
    ...
    \end{enumerate}
  \end{enumerate}
  For example, demonstrate, see, receive testimony, all processes.
  Presuppose that the agent has not made a mistake, is not being presented with an illusion, informer is speaking truthfully.
  These are factive examples.
  Non-factive, estimate, foresee.
  Presuppose some accuracy, previous events offer some guidance.

  In line with our understanding of non-ideal agents.
  And, focus on doxastic matters.

  Part~\ref{Att:Prem:Presupp:1} is primarily motivated by inclusion of a verb.
  Demonstrating is the main focus here.
  Fairly intuitive.
  Seeing may be a little less clear.
  However, plausible examples.
  ...
  Still, cases in which presuppositions are plausible.
  Seeing is straightforward, demonstrating likewise the agent may presuppose that they have not made mistakes at earlier steps, or that they would have identified a mistake were they to have made one.

  This will lead to transmission failure just in case the agent attempts to obtain support for a presupposition made in order to claim an instance of the relevant process.
  ...
  \ref{Att:Prem:No-Presupp} is appealing.

  Difficulty is that intuitively support is lacking.
  If \ref{Att:Prem:Presupp}, then recast support as for some broader disjunction.

  The trouble is that this, at least in the case of factive verbs, is quite problematic.
  Given the possibility of a mistake, there's no clear sense to demonstrating \(\phi\) or \(\psi\) where \(\psi\) leads to \(\lnot\phi\).


  However,~\ref{Att:Prem:Presupp} suggests failure of transmission, because required presuppositions may limit.
\end{note}

\begin{note}[Mistakes and presuppositions, idea]
  Some plausibility to the distinction.
  Suppose \(\phi\) is not the case.
  Either due to misleading support.
  Or, due to mistaken presupposition.
\end{note}
...
\begin{note}[How the argument works]
  So, I now have transmission and the key premise.
  The idea is that we get failures of transmission.
  However, \AR allows the agent to avoid such failures.

  Part walking through transmission failure.
  While doing so we establish links between witness of ability, and proposed attributive reasoning.

  On one hand, attribute reasoning.
  On the other, witnessing ability.
  Witnessing involves failures of transmission.
  \AR does not.

  Key is that by \AR agent obtains support for requirements of witnessing event.
  Some such requirements would be presupposed by the agent were the agent to witness the relevant ability.

  The agent:
  \begin{enumerate*}
  \item Obtains support for \(\psi\) given attribute of being able to \emph{V} that \(\phi\), however,
  \item may need to presuppose that \(\psi\) is the case in order to consider witnessing event as an event of \emph{V}ing that \(\phi\).
  \end{enumerate*}

  Alternatively:
  \(\psi\) must be the case given ability to \emph{V} that \(\phi\) but do not expect to be able to obtain support for \(\psi\) by \emph{V}ing that \(\phi\).

  Tension, rather than contradiction.
  Attribute gets more than potential witness.
