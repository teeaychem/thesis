...
\begin{note}[Final premise explanation]
We now turn to the final premise of the negative argument:

\begin{enumerate}[label=(\arabic*), ref=(\arabic*)]
\item\label{xx:1} support for the general ability
\item\label{xx:2} (novel) information that the agent has the specific ability if the agent has the general ability
\item\label{xx:3} having the specific ability to demonstrate the existence of some strategy
\item\label{xx:4} existence of a strategy
\end{enumerate}

\begin{itemize}
\item If the agent has irresolute support for \ref{xx:3} from \ref{xx:1} and \ref{xx:2}, then the agent does not obtain support for the \ref{xx:4} by virtue of the support the agent has for \ref{xx:3} and the observation that \ref{xx:3} entails \ref{xx:4}.
\end{itemize}
\end{note}

\begin{note}[Final premise as transmission failure]
  This follows transmission failure.
  However, doxastic.
\end{note}
...
\begin{note}[Structure]
  Start by clarifying the argument of interest, and introducing general understanding of transmission.
\end{note}

\subsection{Failure of transmission occurs}
\label{sec:fail-transm-occurs}

\begin{note}[A quick summary of the reasoning of interest]
  ...
  Earlier in the paper it has (or rather will be) argued that the agent has not been provided with the information that they are able to demonstrate the existence of a strategy.
  ...
  \begin{enumerate}
  \item\label{as:1} I (have support for) possess(ing) the ability to reason with the rules of chess.
  \item The informer has provided support for the conditional:
    \begin{enumerate}
    \item\label{as:2} If I are able to reason with the rules of chess then you are able to demonstrate the existence of a (particular) strategy.
    \end{enumerate}
  \end{enumerate}
  As~\ref{as:1} is the antecedent of~\ref{as:2} and the consequent of~\ref{as:2} is novel information, I may combine~\ref{as:1} and~\ref{as:2} to observe that:
  \begin{enumerate}[resume]
  \item\label{as:4} I (have support for holding that I) am able to demonstrate the existence of a (particular) strategy.
  \end{enumerate}
  Finally, I know:
  \begin{enumerate}[resume]
  \item\label{as:3} An agent has the ability to demonstrate the existence of some (specific) strategy only if the strategy exists.
  \end{enumerate}
  Therefore:
  \begin{enumerate}[resume]
  \item\label{as:5} (I have support that) the (particular) strategy exists
  \end{enumerate}

  In summary, the desired result of the reasoning is to transmit support from the ability to demonstrate the existence of a (particular) strategy across a (known) entailment to obtain support for the existence of a (particular) strategy.
...
\begin{note}[The goal of this section]
  The goal of this section is to argue that the agent does obtain support for the existence of a particular strategy from the reasoning sketched.

  This is, unfortunately, somewhat difficult.
  In particular, on first pass the reasoning sketched seems quite compelling.

  Foreshadowing some eventual conclusion, there is something strange about the reasoning due to what it lacks.
  For, the reasoning sketched proceeds on the agent having the ability to do certain things, and does not appeal at any point to the agent witnessing the relevant abilities.
  However, this conclusion is some way off.
  The immediate goal is to argue that the argument requires (at least) a few more passes before granting that the agent does obtain support.
\end{note}
...
\begin{note}[Irresolute support]
  ...
  The agent may be mistaken about their general ability to reason with the rules of chess, that the informer is providing truthful information, that the agent possess the ability to demonstrate the existence of the (specific) strategy, and that the (specific) strategy exists.
...
\begin{note}[Goal of argument]
  So, the goal of the reasoning is to observe that the agent is in at least as good a position for the existence of a strategy as the agent is in for the ability to demonstrate the existence of a strategy.

  This does not rule out the possibility that a strategy exists, but this is a good position for the agent to be while it remains the case that the agent has not demonstrated the existence of a strategy.
...
\begin{note}[Transmission characterisation applied to argument]
  ...
  \item \emph{E} is the result of combining:
    \begin{enumerate}[label=\emph{E}\roman*.]
    \item the agent's support for the general ability to reason with the rules of chess, and
    \item the conditional information provided by the informer that if the agent has the general ability to reason with the rules of chess then the agent has the specific ability to demonstrate that some (particular) strategy exists.
    \end{enumerate}
  \end{itemize}
...
% \begin{note}[Transmission without focus on reasoning]
%   This high-level description of transmission does not require the specification of reasoning from the premises to the conclusion.

%   Covers interest in which the conclusion to bear some relation to the premises, relative to the background state, which ensures that it is not possible for transmission to occur, no matter the reasoning.

%   So, no specification of reasoning is required for failure, viewed at a high level.
...
\begin{note}[Next steps]
  The next step is to introduce transmission failure and walk through a basic example of transmission failure.
  Initially the example will aid in understanding transmission failure.
  However, in time the example will be developed to suggest that there is something problematic about the reasoning sketched.
\end{note}
...
\begin{note}[Handful of observations]
  ...
  The first part of the argument will build on these three observations to develop tension.
  Intermediate conclusion that it is not straightforward to hold that support transmits.
\end{note}
...
\begin{note}[Failure of transmission, classic]
  ...
  We begin with a familiar instance due to \textcite{Dretske:2005vy}.
  \begin{itemize}
  \item[\emph{E}:] It looks as though the animal in the pen is a zebra.
  \item[\emph{A}:] The animal in the pen is a zebra.
  \item[\emph{B}:] The animal in the pen is not a cleverly disguised mule.
  \end{itemize}
  There is something intuitively problematic about obtaining \emph{A} on the basis of the support given by \emph{E} and in turn extending \emph{E}, the visual impression that the animal in the pen is a zebra to discount some possibility that would suggest some fault in obtaining \emph{A} on the basis of the support given by \emph{E}.

  Of course, if a groundskeeper were to testify that the animal in the pen is a zebra, the agent may obtain support for \emph{B}.
  Still, it seems odd that the agent may obtain support for \emph{B} given the particular support the agent has for \emph{A}, even though from \emph{A} to \emph{B} is straightforward entailment.
\end{note}

\begin{note}[Potential intuitive diagnosis --- sketch, \emph{very} tentative]
  The problem, roughly, is that it's not clear that the support the agent has distinguishes between \emph{A} and \emph{A'}.
  \begin{enumerate}
  \item \emph{A'}: The animal in the pen is a cleverly disguised mule.
  \end{enumerate}
  So, given \emph{E}, it seems we obtain the weaker disjunction.
  \begin{enumerate}
  \item \emph{A or A'} The animal in the pen is a zebra or the animal in the pen is a cleverly disguised mule.
  \end{enumerate}
  \emph{A or A'} does not entail \emph{B}.
  And, given further possibilities this suggests expand this disjunction with further possibilities.

  So, it seems the agent is granted support for \emph{A} over \emph{A or A'}, but this means the agent focuses on some subset of what is possible, hence the agent takes for granted \emph{not-A'}.
  In turn, because \emph{A'} remains compatible with support the agent has, the agent is not in a position to claim that the support the do have extends.
\end{note}

\begin{note}[Classic]
  Granting for the moment that there is something odd about obtaining \emph{B} from \emph{E} and the entailment from \emph{A} to \emph{B}, let us reformulate \citeauthor{Dretske:2005vy}'s case with the use of a factive verb to establish closer surface parallels to the chess case.
  \begin{enumerate}
  \item \emph{A?} I am seeing that the animal in the pen is a zebra.
  \end{enumerate}
  The support from \emph{E} to \emph{A} seems preserved in going from \emph{E} to \emph{A?}.
  Given that it looks as though the animal in the pen is a zebra, then it seems this supports seeing that the animal in a pen is a zebra.
  While this involves some speculation, I doubt that \emph{E} plausibly supports \emph{A} without also supporting \emph{A?}.
  ...
  \emph{A?} entails \emph{A}, and hence also entails \emph{B}.
  Reformulating \citeauthor{Dretske:2005vy}'s example in this way skips over \emph{A}, that the animal in the pen is a zebra, but the move from \emph{A?} to \emph{B} seems equally problematic.

  A slight worry here is that it is entailment that is at issue, and therefore if moving from \emph{A?} to \emph{A} is problematic, then moving from \emph{A?} to \textsc{A} is equally problematic.
  For now we leave this aside, noting only that the agent need not move from \emph{A?} to \emph{A}, as \emph{E} seems to support both \emph{A} and \emph{A?}.
  The source of the puzzle seems to be the relations between \emph{E}, \emph{A} or \emph{A'}, and \emph{B}.
\end{note}
...
\begin{note}[Still some ways to go]
  As things stand, reformulating \citeauthor{Dretske:2005vy}'s case in terms of a factive verb has established some surface parallels to the reasoning from ability to demonstrate the existence of a (specific) strategy to the existence of a strategy.
  ...
  Second, the support the agent has for the witnessing the verb also extends to the fact --- \emph{A} and \emph{A?} are both supported by \emph{E}.
\end{note}
...
\begin{note}[Hard line]
  ...
  For, as the agent is not in a position to rule out that they do not have the ability to reason with the rules of chess, and hence the possibility that the agent is not able to demonstrate that the (specific) strategy exists, the agent seems in trouble as the ability to demonstrate the (particular) strategy exists entails that the agent is not mistaken about having the ability to demonstrate the (particular) strategy exists.
...
\begin{note}[Moving on]
  Our attention now turns to a framework/template developed by \citeauthor{Wright:2011wn} in order to capture the intuition that there is something problematic about obtaining support for \emph{B} in \citeauthor{Dretske:2005vy}'s case and other related cases.

  We will note the template in general, how it applies to \citeauthor{Dretske:2005vy}'s case, and raise concerns that it does not (at least immediately) apply to the reasoning from the rules of chess.
  
  In the short term, this may be seen as promising for the agent obtaining support for the existence of a strategy.
...
\begin{note}[\citeauthor{Wright:2011wn}'s template, 1]
  ...
  In short, find some \emph{C} entailed by not-\emph{B}, and as \emph{C} is a presupposition, it follows that without the presupposition, the support that the agent has would underdetermine.
\end{note}

\begin{note}[The zebra, failure]
  \citeauthor{Wright:2011wn}'s template is straightforward to apply to the zebra case.
  \begin{enumerate}
  \item \emph{not-B}: The animal in the pen is a cleverly disguised mule.
  \item \emph{C}: Those animals are not zebras. (\citeyear[90]{Wright:2011wn})
  \end{enumerate}
  ...
  The proposed \emph{C} is entailed by not-\emph{B}.
...
\begin{note}[\citeauthor{Wright:2011wn}'s template, 2]
  ...
  % Still, remain entailments for which support doesn't extend.
  % In particular, that the informer is not misleading.
  % Start out with this doubt, then it's hard to figure out what the informer could do.
  % Conversely, not too hard to rule out testimony.
\end{note}
...
\begin{note}[\citeauthor{Wright:2011wn}'s template, no failure 2]
  ...
  % The required \emph{C} needs to be entailed by the non-existence of a strategy.
  ...
  % {
  %   \color{red}
  %   Because the agent is moving from the attribute, this tolerates mistakes about how things appear, roughly.
  % }
\end{note}

% \begin{note}[\citeauthor{Wright:2011wn}'s template, no failure 3]
...
\begin{note}[Warping Zebra]
  ...
  \begin{itemize}
  \item[\emph{E}:] The agent seems to have the ability to visually identify zebras and the information that if the agent has the ability to visually identify zebras, then they have the ability to see the animal in the pen is a zebra.
  \item[\emph{A}:] The agent has the ability to see the animal in the pen is a zebra.
  \item[\emph{B}:] The animal in the pen is not a cleverly disguised mule.
  \end{itemize}
\end{note}
...
\begin{note}[Making the final step]
  The final step is deny transmission to the existence of a (particular) strategy.

  Intuitive problem here is that in the zebra case, it is plausible that the agent does obtain support for the proposition that the animal in the pen is a zebra.

  If ability fails, then this is stronger.

  Two step.
  Sometimes presuppositions block.
  Agent isn't able to guard against blocking presupposition given ability.
\end{note}
...
\begin{note}[Introduction, 1]
  In the previous section we focused on how the agent obtains support for the ability to demonstrate \(\psi\).
  In brief, the support rests on the agent's support for possessing the (general) ability to reason with the rules of chess, which is extended to the (specific) ability to demonstrate the existence of a (particular) strategy.

  In turn, the (specific) ability to demonstrate the existence of a (particular) strategy requires the existence of the (particular) strategy.

  Plausible that the agent obtains support for the existence of a (particular) strategy from the (specific) ability to demonstrate the existence of a (particular) strategy.
  Our interest is in how the agent obtains such support.
\end{note}

\begin{note}[Introduction, 2]
  In the opening section we sketched two paths to obtaining support for the existence of a (particular) strategy given support for the (specific) ability to demonstrate the existence of a (particular) strategy.
  \begin{enumerate}
  \item \AR
  \item \WR
  \end{enumerate}
  The present section focuses on attributive reasoning, with the goal to suggest a dilemma.
  ...
  While there may be something immediately appealing for \WR, block potential reasons on the basis that there are (is) reasons (reasoning) that the agent has access to.
...
\begin{note}[Introduction, 3]
  The goal of the present section is to show that \AR is not quite so straightforward.
  The core of the argument is that \AR achieves too much.
  Allows the agent to extend support from attribute of ability to propositions to which it seems questionable whether support does extend.

  If the argument is successful, then, this establishes something of a dilemma.
  \begin{enumerate}
  \item\label{RDil:W} If go via witnessing, then agent isn't aware of support appealed to, but limited by the witnessing events.
  \item\label{RDil:A} If go via attribution, then agent is aware of support they have, but extends beyond witnessing events.
  \end{enumerate}

  However, do go a little further.
  Establishing~\ref{RDil:A} relies on rejecting \WR.
  Falls a little short, but suggests that \WR is quite good.
\end{note}

\begin{note}[Introduction, 4]
  Ideally, argument for~\ref{RDil:A} with few premises.
  This is not the case.
  The argument relies on transmission failure, and in particular the broad stokes of \citeauthor{Wright:2016wl}'s characterisation of transmission failure.

  [Example of transmission failure]

  Transmission failure is something of an ask, as it \citeauthor{Wright:2016wl}'s understanding.
  It is controversial to some extent, and this offers a relatively clean way to avoid the conclusion.
...
\begin{note}[Introduction, 5]
  Transmission failure.
  Involves limits on witnessing.
  \AR bypasses reasoning.
  \AR isn't limited in the same way.

  That's basically it for transmission failure.

  However, also need to argue that \AR does/would transmit.
\end{note}
...
\begin{note}[Mistakes and presuppositions, introduction]
  We begin by outlining a basic premise regarding support.
  Before dealing with transmission failure proper, as
  \begin{enumerate*}
  \item development of failure relies on the premise, and
  \item provides the basic structure of important objection.
  \end{enumerate*}
  Premise is that
  \begin{enumerate}[label=(A\arabic*), ref=(A\arabic*)]
  \item\label{Att:Prem:Presupp} obtaining support requires certain things, whose failure to obtain results in absence of support.
  \end{enumerate}
\end{note}

\begin{note}[Mistakes and presuppositions, idea]
  Obtaining support is a process.
  Get into situations where the support the agent has is compatible with some alternatives.
  However, the relevant process is incompatible with the alternatives.

  So, obtaining support for length by measuring object.
  Requires that the measuring device is accurate.
  However, compatible with what happens that the device is inaccurate.

  If as proposed, then get an accurate measurement.
  However, given process, possibility of inaccurate measurement.

  Could redescribe process to obtain something compatible with an approximate measurement.
  So, collating support from accuracy of ruler and so on\dots hence there's some approximate length.

  If so, and the measurement is off, then the agent is misled.
  For sure, but then quite easy for skepticism to take hold.
  Some imagination is required to generate the alternatives, but generating the alternatives is not the problem.
  Rather, it's the insight that rarely, if at all, is support suitable for purpose.

  Well, the key assumption is that it's okay for an agent to employ some process, though there are agent doesn't have sufficient support to ensure that they are witnessing the relevant process.

  From this, find instances where the agent would not be misled.
  For, doubts about the process, rather than ambiguity with the results.
\end{note}
...
\begin{note}[Mistakes and presuppositions, example]
  Using a ruler is a nice example.
  Measure, get some quantity.

  Possible that the ruler isn't accurate.
  Background, there's support that the ruler is accurate, but the possibility is live.

  Subjectively compatible, given the support the agent has.
  So, the agent doesn't have support to rule out (pun) that these alternatives hold.
  So, then, it seems the agent obtains only an approximation to the length.
  
  
  So, looks as though one gets something of an approximation.

  So, plausible that if the object isn't roughly, then one is misled.

  

  However, plausible that the agent obtains support for the measurement.
  So long as the measure is accurate, this goes through.
\end{note}

\begin{note}[Minor qualifications]
  First, we're interested in doxastic support, roughly.
  Second, we're interested in non-ideal agents.
  Third, more-or-less an existential premise, so don't need specific failures.
\end{note}

\begin{note}[Why the distinction is required]
  Argue that limitations on process conflict with attribute.
  If agent gets support in all cases, then there's no limits on witnessing, and hence no argument that \AR over-generates support.
\end{note}

\begin{note}[Transmission failure]
  The premise is motivated by transmission failure.
  Primarily exposition, but helpful exposition.
\end{note}
...
So, the set up has been a distinction between two kinds of reasoning.
