...
\begin{note}[\citeauthor{Wright:2011wn}'s template, 3]
...
\begin{note}[\citeauthor{Wright:2011wn}'s template, 4]
  Worry.
...
\begin{note}[\citeauthor{Wright:2011wn}'s template, 5]
...
\begin{note}[Factive and belief, example 1]
  ...
  \begin{enumerate}
  \item\label{belief:strange:1} Sam is shorter than Taylor.
  \item\label{belief:strange:2} If Sam is shorter than Taylor, then Taylor is taller than Sam.
  \item\label{belief:strange:3} Taylor is taller than Sam.
  \end{enumerate}
  It seems plausible that if the agent has support for believing that Sam is shorter than Taylor, then the same support transmits to Taylor being taller than Sam.\nolinebreak
  \footnote{
  ...
  \begin{enumerate}
  \item I believe that Sam is shorter than Taylor.
  \item So, Sam \emph{is} shorter than Taylor.
  \item The fact of Sam \emph{being} shorter than Taylor entails that Taylor \emph{is} taller than Sam.
  \item So, I believe that Taylor is taller than Sam.
  \end{enumerate}
  The included stress on the copula indicates that the reasons to the belief that Taylor is taller than Sam by taking their initial belief that Sam is shorter than Taylor to reflect how things actually are.

  The agent's attitude toward the proposition that Sam is shorter than Taylor is recognised by the agent as belief because the agent recognises that it may be the case that Sam is not (actually) shorter than Taylor.
  While, the agent's belief that Taylor is taller than Sam is premised on the agent's belief that Sam is (actually) shorter than Taylor.
  Given the agent's cognitive state they (the agent) considers it possible that Sam is not (actually) shorter than Taylor, so it is unclear why the agent has support to believe that Taylor is taller than Sam based on what is (actually) the case.

  {
    \color{red}
    The agent fails to obtain support for their belief that Taylor is taller than Sam by the proposed reasoning.
    For, the support the agent has for the proposition that Sam is shorter than Taylor is not support for Sam (actually) \emph{being} shorter than Taylor.
  }
  ...
  For, if the agent doubts that their support for some proposition \(\phi\) reflects how things actually are, then it seems this doubt rationally commits the agent to doubting the significance, or competence, of reasoning on the basis of how things actually are, irrespective of the outcome.
  (Contrast to \cite[92]{Wright:2011wn})
\end{note}

\begin{note}[Factive and belief, example, 2]
  To contrast, suppose the agent reasons that, even though they may be mistaken about how things actually are, the support they have points to Sam being shorter than Taylor, and hence the given the most plausible candidate for how things are, Taylor is taller than Sam.
  The minor difference is that the agent does not move from their belief about how things are to how things are.
  Even if the agent is mistaken and Sam is (actually) taller than Taylor, Taylor is taller than Sam given the information the agent has support for.
\end{note}

\begin{note}[Factivity and belief, to ability]
...
\begin{note}[Looking back to Wright's template]
  With respect to the template provided by \citeauthor{Wright:2011wn} the difference is slight.
  Extend cognitive project to cover the application of entailment from \emph{A} to \emph{B}.
  At least, to the extent that one is interested in doxastic warrant/justification/support and inferential transmission.
  For, as noted, it is not clear that the concerns arising from ability apply to propositional warrant/justification/support.
  And, pushing the issue further to related factive attitudes is not within the bounds of this paper.
\end{note}
