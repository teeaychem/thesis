...
\begin{note}[Attribution sketch, part 2.5 --- trouble with transmission]
  ...
  So, no support or commitment.
  
  
  

  So, if restrict attention to transmission then agent is committed to \(\psi\).
  And, no support for \(\psi\).
  For, 
  ...
  However, granting informer's information, witnessing would put the agent in position of resolute support for \(\psi\).

  And, novel information doesn't put the agent in a worse position with respect to application of the general ability.
  Novel information, and this shouldn't be taken against \(A(\phi)\).
  If the agent has the general ability, and the information is true, then the agent has the specific ability.

  Argument concludes that the agent can't extend based on attribution alone.
  However, no particular reason for the agent to limit either.
  Reasoning by attribution is limited, but by the principles of transmission --- and the principles of transmission don't touch on the specific contents of the argument.

  It does not follow from transmission failure that the agent has opposing support.
  And, agent has support for \(A(\psi)\) in the form of \(A(\phi)\).
  It seems there's some internal tension here.
  For, though one needs to assume \(\psi\) when working with the conditional, one doesn't need to assume \(\psi\) when witnessing ability.
  \(\psi\) isn't required for \(A(\phi)\).
  And, if reason then \(A(\phi) \rightarrow A(\psi)\) doesn't do any essential work (though may tell agent where to look, etc.\ --- guidance).

  \hozlinedash

  Hm, so the problem is that the agent still has support for \(A(\psi)\).
  This much is clear.
  The parallel to transmission failure cases is that the appearance of hands still seems like some kind of support.
  So, the agent is in a position where their support for \(A(\phi)\) gives support to \(A(\psi)\), but this does not extend to support to \(\psi\).
  This, itself, seems like a problem, because it seems a logical consequence like this should inherit the same support.
...
\begin{note}[Attribution sketch, part 3]
  ...
  So, the reasoning is problematic because it requires stronger premises to support the factive reasoning.
