...
The goal is to motivate interest in \WR.
The introduction focused on the distinction between \AR{} and \WR.
Here, we also focus on whether the agent has an alternative route to support for the existence of the (particular) strategy.
...
\subsection{Transmission alternative?}
\label{sec:transm-altern}
...
\begin{note}[X]
  Agent is assuming that the relevant conditions obtain in order to hold that they have the attribute.
  In turn, the agent is not in a position to appeal to having the attribute in order to support the background conditions holding.
\end{note}

\begin{note}[Y]
  If I am going to argue for some kind of transmission failure, then it should only be with respect to the fact.
  I do not want to argue that the agent fails to have support for the ability.
\end{note}
...
\begin{note}[Still]
  The issue is with whether~\ref{inf:gae} is suitable for transmission.
  Suggest that if~\ref{inf:gae} is suitable, then plausible that support does transmit.
