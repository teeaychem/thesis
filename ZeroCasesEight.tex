...
\begin{note}[Literature]
  ...
  \begin{quote}
    ...
    \item The process/state of accomplishing the relevant putative warrant for A is subjectively compatible with C’s holding: things could be with one in all respects exactly as they subjectively are yet C be true
    \item C is incompatible (not necessarily with A but) with some presupposition of the cognitive project of obtaining a warrant for A in the relevant fashion, and
    \item Not-B entails C
    \end{enumerate}
  \end{quote}
  ...
  So, testimony is working out, then, consider that the animal is not a zebra, C is compatible with testimony.
  C is incompatible with the specific result, but not the process.
  ...
  Whether this is a appropriate C is unclear.
...
\begin{note}[Complex fault]
  ...
  Our interest is what follows from the information state.
  ...
  For, this means that the support the agent would obtain from \(A(\psi)\) given the quick argument is premised on something stronger than the agent has.

  The agent only gets the non-factive variant of \(A(\psi)\) at the relevant states.
  The agent's reasoning will lead them to \(\psi\).
  For, the agent doesn't get to add in the additional assumption of factivity.
