...
\begin{note}[Support for a strategy]
  Agent is argument on the basis of possessing ability.
  Agent does not appeal to any execution of ability.
  Agent's support is irresolute, so the agent isn't looking to show that a strategy exists, but that they have support for a strategy existing, given the information the agent has.
\end{note}

\begin{note}[Somewhat subtle point]
  Irresolute support is a subtle point.
  In the argument of interest, the agent is not looking to move from state to how things are.
  Instead, to expand their state.

  If demonstrate strategy, then strategy exists.
  Here, agents state of demonstrating establishes something independent of the agent's state.
  Demonstration is demonstration of some fact.
  Agent rules out the possibility that a strategy does not exist.

  Attributive reasoning, the agent looks transfer irresolute support.
  Agent does not rule out the possibility that a strategy does not exist.
  However, support establishes that the best candidate for how things are is that a strategy does exist.
\end{note}

\begin{note}[Expanding a little]
  Clear problem if the agent attempt to rule out the possibility of a strategy not existing.
  For, given irresolute support, agent isn't in a position to rule out the possibility.
  Hence, no sound argument.

  Not a failure of transmission, as agent lacks required support \emph{to} transmit.

  Compare:
  Sequence of flips.
  No clear pattern.
  Not possible that this coin is biased.

  Clearly, still possible.

  Parallel here is that the second proposition is significantly stronger.

  Another way to put this is that the agent isn't in a position to transition from information they have to what is actually the case.
  Given the support the agent has, things could turn out differently without the agent having made some mistake.
  Generating more data may reveal bias, and dealing with further problems may reveal that the agent misunderstood some part of chess, and lacked the ability.
  Or, that the informer was misleading, though we will bracket this possibility.
\end{note}

\begin{note}[So, the argument]
  So, the argument is that because the agent has good support for having the ability, then this provides support for the existence of a strategy.
...
\begin{note}[The zebra]
  ...
  \item \emph{C}: Appearances are not misleading.
  \end{enumerate}

  The proposed \emph{C} is entailed by not-B.
  ...
  This is a little cleaner, as the agent does require that appearances are to go by in order to obtain support for \emph{A}.
  ...
  One problem is that the agent could be mistaken about \emph{A}.
  However, this doesn't raise a serious problem, so long as the agent is equally committed to mistake about \emph{B}.

  However, the problem diagnosed by \citeauthor{Wright:2011wn} is slightly different.
  The agent could be mistaken about whether the support the agent has for \emph{A} is appropriately support for \emph{A}, and not some more general proposition expanding \emph{A}.
  For example, \emph{A} or \emph{A'} or \dots \emph{A''''}.
  The issue is not that the support the agent has for \emph{A} might be mistaken, but that the support the agent has for \emph{A} may fall short of \emph{A}.

  If so, the agent may have support for \emph{A}, but then the agent will not be in a position to exclude any other alternative proposition, \emph{A'}, etc.

  So, it may be the case that the animal in the pen is a cleverly disguised mule.
  However, that's okay.
  The agent may be mistaken.
  The problem is that the agent is not in a position to separate zebra from cleverly disguised mule given the information that they have.

  Now, may wish to deny support for the zebra.
  If so, then likewise drop support for general ability, and for the existence of a strategy.
  The agent isn't in a position to rule out a mistake, so the agent doesn't have support.
...
\begin{note}[\citeauthor{Wright:2011wn}'s template, 2]
  So, the key here is that \emph{C} needs to be incompatible with some presupposition of the cognitive project.
...
\begin{note}[\citeauthor{Wright:2011wn}'s template, 2.5]
  ...
  Argue that the attributive reasoning does not satisfy \citeauthor{Wright:2011wn}'s template.
  Hence, unable to use \citeauthor{Wright:2011wn}'s to generate failure of transmission.

  Two further aspects.

  First.
  If transmission does fail, then it is not due to the failure identified by \citeauthor{Wright:2011wn}.
  So, then, something about the entailment.

  Second.
  Identifies failure in a closely related case.
\end{note}
...
\begin{note}[\citeauthor{Wright:2011wn}'s template, no failure 1]
  ...
  Here, the relevant project is broader than seeing.
  The final stage is combining support for general ability with conditional provided by the informer.
  Still, the informer seems okay, if a strategy doesn't exist, then this doesn't seem to impact obtaining information by the informer.
  So, focus on general ability.
  ...
  Agent didn't need to presuppose that they had the general ability.
  ...
  If the agent doesn't have the ability, then this holds because the support ended up being misleading, rather than the agent having some presupposition which led to/favoured support for ability to reason with chess over some other possibility.
...
\begin{note}[Warping zebra]
  ...
  Here, informer.
  \begin{enumerate}
  \item If you've been able to see the visually identify members of the ??? species, then you'll be able to see a zebra in the pen.
  \end{enumerate}

  Haven't looked in the pen.
  Possibility that it's not a zebra.
  However, support seems okay, just as with chess.

  Note, the role of the informer here is important.
  For, if the agent reasons to the attribute from the appearance of zebra, then it's plausible that there is failure of transmission.
...
\begin{note}[???]
  Now, entailment, if able to see zebra in pen, then the animal in the pen is not a cleverly disguised mule.
  From the recognised entailment from zebra to not a cleverly disguised mule and able to see a zebra to there is a zebra.

  Agent has good support for the general ability.
  Studied the species, etc. etc.\

  Something like this.

  First to note is that there remains a problem with the agent moving from appearances to not a cleverly disguised mule.
  Problem is that the agent isn't able to secure that there's nothing misleading.
  Subjectively compatible, hence a worry.

  So, the agent has quite good support.
  The agent may have some doubts about edge cases, here and there, but given the information provided, seems as though there's fairly good support.
  So, combine background with conditional and fairly good support for being able to see a zebra in the pen.

  Looking for support.
  Possibility that something has gone wrong.
  Perhaps there isn't a zebra in the pen, but as things stand, there's no real problem with either of the \emph{C}s.

  If the animal in the pen is a cleverly disguised mule, then this shows that there was something misleading, either in general ability or informer, but that's fine.
  

  
  However, now based on ability to see, rather than sight itself.
\end{note}
