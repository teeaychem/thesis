...
\begin{note}
  ...
  Still, when discussing events we will almost always have a specific event in mind and the role of the relevant description will be to identify particular features of the event.%
  \footnote{
    In particular, both \qWhy{} and \qHow{} are asked with respect to a specific event.
  }
  Hence, the ease the way we talk about events, we make use of the following notation:
  ...
  \begin{notation}[Events and descriptions]%
    ...
    \begin{itemize}
    \item
      \(\edn{}\) picks out a specific event (i.e.\ an event under some exhaustive description).
    \item
  ...
  \noindent%
  For example, \ref{ESERIES:gistCalcEq} intuitively captures a specific event under a particular description.
  Hence, we may re-express \ref{ESERIES:gistCalcEq} as:
  %
  \begin{enumerate}[label=\arabic*\('\)., ref=(\arabic*\('\))]
    \setcounter{enumi}{1}
  \item
    \(\ed{}\) is the event \(\edn{}\) under description \(\edo{}\) `Max concludes \gistCalcEq{} has value \valI{True}'.
  \end{enumerate}
\end{note}
