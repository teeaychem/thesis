...
\begin{note}
  ...
  \begin{application}[A \se{} of \autoref{illu:gist:roots:F}]
    ...
    \item
      ...
      `The agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\)'.
    \item
  ...
  \noindent%
  Note, Step~\ref{illu:gist:roots:F:factor} of \autoref{illu:gist:roots:F} amounts to the agent figuring out \((2x + 1)(x - 1) = 0\).
  However, \autoref{illu:gist:roots:F} did not explicitly characterise the agent attempting to identifying the factors of \(2x^{2} - x - 1 = 0\).
  Still, we take it to be implicit the agent was attempting to identifying the factors of \(2x^{2} - x - 1 = 0\).
...
\begin{note}
  ...
  No other detail for an application is quite as complex as the detail for \autoref{obs:se-inst}.
  And, indeed, not other motivation for an observation or argument for a proposition is quite as complex as the detail for \autoref{obs:se-inst}.
  Still, I sort of recommend working the detail for \autoref{obs:se-inst} as it highlights the way events and event descriptions interact.

  Still, if you would like to skip ahead the very brief argument is:

  \begin{itemize}
  \item
    Clause~\ref{assu:p:se:prog} of \autoref{def:se} is satisfied as an agent figuring out \((2x + 1)(x - 1) = 0\) with the aim to identify the factors of \(2x^{2} - x - 1 = 0\) is clearly working towards an event in which the conclude \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\).
  \item
    Clause~\ref{assu:p:se:hCon} of \autoref{def:se} is satisfied as it makes no sense for an agent to conclude \propM{\rootsCon{}} has \val{0} \valI{True} from a \pool{} which captures the \agents{} understanding of factorisation is the conclusion does not partly happen as a result of factorisation, which is exactly what Step~\ref{illu:gist:roots:F:factor} of \autoref{illu:gist:roots:F} captures.

    Further, it is plausibly the case that the \agents{} conclusion is party a result of the agent aiming to figure out the factors of \(2x^{2} - x - 1 = 0\).

    So, the \agents{} conclusion is party a result of factoring with the aiming to figure out the factors of \(2x^{2} - x - 1 = 0\).
 \end{itemize}
  ...
  \begin{dets}{obs:se-inst}
    ...
    Now, \(\edn{\flat}\) as described by \(\edo{\flat}\) is an event in which the agent figures out \((2x + 1)(x - 1) = 0\) with the aim of finding the relevant factors.
    And, by assumption the agent has a good understanding of factorisation.

    So, as the agent figures out \((2x + 1)(x - 1) = 0\), the only reasoning which remains is captured by steps \ref{illu:gist:roots:F:zero} to \ref{illu:gist:roots:F:factor:done} of \autoref{illu:gist:roots:F}.
    These steps are more-or-less straightforward.
    Hence, I take it to be clear an event in which the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\) \emph{may} be in progress.

    Further, as by \(\edo{\flat}\) the agent has the aim to identify the factors of \(2x^{2} - x - 1 = 0\) the previous observation may be strengthened to state an event in which the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\) \emph{is} be in progress.

    In short, the reasoning which remains to conclude \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\) is more-or-less straightforward and the agent plans to complete the relevant reasoning.
    So, \(\ed{\flat}\) is building to an event in which the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\).

    This is what we wanted to show, Clause~\ref{assu:p:se:prog} of \autoref{def:se} holds.
    \medskip

    \noindent
    Note, in particular, our goal was only to show \(\edn{}\) under the description `the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\)' is in progress.
    Our goal was not to show \(\edn{}\) under any other description is in progress.

    And, note the importance of the \agents{} aim to identify the factors of \(2x^{2} - x - 1 = 0\).
    If \(\edn{\flat}\) is only described as an event in which the agent figures out \((2x + 1)(x - 1) = 0\) then there is nothing to guarantee the agent puts the factorisation of \(2x^{2} - x - 1 = 0\) to any further use.
    \bigskip
    ...
    \item
      ...
      \item
        ...
        \begin{itemize}
        \item
          \(\edo{\sharp}\) is the description: `The agent figures out \((2x + 1)(x - 1) = 0\)'
        \end{itemize}
      \item
    ...
    \end{enumerate}
    %
    In short, \ref{obs:se-inst:phew} states that an \agents{} conclusion of \propM{\rootsCon{}} has \val{0} \valI{True} from a \pool{} which captures the \agents{} understanding of factorisation prior to \(\edn{}\) partly happens as the result of the agent figuring out \((2x + 1)(x - 1) = 0\).
    \medskip
    ...
    In short, \(\ed{}\) does not partly happens as a result of an event in which the agent figures out \((2x + 1)(x - 1) = 0\).

    I take this to be a contradiction.
    For, \(\edo{}\) describes \(\edn{+}\) as an event in which the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\).
    ...
    \footnote{
      Indeed, it is built into \(\edn{+}\) being described as an event in which the agent \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\) that \(\edn{+}\) partly happens as a result of reasoning which amounts to steps~\ref{illu:gist:roots:F:eq}~to~\ref{illu:gist:roots:F:factor:done} of \autoref{illu:gist:roots:F}.
    }
    \medskip
    ...
    \footnote{
      However, note that if \(\edn{\flat}\) is \emph{not} the relevant instance of \(\edn{\times}\) then by \ref{obs:se-inst:phew} there must be some other event in which the agent figures out \((2x + 1)(x - 1) = 0\) that \(\ed{}\) partly happens as a result of.
      And, \autoref{illu:gist:roots:F} does not involve the agent (re-)figuring out \((2x + 1)(x - 1) = 0\) after \(\edn{\flat}\) so any such event must happen prior to \(\edn{\flat}\).
    ...
    For, \(\edo{\flat}\) adds to \(\edo{\sharp}\) that the agent figures out \((2x + 1)(x - 1) = 0\) with the aim to identify the factors of \(2x^{2} - x - 1 = 0\).
    ...
    So, that the agent has the aim to identify the factors of \(2x^{2} - x - 1 = 0\) is a feature that \(\ed{}\) happens as a result of.
  ...
  % In particular, most of the detail of \autoref{obs:se-inst} amounted to arguing it makes no sense for an agent to conclude \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\) without the conclusion partly happening as a result of the agent figuring out \((2x + 1)(x - 1) = 0\).
\end{note}
...
\begin{note}
  Before starting, recall we understand the sense of `why' present in \qWhy{} in terms of why did the event \(\ed{}\) in which an agent concludes some \prop{0} \(\phi\) has \val{0} \(v\) from some \pool{0} \(\Phi\) happen, rather than any incompatible event --- e.g., an event in which the agent failed to conclude \(\phi\) has \val{0} \(v\) from \(\Phi\).
\end{note}
...
\begin{note}
  ...
  However, \qWhy{} asks about \ros{1}, and we have not connected \ros{1} to \se{1}.
  There are (at least) two possible way to argue for a connexion:
  %
  \begin{itemize}
  \item
    Argue \ros{1} are part of some descriptions which explains `why' some event happened.
  \item
    Argue that in some cases what is entailed by a description which explains `why' some event happened \emph{also} explains `why' the event happened, and that \ros{1} are entailed by certain descriptions which explain `why'.
  \end{itemize}
  %
  We opt for the second option.
  For, the second option allows us to easily make general observations about when a \ros{} answers \qWhy{} based on descriptions which do not (directly) appeal to \ros{1}.
  ...
  And later, in \autoref{cha:ros}, we establish the way \ros{3} are entailed by certain descriptions.
...
\begin{note}
  ...
  \begin{argument}{sketch:PE:cROS}
    ...
    By Condition~\ref{sketch:PE:cROS:b}, the \ros{} between \(\psi\), \(v'\) and \(\Psi\) holds throughout \(\ed{\flat}\) explains `why' \(\ed{}\) happened, in the sense of `why' present in \qWhy{}.
    And, by Condition~\ref{sketch:PE:cROS:a}, \(\ed{}\) just is an event in which agent concludes \(\phi\) has \val{0} \(v\) from \(\Phi\).
  ...
  \item
    \label{eip:task:2}
    Show some \se{} of an event in which an agent concludes some \prop{0} \(\phi\) has \val{0} from some \pool{0} \(\Phi\) entails a \ros{} holds.
  \item
    \label{eip:task:3}
    Show some \se{} entails a \ros{} between \(\psi\), \(v'\) and \(\Psi\) holds even when an agent has not concluded \(\psi\) has \val{0} \(v'\) from \(\Psi\) when the agent concludes \(\phi\) has \val{0} from \(\Phi\).
  \end{enumerate}

  Task~\ref{eip:task:1} establishes a \ros{0} may explain `why' an event happened, given \progExII{}/\autoref{sketch:PE:cROS}.

  Task~\ref{eip:task:2} establishes \ros{1} do explain `why' an event happened, given \progExII{}/\autoref{sketch:PE:cROS} and hence establishes certain answers \qWhy{}.

  And, Task~\ref{eip:task:3} establishes \issueInclusion{} fails to hold.
  For, by \autoref{sketch:PE:cROS} the relevant \ros{} between \(\psi\), \(v'\) and \(\Psi\) answers \qWhy{}, and \issueInclusion{} holds only if the agent has also concluded \(\psi\) has \val{0} \(v'\) from \(\Psi\) when the agent concludes \(\phi\) has \val{0} from \(\Phi\).
\end{note}
