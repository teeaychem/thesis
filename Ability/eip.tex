...
\begin{note}
  \begin{definition}[\se{3}]
    ...
    \begin{itemize}
    \item
      For some description \(\edo{\sharp}\), clauses~\ref{assu:p:se:prog}, \ref{assu:p:se:desc}, and \ref{assu:p:se:hCon} hold:
      \begin{enumerate}[label=\Alph*., ref=\Alph*]
      \item
        \label{assu:p:se:prog}
        \(\ed{\flat}\) is such that an event \(\edn{\circ}\) under description \(\edo{\sharp}\) is in progress.
      \item
        \label{assu:p:se:desc}
        \(\edo{}\) is true of \(\edn{}\) \emph{only if} \(\edo{\sharp}\) is true of \(\edn{}\).
      \item
        ...
        \item[\emph{If}:]
          \(\ed[\sharp]{}\) happens.
        \item[\emph{Then}:]
          \(\ed[\sharp]{}\) happens as a result of \(\ed{\flat}\).
        \end{itenum}
      \end{enumerate}
    \end{itemize}
    \vspace{-\baselineskip}
  ...
  Though, we prefer `happens as a result of' to avoid requiring a causal relation.
...
\begin{note}
  \begin{observation}[A \se{} of \autoref{illu:gist:roots:F}]
    \label{obs:se-inst}%
    There is some description such that the event which covers Step~\ref{illu:gist:roots:F:factor} of the \agents{} reasoning is a \se{} of the event described by \autoref{illu:gist:roots:F}.
  \end{observation}
  ...
  \begin{motivation}{obs:se-inst}
    ...
    Therefore, we may fix a description \(\edo{\flat}\) of \(\edn{\flat}\) which states, roughly, `the agent is concluding \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\) in \(\edn{\flat}\) and figures out \((2x + 1)(x - 1) = 0\)'.%
    \footnote{
    ...
    \noindent%
    Our task is to show the clauses of of \autoref{def:se} hold with respect to some description \(d'\).
    Let \(d'\) be `the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\)'.

    We argue for each clause in turn.
    \medskip

    \noindent%
    Clause~\ref{assu:p:se:prog} requires \(\ed{\flat}\) is such that \(\edn{}\) under description `the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\)' is in progress.

    By taking the left conjunct of \(\edo{\flat}\) it is immediate that \emph{an} event such that the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\) is in progress.%
    \footnote{
      For, if the agent is concluding \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\), then an event in which the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\) is in progress (cf.\ \autoref{def:es-in-prog}), {
        \color{red} and \assuPP{}.
      }
    }
    \medskip

    \noindent%
    Clause~\ref{assu:p:se:desc} requires:
    \(\edo{}\) is true of \(\edn{}\) \emph{only if} \(d'\) is true of \(\edn{}\).

    Our interest with \(\ed{}\) is that the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\).
    So, no matter any further details included in \(\edo{}\), it also holds that the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\).
    \medskip

    \noindent%
    Clause~\ref{assu:p:se:hCon} requires the following conditional to be true:
    \emph{If} \(\ed{}\) happens \emph{then} \(\ed{}\) happens as a result of \(\ed{\flat}\).

    We obtain this conditional by a slightly stronger result.
    Specifically, we argue:

    Any event which satisfies \(\edo{\sharp}\) and happens after \(\edo{\flat}\) \emph{without} an intermediary event \(\edn{\triangle}\) such that \(\edo{\flat}\) is true of \(\edn{\triangle}\) happens as a result of \(\ed{\flat}\).%
    \footnote{
    ...
    Suppose \(\edn{\square}\) is an event which satisfies \(\edo{\sharp}\), happens after \(\edo{\flat}\), and without an intermediary event as described.
    And, suppose \(\edn{\square}\) does not happen as a result of \(\ed{\flat}\).

    Now, \(\edn{\square}\) is an event in which the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\).
    And, \(\Phi\) captures the \agents{} understanding of factorisation.

    By assumption, \(\edn{\square}\) does not happen as a result of \(\ed{\flat}\) and there is intermediary event \(\edn{\triangle}\) between \(\edn{\flat}\) and \(\edn{\square}\) in which the agent figures out \((2x + 1)(x - 1) = 0\).
    Therefore, \(\edn{\square}\) does not happen as a result of an event in which the agent figures out \((2x + 1)(x - 1) = 0\).

    This is a contradiction.
    For, \(\edn{\square}\) does not happen as a result of an event in which the agent figures out \((2x + 1)(x - 1) = 0\).
    And, \(\edn{\square}\) is an event in which the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from a \pool{} of premises which captures their understanding of factorisation.
    ...
    So, any event which satisfies \(\edo{\sharp}\) and happens after \(\edo{\flat}\) \emph{without} an intermediary event \(\edn{\triangle}\) such that \(\edo{\flat}\) is true of \(\edn{\triangle}\) happens as a result of \(\ed{\flat}\).
    And, as \(\ed[\sharp]{}\) satisfies these constraints, if \(\ed[\sharp]{}\) happens, \(\ed[\sharp]{}\) happens as a result of \(\ed{\flat}\).
  \end{motivation}

  \noindent%
  Establishing that some event \(\ed{\flat}\) is a \se{} of some other event \(\ed{}\)  takes some effort.
  Still, the key idea is somewhat straightforward:
  Observe that it makes no sense for \(\edo{\flat}\) to be true of \(\edo{}\) if \(\ed[\flat]{}\) does not happen as a result of \(\ed{\flat}\).
\end{note}
