...
\begin{note}
  \autoref{cha:intro} introduced \eiw{0} an agent concludes, two questions \qWhy{} and \qHow{} --- concerning why and how an \eiw{0} an agent concludes happens, and a constraint on answers to \qWhy{} in terms of answers to \qHow{}.

  This chapter establishes the way we obtain answers to \qWhy{}, in line with \autoref{idea:why} and in doing so establishes a foundation for counterexamples to \issueInclusion{}.
\end{note}

\begin{note}
  We start by outlining a broad understanding of both events and events (such that some other event is) in progress.

  In particular, our understanding of both events and events in progress is applied to both:

  \begin{itemize}
  \item
    An \eiw{1} an agent concludes.
  \item
    An \eiw{0} some other \eiw{} an agent concludes is in progress --- or colloquially an \eiw{0} an agent is concluding.
  \end{itemize}
\end{note}

\begin{note}
  With an understanding of both events and events in progress in hand, we introduce `\progEx{}' with the aid of a particular kind of event in progress termed a `\se{}'.

  \progEx{2}  expands on \autoref{idea:why} (\autoref{cha:intro}, \autopageref{idea:why}) and characterises the sense of `why' present in \qWhy{} in terms of features of an event in progress.
  The task of the remaining argument is to show some answers to \qWhy{} obtained by \progEx{} do not satisfy \issueInclusion{}.
\end{note}
...
\begin{note}
  ...
  \begin{intuition}[Events in progress and the progressive]
    ...
    \begin{itemize}
    \item
      \(\ed{\flat}\) is an \eiw{0} \(\eXd{}\) is in progress.
    \end{itemize}
    % 
    \emph{If and only if}:
    % 
    \begin{itemize}
    % \item
    %   \(\edo{}\) is true of some (maybe non-actual) event \(\edn{}\).
    % \item
  ...
  \begin{assumption}[\assuPP{2}]%
    ...
    \item[\emph{If}:]
      \(\ed{\flat}\) is an \eiw{0} \(\eXd{}\) is in progress.
    \item[\emph{Then}:]
      There is some \progAdj{0} event \(\edn{}\) such that~\ref{assu:PP:pe:dev} and~\ref{assu:PP:pe:verb} are both true:
      \begin{enumerate}[label=\roman*., ref=(\roman*)]
      % \item
      %   \(\edo{\flat}\) is true of \(\edn{\flat}\).
      \item
        \label{assu:PP:pe:dev}
        \(\edn{}\) is a development of \(\edn{\flat}\).
      \item
        \label{assu:PP:pe:verb}
        \(\edo{}\) is true of \(\edn{}\). %, where \(\edo{\sharp}\) is the perfective correlate of \(\edo{}\).
      \end{enumerate}
    \end{itenum}
    \vspace{-\baselineskip}
  \end{assumption}
...
\begin{note}
  ...
  \begin{observation}[Events in progress and common satisfaction]%
    ...
    Sometimes, when \(\ed{\flat}\) is an \eiw{0} \(\eXd{}\) is in progress, \(\eXdn{}\) may be satisfied by multiple events.
  \end{observation}
...
%   In this respect, it is a little unethical to mention \(\edn{}\).
%   For, to say \(\ed{}\) is in progress is just to say there is \emph{some} event in progress such that \(\edo{}\) is true of the event.
%   However, this simplifies talk about events in progress.
% \end{note}
...
\begin{note}
  ...
  % XXX
  % So, to fix, the student has arranged to be passed notes.
  % Then, student is passing the exam.
  % Still, the student then gets the notes.
  % The issue is, haven't included notes in the description.
  % And, the student passes as a result of the notes.
...
\begin{note}
  ...
  \begin{dets}{obs:se-inst}
    ...
    \noindent
    Note, in particular, our goal was only to show some event \(\edn{}\) under the description `the agent concludes \pv{\propM{\rootsCon{}}}{\valI{True}} from \(\Phi\)' is in progress.
...
\begin{note}
  ...
  \begin{motivation}{prop:PEbasic}
    ...
    \noindent%
    By Clause~\ref{assu:p:se:prog} of \autoref{def:se} (\autopageref{def:se}), \(\ed{\flat}\) is such that \(\eXd{}\) is in progress.
...
\begin{note}
  ...
  Broadly, \qWhy{} asks for (partial) explanations of why some event is an event where an agent concluded \(\pv{\phi}{v}\) from \(\Phi\), rather than an \eiw{0} some other thing happened.
