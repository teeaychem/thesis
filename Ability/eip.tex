...
\begin{note}
  ...
  \begin{itemize}
  \item
    \(\ed{\flat}\) is an event in which Max concludes \pv{\propM{\gistCalcEq{}}}{\valI{True}} is in progress.
  \item
...
\begin{note}
  ...
  \begin{itemize}
  \item
    Clause~\ref{assu:p:se:prog} of \autoref{def:se} is satisfied as an agent figuring out \rootsConEqFac{} with the aim to identify the factors of \rootsConEq{} is clearly working towards an event in which the agent concludes \pv{\propM{\rootsCon{}}}{\valI{True}} from \(\Phi\).
  \item
    Clause~\ref{assu:p:se:hCon} of \autoref{def:se} is satisfied as it makes no sense for an agent to conclude \pv{\propM{\rootsCon{}}}{\valI{True}} from a \pool{} which captures the \agents{} understanding of factorisation if the conclusion does not partly happen as a result of factorisation, which is exactly what Step~\ref{illu:gist:roots:F:factor} of \autoref{illu:gist:roots:F} captures.
 ...
 \end{itemize}
  ...
  \begin{dets}{obs:se-inst}
    ...
    Hence, I take it to be clear an event in which the agent concludes \pv{\propM{\rootsCon{}}}{\valI{True}} from \(\Phi\) \emph{may} be in progress.

    Further, as by \(\edo{\flat}\) the agent has the aim to identify the factors of \rootsConEq{} the previous observation may be strengthened to state an event in which the agent concludes \pv{\propM{\rootsCon{}}}{\valI{True}} from \(\Phi\) \emph{is} be in progress.
    ...
    So, \(\ed{\flat}\) is building to an event in which the agent concludes \pv{\propM{\rootsCon{}}}{\valI{True}} from \(\Phi\).
    ...
    \end{enumerate}
    %
    In short, \ref{obs:se-inst:phew} states that an \agents{} conclusion of \pv{\propM{\rootsCon{}}}{\valI{True}} from a \pool{} which captures the \agents{} understanding of factorisation prior to \(\edn{}\) partly happens as the result of the agent figuring out \rootsConEqFac{}.
    \medskip
    ...
    For, \(\edo{}\) describes \(\edn{+}\) as an event in which the agent concludes \pv{\propM{\rootsCon{}}}{\valI{True}} from \(\Phi\).
    ...
    \footnote{
      Indeed, it is built into \(\edn{+}\) being described as an event in which the agent concludes \pv{\propM{\rootsCon{}}}{\valI{True}} from \(\Phi\) that \(\edn{+}\) partly happens as a result of reasoning which amounts to steps~\ref{illu:gist:roots:F:eq}~to~\ref{illu:gist:roots:F:factor:done} of \autoref{illu:gist:roots:F}.
    }
    \medskip
  ...
  % In particular, most of the detail of \autoref{obs:se-inst} amounted to arguing it makes no sense for an agent to conclude \pv{\propM{\rootsCon{}}}{\valI{True}} from \(\Phi\) without the conclusion partly happening as a result of the agent figuring out \rootsConEqFac{}.
\end{note}
...
\begin{note}
  Before starting, recall we understand the sense of `why' present in \qWhy{} in terms of why did the event \(\ed{}\) in which an agent concludes some \prop{0} \(\phi\) has \val{0} \(v\) from some \pool{0} \(\Phi\) happen, rather than any incompatible event --- e.g., an event in which the agent failed to conclude \(\pv{\phi}{v}\) from \(\Phi\).
\end{note}
...
\begin{note}
  ...
  Specifically, \(\edo{\flat}\) was `the agent is concluding \pv{\propM{\rootsCon{}}}{\valI{True}} from \(\Phi\) in \(\edn{\flat}\) and figures out \rootsConEqFac{}'
...
\begin{note}
  ...
  Hence, given \autoref{obs:se-inst} the agent figuring out \rootsConEqFac{} explains why the agent concluded \pv{\propM{\rootsCon{}}}{\valI{True}} from \(\Phi\) in \autoref{illu:gist:roots:F}.
\end{note}
