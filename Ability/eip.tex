...
\begin{note}
  ...
  A key takeaway of this chapter is `\progEx{}' (\autoref{obs:PE}, \autopageref{obs:PE}) which characterises the sense of `why' present in \qWhy{} in terms of features of an event in progress.
  We sketch the way in which \progEx{} applies to an event in which an agent concludes happens, and return to the sketch in \autoref{cha:var} after the way in which we understand conclusions and \ros{1} has been clarified.
\end{note}
...
\begin{note}
  ...
  Some technicalities (such as our use of \prop{1}, \val{1}, and \pool{1}) constrain the relevant description, but something like `the agent concluded \gistCalcEq{} has value \valI{True} for the first time by using a calculator' is about the level of detail I have in mind, though what follows is compatible with additional detail.
\end{note}
...
\begin{note}
  ...
  \item
    \(d\) is true of \(e\) and \(d\) entails Max is concluding \propM{\gistCalcEq{}} is \valI{True}.
  \end{itemize}
...
\begin{note}
  A brief observation, which contrast with \autoref{assu:HaUniqueD}:
...
\begin{note}
  ...
  \noindent%
  We understand events in progress via the progressive, and \autoref{assu:p:ex} seems to follows from an understanding of the progressive.
  ...
  So, it is not possible for team A to be winning the game of hockey and for team B to be winning the game of hockey.
\end{note}
...
\begin{note}
  ...
  \footnote{
    See also \citeauthor{Bonomi:1997uq} for additional discussion.
  }
...
\begin{note}
  Though \autoref{assu:p:ex} captures a way in which our understanding of an event in progress may differ from the progressive aspect, \autoref{def:es-in-prog} remains a important intuition.
  For, we assume a common feature of analyses of the progressive holds of events in progress:%
  \footnote{
    See, e.g.:
    \cite{Bennett:1972uw},
    \cite{Dowty:1979vq},
...
\begin{note}[Interest with the progressive]
  A few observations to highlight some features.
  ...
  \begin{motivation}{obs:prog-not-cfs}
    ...
    Then, a classmate passes John some answers, which John uses.
    It is no longer true that John is passing the exam without external help.
    And, in the closest possible world where the classmate does not pass John answers, some other classmate may pass John the same answers.
  \end{motivation}
