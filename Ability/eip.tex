...
\begin{note}
  ...
  \begin{dets}{obs:se-inst}
    \color{blue}
    By assumption, \(\edn{\flat}\) is the event which covers Step~\ref{illu:gist:roots:F:factor} of the \agents{} reasoning in \autoref{illu:gist:roots:F}.
    So, \(\edn{\flat}\) is an event in which figures out \((2x + 1)(x - 1) = 0\).
    And, I take it to be sufficiently clear that \(\edn{\flat}\) is such that an event in which the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\) is in progress.
    Hence, the description \(\edo{\flat}\) which states:%
      \footnote{
        We fix a description such that clauses~\ref{assu:p:se:prog} and \ref{assu:p:se:hCon} are mostly immediate.
    ...
    \begin{quote}
      `The agent is concluding \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\) while figuring out \((2x + 1)(x - 1) = 0\)'.
    \end{quote}
    is true of \(\edn{\flat}\).
    \medskip

    \noindent%
    We observe clauses~\ref{assu:p:se:prog} and \ref{assu:p:se:hCon} of \autoref{def:se} hold.
    \medskip

    \noindent%
    Clause~\ref{assu:p:se:prog} is more-or-less immediate.

    For, \(\edo{}\) is the description `the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\)'.
    And, \(\edo{\flat}\) describes \(\edn{\flat}\) as an event in which the agent is concluding \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\).
    So, \(\edo{\flat}\) describes \(\edn{\flat}\) as an event in which an event under the description `the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\)' is in progress.
    Hence, \(\edn{}\) as described by \(\edo{}\) is in progress.%
    \footnote{
      Note, this is not the same as claiming \(\edn{}\) is in progress.
      For, \(\ed{}\) captures \(\edn{}\) under description \(\edo{}\), and hence \(\ed{}\) does not capture any features of \(\ed{}\) which are not entailed by \(\edo{}\).
    }
    \medskip

    \noindent%
    For Clause~\ref{assu:p:se:hCon} our goal is to show \(\ed{}\) happens --- in part --- as a result of \(\ed{\flat}\).

    Start with a slightly distinct claim:
    %
    Description \(\edo{\sharp}\) `figures out \((2x + 1)(x - 1) = 0\)'
    ...
    \item
      ...
      For any event \(\edn{+}\) such that \(\edo{}\) is true of \(\edn{+}\), there is some event \(\edn{\times}\) such that \(\edo{\sharp}\) is true of \(\edn{\times}\) and \(\ed[]{+}\) happens --- in part --- as a result of \(\ed[\sharp]{\times}\).
    \end{enumerate}

    The argument for \autoref{obs:se-inst:phew} is by contradiction.
    \medskip

    \noindent%
    Suppose \(\edn{+}\) such that \(\edo{}\) is true of \(\edn{+}\) but 

    Take arbitrary \(\edn{+}\).
    \(\edo{}\) describes \(\edn{+}\) as an event in which the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\).
    And, \(\Phi\) captures the \agents{} understanding of factorisation.

    In other words, \(\edo{}\) describes \(\edn{+}\) as an event in which the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} by factorisation.

    Now, there is no event \(\edn{\times}\) such that \(\edo{\sharp}\) is true of \(\edn{\times}\) and \(\ed[]{+}\) happens --- in part --- as a result of \(\ed[\sharp]{\times}\).

    This means either there is no event in which agent figures out \((2x + 1)(x - 1) = 0\).
    However, factorisation.
    This is not the relevant \pool{}.
    The description is not true.

    So, not a result.
    However, repeat reasoning.
    If not result then  description is not true.

    So, it must be the case there is some event \(\edn{\times}\) such that \(\edo{\sharp}\) is true of \(\edn{\times}\) and \(\ed[]{+}\) happens --- in part --- as a result of \(\ed[\sharp]{\times}\).
    \medskip
    ...
    Now, \autoref{obs:se-inst:phew} concerned arbitrary events.
    To complete the argument, observe \(\edn{}\) for \(\edn{+}\).
    So, with respect to \(\edn{}\) in particular.
    What remains is to observe there is no event other than \(\edn{\flat}\) for \(\edn{\times}\).

    I take this to be intuitive.

    Though we can do better.

    It is explicit in the \scen{} that there is no event after.
    So, event before.

    If you want a full argument there are two options.

    Additional argument that result of that event rather than this.
    Or, a retcon of THE SCENARIO to make explicit the agent has not reasoned through STEP before.
    \medskip

    \noindent%
    In short, if the agent did not conclude by factoring \(2x^{2} - x - 1 = 0\), it cannot be the case that the \agents{} concluded by their understanding of factorisation.
    The agent must have concluded \propM{\rootsCon{}} has \val{0} \valI{True} by some other \pool{}.
    \medskip

    Now, given previous claim, \(\edn{\flat}\) is the only event.
    This is given by scenario.
    For, there is no other event.
