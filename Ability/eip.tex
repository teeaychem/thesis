...
\begin{note}
  ...
  \begin{itemize}
  \item
    Clause~\ref{assu:p:se:prog} of \autoref{def:se} is satisfied as an agent figuring out \rootsConEqFac{} with the aim to identify the factors of \rootsConEq{} is clearly working towards an event in which the conclude \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\).
  \item
 ...
 \end{itemize}
  ...
  \begin{dets}{obs:se-inst}
    ...
    Further, as by \(\edo{\flat}\) the agent has the aim to identify the factors of \rootsConEq{} the previous observation may be strengthened to state an event in which the agent concludes \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\) \emph{is} be in progress.
    ...
    \end{enumerate}
    %
    In short, \ref{obs:se-inst:phew} states that an \agents{} conclusion of \propM{\rootsCon{}} has \val{0} \valI{True} from a \pool{} which captures the \agents{} understanding of factorisation prior to \(\edn{}\) partly happens as the result of the agent figuring out \rootsConEqFac{}.
    \medskip
  ...
  % In particular, most of the detail of \autoref{obs:se-inst} amounted to arguing it makes no sense for an agent to conclude \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\) without the conclusion partly happening as a result of the agent figuring out \rootsConEqFac{}.
\end{note}
...
\begin{note}
  ...
  Specifically, \(\edo{\flat}\) was `the agent is concluding \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\) in \(\edn{\flat}\) and figures out \rootsConEqFac{}'
...
\begin{note}
  ...
  Hence, given \autoref{obs:se-inst} the agent figuring out \rootsConEqFac{} explains why the agent concluded \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\) in \autoref{illu:gist:roots:F}.
\end{note}
