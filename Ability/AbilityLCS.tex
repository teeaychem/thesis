...
\begin{note}
  \autoref{prop:lcs} draws out a consequence of \ideaCS{}.
\end{note}
...
\begin{note}
  \begin{restatable}[\LCS{0}  --- \LCS{}]{proposition}{propLCS}
    \label{prop:lcs}
    \label{prem:ni}
    For an agent \(S\), propositions \(\phi\), \(\psi\), and values \(v\), \(v'\).
    ...
    \item
      ...
      \item
        \(\phi\) not having value \(v\) is \epVAd{}.
      \item
    ...
    \item
      \label{nI:inclusion}
      Suppose \(S\) contemplates some step \(\delta\) such that \(S\) is confident that:
      \begin{enumerate}[label=\alph*., ref=\named{\LCSAcro{}:2\alph*}]
      \item
  ...
  {
    \color{red}
    This is more or less the definition of a \crequ{}.
    The thing is teasing out the relevant structure.
    This is straightforward.

    The other, is reasoning to \(\psi\).
    Here, rather than direct, the reasoning is drawn through \epVW{}.\nolinebreak
    \footnote{
      Narrowing down \ideaCS{} has led to the force of this proposition being somewhat blunted.
    }
  }
\end{note}

\begin{note}
  If \crequ{}, then no claiming support.
  In particular, claim support for \(\psi\)
\end{note}
