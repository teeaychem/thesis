...
\label{sec:claiming-support}

\begin{note}
  \begin{restatable}[Claiming support and how things are]{assumption}{assuCSWordly}
    \label{assu:CSWordly}
...
\begin{note}[]
  ...
  \begin{restatable}[\ideaCSB{0} --- \ideaCSB{}]{idea}{ideaEIS}
    ...
    \begin{enumerate}
    \item indicates that \(\phi\) has value \(v\)
    \item indicates that any proposition-value pair appealed to in reasoning is the case.
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{restatable}
...
\begin{note}
  From~\autoref{assu:CSWordly}, claiming support is reasoning about how things are.
  \ideaCSA{} narrows down claiming support to proposition-value pair which are \ep{}.
  Hence, claiming support is a weak attitude\dots

  \ideaCSB{} adds strength\dots
\end{note}
...
\begin{note}
  Indication is a variable, and we allow for really weak interpretation.
  Some positive account of why \(\psi\) has value \(v\) over any other possible value.

  In particular, indication may fall short of justification.
...
\begin{note}
  ...
  \item relation to \autoref{assu:CSWordly}.
  \end{itemize}
\end{note}
...
\begin{note}
  ...
  Problem here is that whether the reasoning is an instance of claiming support depends on factors external to the epistemic state of the agent.

  In particular, given \ideaCSA{}, factors which the agent thinks are \ep{}.
  Reject this.
  Then:
  \begin{enumerate}
  \item Permissible to appeal to \(\phi\) having value \(v\) as a premise and for \(\phi\) not to have value \(v\).
  \end{enumerate}
  Question is why is this permissible?

  This we don't need an detailed answer to.
  We need is that it is possible for \(\phi\) to not have value \(v\), but for it to be okay for the agent to appeal to \(\phi\) having value \(v\).
  Here, some reasoning that \emph{indicates} that \(\phi\) has value \(v\).

  Where, indication is some reasoning which suggests that \(\phi\) has value \(v\) that is compatible with \(\phi\) not having value \(v\).
  Where, compatible just reduces to not requiring that \(\phi\) has value \(v\).
\end{note}
