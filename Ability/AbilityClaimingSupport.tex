...
\begin{note}[Support]
  ...
  \ideaCSA{} states, roughly, that an agent has \support{} for \(\phi\) having value \(v\) \emph{only if} the agent would conclude that \(\phi\) has value \(v\) from any \epPW{0}.
...
\begin{note}[How support matters]
  ...
  In particular, incompatible because the step of reasoning only applies to certain \epVW{1}.
  Further constraint would be that current \epVW{1} are sufficiently `robust'.
...
\subsection{\ideaCSB{0}}
\label{sec:ideaCSB}


\subsubsection{\ideaCSB{1}}
\label{sec:ideaCSB}
...
\begin{note}[Intuition for \ideaCSB{}]
...
\begin{note}
  \color{red}
  Introduce the \emph{idea} of \requ{1} here, and then use for \ideaCSB{} and \ideaCSC{}.
