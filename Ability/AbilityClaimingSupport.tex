...
\begin{note}
  ...
  \item
    \label{CS:delicacy:A}
    That concludes \(\phi\) has value \(v\) \emph{assuming} \dots\space --- where `\emph{assuming} \dots\space' is expanded to state various proposition-value pairs which do not (jointly) hold for any \epVW{}.
  \end{enumerate}
\end{note}
...
\begin{note}[Epistemic operator]
  ...
  \begin{quote}
    \vAgent{} \(\{\) hopes, imagines, desires, thinks \dots \(\}\) \(\phi\) is true.
  \end{quote}
...
\begin{note}
  \begin{restatable}[\indicateN{2}]{definition}{defIndicate}
    ...
    \begin{itemize}
    \item It is not \epPAd{}, relative to \vAgent{}' epistemic state, \(\psi\) has value \(v'\) while \(\psi\) does not have value \(v'\).
    \end{itemize}
    \vspace{-\baselineskip}
  \end{restatable}
\end{note}
