...
\begin{note}
  \begin{restatable}[\ideaCSA{0} --- \ideaCSA{}]{idea}{iCSA}
    ...
    An agent \emph{S} claims support that \(\phi\) has value \(v\) \emph{only if} \(\phi\) having value \(v\) is a \sink{} with respect to the agent's current epistemic state with no constraints.
\end{restatable}
\end{note}
  ...
  \begin{definition}[A \sink{0}]
    ...
    \item
      ...
      \item
        ...
        \begin{enumerate}[label=\roman*., ref=(1b\roman*)]
        \item
...
\begin{note}
  ...
  Clause \ref{def:sink:restrict:phi:get} of \autoref{def:sink} specifies a base case, while clause \ref{def:sink:restrict:psi} specifies the relevant recursive step.
\end{note}
...
\begin{note}
  In order to ensure the agent has some way of concluding that \(\phi\) has value \(v\), the quantification of all \epN{} \world{1} is important.
...
\begin{note}
  ...
  \footnote{
    I implicitly assume that relaxing an epistemic state does not introduce novel \epN{} \world{1}.

    Still, if this assumption is discarded, then the collection of constraints is somewhat interesting.
    ...
    However, \eState{}{'} allows that \(\psi_{2}\) not having value \(v_{2}\) as a novel \epAdv{}.
    ...
    Hence,~\autoref{def:sink} implicitly rules out certain novel \epN{} \world{1}.
    ...
    Depending on your intuition (if you have any) then the definition may need to be revised to extend relaxing to the collection of proposition-value pairs.
    However, as I have no clear intuition regarding such a possibility and as excluding such a possibility will increase the complexity of the definition, I leave the assumption implicit.
  }
\end{note}
