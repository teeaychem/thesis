...
\subsubsection{Proposition and evaluation}
...
\subsubsection{`Confined' reasoning}
...
\begin{note}[]
  ...
  \begin{restatable}[\ideaCSB{0} --- \ideaCSB{}]{idea}{ideaEIS}
    \label{idea:eiS}
    An instance of reasoning is an instance of claiming support just in case the reasoning is about how things are.

    In particular:

    An instance of reasoning that concludes that \(\phi\) has value \(v\) is an instance of claiming support for \(\phi\) having value \(v\) \emph{only if}:
    \begin{enumerate}
    \item The instance of reasoning indicates that \(\phi\) has value \(v\).
    \item For any proposition-value pair \(\psi\) having value \(v'\) appealed to in reasoning, the agent has indication that \(\psi\) has value \(v'\).
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{restatable}
...
\begin{note}
  Given \ideaCSA{}.
  \ep{1}.
  So, possible (and in particular \possibilityAdv{}) that conclusion or some proposition-value pair appealed to does not have value.

  \autoref{idea:eiS} has two components.
  \begin{enumerate}
  \item Indication.
  \item Appeal.
  \end{enumerate}
...
\paragraph{How things are}

\begin{note}[Core constraint]
...
\begin{note}
  ...
  \begin{figure}[h!]
    ...
    \begin{subfigure}{0.45\linewidth}
      ...
      \end{enumerate}
      \caption{(Possibly) claiming support}
      \label{fig:Rover:CS}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.45\linewidth}
...
\begin{note}
  ...
  \autoref{fig:Rover:nCS} is not an instance of claiming support because the premise does not concern the actual state of Rover, and hence the conclusion (also) does not concern the actual state of Rover.
  Of course, if Rover is (in fact tired) then it may be the case that Rover will fall asleep soon, but is it possible for the agent to obtain~\ref{fig:Rover:nCS:2} from~\ref{fig:Rover:nCS:1} while maintaining that Rover is not tried.

  By contrast,~\autoref{fig:Rover:CS} is (possibly) an instance of claiming support because the reasoning is about how things are.
  ...
  The `(Possibly)' qualification is added for the simple reason that reasoning being about how things are is not itself sufficient for the reasoning to be an instance of claiming support.
\end{note}

\begin{note}
  Explicit supposition.
  However, extend to implicit supposition.

  No indication that some premise is the case.
  And, no indication that some proposition-value pair appealed to is the case.

  Begin by developing with {\color{red} N} implicit ideas, focusing on conclusion.
  Then extend to implicit ideas covering other aspects of reasoning.
\end{note}

\paragraph{Implicit ideas}

\begin{note}
  \begin{implicitIdea}
    If \(\phi\) not having value \(v\) is an \ep{}, then reasoning \indicateV{1} that \(\phi\) has value \(v\) only if \indicateV{1} even if it happens to be the case that \(\phi\) does not have value \(v\).
  \end{implicitIdea}

  Implicit idea does not provide any analysis of `\indicateV{0}'.
  However, informative in that it shows preserved under strengthening.

  Consider Rover.
  It is quite possible that Rover will not, in fact, fall asleep soon.
  However, that Rover is tired suggests that Rover will fall asleep soon, even if Rover does not.
\end{note}
...
\begin{note}
  ...
  \begin{figure}[h!]
    ...
    \begin{subfigure}{0.45\linewidth}
      \begin{enumerate}
      \item The die has rolled even if \(5251\) of \(6000\) samples.
    ...
    \caption{}
    \label{fig:ideaCSB:basic-examples}
  \end{figure}

  Intuitive `even if\dots' test.

  \begin{enumerate}[label=\ref{fig:ideaCSB:basic-examples}\alph*., ref=(\ref{fig:ideaCSB:basic-examples}\alph*)]
...
\subparagraph{Sensitivity}

\begin{note}
  Contrast to sensitivity
  \begin{enumerate}
  \item If \(\phi\) were not to have value \(v\), \(S\) would not reason to \(\phi\) having value \(v\).
  \end{enumerate}
  I.e.\ reasoning only in good cases.

  No.

  \begin{enumerate}
  \item If \(\phi\) were not to have value \(v\), \(S\)' reasoning would (still) indicate that \(\phi\) has value \(v\).
  \end{enumerate}

  This seems weaker, though implied.
  For, if not, then I wouldn't reason, and so as I have reasoned, then because I think I would not have if \(\phi\) was not the case, this suggests that \(\phi\) is the case.
\end{note}

\paragraph{Second implicit}
...
\begin{note}
  ...
  For, if an instance of claiming support, then \(\phi\) not having value \(v\) is an \ep{}.
  Hence, if the agent's reasoning only indicates that \(\phi\) has value \(v\) if it is in fact the case that \(\phi\) has value \(v\), then the agent's reasoning does not \indicateV{0} that \(\phi\) has value \(v\) in the \epAdv{} state of affairs in which \(\phi\) does not have value \(v\).
\end{note}


\paragraph{Extends to premises}

\begin{note}
  As noted with `\emph{supposing}'.
  So, the implicit ideas apply not only to conclusion, but also to premises.
\end{note}

\begin{note}
  With Rover, appearance of being tired.
  That this is not the case is not an \ep{}.
  Then, with figure 4, arguably the premises are not \ep{1}.

  This gives one way.
\end{note}

\begin{note}
  However, variations on the Rover case.
\end{note}

\paragraph{Beyond premises}

\begin{note}
  Also, other proposition-values that are appealed to.
\end{note}

\begin{note}
  Issue is that this is now a strong requirement.
  However, the core idea remains.
  To the extent that these proposition-value pairs are important for the reasoning to be about how things are, if they fail to have value, then problem.
\end{note}
...
\begin{note}
  `Appeal'.
  From the examples given, the scope of appeal is not clear.
  That is, there is no clear bound.
  Depending on how you view premises or commitments of the agent.
\end{note}

\begin{note}
  Presents a difficulty.
  For, given scope of appeal, it seems, perhaps, implausible for the agent to either reason or have reason about all the proposition-value pairs appealed to.

  So,
  \begin{itemize}
  \item explicit reasoning,
  \item recognition,
  \item access.
  \end{itemize}

  However, \indicateO{0} is such that it does not require that the agent recognises.

  What we're interested in is a this core intuition.
  It doesn't follow that the agent needs to recognise that they have satisfied the core intuition.
  ...
  Do not need to recognise that I have reasoned about any proposition-value pair appealed to in order to have claimed support.
...
\begin{note}
  Important thing is don't assume a truth connexion.
  It being the case that \(\phi\) has value \(v\) is not taken to be important.
  This may be the case, and you may view taking with the addition of this idea.
  However, is not assumed.
\end{note}
...
\begin{note}
  Key restriction is that no indication for \(\psi\) having value \(v'\) if reasoning involves appeal to \(\psi\) having value \('\).

  Extend implicit ideas to proposition-value pairs appealed to.

  \begin{implicitIdea}
    Key thing is that positive for \(\psi\) having value \(v'\) even if \(\psi\) does not have value \(v'\).
  \end{implicitIdea}


  \begin{implicitIdea}
    Not if requires \dots
  \end{implicitIdea}
...
\begin{note}[The scope of appeal]
  Broad.
\end{note}
...
\begin{note}
  \begin{enumerate}
  \item Permissible to appeal to \(\phi\) having value \(v\) only if \(\phi\) has value \(v\).
  \end{enumerate}

  The notion of support here is that some things are a certain way, and therefore, some other thing is a certain way.
  Not the weaker: If things are a certain way, then therefore some other thing is a certain way.

  In other words, sound.

  If \ep{} that \(\phi\) does not have value \(v\), then this carries a risk.
  For, it may be that \(\phi\) does not have value \(v\), and therefore it may be that the agent violates the ideal.

  \begin{enumerate}
  \item Permissible to appeal to \(\phi\) having value \(v\) only if think it is the case that \(\phi\) has value \(v\).
  \end{enumerate}
  In other words, \emph{subjectively} sound.
\end{note}
...
\subsection{Appeal: \result{3} and \requ{1}}
  ...
  \begin{restatable}[\eiS{0} --- \eiS{}]{assumption}{assuCSRReq}
    ...
    \item
      ...
      \item
        ...
        \begin{itemize}
        \item
          The reasoning of \ref{assu:supp:ind:indicate} did or does not involve appeal to a proposition-value pair that is only obtained from the step of reasoning and \crequ{1} which leads to \(\psi\) having value \(v'\) being a \prequ{}.
        \end{itemize}
      \end{enumerate}
    \end{enumerate}
    \vspace{-\baselineskip}
...
\begin{note}
  ...
  For, \autoref{assu:supp:independence} requires that the agent either does or has reasoned about any \requ{0} of a step of reasoning if they are claiming support, while meta-proposition-value pairs suggest that this requirement is either unreasonable due to the abundance of proposition-value pairs which follow from a source proposition-value pair, or implausible due to the existence of proposition-value pairs regarding the step of reasoning itself.

  We briefly elaborate on two ways in which the concern may be raised and offer brief responses.
\end{note}

\subparagraph*{Meta-proposition-value pairs: Proposition-value pairs}
