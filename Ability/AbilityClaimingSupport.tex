...
\begin{note}
  ...
  \begin{restatable}[\requ{3} of (making a step of) some instance of reasoning]{idea}{ideaRequisite}
    A requ{} is a proposition-value pair which
    \begin{enumerate}
    \item Follows from some step or steps.
    \item Would not get conclusion without.
    \item \ep{}
    \end{enumerate}
  \end{restatable}
  Difficulty is the second.
  Would not get conclusion without.
...
\paragraph{\result{3} of (making a step of) some instance of reasoning}

\begin{note}
  \begin{restatable}[\result{3} of (making a step of) some instance of reasoning]{definition}{defResult}
    \label{def:result}
    \(\psi\) has value \(v'\) is \result{} of a step of reasoning that concludes that \(\phi\) has value \(v\) (from an agent's perspective) if and only if:
    \begin{enumerate}
    \item
      \label{def:requi:not-v-poss}
      \(\psi\) not having value \(v'\) is epistemically possible.
    \item\label{def:requi:prop}
      \begin{enumerate}
      \item
        \label{def:requi:prop:conseq}
        Follows as a result of making the step of reasoning no possible \(\psi\) no have value \(v'\).
      \item
        \label{def:requi:prop:persists}
        No other part of reasoning shows that \(\phi\) has value \(v\) which does not involve a step for which \(\psi\) having value \(v'\) follows as a making step. (Persists).
      \end{enumerate}
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{restatable}
...
\begin{note}
  ...
  Here, consider cases.
  Does not follow from step if some other case for which no step.
  Still, each case.
  If all of these.
\end{note}

\paragraph{\requ{3} of a step of reasoning}

\begin{note}
  With~\autoref{def:result} in hand, we now turn to \requ{1}.
  Start with two basic types of \requ{}: \prequ{} and \crequ{}.
  A final more complex kind of \prequ{}, a \cprequ{}.
  Then, close with general definition of a \requ{}.
\end{note}

\begin{note}
  These definitions include a general clause.
  This is a circular definition, but uninteresting.
  Expands to a disjunction of cases.
  Hence, upshot of circularity is infinite disjunction of three cases.
  Which is equivalent to the three cases.
\end{note}

\begin{note}
  \begin{restatable}[\prequ{2} of a step of reasoning]{definition}{defRequisiteP}
    \label{def:requisite:p}
    \(\psi\) has value \(v'\) is a \emph{\prequ{0}} of a step of reasoning if and only if
    \begin{enumerate}
    \item \(\psi\) has value \(v'\) is a \result{0} of a that step of reasoning.
    \item\label{def:requi:subjunctive} If \(\psi\) were not to have value \(v'\) then that step of reasoning would involve:
      \begin{enumerate}
      \item\label{def:requi:subjunctive:p} Appealing to something that is not the case.
      \end{enumerate}
    \item Any other, also a \requ{}.
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{restatable}
\end{note}
...
\begin{note}
  ...
  Key idea is that relevance to reasoning by reasoning introducing those premises.
  Simple example.
  Some independent belief.
  Not a \prequ{}.
  Add in link.
  Now a \prequ{}.
  However, some other premises, then not a \prequ{}.
\end{note}

\begin{note}
  Important things.

  First, may prefer to give up other information that leads to \(\phi\) having value \(v'\).
  If so, not a \requ{}.

  Second, though \result{} has global clause, way introduced may be different.
  Cases.
  Two different links.
  Hence, \ref{def:requi:subjunctive} may have different conditions.

  \result{1}.
  However, \prequ{} does not require that for any other step, also following from premises.
\end{note}

\begin{note}
  \begin{restatable}[\crequ{2} of a step of reasoning]{definition}{defRequisiteC}
    \label{def:requisite:c}
    \(\psi\) has value \(v'\) is a \emph{\crequ{0}} of a step of reasoning if and only if
    \begin{enumerate}
    \item \(\psi\) has value \(v'\) is a \requ{0} of a that step of reasoning.
    \item\label{def:requi:subjunctive} If \(\psi\) were not to have value \(v'\) then that step of reasoning would involve:
      \begin{enumerate}
      \item\label{def:requi:subjunctive:c} Moving to something that is not the case.
      \end{enumerate}
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{restatable}
...
\begin{note}
  In cases of purely deductive reasoning, no different from a \prequ{}.

  In other cases of reasoning, distinct from a \prequ{}.
  Making the step adds something extra.
  For example, in strengthening cases.
\end{note}

\begin{note}
  \begin{restatable}[\cprequ{2} of (a step of) reasoning]{definition}{defRequisiteCP}
    \label{def:requisite:cp}
    \(\psi\) has value \(v'\) is a \emph{\cprequ{0}} of (a step of) reasoning \(\delta\) iff
    \begin{enumerate}
    \item \(\chi\) having value \(v''\) is a \crequ{0} of a the step of reasoning \(\delta\).
    \item \(\chi\) having value \(v''\) then \(\psi\) has value \(v'\) follows from premises of \(\delta\) together with \(\chi\) having value \(v''\).
    \item 1 and 3 of result hold.
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{restatable}
\end{note}
...
\begin{note}
  Special case of a \prequ{}.
  Something new from premises when combined with 

  Here, only when the conclusion is introducing something new.
  So, no \cprequ{1} in cases of purely deductive reasoning.
\end{note}
...
\begin{note}
  \autoref{def:requisite} is how we build on \ideaCSA{} and \ideaCSB{}.
  ...
  Still, as \autoref{def:requisite} is quite complex, we will first work through the clauses and how they combine in some detail before providing a handful of illustrations.
\end{note}
...
\begin{note}
  The clauses are arranged as follows:
  \begin{itemize}
  \item Clause \ref{def:requi:prop} states how \(\psi\) having value \(v'\) relates to the instance reasoning.
  \item Clause \ref{def:requi:not-v-poss} highlights the relevance of subjective conditional of \ref{def:requi:subjunctive}.
  \item Clause \ref{def:requi:subjunctive} states a specific way in which \(\psi\) having value \(v'\) matters to the instance of reasoning.
  \end{itemize}
  % As such, our focus will be on clauses~\ref{def:requi:prop}~and~\ref{def:requi:subjunctive}.\nolinebreak
  % \footnote{
  %   Clause \ref{def:requi:not-v-poss} is separated from \ref{def:requi:subjunctive} simply to ease readability.
  % }
\end{note}

\subparagraph*{Clause \ref{def:requi:prop}}

\begin{note}[Clause \ref{def:requi:prop}]
  Clause \ref{def:requi:prop} requires two properties of \(\psi\) having value \(v'\).

  First, sub-clause~\ref{def:requi:prop:conseq} requires that \(\psi\) having value \(v'\) is a result of making the step of reasoning.
  Second, sub-clause~\ref{def:requi:prop:persists} requires that \(\psi\) having value \(v'\) persists through to the conclusion of the instance of reasoning.

  We take each sub-clause in turn.
\end{note}

\subparagraph*{Sub-clause~\ref{def:requi:prop:conseq}}

\begin{note}
  Starting with sub-clause~\ref{def:requi:prop:conseq}, emphasis is on \(\psi\) having value \(v'\) as a result of \emph{making} the step of reasoning.

  Suppose the step of reasoning concludes with \(\alpha\) having value \(v''\).
  And, suppose the agent knows that \(\alpha\) has value \(v''\) only if \(\beta\) has value \(v'''\).
  Then, \(\beta\) having value \(v'''\) follows from the agent making the step of reasoning.

  Suppose the agent has good inductive reasons for thinking that \(\alpha\) has value \(v''\).
  The step of reasoning involves moving from those good inductive reasons to holding that \(\alpha\) has value \(v''\).
  And, suppose as before the agent knows that \(\alpha\) has value \(v''\) only if \(\beta\) has value \(v'''\).
  Then, \(\beta\) having value \(v'''\) follows from the agent making the step of reasoning.

  The term `follows as a result' is quite general.
  I doubt that it is possible, in general, to provide a clear account of what this amounts to.
  Further, `follows from as a result' is relative to the agent's perspective.
  We will try to keep any instance of a \requ{} as uncontroversial as possible.
\end{note}

\subparagraph*{Sub-clause~\ref{def:requi:prop:persists}}

\begin{note}
  Turning to sub-clause~\ref{def:requi:prop:persists}, by `persistence' we relate the step of reasoning to the instance of reasoning more broadly.
  From sub-clause~\ref{def:requi:prop:conseq}, it is a necessary condition of \(\psi\) having value \(v'\) being a \requ{} that \(\psi\) having value \(v'\) follows from a making some step.
  However, we also wish to ensure that \(\psi\) having value \(v'\) matters with respect to the instance of reasoning as a whole.
  ...
  In the first, the step of reasoning concludes with \(\alpha\) having value \(v''\).
  Suppose the agent goes on to observe that \(\alpha\) having value \(v''\) only if \(\gamma\) has value \(v''''\), and concludes their reasoning with \(\gamma\) having value \(v''''\).
  Then, as \(\beta\) having value \(v'''\) followed as a result of \(\alpha\) having value \(v''\), and the agent obtained \(\gamma\) having value \(v''''\) from \(\alpha\) having value \(v''\), \(\beta\) having value \(v'''\) persists through to the conclusion of reasoning.

  The same observation also holds for the second example.

  By contrast, consider an instance of reasoning by cases.
  The agent reasons about \(\alpha\) having value \(v''\) and \(\alpha\) not having value \(v''\) and observes that \(\beta\) has value \(v'''\) follows regardless.
  In the first case, \(\alpha\) having value \(v''\) (trivially) follows as a result of assuming that \(\alpha\) having value \(v''\), but \(\alpha\) having value \(v''\) does not persist through to the conclusion of the instance of reasoning as the agent discharges their assumption that \(\alpha\) has value \(v''\) as part of reasoning by cases.
\end{note}

\subparagraph*{Clause \ref{def:requi:not-v-poss}}

\begin{note}[Clause \ref{def:requi:not-v-poss}]
  Clause \ref{def:requi:not-v-poss} highlights the relevance of subjective conditional of \ref{def:requi:subjunctive}:
  The subjunctive antecedent of clause~\ref{def:requi:subjunctive} concerns \(\psi\) not having value \(v'\), and clause~\ref{def:requi:not-v-poss} simply states that \(\psi\) not having value \(v'\) is an epistemic possibility.

  The explicit assumption is important for two reasons.

  First, we have no interest in a consequence of reasoning if there is no epistemic possibility of subjunctive of clause \ref{def:requi:subjunctive} obtaining.
  The purpose of identifying \requ{1} is to highlight features of an agent's reasoning which are relevant to whether the reasoning indicates that \(\phi\) having value \(v\) is the case.

  Second, that \(\psi\) may not have value \(v'\) is an epistemic possibility ensures that the subjunctive conditional of clause \ref{def:requi:subjunctive} may be evaluated from the agent's epistemic state when making the relevant step of reasoning.\nolinebreak
  \footnote{
    Clause~\ref{def:requi:subjunctive} could be expanded to apply in cases of epistemically impossible proposition-value pairs, but it is not clear why such epistemically impossible proposition-value pairs would matter to the agent's reasoning.
    }
\end{note}

\subparagraph*{Clause~\ref{def:requi:subjunctive}}

\begin{note}
  With clause~\ref{def:requisite} we stated how \(\psi\) having value \(v'\) relates to the agents reasoning.
  With clause~\ref{def:requi:subjunctive} we state a specific way in which \(\psi\) having value \(v'\) matters to the instance of reasoning.\nolinebreak
  \footnote{
  May think that clause \ref{def:requi:prop:conseq} follows from \ref{def:requi:subjunctive}.
  For, \(\lnot p \leadsto \lnot q\), \(\lnot p \rightarrow \lnot q\), so \(q \rightarrow p\).
  However, this is not quite right.

  Not appeal to something that is not the case, then it is not the case that \(\psi\) were not to have value \(v'\).

  For this to work, would need that in appealing to something, also appealing to it not being the case that one is appealing to something that is not the case.
  This does not obviously follow.
  Return to this with illustration.
  }
\end{note}
...
\begin{note}
  ...
  A step of reasoning concludes with \(\alpha\) having value \(v''\) such that the conclusion of the instance of reasoning is obtained in part from \(\alpha\) having value \(v''\).
  And, the agent is confident that if \(\alpha\) has value \(v''\) then \(\psi\) has value \(v'\) (and if \(\psi\) does not has value \(v'\) then \(\alpha\) does not have value \(v''\)).
  Further, the agent considers is epistemically possible that \(\psi\) does not have value \(v''\).
  So, clauses~\ref{def:requi:prop} and~\ref{def:requi:not-v-poss} are both satisfied.

  However, if the agent were to consider the case that \(\psi\) does not has value \(v'\), then the agent would consider this to indicate that \(\alpha\) may have value \(v''\) (even) when \(\psi\) does not has value \(v'\), rather than considering that \(\alpha\) would not have value \(v''\).

  In other words, step of reasoning concludes with \(\alpha\) having value \(v''\), and it is only \(\alpha\) having value \(v''\) that matters with respect to obtaining the conclusion of the instance of reasoning.
  And, \(\psi\) has value \(v'\) follows as a result of the step of reasoning as the agent is confident that if \(\alpha\) has value \(v''\) then \(\psi\) has value \(v'\).
  However, as the agent is more confident with the step of reasoning concluding with \(\alpha\) having value \(v''\) over the conditional from \(\alpha\) having value \(v''\) to \(\psi\) having value \(v'\), \(\psi\) not having value \(v'\) would not indicate to the agent that there is any issue with the step of reasoning.

  In this respect, clause~\ref{def:requi:subjunctive} ensures that \(\psi\) (not) having value \(v'\) matters with respect to making the step of reasoning.
\end{note}

\begin{note}
  Still, clause~\ref{def:requi:subjunctive} is light on the way in which \(\psi\) (not) having value \(v'\) matters.
  Sub-clause~\ref{def:requi:subjunctive:p} involves the agent appealing to something that is not the case, while sub-clause~\ref{def:requi:subjunctive:c} involves the agent moving to something that is not the case.
  In other words, the step of reasoning involves either appealing to some premise which would not be the case, or establishing some sub-conclusion that is not the case.

  With respect to the example in play, \(\psi\) (not) having value \(v'\) would either lead to \(\alpha\) not having value \(v''\), or some premise of the step of reasoning \(\xi\) not having \(v'''\).

  In both cases, we return to the idea of an \emph{undercutting} defeater.
  If \(\alpha\) does not have value \(v''\) then the agent's reasoning for \(\phi\) having value \(v\) is undercut by appeal to some premise that is would not be the case.
  And, if \(\xi\) does not have value \(v'''\) then, likewise, the agent's reasoning for \(\phi\) having value \(v\) is undercut by appeal to some premise that is would not be the case.
\end{note}
...
\begin{note}
  \begin{restatable}[\mistaken{0} and \misled{}]{definition}{defMoM}\label{def:MoM}
    Some step of reasoning such that reasoning culminates with \(\phi\) having value \(v\).
    \begin{itemize}
      \item The reasoning is \emph{\mistaken{}} if \(\psi\) having value \(v'\) is a \prequ{} and \(\psi\) does not have value \(v'\).
    \item The reasoning is \emph{\misled{}} if \(\psi\) having value \(v'\) is a \crequ{} and \(\psi\) does not have value \(v'\).
    \end{itemize}
    \vspace{-\baselineskip}
  \end{restatable}
