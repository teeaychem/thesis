...
\begin{note}[Statement of a \sink{0}]
  ...
  \begin{restatable}[A \sink{0}]{notion}{defSink}
    ...
    \item
      ...
      \item
        There is no other proposition-value pair \(\psi\) having value \(v'\) such that \vAgent{} would conclude that \(\psi\) has value \(v'\).
      \end{enumerate}

      Else:
      \begin{enumerate}
      \item {\color{red} This does the work of the next clause. For, either conclude \(\phi\) or make some other conclusion.}
        For any other proposition-value pair \(\psi\) having value \(v'\) such that \vAgent{} would conclude that \(\psi\) has value \(v'\),
        \begin{enumerate}
        \item \(\phi\) is a \sink{} with respect to \(\eState{\vAgent{}}[]\) and the collection \(\Phi\) \emph{combined with} \(\psi\) having value \(v'\).
        \end{enumerate}
      \end{enumerate}
      Or:
...
\begin{note}[Key idea of \sink{}]
  The key idea of some proposition-value pair \(\phi\) having value \(v\) being a \sink{} is that for any \epPAd{} \world{} that the agent may entertain, the agent both has some way of concluding that \(\phi\) has value \(v\), and would conclude that \(\phi\) has value \(v\).
...
\begin{note}
  ...
  In each instance, it seems plausible that the agent may conclude the conclusion from the premise, and further, would continue to maintain the premise even if the conclusion is not the case.
...
\begin{note}[Notion: \requ{2}]
  ...
  \begin{restatable}[A \requ{0}]{notion}{notionRequisite}
    ...
    \item
      ...
      It is not \epVAd{} for agent to conclude \(\phi\) has value \(v\) from \(\delta\) without requiring that \(\psi\) hasn't value \(v'\).
    \item
      \label{notion:requ:possible-psi}
      If step of reasoning  it is \epVAd{} for the agent to conclude that \(\psi\) has value \(v'\) prior to making \(\delta\), and without the conclusion of \(\delta\) given their present epistemic state.
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{restatable}
\end{note}
...
\begin{note}
  ...
  First, however, we will consider the intuition supporting the notion of a \requ{}.

  Antecedent check on whether the agent would conclude that \(\phi\) has value \(v\) regardless of the reasoning the agent may perform from their current epistemic state.
  Or, in other words, regardless of how the agent may refine their present epistemic state.
...
\begin{note}
  ...
  Two cases.
  Either the agent reasons about whether \(\psi\) has value \(v'\) and concludes that \(\psi\) \emph{does} have value \(v'\).
  Or, the agent reasons about whether \(\psi\) has value \(v'\) and \emph{fails} to conclude that \(\psi\) has value \(v'\).

  In first case, conclude that \(\psi\) has value \(v'\), hence \(\psi\) not having value \(v'\) would not be \epVAd{}.
  And, so though \(\phi\) needs \(\psi\), the agent will already have \(\psi\).

  The first case is not, however, of particular interest.
  Rather, it is the second case that is important.

  Simple case, conclude that \(\psi\) does not have value \(v'\).
  Then, no \epVW{} where \(\phi\) has value \(v\).
  Hence, the agent would not conclude that \(\phi\) has value \(v\).
