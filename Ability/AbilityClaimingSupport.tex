...
\begin{note}[Statement of a \sink{0}]
  ...
  \begin{restatable}[A \sink{0}]{notion}{defSink}
    ...
    \item
      ...
      \item
        There is no other proposition-value pair \(\psi\) having value \(v'\) such that \vAgent{} would conclude that \(\psi\) has value \(v'\) and \(\phi\) is not a \sink{} with respect to \(\eState{\vAgent{}}[]\) and the collection \(\Phi\) \emph{combined with} \(\psi\) having value \(v'\).
      \end{enumerate}
      Else:
