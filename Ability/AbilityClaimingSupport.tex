  ...
  \begin{restatable}[\ideaCSB{0} --- \ideaCSB{}]{idea}{ideaEIS}
    ...
    An instance of reasoning that concludes that \(\phi\) has value \(v\) is an instance of claiming support for \(\phi\) having value \(v\) \emph{only if}:
      ...
      \item
        ...
        Reasoning past or present that \indicateV{1} that \(\psi\) has value \(v'\), relative to current \ep{1}.
      \end{enumerate}
    \vspace{-\baselineskip}
  \end{restatable}
\end{note}
...
\begin{note}
  ...
  \begin{idea}
    Does not require the agent to recognise that reasoning indicates.
  \end{idea}

  This is to ensure that \indicateO{0} is not excessive.
\end{note}
...
\paragraph{The core of the assumption}
...
\begin{note}
  A step from \(X\) to \(\psi\) is impotent just in case:
  \(X\) from \(Y\).
  \(X\) from \(Y\) only if \(\psi\).
  Possible not \(\psi\).
\end{note}

\begin{note}
  So, the step brings out something that is already required to be the case from previous steps.
  If so, then this doesn't tell us anything about whether \(\psi\) has value \(v'\).
  For, \(X\) only when \(\psi\) has value \(v'\).
\end{note}

\begin{note}
  Motivated by the ideal of claiming support.
\end{note}
