...
\begin{note}[Expectation, assumption]
  ...
  \begin{restatable}[\expec{3} of reasoning]{definition}{defExpectation}
    \label{def:expectation}
    If claimed support for \(\phi\) having value \(v\) and then come to consider \(\psi\) has value \(v'\) as \requ{} of reasoning that \(\phi\) has value \(v\), then \(\psi\) has value \(v'\) is an \emph{\expec{}} of the reasoning for \(\phi\) having value \(v\).
  \end{restatable}
...
\begin{note}
  \begin{restatable}[\expec{3} and claiming support]{assumption}{assuCSRExp}
    \label{assu:independence-expec}
    Appeal to reasoning for \(\psi\) having value \(v'\) such that \(\xi\) having value \(v''\) is an \expec{} of that reasoning for \(\psi\) having value \(v'\).

    Then:

    Claiming support for \(\phi\) having value \(v\) involves:
    \begin{itemize}
    \item Reasoning about \(\xi\) having value \(v''\) as \expec{} to \(\psi\) having value \(v'\), {\color{red} given that \(\xi\) may not have value \(v''\)}.
      \vspace{-\baselineskip}
    \end{itemize}
  \end{restatable}
...
\begin{note}
  ...
  However, reasoning does not address the \expec{}.

  {\color{red}
    Again, definition of a \requ{}.
    This motivates the second part of the clause.
    As with \requ{1}, focus for the moment will be on reasoning.
    Then, with \nI{} this second part will be important.
  }
\end{note}
...
\begin{note}[Spot the difference]
  ...
  The reasoning is mentioned but not stated in the \illu{0}.
...
\begin{note}
  \illu{3} \ref{illu:CS:spot-the-diff} and \ref{illu:CS:wheres-wally} looked at \expec{1} and reasoning.
