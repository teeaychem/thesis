...
\begin{note}[Notion: \requ{2}]
  ...
  For the moment we provide the intuitive notion of a \requ{}.
  ...
  \begin{restatable}[A \requ{0}]{notion}{notionRequisite}
    ...
    \item
      ...
      It is not \epVAd{} for agent to conclude \(\phi\) has value \(v\) from \(\delta\) unless \(\psi\) has value \(v'\).
    \item
...
\begin{note}
  We will shortly replace this notion with a definition covering two specific types of \requ{}.
\end{note}
...
\begin{note}[\crequ{3}]
  \begin{illustration}[Where's Wally]
    ...
    \item
      \label{illu:CS:wheres-wally:ante}
      Had features.
      % \item The individual identified is Wally only if the individual is holding a cane.
    \item
      The individual was holding a cane. \hfill (From \ref{illu:CS:wheres-wally:info} and \ref{illu:CS:wheres-wally:ante})
    \end{enumerate}
  \end{illustration}
...
\begin{note}
  Here, there is something weaker.
  If really is Wally, then good for cane.
  Still, strictly weaker.
  Only a cane with other attributes.
  So, get this and combine with what's been observed.
\end{note}
...
\begin{note}
  Focus is with \crequ{}, in particular.
\end{note}
...
\begin{note}
  ...
  \begin{enumerate}[label=\arabic*., ref=(\arabic*)]
  \item
    \label{crequ:int:structure}
    \(\psi\) not having value \(v'\) is \epVAd{} and the relevant \emph{conclusion} proposition-value pairs of the step are paired only if \(\psi\) has value \(v'\), and
  \item
  ...
  \begin{restatable}[A \crequ{0}]{definition}{defCRequisite}
    ...
    \item
      ...
      \item
        ...
        \end{itemize}
      \item
        \label{def:requ:prequ:strcture:psi-subset-psi}
        There are no \epVW{1} such that:
        \begin{itemize}
        \item \(\phi\) has value \(v\) and \(\psi\) does not have value \(v'\).
        \end{itemize}
      \item
        \label{def:requ:prequ:strcture:phi-propersubset-psi}
        There is some \epVW{} such that:
        \begin{itemize}
...
\paragraph{The structure of the agent's epistemic state}

\begin{note}[Structure of sub-defs]
  With respect to the (sub-)definitions of a \crequ{}, these describe particular epistemic states an agent may have.
...
\begin{figure}[h!]
  ...
  \begin{subfigure}{0.3\linewidth}
    \begin{tikzpicture}[scale=.85]
      ...
      \draw[dashed, rounded corners] (psiLL) rectangle (psiUR);
    \end{tikzpicture}
    \caption{\ref{def:requ:prequ:strcture:psi-subset-psi} \& \ref{def:requ:prequ:strcture:phi-propersubset-psi}}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.3\linewidth}
    \begin{tikzpicture}[scale=.85]
...
\subparagraph{Library indexes: Limitation, broadening}
...
\begin{note}
  In order to keep things a straightforward as possible, the definition of a \prequ{} does not cover such cases.
  Focus only on what the agent considers \epVAd{}.
\end{note}

\begin{note}
  \color{red}
  The point here is that \support{} motivates a broader constraint.
  In this case, as the agent has performed the search, it is not the case that there is an \epVW{} in which the right structure holds.
  Still, if we think about \support{} and relaxing an agent's epistemic state, we can re do the constraint.
\end{note}

\begin{note}
  \color{red}
  However, we will see that \crequ{} covers this case\dots
  (For, here it's fine for the premises to hold throughout all \epVW{1}.)
  So, whether or not something additional is needed will depend on whether view premises or moving to conclusion as the problem.
\end{note}
...
\begin{note}
  Start with a recent \illu{0}.
  Back to Library.
  In this case, premises are no longer an issue.
  Instead, there's something weaker in-between.
\end{note}
