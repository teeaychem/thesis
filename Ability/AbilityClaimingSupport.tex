...
\begin{note}
  \begin{restatable}[\eiS{-} --- \eiS{}]{assumption}{assuCSRReq}
    ...
    \begin{enumerate}
    \item\label{assu:supp:ind:indicate} The reasoning that concludes with \(\phi\) having value \(v\) includes some sub-reasoning that:
      \begin{enumerate}
      \item Indicates that \(\psi\) has value \(v'\).
      \item\label{assu:supp:ind:circular} Does involve not appeal to proposition-value pair that only follows from the step of reasoning for which \(\psi\) having value \(v'\) is a \requ{}.
      \end{enumerate}
...
\begin{note}
  No-circularity is fairly straightforward.
