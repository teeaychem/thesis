...
\begin{note}
  \begin{restatable}[\requ{3} of a step of reasoning]{definition}{defRequisite}
    \label{def:requisite}
    \(\psi\) has value \(v'\) is \requ{} of a step of reasoning that concludes that \(\phi\) has value \(v\) (from an agent's perspective) iff:
    \begin{enumerate}
    ...
    \item\label{def:requi:subjunctive} If \(\psi\) were not to have value \(v'\) then that step of reasoning would involve appealing to:
      \begin{enumerate}
      \item\label{def:requi:subjunctive:p} something that is not the case, or
...
\begin{note}
  \begin{restatable}[\eiS{-} --- \eiS{}]{assumption}{assuCSRReq}
    ...
    Suppose that \(\psi\) having value \(v'\) is a \requ{} of some step of reasoning that concludes with \(\phi\) having value \(v\).
    ...
    % \item\label{assu:supp:ind:circular} The sub-reasoning does involve not appeal to proposition-value pair which only follows from the step of reasoning for which \(\psi\) having value \(v'\) is a \requ{}.
      \item\label{assu:supp:ind:indicate} The reasoning that concludes with \(\phi\) having value \(v\) includes some sub-reasoning concludes with \(\psi\) has value \(v'\).
    \item\label{assu:supp:ind:circular} The sub-reasoning does involve not appeal to proposition-value pair which only follows from the step of reasoning for which \(\psi\) having value \(v'\) is a \requ{}.
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{restatable}
  ...
  Reasoning, and a minimal constraint on such reasoning.
  In the background, that this reasoning indicates.
  However, not clear on what this really amounts to.
  Hence, avoid.

  This is a trade-off between having assumptions and propositions which are relatively clear, and dealing with intuitively problematic cases as part of assumptions and propositions.

  For now, this will not be an issue.
  Focus mostly on cases where agent fails to reason, and hence when this assumption is not satisfied.
  However, will return with respect to \nI{}.
  Deny that under an addition assumption, not possible for an agent to claim support.
  Building in impossibility of reasoning, rather than the impossibility of `good' reasoning.
...
\begin{note}
  \autoref{assu:supp:independence} is a core assumption of the argument to follow.
  However, as we have not stated what it is for reasoning to `indicate' that some proposition \(\xi\) has some value \(v''\), the content of \ref{assu:supp:ind:indicate} is open to interpretation.

  Intuitively, some reasoning to `indicates' that some proposition \(\xi\) has some value \(v''\) just in case the reasoning
  \begin{itemize}
  \item provides considerations which count in favour of (Hieronymi)
  \item raises the probability above some threshold (???)
  \item Involves the agent taking to support (Boghossian)
  \item responds to reasons for thinking that (Lord)
  \item It would be fitting (McHugh and Way)
  \end{itemize}

  From a certain perspective, leaving part of \autoref{assu:supp:independence} open to interpretation is unsatisfying.
  Without any account of what it is for some reasoning to indicate that some proposition \(\xi\) has some value \(v''\), it is not clear what commitments follow from this assumption.
  From a different perspective, however, interpretation is unavoidable.
  We do not wish to place any constraints on what `indication' amounts to in order for \autoref{assu:supp:independence} to be compatible with a
