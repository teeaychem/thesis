...
\begin{note}
  \begin{restatable}[A \result{0} of (making a step of) some instance of reasoning]{definition}{defResult}
    \label{def:result}
    ...
    \item
      ...
      Making step \(\delta\) while holding that \(\psi\) does not have value \(v'\) is not epistemically possible, relative to background commitments of the agent (if any).
    \item
      \label{def:requ:persists}
      No other part of reasoning shows that \(\phi\) has value \(v\) which does not involve a step \(\delta'\) such that \(\psi\) having value \(v'\) follows from making step \(\delta'\).
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{restatable}
\end{note}
...
\subparagraph{Clause~\ref{def:result:ep}}

\begin{note}
  Only interested in these instances.
\end{note}

\subparagraph*{Clause~\ref{def:result:conseq}}

\begin{note}
  Starting with sub-clause~\ref{def:result:conseq}, emphasis is on \(\psi\) having value \(v'\) as a result of \emph{making} the step of reasoning.

  \begin{example}
    Suppose the step of reasoning appeals to premise that \(\alpha\) has value \(v''\).
    And, suppose the agent holds that \(\alpha\) has value \(v''\) only if \(\psi\) has value \(v'\).
  \end{example}

  In this example, \ref{def:result:conseq} is satisfied due to appealing to a premise.
  By contrast, conclusion:
  ...
  \begin{example}
    Suppose the agent has available inductive for \(\alpha\) having value \(v''\).
    The step of reasoning involves moving from inductive premises to \(\alpha\) having value \(v''\).

    As before the agent holds that \(\alpha\) has value \(v''\) only if \(\psi\) has value \(v'\).
  \end{example}
\end{note}

\subparagraph*{Clause~\ref{def:requ:persists}}
...
\begin{note}
  ...
  \end{example}
  So, \ref{def:requ:persists} fails.

  \begin{example}
    Both cases need \(\psi\) having value \(v'\).
  \end{example}
\end{note}
...
\begin{note}
  ...
  The definition of a \result{} clarifies how a proposition-value pair is involved in some instance of reasoning, but the definition of a \result{} alone does not account for why it is not possible for the agent to conclude the instance of reasoning without (indirectly) appealing to the proposition-value pair.
  To capture the mentioned role of proposition-value pair, definition of a \requ{} is broken down into three distinct types of \requ{}, and the definition of a \result{} is then used to clarify two basic \requ{}-types.
\end{note}
...
\begin{note}
  \begin{restatable}[\prequ{2} of a step of reasoning]{definition}{defRequisiteP}
    ...
    \begin{enumerate}[label=\arabic*., ref=\named{pR:\arabic*}]
    \item
      \(\psi\) has value \(v'\) is a \result{0} of the \emph{premises} of a the step of reasoning \(\delta\).
    \item
      \label{def:prequ:subjunctive}
      It is not epistemically possible for the agent to make step \(\delta\) without either:
      \begin{enumerate}[label=\alph*., ref=\named{pR:2\alph*}]
      \item \(\psi\) having value \(v'\), or
      \item the relevant background conditions (if any) which lead to \(\psi\) having value \(v'\) being a \result{0} of step \(\delta\).
      \end{enumerate}
    \item
      \requGlobalClause{}
    \end{enumerate}
...
\begin{note}[Summary of \prequ{0}]
  In short, a \prequ{0} is simply a \result{0} which follows from the \emph{premises} of the step of reasoning with the added constraint regarding the relevant background commitments of the agent, if there are any.
\end{note}

\begin{note}
  To clarify \ref{def:prequ:subjunctive}, let us consider an example.

  \begin{example}
    Suppose the step of reasoning appeals to premises that \(\alpha\) has value \(v''\) and that \(\beta\) has value \(v'''\).
    And, suppose the agent holds that
    \(\alpha\) has value \(v''\) only if \(\psi\) has value \(v'\)
    and
    that \(\beta\) has value \(v'''\) was obtained from \(\psi\) having value \(v'\).
    Such that, without \(\psi\) no \(\beta\).
  \end{example}
  \(\alpha\) has value \(v''\) only if \(\psi\) has value \(v'\) is background commitment of the agent.
  However, so long as \(\alpha\) has value \(v''\) and \(\beta\) value \(v'''\), then step.
  So, background from relation to \(\alpha\) alone does not satisfy.
  However, relation to \(\beta\).
  Here, important.
\end{note}

\begin{note}[Also no background]
  When no background commitments.
  For, the \(\psi\) having value \(v'\) is really important.
  However, if background, then these matter for making the step.
\end{note}
...
\begin{note}
  \begin{restatable}[\crequ{2} of a step of reasoning]{definition}{defRequisiteC}
    ...
    \begin{enumerate}[label=\arabic*., ref=\named{cR:\arabic*}]
    \item
      \(\psi\) has value \(v'\) is a \result{0} of the \emph{conclusion} of the step of reasoning \(\delta\).
    \item
...
\begin{note}[No \crequ{1} wrt.\ deductive reasoning]
  ...
  For, in the case of deductive reasoning, any conclusion of the step of reasoning follows from the relevant premises, in the sense that it is not (epistemically) possible for the agent to jointly hold that the premises of the step have their respective values while the conclusion of the step does not have some value.\nolinebreak
  \footnote{
    This is only to state that, with respect to \emph{conditional detachment}, it is not (epistemically) possible for an agent to hold that \(\phi\) is true and \(\phi \rightarrow \psi\) is true and not to hold that \(\psi\) is false.
...
\begin{note}
  \begin{restatable}[\cprequ{2} of (a step of) reasoning]{definition}{defRequisiteCP}
    ...
    \item It is the case that both:
      \label{def:cprequ:conseq}
      \begin{enumerate}[label=\alph*., ref=\named{cpR:2\alph*}]
      \item
        \label{def:cprequ:crequ}
        \(\chi_{i}\) having values \(v''_{i}\) are \crequ{1} of a the step \(\delta\).
      \item
        \label{def:cprequ:subjunctive}
        It is not (epistemically) possible for the agent to jointly hold
          \begin{enumerate}
          \item the premises of \(\delta\),
          \item that \(\chi_{i}\) have values \(v''_{i}\), and
          \item that \(\psi\) does not have value \(v'\).
          \end{enumerate}
      \end{enumerate}
    \item
      \requGlobalClause{}
...
\begin{note}
  Intuitively, a \cprequ{} is a special case of a \prequ{} which takes into account proposition-value pairs introduced by making the step of reasoning.
  Something new from premises when combined with \crequ{1} of the relevant step of reasoning.

  For, \ref{def:cprequ:crequ} requires that the relevant proposition-value pairs which lead to \(\psi\) having value \(v'\) are \crequ{1}.

  Hence, follow from the note \hyperref[crequ-only-non-deductive]{above}, \cprequ{1} are unique to non-deductive reasoning.
\end{note}

\begin{note}
  Turning to the clauses of \autoref{def:cprequ}.

  As with the definitions of \pandcrequ{1}, \ref{def:cprequ:conseq} the first clause of the definition of a \cprequ{} requires that \(\psi\) having value \(v'\) is a \result{0} of the step of reasoning \(\delta\).
  Unlike the definitions of \pandcrequ{1}, \ref{def:cprequ:conseq} does not constrain the way in which \(\psi\) having value \(v'\) is a \result{0}.
  Instead, the relevant clarification is found in~\ref{def:cprequ:subjunctive}.

  In contrast, an analogue of clauses~\ref{def:prequ:subjunctive} and~\ref{def:crequ:subjunctive} is omitted from the definition of \cprequ{}.
  For, the importance of \(\psi\) having value \(v'\) to reasoning is secured by the proposition-values pairs \(\chi_{i}\) having values \(v''_{i}\) being a \crequ{} and~\ref{def:cprequ:subjunctive}.
\end{note}
