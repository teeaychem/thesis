...
\begin{note}
  \begin{restatable}[\requ{3} of a step of reasoning]{definition}{defRequisite}
    \label{def:requisite}
    \(\psi\) has value \(v'\) is \requ{} of a step of reasoning that concludes that \(\phi\) has value \(v\) (from an agent's perspective) if:
    \begin{enumerate}
    \item\label{def:requi:prop} \(\psi\) having value \(v'\):
      \begin{enumerate}
      \item\label{def:requi:prop:conseq} Is a consequence of the step of reasoning.\nolinebreak
        \footnote{
          Or rather, is a consequence of what is established as a result of the step of reasoning.
        }
      \item\label{def:requi:prop:persists} Persists through to the conclusion of the instance of reasoning.\nolinebreak
        \footnote{
          Or rather, what is established as a result of the step of reasoning persists through to the conclusion of the instance of reasoning.
          For, \(\psi\) having value \(v'\) is a consequence of what is established as the result of the step of reasoning, and as such if the consequence persists through to the conclusion of the instance of reasoning, then \(\psi\) having value \(v'\) must also likewise persist.
        }
      \end{enumerate}
    \item\label{def:requi:not-v-poss} \(\psi\) not having value \(v'\) is epistemically possible.
    \item\label{def:requi:counterfactual} If \(\psi\) were not to have value \(v'\) then that step of reasoning would involve appealing to:
      \begin{enumerate}
...
\begin{note}
  ...
  \begin{itemize}
  \item Clause \ref{def:requi:prop} How \(\psi\) having value \(v'\) relates to the instance reasoning.
  \item Clause \ref{def:requi:not-v-poss} is a precondition for the relevance of subjective conditional of \ref{def:requi:counterfactual}.
  \item Clause \ref{def:requi:counterfactual} highlights a specific issue that arises from how \(\psi\) having value \(v'\) relates to the instance reasoning.
  \end{itemize}
  As such, our focus will be on clauses~\ref{def:requi:prop}~and~\ref{def:requi:counterfactual}.\nolinebreak
  \footnote{
    Clause \ref{def:requi:not-v-poss} is separated from \ref{def:requi:counterfactual} simply to ease readability.
  }
\end{note}

\begin{note}[Clause \ref{def:requi:prop}]

  If consequence of a step, then consequence of reasoning.
...
\begin{note}[Clause \ref{def:requi:not-v-poss}]
  Clause \ref{def:requi:not-v-poss} is a precondition for the relevance of subjective conditional of \ref{def:requi:counterfactual}.
  Further, no interest in a consequence of reasoning if there is no epistemic possibility of subjunctive of clause \ref{def:requi:counterfactual} obtaining, and hence clause \ref{def:requi:not-v-poss} ensures the epistemic possibility.

  \ref{def:requi:counterfactual} may apply in cases of epistemically impossible proposition-value pairs, but it is not clear why such epistemically impossible proposition-value pairs would matter to the agent's reasoning.
\end{note}

\begin{note}[Clause \ref{def:requi:counterfactual}]
...
\begin{note}
  Footnote:\nolinebreak
  \footnote{
  May think that clause \ref{def:requi:prop:conseq} follows from \ref{def:requi:counterfactual}.
...
\begin{note}
  ...
  Finally, \ref{def:requi:prop:persists} ensures that the proposition-value pair remains important.
  In particular, avoids classifying proposition-value pairs appeals to as part of reasoning by cases as \requ{1}.
\end{note}

\begin{note}
  The subjunctive here, generally speaking, highlights a possible defeater for reasoning.
...
\begin{note}
  \begin{restatable}[\eiS{-} --- \eiS{}]{assumption}{assuCSRReq}
    ...
    \begin{itemize}
    \item Reasoning about why \requ{} is fine.
    \item Without consequence of step. (Where this is motivated by possibility that it doesn't hold. If only with consequence, then only when does hold.)
    % \item Why appeal to source of \requ{} is compatible with \requ{} not having value.
    \end{itemize}
    \vspace{-\baselineskip}
    \end{restatable}
