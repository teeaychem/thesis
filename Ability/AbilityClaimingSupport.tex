...
\begin{note}
  Intuitively, \(\psi\) having value \(v'\) is a \requ{} of a step of reasoning just in case the agent could not conclude that \(\phi\) has value \(v\) without the conclusion also \indicatePr{0} that \(\psi\) has value \(v'\) \emph{such that} prior to making the step the agent conclude that \(\psi\) having value \(v'\), and hence check whether they would conclude that \(\psi\) has value \(v'\).
...
\begin{note}[Structure of \autoref{def:requ}]
  ...
  Still, for the purposes of this document we will only be interested in \prequ{1} and \crequ{1}.
...
\begin{note}[Structure of sub-defs]
...
\paragraph{The possibility of reasoning}

\begin{note}
  \ref{prequ:int:reasoning} is captured by \ref{def:requ:prequ:possible-reason}.
...
\subparagraph{The core constraint}
...
\subparagraph{The role of the agent}
...
\begin{note}
  ...
  \begin{itemize}
  \item
    First, we ensure that \(\psi\) having value \(v'\) is relevant to the agent's own reasoning in general.
  \item
...
\begin{note}[Problem]
  In order for \(\psi\) having value \(v'\) to be a \prequ{} the agent must consider it \epVAd{} that \(\psi\) does not have value \(v'\).
...
\begin{note}[Solution]
  Still, quantifying over \epVW{1} in which the premises of the step have appropriate values provides a motivated solution.

  For, we are only interested in the idea \(\phi\) having value \(v'\) being \prequ{} as a check on whether it would make sense for an agent to appeal to a collection of premises in order to make some conclusion.
  Hence, in order for the \prequ{} to be a check of the relevant kind, we are only interested in cases where \(\psi\) must have value \(v'\) in order for the premises of the step of reasoning to have appropriate values.
  Therefore, we may restrict the interest in the possibility of concluding that \(\psi\) has value \(v'\) to those \epVW{1} in which the premises propositions have the values of interest.
\end{note}
...
\begin{note}[Illustration]
  ...
  \begin{illustration}
    Suppose an agent wishes to conclude that they should report that their parcel has gone missing from the premise that their parcel is missing.
    Still, the agent considers it \epVAd{} that a delivery was attempted, and that a note was left on their front door.
    Naturally, if the parcel has gone missing, then no delivery attempt will have been made, and no note will be on their front door.

    Still, if the parcel has not gone missing, then a delivery may have been attempted, and a note left on the agent's front door.
    Yet, if a note has been left on the agent's front door, then it will not be possible for the agent to conclude that there is no note on the front door.
    For, the agent will only conclude that there is no note on the front door by checking the front door.
  \end{illustration}

  The agent's epistemic state plausibly satisfies \ref{def:requ:prequ:strcture}.\nolinebreak
  \footnote{
    `Missing' will plausibly be a proper subset of `no not on front door' as it may remain \epVAd{} that the parcel is not missing, but not yet delivered.
  }
  However, if a note has been left on the agent's front door, then it will not be possible for the agent to conclude that there is no note on the front door.
  Hence, the core of the constraint would fail given the \epVAd{} that the parcel is not missing.

  Still, as the step of reasoning would move from a missing parcel to a resolution for action, at issue is only whether the agent would fail to conclude that there is no note on the door in the case that the package really is missing.
  If the agent's reasoning were to fail because of the presence of a note on the agent's front door, then it seems this would be sufficient to indicate to the agent that the parcel is not missing, and that they should not report the parcel missing.
\end{note}

\hozline

\subparagraph{Motivation from \support{}}
...
\begin{note}
  ...
  \ref{notion:requ:p} observes that the premises appealed to cover all \epVW{1} only if \(\psi\) has value \(v'\).
...
\begin{note}
  Seen \ideaCS{}, and a handful of \illu{1}.

  Now, \illu{1} may be sufficient motivation.
