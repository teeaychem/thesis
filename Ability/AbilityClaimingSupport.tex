...
\paragraph{\indicateO{1}}

\begin{note}
  \begin{definition}[\indicateO{1}]
    Reasoning that concludes that \(\psi\) has value \(v\) \indicateV{1} \(\psi\) has value \(v'\) if it is not a \ep{} that \(\psi\) has value \(v\) and not \(\psi\) does not have value \(v'\).
  \end{definition}
\end{note}
...
\begin{note}
  ...
  Do not need to recognise that I have reasoned about any proposition-value pair appealed to in order for reasoning to \indicateO{0}.
...
\begin{note}
  Above we paraphrased \ideaCSA{} as holding that that an instance of reasoning that concludes \(\phi\) has value \(v\) is an instance of claiming support that \(\phi\) has value \(v\) \emph{only if} the agent would conclude that \(\phi\) has value \(v\) were it to be the case that the actual state of affairs is some \ep{} state of affairs.
...
\begin{note}
  \begin{restatable}[\ideaCSA{0} --- \ideaCSA{}]{idea}{iCSA}
    ...
    An agent \emph{S} claims support that \(\phi\) has value \(v\) just in case:
    \begin{enumerate}
    \item
      \label{idea:defs-for-CS:clause}
      For any \ep{} \world{} relative to what \emph{S} takes the actual \world{} to be, there is some candidate \world{} \(S_{c}\) that \emph{S} would identifies as the actual \world{}, and either
    \begin{enumerate}
    \item
      \label{idea:defs-for-CS:clause:base}
      \(S_{c}\) is such that \(\phi\) has value \(v\), or
    \item
      \label{idea:defs-for-CS:recursive}
      There is some other state of affairs satisfying~\ref{idea:defs-for-CS:clause}.
    \end{enumerate}
  \end{enumerate}
\end{restatable}
\end{note}
...
\begin{note}
  \ideaCSA{} takes the form of a recursive definition.

  Take any \ep{} \world{0} relative to what \emph{S} considers to be the actual \world{}.
  Then, either:
  There is some candidate \world{} which \emph{S} identifies as the actual \world{} such that \(\phi\) has value \(v\).
  Or, relative to the \ep{} \world{0}, there is some candidate \world{} such that for any \ep{} \world{0} \emph{relative to the candidate \world{0}}, there is some candidate \world{} that \emph{S} would identify as actual, and either:
  \(\phi\) has value \(v\).
  Or, \dots
\end{note}

\begin{note}
  Candidate \world{} such that \(\phi\) has value \(v\).
  Still the case that \emph{S} must identify the candidate \world{} as the actual world.

  Candidate \world{} such that \(\phi\) does not have value \(v\).
  There are two options.
  Either straight to \(\phi\) having value \(v\).
  Or, some other world such that\dots
\end{note}

\begin{note}
  Different epistemic states is important.
  For, \ep{}.
  This is different from possibility.
...
\begin{note}
  \begin{restatable}[\ideaCSA{0} --- \ideaCSA{}]{idea}{iCSA}
    Claiming support for \(\phi\) having value \(v\) is compatible (from the perspective of the agent) with:
    \begin{enumerate}
    \item
      \(\phi\) not having value \(v\), if \(\phi\) not having value \(v\) is an \ep{}.
    \item
      The reasoning appealing to some proposition \(\psi\) having value \(v'\) while \(\psi\) does not have value \(v'\), if \(\psi\) not having value \(v'\) is an \ep{}.
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{restatable}
\end{note}
...
\begin{note}[]
  ...
  \begin{restatable}[\ideaCSB{0} --- \ideaCSB{}]{idea}{ideaEIS}
    \label{idea:eiS} % to remove, as I've used eiS too much
    \label{idea:cs:EIS}
    Suppose that:
    \begin{enumerate}
      \item
        \label{idea:eiS:p-v-pair:ep}
        \(\psi\) not having value \(v'\) is an \ep{}.
      \item
        There is some step of reasoning \(\delta\) which moves from premises \(\chi_{i}\) having values \(v_{i}''\) to the conclusion that \(\xi\) has value \(v'''\).
      \item
        \label{idea:eiS:p-v-pair:commit}
        \emph{S} is committed to it being the case that if \(\psi\) does not have value \(v'\) then either:
        \begin{enumerate}
        \item
          \label{idea:eiS:p-v-pair:commit:p}
          Some premise \(\chi_{i}\) of \(\delta\) does not have value \(v_{i}''\).
        \item
          \label{idea:eiS:p-v-pair:commit:c}
    ...
    An instance of reasoning that concludes that \(\phi\) has value \(v\) is an instance of claiming support for \(\phi\) having value \(v\) \emph{only if}:
      ...
      \item
        \label{idea:eiS:p-v-pair:recurse}
        The agent has witnessed some reasoning which \indicateV{1} --- relative to the agent's current epistemic state --- that \(\psi\) has value \(v'\).\nolinebreak
        \footnote{
...
\begin{note}
  \autoref{idea:eiS} expresses a necessary condition for an instance of reasoning to be an instance of claiming support.
\end{note}
...
\begin{note}[Breaking down idea]
  ...
  Hence, \ref{idea:eiS:p-v-pair:recurse}.
  Past or present reasoning which \indicateV{1} that \(\psi\) has value \(v'\).

  \begin{idea}
    If \(\phi\) not having value \(v\) is an \ep{}.
    Then:
    Reasoning indicates that \(\phi\) has value \(v\) only if things appealed to are compatible with \(\phi\) not having value \(v\), and without further things, suggest that \(\phi\) has value \(v\).
  \end{idea}

  Tweety flies, so Tweety is not a mammal.
  \ep{} that Tweety is a bat.
  Bats are mammals.
  However, suggests that Tweety is not a mammal.

  Other examples.

  This is hard to evaluate.
  Hence, treat \autoref{idea:eiS} not only as an idea, but as an \emph{ideal}.
  To the extent non-deductive reasoning is possible, this ideal is going to be satisfied.
  What we avoid is \emph{how} it is satisfied.
  Assume weak.

  Important idea is that this ensures that the scope of the step of reasoning \(\delta\) is not restricted to candidates such that \(\psi\) has value \(v'\).
  For, in \ep{} where \(\psi\) does not have value \(v'\), agent's epistemic state points to \(\psi\) having value \(v'\).

  Second.
  \indicateV{1}.

  Issue is restriction, not whether agent recognises that their reasoning is restricted.
\end{note}
...
\begin{note}
  ...
  Hence, \ref{idea:eiS:p-v-pair:recurse}, that present or past reasoning which concludes that \(\psi\) has value \(v'\).
...
\begin{note}
  ...
  \begin{enumerate}
  \item Even if Rover will not fall asleep soon\dots
  \end{enumerate}
...
\begin{note}
  ...
  Hence, \label{idea:eiS:p-v-pair:commit} constrains the reasoning in general.
\end{note}
...
\begin{note}
  ...
  Here, \indicateV{1} terminology of \ref{idea:eiS:p-v-pair:recurse}.
  ...
  On the other hand, \indicateO{0} may be too weak.
...
\begin{note}
  ...
  For, \indicateO{0}.
  May think that \indicateV{1} only if satisfies idea.
...
\begin{note}
  Above, given some intuition.
  Here, motivate intuition.
  Relate to safety.
\end{note}
...
\subparagraph{Contrast to safety}
...
\section{Impotent steps of reasoning}
...
\begin{note}
  If \autoref{idea:eiS} is satisfied, then reasoning.
...
\begin{note}
  ...
  Hence, following is compatible with rejecting \autoref{idea:eiS} while maintaining \autoref{idea:cs:imp-stp}.
