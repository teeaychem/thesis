...
\begin{note}
  ...
  So, here, plausibility of either.
\end{note}

\begin{note}
  However, does not entail that there are defeaters, nor that there are a defeater is a genuine (i.e.\ non-epistemic) possibility --- only that defeat is an epistemic plausibility.
\end{note}
...
\begin{note}
  \begin{restatable}[\requ{3} of a step of reasoning]{definition}{defRequisite}
    \label{def:requisite}
    \(\psi\) has value \(v'\) is \requ{} of a step of reasoning that concludes that \(\phi\) has value \(v\) (from an agent's perspective) if and only if:
    \begin{enumerate}
    ...
    \item\label{def:requi:subjunctive} If \(\psi\) were not to have value \(v'\) then that step of reasoning would involve either:
      \begin{enumerate}
...
\begin{note}
  \begin{restatable}[Premise \requ{0} of a step of reasoning]{definition}{defRequisiteP}
    \label{def:requisite:p}
    \(\psi\) has value \(v'\) is a \emph{premise \requ{0}} of a step of reasoning if and only if
    \begin{enumerate}
    \item \(\psi\) has value \(v'\) is a \requ{0} of a that step of reasoning.
    \item \(\psi\) has value \(v'\) is a \requ{0} of a that step of reasoning because satisfies \ref{def:requi:subjunctive:p} of \autoref{def:requisite}.
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{restatable}

    \begin{restatable}[Conclusion \requ{0} of a step of reasoning]{definition}{defRequisiteC}
    \label{def:requisite:c}
    \(\psi\) has value \(v'\) is a \emph{conclusion \requ{0}} of a step of reasoning if and only if
    \begin{enumerate}
    \item \(\psi\) has value \(v'\) is a \requ{0} of a that step of reasoning.
    \item \(\psi\) has value \(v'\) is a \requ{0} of a that step of reasoning because satisfies \ref{def:requi:subjunctive:c} of \autoref{def:requisite}.
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{restatable}
\end{note}
...
\begin{note}
  \begin{restatable}[\eiS{-} --- \eiS{}]{assumption}{assuCSRReq}
    ...
    \item Step is compatible with \(\psi\) not having value \(v'\).
      (is a `premise \requ{}')
    \end{enumerate}
    Then.
    \begin{enumerate}[resume]
    \item\label{assu:supp:ind:circular} Does involve not appeal to a proposition-value pair that is only obtained from the step of reasoning for which \(\psi\) having value \(v'\) is a \requ{}.
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{restatable}
  {
  ...
  And, one instance of clear failure.
\end{note}
