...
\begin{note}
  ...
  Collected, these definitions may be used to form a theory about events in which an agent concludes (where here `concludes' is read in laypersons terms).
  However, these definitions are intended to be theory neutral.
  In other words, rather than forming a theory about conclusions from the definitions, the definitions are designed to be quite general --- general in the sense that specific theories of conclusions may be captured using these definitions.

  In particular, our interest is with \issueInclusion{}, which is understood as a pre-theoretical constraint on understanding the way an event in which an agent concludes happens.

  This section adapts the definitions to a handful of accounts of conclusions from the literature.
\end{note}
...
\begin{note}
  ...
  \end{quote}
  %
  As noted above, propositional attitudes.
  Further, we do not assume that a conclusion is caused by whatever is captured by the relevant \pool{0}, however the definitions given are compatible with this idea.
  Hence, expressed in our terminology:

  An agent infers \emph{p} from \emph{q} just in case the agent concludes \pv{p}{\valI{True}} from a \pool{} which contains \pv{q}{\valI{True}}, and the conclusion is a result of the \prop{0}-\val{0} pairs in the \pool{} causing the agent to conclude \pv{p}{\valI{True}}.
\end{note}


\begin{note}
  \citeauthor{Armstrong:1968vh}'s (\citeyear{Armstrong:1968vh}) account of inferring focused on causation.
  Other accounts of conclusion appeal to rule following.
  For example consider, \citeauthor{Broome:2002aa}'s (\citeyear{Broome:2013aa}) rule following account of (active) reasoning:
  %
  \begin{quote}
  ...
  Hence, expressed in our terminology:%
  \footnote{
    As an aside I am impressed with how \citeauthor{Broome:2002aa} smuggles agency into their account of reasoning.
    The premise attitudes cause \emph{you} to acquire a conclusion --- the premise attitudes do not cause the conclusion via a causal path that captures adherence to a rule.
  }

  An agent concludes \(\pv{\phi}{v}\) from some \pool{} \(\Phi\) by active reasoning just in case:
  \begin{itemize}
  \item
    The agent concludes \(\pv{\phi}{v}\) from \(\Phi\).
  \item
    The conclusion is a result of the \prop{0}-\val{0} pairs in the \pool{} causing the agent to conclude \pv{p}{\valI{True}}.
    Where:
    \begin{itemize}
    \item
      The agent operates on the \prop{0}-\val{0} pairs in \(\Phi\) by following some rule to construct \(\pv{\phi}{v}\).
    \end{itemize}
  \end{itemize}
  %
  ...
  Extends to non-causal accounts such as \citeauthor{Wright:2014tt}'s (\citeyear{Wright:2014tt}) `Simple Proposal' (discussed in a different context on \autopageref{wrightSimp}), or  \citeauthor{Longino:1978wv}'s (\citeyear{Longino:1978wv}) account of inferring:
  \begin{quote}
