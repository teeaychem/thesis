...
\begin{note}
  ...
  \begin{notation}
    \item
      ...
      For example \propI{The hedge is shaped like a squirrel} is the proposition/state of affairs in which a hedge is shaped like a squirrel.
      \valI{True} is the value `True'.
    \item
      ...
      \end{itemize}
    \item
      \(\pv{\emph{\phi}}{v}\) links a proposition and a value.
...
\begin{note}
  ...
  \begin{definition}[\evalN{3}]
    ...
    \item
      ...
      \begin{itemize}
      \item
        The pair captures the way things are from an \agpe{}.
      \end{itemize}
    \end{itemize}
    \vspace{-\baselineskip}
  \end{definition}
...
\begin{note}
  ...
  Indeed, the relevant values may be restricted to \valI{True} and \valI{False}, which we understand as synonyms for \valI{Actual} and \valI{Not actual}.
  ...
  I both \emph{believe} \propI{The coffee is bitter} and \emph{desire} \propI{The coffee is bitter}.
...
\begin{note}
  ...
  So, from \agpe{your}, \propI{The long lines are parallel} is \evaled{} \valI{False}.
...
\begin{note}
  As \evalN{1} capture an \agpe{} on the way the things are we distinguish \evalN{1} from a proposition-value pairs \emph{given various assumptions}.
  ...
  By contrast, if the agent has been entertaining the idea that Rover is tired then the agent is entertaining the idea that the proposition \propI{Rover is tired} has the value \valI{True}, but we do not consider the proposition-value pair \evaled{0}.
...
\begin{note}
  ...
  \noindent%
  So, the progressive allows us to capture conclusions which are available to the agent, but not yet made.
\end{note}
...
\begin{note}
  ...
  \begin{argument}{obs:cds-arb}
    ...
    %   For example, an agent looks outside, sees a bird in a tree and concludes \propI{There is a bird in that tree} has value \valI{True}.%
    %   \footnote{
...
\begin{note}
  ...
  In our terminology, evaluation \emph{p} has value \valI{True}.
...
\begin{note}
  ...
  Add to \pool{}.
  Parallel considerations apply to \textcite{Thomson:1965vv}, where \pool{} involves a belief that other elements of \pool{} are a reason for evaluating \(\phi\) with value \(v\).
