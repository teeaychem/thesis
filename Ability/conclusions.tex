...
\begin{note}
  ...
  Our use of the term \textquote{concluding} will always capture an event in progress.
  \textquote{The agent is concluding \(\phi\) has \val{0} \(v\)} is read in parallel to a natural reading of  \textquote{Taylor walking to the shops} or \textquote{Riley is filling a bathtub}.
...
\begin{note}
  \begin{assumption}[Conclusions from \pool{3}]
    ...
    \item
      \begin{itenum}
      \item[\emph{If}:]
        \(\phi\) having \val{0} \(v\) is a conclusion by \vAgent{}.
      \item[\emph{Then}:]
        \(\phi\) having \val{0} \(v\) is a conclusion by \vAgent{} from some \pool{} \(\Phi\).
      \end{itenum}
    \end{itemize}
    \vspace{-\baselineskip}
  \end{assumption}
...
\begin{note}
  Final assumption is about what it is for an event to be in progress.
...
\begin{note}
  Our interest with events in progress is with respect to events in which an agent is concluding.
  \assuPP{} entails there is some possible event in which the agent concludes:
  ...
  \begin{argument}{obs:cds-arb}
    ...
    % \item
    %   It may be the case that an agent instantly concludes \(\phi\) has \val{0} \(v\).

    %   For example, an agent looks outside, sees a bird in a tree and concludes \propI{There is a bird in that tree} has \val{0} \valI{True}.%
    %   \footnote{
