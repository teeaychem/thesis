...
\begin{note}[Expanding on~\ref{def:itp:b:pR}]
  ...
  For, if it is possible for an agent to conclude \(\phi\) has value \(v\) by witnessing reasoning from \(\Phi\) to \(\pv{\phi}{v}\), then (from the \agpe{} at least), \(\pv{\phi}{v}\) whenever \(\pv{\phi_{i}}{v_{i}}\) for each \(\pv{\phi_{i}}{v_{i}}\) in \(\Phi\).
...
\begin{note}
  ...
  And, our interest with \itp{1} will typically be from the \agpe{}.
\end{note}
...
\begin{note}[Expanding on~\ref{def:itp:b:distinct}]
  ...
  Where, again, equivalence is evaluated from the \agpe{}.
...
\begin{note}
  ...
  Still, even though memory is contingent, we may briefly observe that the way in which the agent claim support in~\autoref{ill:ad:proof:eve} is compatible with \issueConstraint{}.
  For, \issueConstraint{} requires that an agent may claim support for some conclusion from premises and steps of reasoning only if the agent has witnessed reasoning to the conclusion from those premises via those steps of reasoning.
  So, if the initial instance of claiming support conformed to \issueConstraint{} then the agent will have witnessed reasoning from those steps and premises to the conclusion --- the instance of claiming support in~\autoref{ill:ad:proof:eve} does not involve such witnessing, but the agent's memory would be about how the relevant premises and steps were used to claim support.
\end{note}
