...
\begin{note}
  \autoref{cha:zS} introduced \zSN{2} via a question, \qzS{}.
  \zSN{2} defined as property, \zS{}.
  Primarily in terms of positive answers to \qzS{}.

  In this chapter, an equivalent statement of \zS{}, which we term \zetaS{}.
  ...
  \item
    Same time
  \item
    Present epistemic state, no fail.
  \end{enumerate}

  First two options, fairly straightforward.
  Witnessing.
  Third is in exclusion to the other two.

  Not an instance of \zS{}.
  Thing here, and why such detail, is the strength of this.
  Possibility of a different conclusion, unless concluded.

  Two things.
  Understand why this leads to some tension.
  Set up for next chapter.
  Not present epistemic state.
\end{note}
...
\begin{note}
  Okay, so can understand \qzS{} in terms of relations of support.

  This is interesting, as it's a purely positive principle.
  Though, I don't think this is too important.
  For, consider any \(\overline{v}\).
  Either it's not the case that the agent has the option of concluding, or the agent has the option of concluding.

  So, it is either the case that there's a \requ{} regarding any incompatible proposition-value pair, or there's nothing that can be said from the agent's present epistemic state.
  I.e.\ it may be the case that incompatible, but nothing about this follows from the agent's present epistemic state.
\end{note}

\begin{note}
  \begin{proposition}
    Suppose \zS{} as result of concluding.

    If support only if witnessing then, for any \requ{}:
    \begin{itemize}
    \item
      Concluded.
      \begin{itemize}
      ...
      \end{itemize}
    \item
      When fixing, either no option to reason or possible to have reasoned otherwise.
    \end{itemize}
  \end{proposition}

  The interesting thing here is the second.
  This is not immediate, right.
  It does not follow that \requ{} was fixed.
  In general, may be the case that conclude would not reason otherwise from something else.

  Applied to \cScen{}, then this second option is immediate.

  This is where tension is.

  And, the argument is fairly simple.
  Whenever get something entailing, then the pairing serves as a \requ{}.
  As \requ{} was not explored, may have reasoned otherwise.
\end{note}
...
\begin{note}
  \begin{definition}[Fixed]
    \(\pvp{\psi}{v'}{\Psi}\) is \emph{fixed} with respect to an agent's epistemic state \emph{if and only if} from agent's perspective, would not fail to conclude \(\pv{\psi}{v'}\) from \(\Psi\).
    And no relation of support between \(\pv{\psi}{v'}\) and \(\Psi\).
  \end{definition}

  This isn't quite right.
  The option is much stronger.
  Fixed, such that if option to conclude, when concluding, possibility of some other conclusion.
\end{note}
