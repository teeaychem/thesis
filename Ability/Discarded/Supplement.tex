...
\begin{note}[Looking ahead]
  ...
  Claiming support for \abgen{} ability, so we add claimed (inductive) support for theory together with understanding of theory.
  Follows to \abspec{} ability as instance of \abgen{}.
...
\begin{note}
  ...
  So, would need to be the case that \abgen{} ability does not make \(\psi\) sufficiently probable.

  Well, it's hard to evaluate this.
  Given the information, it seems it does.
  But, suppose it does not.
  Then, would need it to be the case that \abgen{} to \abspec{} requires \(\psi\).
  ...
  However, if this is the case, then it seems \abgen{} alone is going to be sufficient.
