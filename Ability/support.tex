...
\begin{note}
  ...
  In turn, \qWhy{} asks about explanatory \fingfr{0}, and \issueInclusion{} states \(\pv{\psi}{v'}\) \fingf{0} \(\Psi\) is a candidate for explaining why \(\ed{}\) is an event when an agent concluded \(\pv{\phi}{v}\) from \(\Phi\) \emph{only if} the agent has concluded \(\pv{\psi}{v'}\) from \(\Phi\) prior to, or concludes \(\pv{\psi}{v'}\) from \(\Phi\) at the same time, as the conclusion of \(\pv{\phi}{v}\) from \(\Phi\).
\end{note}
...
\begin{note}
  ...
  The first sufficient condition helps fix our understanding of \fingfr{1}, while the second sufficient condition is key to obtaining counterexamples to \issueInclusion{}.

  In line with the introduction of \fc{1} in \autoref{cha:fcs}, this is a short chapter which focuses on stating and illustrating idea of \fingfr{}.
  \autoref{cha:requs} details the way details the way \fc{1} and \fingfr{1} combine to form a path toward counterexamples to \issueInclusion{}.
  And, \autoref{cha:typical} will introduce a final key idea before \autoref{cha:ces} establishes a pattern of counterexamples.
\end{note}


\begin{note}
  This chapter closes with a handful of option notes, two of which connect and contrast \fingfr{1} to similar ideas in the literature.
\end{note}
...
\begin{note}
  ...
  \begin{dets}{appl:qWhyV-s1}
    ...
    Further, as \(\ed{\flat}\) is a \se{} of \(\ed{}\) it follows \(\ed{\flat}\) is such that \(\eXd{}\) is in progress (Clause~\ref{assu:p:se:prog} of \autoref{def:se}, \autopageref{def:se}).
...
\begin{note}
  ...
  \footnote{
    ...
    Given the parallels between \fingfr{1} and \emph{ex ante} justification, you may think of \fingfr{1} in line with propositional justification.
  }%
  \(^{,}\)%
  \footnote{
...
\begin{note}
  ...
  When we speak of \fingfr{} we do not consider \fingfr{1} embedded within \fingfr{}.
  ...
  \footnote{
    ...
    If you like, relevant definitions may be be refined to explicitly exclude embedded \fingfr{1}.
