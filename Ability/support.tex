...
\begin{note}
  ...
  \footnote{
    As an event in which an agent concludes may span the \agents{} reasoning to \(\pv{\phi}{v}\) from \(\Phi\) (\autoref{assu:ConRea}), the existence of a \ros{} by \supportI{} is restricted to the sub-event when the agent \evals{} \(\phi\) as having \val{} \(v\).
  }
\end{note}
...
\begin{note}
  ...
  Still, in \autoref{scen:countS} \supportII{} does not assume a \ros{} between \pv{\propI{What Pritcher said was a sign}}{\valI{True}} and some \pool{} \(\Psi\) holds \emph{prior to} Fox's conclusion of \pv{\propI{What Pritcher said was a sign}}{\valI{True}} from \(\Psi\).
...
\paragraph*{\supportI{} and \citeauthor{Boghossian:2014aa} Taking Condition \hfill (Optional)}
...
\begin{note}
  ...
  When speak of \ros{}, \prop{0}.
  Not a \prop{0} from \agpe{an \agents{}}.
