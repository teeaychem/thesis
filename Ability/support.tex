...
\begin{note}
  ...
  \begin{idea}[\supportII{}]%
    ...
    \item[\emph{Then}:]
      ...
      % Following conditional does not hold:
      ...
      %   \vAgent{} has a (\pwP{}) \wit{} for the \ros{} between \(\pv{\phi}{v}\) and~\(\Phi\).
      % \end{enumerate}
    \end{itenum}
    \vspace{-\baselineskip}
  \end{idea}
...
\begin{note}
  ...
  % \noindent%
  % \autoref{prop:attribution} rests on \autoref{def:NScon}.
  % However, I suspect the contrapositive of \autoref{prop:attribution} holds for a range of accounts of what necessary and sufficient conditions are.
