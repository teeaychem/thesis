...
\begin{note}
  ...
  \item
    The way in which \(\pv{\psi}{v'}\) is supported by [the way in which \(\pv{\phi}{v}\) is supported by \(\Phi\), from the \agpe{}], from the \agpe{}.
  \end{itemize}
  %
  Intuitive distinction.

  If \ros{} between \(\pv{\phi}{v}\) and \(\Phi\) occurs within some other \ros{}, we say the \ros{} between \(\pv{\phi}{v}\) and \(\Phi\) is `\rosE{0}'.
  And, we say a \ros{} is `\rosNE{0}'

  Throughout this document reference to a \ros{} is always reference to an \rosNE{0} \ros{}, unless either
  explicitly mention \rosE{}, or
  it is clear from context that I forgot.
\end{note}
...
\begin{note}
  In full, we define an \rosE{} \ros{} in the following way:%
  \footnote{
    To keep things seems we assume:
    \begin{itenum}
    \item[\emph{If}:]
  ...
  The cases of interest to us are where \(\pv{\psi}{v'}\) is \rosE{0} within in a \ros{} between \(\pv{\phi}{v}\) and \(\Phi\), no matter the degree of embedding:
