...
\begin{note}
  ...
  This chapter gives two sufficient conditions for a \ros{} holding from an \agpe{} with respect to some event.
\end{note}
...
\begin{note}
  ...
  In turn, \ros{3} are answers to \qWhy{}, and if \issueInclusion{} holds, for any \ros{} which answers \qWhy{} there is some event where agent concludes.
\end{note}
...
\begin{note}
  ...
  \item[\emph{If}:]
    \begin{itemize}
    \item
      Throughout \(\ed{}\) there is some action \(a\) \vAgent{} may perform such that both \ref{def:fc:act} and \ref{def:fc:result} are true:
...
\begin{note}
  ...
  \begin{dets}{appl:qWhyV-s1}
    ...
    And, that \pv{\rootsCon{}}{\valI{True}} is a \fc{} from \(\Phi\) throughout \(\ed{\flat}\) (\autoref{obs:var:factor:fc}, \autopageref{obs:var:factor:fc}).

    Given \autoref{obs:var:factor:fc} \pv{\rootsCon{}}{\valI{True}} is a \fc{} from \(\Phi\) throughout \(\ed{\flat}\).
  Hence, \supportII{} (\autopageref{idea:support:possible}) entails a \ros{} between \pv{\rootsCon{}}{\valI{True}} and \(\Phi\) holds throughout \(\ed{\flat}\).
