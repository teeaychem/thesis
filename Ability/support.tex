...
\begin{note}
  Our interest is with answers to \qWhy{} and \qHow{} \(e\) is an event in which an agent concludes \(\pv{\phi}{v}\) from \(\Phi\).

  Understand \ros{1} to captures some connexion between \(\pv{\phi}{v}\) and \(\Phi\) from the \agpe{}.
  Intuitively, `follows from' connexion.

  An agent concludes \(\pv{\phi}{v}\) `from' \(\Phi\) where `from' captures act.
  When agent concludes \(\pv{\phi}{v}\) `from' \(\Phi\), \(\pv{\phi}{v}\) `follows from' \(\Phi\), where `follows from' captures something from the \agpe{}.

  In the same way \propI{The time is 14:49} \valI{True}, a \ros{} between \propI{The time is 14:49}, \valI{True} and some \pool{} which captures watch.
  Did not arbitrarily conclude \propI{The time is 14:49} \valI{True} in the same way Humpty Dumpty arbitrary chooses for `glory' to mean `a nice knock-down argument' (\cite[190]{Carroll:2009aa}).
\end{note}

\begin{note}
  So, focus is not on an account of what \ros{} are.
  ...
  \begin{quote}
    ...
    \mbox{ }\hfill\mbox{(\cite[9]{Dudeney:1995aa})}
  \end{quote}

  Interest with the puzzle is that you have all the information required to figure out the answer.
  Task is to identify understood method, and apply the method.
  In this sense, \ros{}.

  Enjoyment is applying understanding.
  Existence of a \ros{} does not entail any awareness of what the \ros{} nor that there is no value.

  In this case, specifying what the \ros{} amounts to in terms of present understanding is, I think, difficult.
  Basing grasp on algebra, representing natural language problems as algebraic problems, and mixed by some \emph{je ne sais quoi}.

  Some mystery may be resolved by considering an explicit method.
  For example, the \(i\)th number in the Fibonacci sequence is defined by the following function \(f\):
  ...
  \ros{} holds between \propI{The fourth number in the Fibonacci sequence is 5}, \valI{True}, and a \pool{} which captures the information just presented, along with any other required \prop{0}-\val{0} pairs.
\end{note}

\begin{note}
  \ros{} answer \qWhy{}.
  Details about the event answer \qHow{}.

  What, in any detail, a \ros{} amounts to depends on details about an agent's psychology which are too fine for the level of abstraction \qWhy{} and \qHow{} are stated at.
  Our interest with \issueInclusion{} is as a constraint on theories rather than the details of any given theory.
  In line with this level of abstraction, we characterise \ros{1} by two ideas referred to as `\supportI{}' and `\supportII{}', respectively.
  In short, these ideas state:

  \begin{itemize}
  \item
    A \ros{} holds between \(\pv{\phi}{v}\) and \(\Phi\), for the agent, when an agent concludes \(\pv{\phi}{v}\).
  \item
    A \ros{} may hold between \(\pv{\phi}{v}\) and \(\Phi\) from an \agpe{agent's} though there is no present or prior event in which the agent (is concluding or) concludes \(\pv{\phi}{v}\) from \(\Phi\).
  \end{itemize}
  %
  ...
  \supportII{} when an agent concludes \(\pv{\phi}{v}\) from \(\Phi\) the agent does not necessarily establish anything novel about the relationship between \(\pv{\phi}{v}\) and \(\Phi\) from their perspective.

  Fix our intuitive understanding of \ros{} in events in which an agent concludes.
  Lots of events in which agents conclude, so lots of events to help fix intuitive understanding of what `follows from' amounts to.
  \supportII{} does two things:
  \begin{enumerate}
  \item
    Helps fix understanding by denying that `follows from' is conceptually identical to `from'.
  \item
    Possibility of cases where `follows from' without an accompanying `from'.
  \end{enumerate}

  So, allows possibility that what we use to fix intuitive understanding does not captures all instances.

  Need to argue there are cases other than those used to fix intuitive understanding.
  Though, argument limited by only way to introduce \ros{} is by event.
  Argument follows in \autoref{part:ing} (specifically \autoref{cha:fc:fc-sII}).%
  \footnote{
    Note, this doesn't get a counterexample to \issueInclusion{}.
  }
\end{note}
...
\begin{note}
  Still, from \agpe{}.
  It's not clear that arbitrary instance of the sequence follow.
  Rather than take a stand, limit scope of \supportII{}.
  Though, we save this restriction for the full statement of \supportII{}, below.
\end{note}


\begin{note}
  Our approach to \ros{1} is similar to \citeauthor{Goldman:1979ui}'s account of \citeauthor{Goldman:1979ui}'s account of \emph{ex ante} justification in terms of \emph{ex post} justification.%
  \footnote{
...
\begin{note}
  ...
  As noted, strong.
  Restrict this idea.
  So long as it is possible there is an event in which the agent concludes, it is possible for a \ros{} to hold without a (\pwP{}) \wit{}.
\end{note}
...
\begin{note}
  \begin{idea}[\supportII{}]%
    ...
    \begin{itenum}
    \item[\emph{If}:]
      There is some action \(a\) \vAgent{} may immediately perform such that:
      \begin{itemize}
      \item
    ...
    \item[\emph{Then}:]
      ...
      \item
        ...
        \vAgent{} does not have a (\pwP{}) \wit{} for the \ros{} between \(\pv{\phi}{v}\) and \(\Phi\).
      \end{enumerate}
    \end{itenum}
    \vspace{-\baselineskip}
  \end{idea}
  ...
  \noindent%
  Consequent of the \itc{} is the `core' of \supportII{}.
  Entails that \ros{} between \(\pv{\phi}{v}\) and \(\Phi\) is distinct to relation which holds when an agent concludes \(\pv{\phi}{v}\) from \(\Phi\).

  Now, consequent.
  Possible without.
  Hence, conceptually distinct.

  The way I understand possibility, if two things are conceptually distinct then it is possible to have one without the other.
  Hence, 

  The antecedent of the \itc{} is designed to place reasonable constraints on this possibility.

  {
    \color{red}
    Note, these restrictions only restrict negative condition.
    Conceptually distinct, hence possibility.
    So, heck.

    TODO:
    SCRAP RESTRICTION. THIS IS USEFUL ONLY WHEN I GO TO FCS.
    HERE, POSSIBILITY IS JUST ENSURING CONCEPTUALLY DISTINCT.
    A RESTRICTION DOESN'T DO ANYTHING HERE.
    THINGS ARE MUCH CLEANER IF I STATE THAT THIS IS WHAT POSSIBILITY IS DOING, AND LEAVE THINGS AS THEY ARE.
  }

  
  As \ros{} are understood in part by an event in which an agent concludes, restriction in terms of events.

  Action \(a\) and concluding.
  If agent is concluding, then by \assuPP{} event in which agent concludes.
  So, there's an action the agent does, and by \supportI{} this is enough for a \ros{} to hold after the agent does the action.
  Role of concluding is to restrict interest to `non-arbitrary' conclusions.
  As observed in previous chapter, this narrows sense in which event when agent concludes is possible.

  Still, \assuPP{}, this does not say anything about what happens in the event.
  Hence, additional restriction, concludes without novel information.
  \ros{} captures something about what is sufficient to conclude.
...
\begin{note}
  ...
  \ros{}, in the sense that follows from present grasp on arithmetic.
...
\begin{note}
  ...
  Or, conclusion headline of tomorrows newspaper contains an even number of vowels.
  Need the headline.
