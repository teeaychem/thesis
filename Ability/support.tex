...
\begin{note}
  ...
  \footnote{
    The existence of a \fingfr{0} is restricted to the sub-event when the agent \evals{} \(\phi\) as having \val{} \(v\) as a we assume a concludes event spans the \agents{} reasoning to \(\pv{\phi}{v}\) from \(\Phi\).
...
\begin{note}
  With \supportI{} in hand we define a `\wit{0}' for \fingfr{}.
  In full:

  \begin{definition}[A \wit{0} for a \fingfr{1}]%
  ...
  \noindent%
  In short, a \wit{} for \fingfb{\(\pv{\phi}{v}\)}{\(\Phi\)} is some (extant) \eiw{0} the agent concludes \(\pv{\phi}{v}\) from \(\Phi\).
\end{note}
...
\begin{note}
  ...
  Hence, so long as the action is available \fofb{\pv{\propI{The six number in the Fibonacci sequence is 5}}{\valI{True}}}{some \pool{} which includes the \agents{} understanding of \(f\)}.
...
\begin{note}
  To help understand the role of \supportII{}, we return to \autoref{illu:gist:roots:F} and argue \fingfb{\pv{\rootsCon{}}{\valI{True}}}{some \pool{} \(\Phi\) which includes the \agents{} understanding of factorisation} answers \qWhy{}.
  ...
  \begin{application}[\qWhy{} and \autoref{illu:gist:roots:F}]%
    ...
    \begin{itemize}
    \item
      \fingfb{\pv{\rootsCon{}}{\valI{True}}}{the \pool{} \(\Phi\) which captures the \agents{} understanding of factorisation} answers \qWhy{} (the agent concluded \pv{\rootsCon{}}{\valI{True}} from \(\Phi\)).
    \end{itemize}
    \vspace{-\baselineskip}
  \end{application}
  ...
  \begin{dets}{appl:qWhyV-s1}
    ...
    Hence, \supportII{} (\autopageref{idea:support:possible}) entails \fofb{\pv{\rootsCon{}}{\valI{True}}}{\(\Phi\)} through \(\ed{\flat}\).
...
\begin{note}
  ...
  The second contrasts our understanding of the connexion between \fingfr{1} and an \eiw{0} an agent concludes to \citeauthor{Boghossian:2014aa}'s understanding of the connexion between `support' an \eiw{0} an agent infers.
...
\begin{note}
  ...
  Likewise, if \support{} characterises an \emph{ex post} \fingfr{0} and \supportII{} characterises an \emph{ex ante} \fingfr{0} we may say, in a broad stroke, an \emph{ex ante} \fingfr{} holds from an \agpe{} just in case there is some action the person may immediately do and as a result of doing the action a \emph{ex post} \fingfr{} holds from the \agpe{}.
