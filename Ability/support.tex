...
\begin{note}
  \autoref{cha:intro} introduced \ros{1} as an abstract way to capture the way a \prop{0}-\val{0} pair \(\pv{\phi}{v}\) `follows from' some \pool{} \(\Phi\) from an \agpe{} --- regardless of whether the agent has concluded \(\pv{\phi}{v}\) `from' \(\Phi\).

  In turn, \ros{0} answer \qWhy{}, and if \issueInclusion{} holds, for any \ros{} which answers \qWhy{} there is some event where agent concludes.
...
\begin{note}
  \supportI{} characterises when a \ros{} holds between some \prop{0}-\val{0} pair and \pool{} \emph{given} an \agents{} conclusion of the \prop{0}-\val{0} pair from the \pool{}.
  ...
  The role of \ros{0} a \ros{} between \(\pv{\phi}{v}\) and \(\Phi\) is to abstractly capture the way \(\pv{\phi}{v}\) `follows from' \(\Phi\) from the \agpe{}.
...
\begin{note}
  Applied to \scen{1}~\ref{illu:gist:roots:F}~and~\ref{illu:gist:roots:QF} \supportI{} entails a \ros{} between \pv{\propM{\rootsCon{}}}{\valI{True}} and a \pool{} which includes the \agents{} understanding of factorisation or the quadratic formula holds when the agent concludes.
  And, applied to \autoref{scen:countS} \supportI{} entails a \ros{} between \pv{\propI{What Pritcher said was a sign}}{\valI{True}} and some \pool{} holds from \agpe{Fox's} when Fox responds `Miran is early this year'.
\end{note}
...
\begin{note}
  With \supportI{} in hand we define a `\wit{0}' for \ros{}.
  In full:
  ...
  \begin{definition}[A \wit{0} for a \ros{1}]%
    ...
    \begin{itemize}
    \item
      An event \(\ed{\ast}\) is a \emph{\wit{0}} for a \ros{} between \(\pv{\phi}{v}\) and \(\Phi\), for \vAgent{} through an event \(\ed{}\).
    \end{itemize}
  ...
  \noindent%
  In short, a \wit{} for a \ros{} between \(\pv{\phi}{v}\) and \(\Phi\) is some (extant) \eiw{0} the agent concludes \(\pv{\phi}{v}\) from \(\Phi\).

  % If an agent has concluded \(\pv{\phi}{v}\) from \(\Phi\), we say the agent `has a \wit{}' for a \ros{} between \(\pv{\phi}{v}\) and \(\Phi\).
\end{note}
...
\begin{note}
  \supportII{} characterises when a \ros{} to hold between some \prop{0}-\val{0} pair and \pool{} from an \agpe{} \emph{without} an \eiw{0} the agent concludes the \prop{0}-\val{0} pair from the \pool{}.
  ...
  \begin{idea}[\supportII{}]%
    ...
    \item[\emph{Then}:]
      A \ros{} between \(\pv{\phi}{v}\) and \(\Phi\) holds from \agpe{\vAgent{}'} through~\(\ed{}\).
    \end{itenum}
    \vspace{-\baselineskip}
  \end{idea}
...
\begin{note}
  ...
  \footnote{
    Perhaps one may say a good puzzle of the kind given in \autoref{scen:fc:chick} is a puzzle where a \ros{} already holds between the intended solution and some \pool{} for the agent (after reading the puzzle) and obtaining a \wit{} for the \ros{} is enjoyable.
  }
...
\begin{note}
  ...
  Hence, so long as the action is available a \ros{} holds between \pv{\propI{The six number in the Fibonacci sequence is 5}}{\valI{True}} and some \pool{} which includes the \agents{} understanding of \(f\).
...
\begin{note}
  To help understand the role of \supportII{}, we return to \autoref{illu:gist:roots:F} and argue a \ros{} between \pv{\rootsCon{}}{\valI{True}} and some \pool{} \(\Phi\) which includes the \agents{} understanding of factorisation answers \qWhy{}.
  ...
  \begin{application}[\qWhy{} and \autoref{illu:gist:roots:F}]%
    ...
    \begin{itemize}
    \item
      A \ros{} between \pv{\rootsCon{}}{\valI{True}} and the \pool{} \(\Phi\) which captures the \agents{} understanding of factorisation answers \qWhy{} (the agent concluded \pv{\rootsCon{}}{\valI{True}} from \(\Phi\)).
    \end{itemize}
    \vspace{-\baselineskip}
  \end{application}
  ...
  \begin{dets}{appl:qWhyV-s1}
    ...
    So, by \autoref{obs:var:factor:fc} (\autopageref{obs:var:factor:fc}) \pv{\rootsCon{}}{\valI{True}} is a \fc{} from \(\Phi\) through \(\ed{\flat}\).
    Hence, \supportII{} (\autopageref{idea:support:possible}) entails a \ros{} between \pv{\rootsCon{}}{\valI{True}} and \(\Phi\) holds through \(\ed{\flat}\).
...
\begin{note}
  ...
  The first highlights the way \ros{1} of interest may be restricted.

  The second contrasts our understanding of the connexion between \ros{1} and an \eiw{0} an agent concludes to \citeauthor{Boghossian:2014aa}'s understanding of the connexion between `support' an \eiw{0} an agent infers.
...
\begin{note}
  Our characterisation of \ros{} by \supportI{} and \supportII{} rests on the way we understand conclusions.
  And, we place no constraints on the conclusions an agent may make.
  (See \autoref{cha:clar} for details).

  In particular, \supportI{} allows for \ros{} to hold between any \prop{0}-\val{0} pair and \pool{0}, so long as the agent concludes the relevant \prop{0}-\val{0} pair from the given \pool{}.
  And, \supportII{} allows for \ros{} to hold between any \prop{0}-\val{0} pair and \pool{0}, so long as the agent may be concluding the relevant \prop{0}-\val{0} pairs from the \pool{}.
  So, a \ros{} may hold between, e.g., \pv{\propI{Fish are mammals}}{\valI{True}} and some \pool{} from the \agpe{}.
  ...
  In short, of interest is only \emph{some} \ros{1} as given by \supportI{} and \supportII{}, rather than \emph{every} \ros{} given by \supportI{} and \supportII{}.
\end{note}
...
\begin{note}
  ...
  In contrast, we do not require that a \ros{} has any particular role \emph{for the agent} in \eiw{0} an agent concludes \(\pv{\phi}{v}\) from \(\Phi\).
...
\begin{note}
  From a structural perspective, our approach to characterising \ros{1} without a given an \eiw{0} the agent concludes is similar to \citeauthor{Goldman:1979ui}'s account of \emph{ex ante} justification in terms of \emph{ex post} justification.%
  \footnote{
