...
\begin{note}
  ...
  \begin{definition}[\wit{3} for a \ros{1}]%
    \label{def:witnessing}%
    \vspace{-\baselineskip}
    \begin{itemize}
    \item
      An event \(e\) is \emph{\wit{0}} for \ros{} between \(\pv{\phi}{v}\) and \(\Phi\), for \vAgent{}.
    \end{itemize}
...
\begin{note}
  ...
  \begin{idea}[\supportII{}]%
    ...
    \begin{itenum}
    \item[\emph{If}:]
      Event in which \vAgent{} is concluding \(\pv{\phi}{v}\) from \(\Phi\) and concludes without novel information.
    \item[\emph{Then}:]
  ...
  {
    \color{red}
    So, concluding entails concludes by \assuPP{}.
    However, it does not follow that the agent concludes without novel information.
...
% \begin{note}
%   In other words, no direct interest in \ros{} 
