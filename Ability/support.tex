...
\begin{note}
  ...
  However, you may also restrict \ros{} independently of conclusions.
  For example, I expect the argument which follows is compatible with restricting \ros{} to justified conclusions.

  That \ros{} hold between certain \prop{0}-\val{0} pairs and \pool{1} is important for the argument to follow, not that \ros{} may hold between arbitrary \prop{0}-\val{0} pairs and \pool{1}.
...
\begin{note}
  ...
  \begin{definition}[A \wit{0} for a \ros{1}]%
    ...
    \begin{itemize}
    \item
      An event \(e^{-}\) is \emph{\wit{0}} for a \ros{} between \(\pv{\phi}{v}\) and \(\Phi\), for \vAgent{} through event \(e\)
    \end{itemize}
...
\begin{note}
  ...
  % \begin{observation}[A conclusion entails witnessed \support{}]
  %   % \label{prop:cws}
...
\begin{note}
  ...
  \begin{definition}[A \pwit{} for a \ros{}]
    ...
    \begin{itemize}
    \item
      An event \(e^{-}\) is \emph{\pwit{0}} for a \ros{} between \(\pv{\phi}{v}\) and \(\Phi\), for \vAgent{} through event \(e\)
    \end{itemize}
