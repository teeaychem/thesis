...
\begin{note}
  ...
  \begin{itemize}
  \item The problem is getting to general ability.
  \item First observation, general ability and specific ability.
    \begin{itemize}
    \item In these cases, the specific ability is strictly weaker than the general ability.
    \end{itemize}
  \item Observation: In general, specific ability is boundless.
    \begin{itemize}
    \item There are many different checks on having a general ability.
    \end{itemize}
  \item That one has the specific ability is a \crequ{}.
  \item Hence, a point of tension.
  \item Intuitively, claim support, nothing particularly problematic about holding one has the general ability.
  \item However, at the same time, it is clear that there are various checks.
  \end{itemize}

  So, the problem is, concluding that one has the general ability.
  Various `inductive' premises, but\dots always a way to prevent.

  Now, either there's something which allows the agent to conclude that the specific instances are the case.
...
\begin{note}[Table]
  \begin{figure}[H]
    \centering
    \saMtxEmpty{}
    \repeatCaption{fig:saMtxEmpty}{Distinction matrix for interpretations of \aben{the}. \\ Rows are interpretations of ability, columns are type of reasoning regarding ability.}
  \end{figure}
\end{note}
