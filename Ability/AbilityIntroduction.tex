...
\begin{note}[Point about ability]
  \begin{enumerate}[label=(IE\arabic*), ref=(IE\arabic*), series=i_ex]
  ...
  So, you do not need to witness you ability to reason with basic algebra and radicals to be confident that the equation is true.\nolinebreak
  \footnote{
    Indicating a marked use of `ability' here.
    Developed in some detail below, will go some way to identifying relevant features.
  }
\end{note}

\begin{note}[Focus]
  The focal point of the above is the observation that:
  \begin{enumerate}[label=(IO\arabic*), ref=(IO\arabic*), series=i_ob]
  ...
  \begin{enumerate}[label=(IO\arabic*), ref=(IO\arabic*), resume*=i_ob]
  \item\label{intro:obs:entail} It would not be possible for an individual to have the (specific) ability if the result of the witnessing the ability were not true.\nolinebreak
    \footnote{The converse does not hold, some lack the ability to reason with basic algebra and radicals --- I lack the ability to read sheet music, though I may have been taught at one point.}
  \end{enumerate}
...
\begin{note}
  ...
  I was careful not to provide such implicit testimony.
...
\begin{note}
  ...
  \begin{enumerate}[label=(IE\arabic*), ref=(IE\arabic*), resume*=i_ex]
...
%   In cases of interest, something independent of ability must be true in order for agent to have ability.
  ...
  Third observation
  \begin{enumerate}[label=(IO\arabic*), ref=(IO\arabic*), resume*=i_ob]
  \item\label{intro:obs:resources} Interpretation of interest, sufficient resources are available to the agent.
  \end{enumerate}
  In position to witness.
...
\begin{note}[Interest in~\ref{intro:obs:uRa}]
  ...
  \item\label{intro:obs:nI} An agent is never in a position to claim support for truth of a proposition \(\psi\) given claimed support for some proposition \(\phi\) by observing that \(\psi\) must be true if \(\psi\) is true when:
    \begin{enumerate}[label=(\alph*), ref=(IO\arabic{enumi}\alph*)]
