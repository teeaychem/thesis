...
\begin{note}
  The parenthetical `due to' appended to~\ref{idea:requ:nPsi-nPhi:fail} ensures that the agent not concluding \(\pv{\phi}{v}\) from \(\Phi\) is tied to failing to conclude \(\pv{\psi}{v'}\) from \(\Psi\) after taking the relevant opportunity.%
  \footnote{
...
\begin{note}
  \begin{proposition}[\requ{3} and \fc{1}]
    \(\pvp{\phi}{v'}{\Psi}\) is a \emph{\requ{}} of concluding \(\pv{\phi}{v}\) from \(\Phi\) is equivalent to the following conditional:

    \begin{itemize}
    \item[\emph{If}:]
      \begin{enumerate}
      \item
        \(\pvp{\psi}{v'}{\Psi}\) is not a \fc{}.
      \end{enumerate}
    \item[\emph{Then}:]
      \begin{enumerate}
      \item
    ...
    \begin{argument}
      \ref{idea:requ:nPsi-nPhi} is of the form `\emph{If}~(\ref{idea:requ:nPsi-nPhi:opp} \emph{or} \ref{idea:requ:nPsi-nPhi:link}) \emph{then}~\ref{idea:requ:nPsi-nPhi:fail}'.
      And, `\ref{idea:requ:nPsi-nPhi:opp}~\emph{or}~\ref{idea:requ:nPsi-nPhi:link}' is equivalent to \emph{not}-(\emph{not}-\ref{idea:requ:nPsi-nPhi:opp}~and~\emph{not}-\ref{idea:requ:nPsi-nPhi:link}).
      Hence, \ref{idea:requ:nPsi-nPhi} is equivalent to `\emph{If}~\emph{not}-(\emph{not}-\ref{idea:requ:nPsi-nPhi:opp}~\emph{and}~\emph{not}-\ref{idea:requ:nPsi-nPhi:link})~\emph{then}~\ref{idea:requ:nPsi-nPhi:fail}'.

    And, `(\emph{not}-\ref{idea:requ:nPsi-nPhi:opp}~and~\emph{not}-\ref{idea:requ:nPsi-nPhi:link})' is equivalent to pair of conditions for \(\pvp{\psi}{v'}{\Psi}\) being a \fc{}.
    \end{argument}
  \end{proposition}
\end{note}
...
\begin{note}
  \color{red}
  Clause~\ref{idea:requ:nPsi-nPhi} expresses that concluding \(\pv{\psi}{v'}\) from \(\Psi\) is a check on whether it makes sense for the agent, from the \agpe{}, to conclude \(\pv{\phi}{v}\) from \(\Phi\).
\end{note}
...
\subsection{\requ{3} and (partial) checks on conclusions}
