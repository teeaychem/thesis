...
\begin{note}[Intro, locating]
  ...
  \autoref{cha:zS:sec:question:illu} will provide additional \illu{1} of \curb{} and \qzS{}.
\end{note}
...
\paragraph{\sCe{2} a \sCon{0}}

\begin{note}
  \sCing{2} extends to an event in which an agent concludes being an event in which an agent \sCon{1}
...
\begin{note}
  ...
  \begin{definition}[\requ{2}]
    ...
    \begin{itemize}
    \item
      For \agpe{\vAgent{}'}, \(\pvp{\psi}{v'}{\Psi}\) is a \curb{} on concluding \(\pv{\phi}{v}\) from \(\Phi\).
    \end{itemize}
  \end{definition}
...
\begin{note}
  Hence, we're not worried about whether or not the agent's conclusion in cases of \cScen{1} amounts to a \sCon{}.
\end{note}
...
\paragraph{\curb{3} and undercutting defeaters}
