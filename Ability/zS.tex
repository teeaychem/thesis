...
\begin{note}
  ...
  \autoref{question:zs} concerns an agent when concluding \(\pv{\phi}{v}\) from \(\Phi\), and asks whether, from the agent's perspective, something is true regarding any proposition-value-premises pairing \(\pvp{\psi}{v'}{\Psi}\) which satisfies a pair of constraints.
\end{note}

\begin{note}
  Important thing, fail to conclude, either something else or no conclusion.

  However, in an intuitive sense, failure in two ways.
...
\begin{note}[Lax variant]
  ...
  \begin{question}[Lax variant of~\autoref{question:zs}]
    For an agent \vAgent{}, when concluding \(\pv{\phi}{v}\) from \(\Phi\), is it the case that:
    \begin{itemize}
    \item
      From \vAgent{}' perspective:
...
\begin{note}[Negative variant]
  \qzS{} is framed so a positive answer is the best outcome.


  So, fail if there is some \(\pvp{\psi}{v'}{\Psi}\) such that fail would block, and might fail.
\end{note}



\begin{note}
  \autoref{question:zs} involves an agent, an understanding of the agent's epistemic state, some pool of premises \(\Phi\), and a proposition-value pair \(\pv{\phi}{v}\) such that the agent has not (yet) concluded \(\pv{\phi}{v}\) from \(\Phi\) and quantification of proposition-value-premises pairing \(\pvp{\psi}{v'}{\Psi}\) of a certain type.
...
\begin{note}
  \qzS{} has two main components.
  First, identifying \(\pvp{\psi}{v'}{\Psi}\) of interest.
  Second, statement about any such \(\pvp{\psi}{v'}{\Psi}\).
\end{note}
...
\begin{note}
  ...
  \item
    \autoref{cha:zS:sec:question:illu:additional}: Additional \illu{1}.
  \item
    \autoref{cha:zS:sec:question:illu:carroll}: Final, involved \illu{1} which links \qzS{} to \citeauthor{Carroll:1895uj}.
  \end{itemize}
\end{note}
...
\subsubsection{Additional \illu{3}}
