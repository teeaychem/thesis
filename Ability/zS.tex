...
\begin{note}
  ...
  \item
    ...
    As the agent entertains the possibility of failing to conclude \(\pv{\psi}{v'}\) from \(\Psi\), they do not conclude \(\pv{\phi}{v}\) from \(\Phi\).
  \end{enumerate}
  \vspace{-\baselineskip}
  \end{sketch}
...
\begin{note}
  ...
  \item
    ...
    As the agent entertains the possibility of concluding \(\pv{\psi}{v'}\) from \(\Psi\), they do not conclude \(\pv{\phi}{v}\) from \(\Phi\).
  \end{enumerate}
...
\begin{note}
  ...
  \begin{sketch}
    ...
    \item
      ...
      As the agent \emph{does not} entertain the possibility of failing to conclude \(\pv{\psi}{v'}\) from \(\Psi\), the agent concludes \(\pv{\phi}{v}\) from \(\Phi\).
    \end{enumerate}%
    \vspace{-\baselineskip}
  \end{sketch}
...
\begin{note}
  ...
  \item
    ...
    Whether or not it is the case that have ability.
  \end{enumerate}

  From the sketches, I and II are the first part, III is the second part.

  Check is key.
  Whether it makes sense to conclude \(\pv{\phi}{v}\) from \(\Phi\).
  Opportunity to check.

  Ability then concerns whether agent would satisfy check.

  \cScen{1}.
  \collateral{} reasoning, serves as check, as sketched.
\end{note}

\begin{note}
  Build this out in two separate parts.
  First, I and II, idea of a \requ{}.
  III, question `\qzS{}'.
\end{note}
...
\begin{note}[\requ{3}]
  ...
  With respect to the breakdown of the \scen{0} in \autoref{cha:zS:sec:lost-keys:analysis}, a \requ{} captures both the opportunity for the agent to reason about whether \(\pv{\psi}{v'}\) follows from \(\Psi\) and the idea of whether \(\pv{\psi}{v'}\) follows from \(\Psi\) being a \check{0} on concluding \(\pv{\psi}{v'}\) from \(\Phi\).
...
\begin{note}[Failure but no option]
  ...
  \citeauthor{Dretske:1970to}'s \scen{0} is framed in terms of knowledge, and is designed to raise a problem for conclude of knowledge under known entailment.
...
\subsection{\requ{3} as a (partial) checks on reasoning}
...
\begin{note}
  ...
  Negated, negative answer to this paraphrase positive answer to \qzS{}.%
  \footnote{
      ...
      \item
        ...
        \item
          ...
          \item[\emph{And}:]
            ...
            \item
              \label{question:zs:may-fail}
              \vAgent{} would \emph{fail} conclude \(\pv{\psi}{v'}\) from \(\Psi\), if \vAgent{} were to attempt to conclude \(\pv{\psi}{v'}\) from \(\Psi\).
            \end{enumerate}
          \end{itemize}
        \end{itemize}
      \end{itemize}
...
\begin{note}
  \qzS{}, clause~\ref{sketch:zS:succeed:no-c} of \autoref{sketch:zS:succeed}.
  However, significantly weaker.

  Clause~\ref{sketch:zS:succeed:no-c} of \autoref{sketch:zS:succeed} links agent not entertaining the possibility to not concluding.
  Positive answer to \qzS{} \emph{only} captures things from the agent's perspective.
  ...
  Chapter X work through the move to failure to conclude in some detail.
  Though,~\autoref{cha:clarification:sec:dis-and-dev} sketched outline.
...
\paragraph{Careful about positive answers}
...
\paragraph{Stronger variant}

\begin{note}
  \emph{When} concluding \(\pv{\phi}{v}\) from \(\Phi\).
  ...
  Stronger condition, though somewhat intuitive.
  ...
  the advantage of focusing on some proposition-value pair `required' by \(\phi\) having value \(v\) is a significant constraint on the range of proposition-value pairs an agent needs to consider in for the agent to get a positive answer to \qzS{}.

  From the agent's perspective, same may hold with respect to revision.

  However, benefit of \requ{} focus on \(\pv{\phi}{v}\) from \(\Phi\).
  Agent's current state is fine.
  What conclusions they may draw.
...
\begin{note}
  ...
  \begin{definition}[\zSX{}]
    ...
    \item
      \begin{enumerate}[label=]
      \item
        \zSX{} held between \(\pv{\phi}{v}\) and \(\Phi\) when \vAgent{} concluded \(\pvp{\phi}{v}{\Phi}\) from \(\Phi\), from \vAgent{}' perspective.
      \end{enumerate}
    \item \emph{If and only if}:
    \item
      \begin{itemize}
      ...
      \begin{enumerate}[label=]
        ...
        \item[\emph{Then}:]
          ...
          \item
            \label{question:zs:may-fail}
            \vAgent{} would have conclude \(\pv{\psi}{v'}\) from \(\Psi\), if \vAgent{} were to have attempt to conclude \(\pv{\psi}{v'}\) from \(\Psi\).
          \end{enumerate}
        \end{enumerate}
      \end{itemize}
    \end{enumerate}
...
\begin{note}
  Though we have no direct interest in \zS{}, additional motivation for \qzS{}.
\end{note}
...
\begin{note}
  ...
  \begin{enumerate}
  \item
    Support only if possible to reason.
  \item
  ...
  So, from the second and third, no support for conflicting.
  From the first, accessible to reasoning.
  Required to grant agent support from reasoning.
