...
\begin{note}
  ...
  \begin{illustration}[Sudoku]
    ...
    \label{fig:sudoku}
  \end{figure}
    An agent presses the `hint' button next to the Sudoku grid.
    The Sudoku grid is updated.

    The agent concludes that the current state of the grid rules out \dots
  \end{illustration}
  ...
  \begin{illustration}[Chess]
    \begin{figure}[H]
      ...
      \hfill
      \mbox{ }
      \caption{Chess.}%\protect\footnotemark}
      \label{fig:chess:intro}
    \end{figure}

    An agent highlights the queen.
    Squares are highlighted.

    The agent concludes that they have the option of moving the queen to \dots
  \end{illustration}

  As with \illu{1} \autoref{illu:gist:calc} and \ref{illu:gist:sudoku}, know whether, but automatic highlighting.
\end{note}

\begin{note}[Support]
  \begin{assumption}[Support]
    If an agent has concluded, then either a pool of premises or some pools of premises which support the conclusion.
  \end{assumption}
  ...
  Perhaps the rules of sudoku and chess are the relevant premises together with the relevant game states.
  Though, understanding of arithmetic is less clear.
...
\begin{note}[How and why]
  Instance of concluding.
  Some process, from inputs to an output, the conclusion.

  \begin{idea}[How and why]
    \mbox{ }
    \begin{itemize}
    \item
      Why has the agent concluded?
      Which pool of premises, or pools of premises, support the conclusion?
    \item
      How did the agent conclude?
      What information was appealed to by the agent to conclude?
    \end{itemize}
  \end{idea}

  In the example given\dots
\end{note}

\begin{note}
  Initial issue:

  \begin{issue}[Inclusion]
    \label{issue:why-inc-in-how}
    Is an answer to why always included in an answer to how?
  \end{issue}
...
\begin{note}[Causation]
  Ways of understanding causation on which this seems right.

  However, question is broader than causation.
\end{note}
...
\begin{note}
  \begin{issue}[Witness]
    ...
    If \(\Phi\) reason why, then agent has answered about how.
  \end{issue}

  In other words, witnessed.

  Simple cases, witnessing is present reasoning.
  However, may have reasoned in the past.
