...
\paragraph{Relationship between questions}
...
\begin{note}
  ...
  \begin{restatable}[Inclusion]{idea}{ideaInclusionFirst}
    \label{idea:how-why-relation-first}
    \label{issue:why-inc-in-how}
    Some proposition-value-premises pairing is, in part, an answer to `why?' only if that proposition-value-premises pairing is (also), in part, an answer to `how?'.
  \end{restatable}
...
\section{Concluding and support}
...
\section{Support, why, and how}
\label{sec:support-why-how}

\begin{note}
  Support is important.
  Relate to the two questions, and relation between these.
\end{note}

\subsubsection{Why}
\label{sec:why-support}

\begin{note}
  Assumption about support.
  Distinguishes from reasoning.

  Relation of support also \emph{why}.

  \begin{restatable}[Proposition-value-premises pairing only if support]{assumption}{assumptionSupportWhy}
    ...
    \begin{itemize}
    \item
      From the perspective of the agent's epistemic state when concluding \(\phi\) has value \(v\) from \(\Phi\), a relation of support holds between \(\Psi\) and \(\psi\) having value \(v'\), in part, answers why the agent concluded \(\phi\) has value \(v\) from \(\Phi\).
    \end{itemize}
    \vspace{-\baselineskip}
  \end{restatable}
\end{note}
...
\begin{note}
    \autoref{assumption:support}, concluding only if relation of support.

  \autoref{assu:why:pvpp:support}, answer to why an agent concludes only if relation of support.

  Constrain understanding of support.
\end{note}

\begin{note}
  Proposition-value-premises pairing as answer to why.
  This is somewhat delicate.
  If viewed as paring between some proposition-value pair and a set of proposition-value pairs, then this is bad.
  In principle, could be that such a pairing answers why, in part, without support.
  For, thought about \(\Phi\), led to thinking about \(\pv{\phi}{v}\), but then concluded \(\pv{\phi}{v}\) from \(\Phi'\).
  However, as conclusion from premises.
...
\begin{note}
  Support is involved.
  And, support is also, in part, an answer why.
  Captured by \autoref{assu:why:support:pvpp}.
\end{note}

\begin{note}
  \begin{restatable}[Support only if proposition-value-premises pairing]{assumption}{assumptionSupportWhy}
...
\begin{note}
  Relations of support constrain answers to why.

  Why an agent concludes.
  \autoref{assumption:support}, support is required for concluding.
  \autoref{assu:why:pvpp:support}, support is required to explain concluding.

  However, take why as fixed, and \autoref{assu:why:pvpp:support} is adding further clarity to what a relation of support amounts to.
  In part an answer to why.
\end{note}

\begin{note}
  Support, something distinctive about relation between premises and conclusion from agent's point of view.
  Hence, in part, why agent concludes.
\end{note}

\paragraph{Variants on `how' and `why'}
...
\begin{note}
  ...
  \begin{question}[\qHow{}?]
    ...
    \item
      Which proposition-value-premises pairings were part of the agent's reasoning?
    \end{itemize}
    \vspace{-\baselineskip}
    \vspace{-\baselineskip}
  \end{question}
...
\begin{note}
  Note, \autoref{idea:how-why-relation-first} was carefully stated.
  Recall:
  \begin{quote}
    \ideaInclusionFirst*
  \end{quote}

  Proposition-value-premises pairings as partial answers.
  \autoref{idea:how-why-relation-first} does not state that \emph{something} is, in part, an answer to `why?' only if that something is, in part, an answer to `how?'.

  Combination of ideas do not entail that if relation of support is, in part, an answer to `why?' then relation of support is also, in part, an answer to `how?'.
\end{note}

\begin{note}
  This is important.

  For, motivated inclusion in terms of causal theories.
  However, lose constraints on involvement of relation of support in reasoning.

  So, either causal theory adopts the same so no need for support, or finds some causal role of relation of support.

  As we've seen with \citeauthor{Boghossian:2014aa}, taking.
  Or, someone like \citeauthor{Wedgwood:2006ui} (see~\autoref{cha:motivation})

  So, more needs to be said.

  Response is:
  Mistake to assume that same things answer both questions.
\end{note}

\begin{note}
  However, important that support is involved.
\end{note}
...
\begin{note}
  So, now have \autoref{issue:why-inc-in-how}.
  This cleans up what went on in {\color{red} chapter} somewhat.

  Two problems.

  First, given weak assumptions regarding support, \autoref{issue:why-inc-in-how} is somewhat strained.

  Second, quite strong.

  Attention will go to a weaker issue, that is somewhat more straightforward.
\end{note}
...
\subsubsection{How support}

\begin{note}
  \begin{idea}
    proposition-value-premises pairing is, in part, an answer how only if witnessed reasoning.
  \end{idea}
\end{note}

\begin{note}
  \color{red}
  Here, join things together.
  So, relation of support why only if witnessing.
  This is the key idea.
\end{note}

\begin{note}
  Now, difficult to relate support and how.

  However, if included to think relation of support answers how, then previous idea may be reformulated.
  \begin{idea}
    Relation of support answers how only if witnessed.
  \end{idea}
...
\begin{note}
  ...
  So, proposition-value-premises pairing is, in part, and answer to `how?' only if the agent has \emph{witnessed} reasoning from pool of premises to conclusion.
...
\begin{note}
  \begin{restatable}[Witnessing]{issue}{issueWitnessing}
    ...
    If a relation of support between \(\psi\) having value \(v'\) and some pool of premises \(\Psi\) is, in part, an answer to, \qWhy{} an agent concluded \(\phi\) has value \(v\), then the agent has witnessed reasoning from \(\Psi\) to \(\psi\) having value \(v'\).
  \end{restatable}
