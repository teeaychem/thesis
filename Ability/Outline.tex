  ...
  \begin{restatable}{question}{questionZS}
    ...
    \item
      ...
      \item
        ...
        \vAgent{} \emph{would not} conclude \(\pv{\phi}{v}\) from \(\Phi\) if \vAgent{} were fail to conclude \(\pv{\psi}{v'}\) from \(\Psi\).%
        \footnote{
      ...
      \item
        ...
        It is not the case that: \vAgent{} would conclude \(\pv{\phi}{v}\) from \(\Phi\) if \vAgent{} were fail to conclude \(\pv{\psi}{v'}\) from \(\Psi\).
      \end{enumerate}
    \item
...
\begin{note}
  ...
  Role of clause~\ref{question:zs:subjunctive-non-trivial}.

  Whether or not clause~\ref{question:zs:subjunctive-non-trivial} is required comes down to the semantics of conditionals.

  Ensure that~\autoref{question:zs} concerns cases where option, and such reasoning terminates.

  Suppose clauses hold.
  However, positive resolution because would not fail.
  Agent reasons, does not conclude, but does not close reasoning.
  Hence, does not conclude \(\phi\) has some value other than \(v\), does not fail to conclude \(\phi\) has value \(v\), and does not conclude \(\phi\) has value \(v\).

  State of perpetual reasoning.
  Have not concluded, but have not failed to conclude.

  \ref{question:zs:option} rules these out?
  Plausibly, but there also seems to be a plausible reading where these are not ruled out.
  Hence,~\ref{question:zs:subjunctive-non-trivial}.
  States that in this case, the agent won't conclude.
  Hence, relevant \(\pvp{\psi}{v'}{\Psi}\) are restricted.
...
\subparagraph*{Revision}

\begin{note}[Revision]
  Before starting with the cases, highlight that \qzS{} is non-trivial, in the sense that possibility of failing to conclude.

  From this, possible to revise \(\chi\).
\end{note}

\begin{note}
  Suppose no revision.
  Then, \ref{question:zs:subjunctive} is trivial.
  Never fails to conclude.
  However, it then follows that \ref{question:zs:subjunctive-non-trivial} is not satisfied.
  If no revision, then no failure, hence, would conclude.

  In order to ask \qzS{}, needs to be the case that the agent allows relevant possibility of failure condition, and considers failure a problem.
\end{note}
...
\begin{note}
  ...
  \qzS{} is from perspective of agent.
  \(\chi\), not the case that the agent may.

  However, may be the case that option, revision.

  \qzS{} intuitively scopes over revision.
  ...
  Indeed, this is required for non-triviality.
\end{note}
