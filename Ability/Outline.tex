...
\paragraph*{Initial \illu{10}}

\begin{note}
  \begin{illustration}[Calculator]
...
\paragraph*{Clarifying}

\begin{note}
  Some process, from inputs to an output, the conclusion.
  Abstract, talk about support.
\end{note}
...
\begin{note}[Support]
  ...
  For example, \autoref{illu:gist:calc} involves an agent concluding \(23 \times 15 = 345\) is true, and the relevant pool of premises involves either the calculator, the agent's understanding of arithmetic, or possibly both.
\end{note}
...
\begin{note}
  ...
  \begin{quote}
    ...
    Explanatory and motivating reasons are not normative\dots
  ...
  Hence, prefer to speak in general terms.

  Proposition has value, other variations.
  Assume pool of premises is also proposition-value pairs.
  ...
  Something the agent appeals to.
  Conclusion doesn't come for free, and pool of premises identifies whatever.

  Pools of premises, for multiple pools.
  For example, testimony of calculator and understanding of arithmetic.
\end{note}
...
\begin{note}[How and why]
  Concluding, relation of support between conclusion and premises.
  Two questions.
\end{note}
...
\begin{note}[Why]
  \begin{question}[\qWhy{}?]
    \label{q:why}
    Why has the agent concluded \(\phi\) has value \(v\)?
  \end{question}

  Which relation, or relations, of support matter.
\end{note}

\begin{note}[How]
  \begin{question}[\qHow{}?]
    \label{q:how}
    How did the agent conclude \(\phi\) has value \(v\)?
  \end{question}
...
\begin{note}
  Relation of support is general, not necessarily tied to concluding.
  However, for any conclusion, relation of support.
\end{note}
...
\begin{note}
  ...
  \begin{restatable}[Witnessing]{idea}{ideaWitnessing}
    ...
    A relation of support holds between a pool of premises \(\Phi\) and a conclusion that \(\phi\) has value \(v\) only if the agent has witnessed reasoning from \(\Phi\) to \(\phi\) having value \(v\).
  \end{restatable}
...
\begin{note}[Audi]
  \begin{quote}
    \emph{S} believes that \emph{p}, for a reason, \emph{r}, at time \emph{t}, if and only if, at \emph{t}, there is a support relation, \emph{C}, such that
    (1) \emph{S} believes that p and that r;
    (2) \emph{S} believes \emph{C} to hold between \emph{r} and \emph{p} (or believes something to the effect that \emph{r} bears \emph{C} to \emph{p});
    (3) \emph{S}'s basis belief, that \emph{r}, and at least one connecting belief, i.e., his believing \emph{C} to hold between \emph{r} and \emph{p}, or his believing something to the effect that \emph{r} bears \emph{C} to \emph{p}, are part of what explains why \emph{S} believes \emph{p}, and, together, fully explain this;
    (4) \emph{S} is non-inferentially disposed, independently of seeking reasons he has, had, or might have had, at or before \emph{r}, for believing \emph{p}, to attribute his belief that \emph{p} to the explaining beliefs specified in (3);
    (5) those explaining beliefs do not (singly or together) accidentally sustain or produce \emph{S}'s belief that \emph{p} and
    (6) those beliefs do not (singly or together) sustain or produce it via an alien intermediary.%
    \mbox{}\hfill\mbox{(\citeauthor[262]{Audi:1986to})}
  \end{quote}

  \citeauthor{Audi:1986to}, as stated, does not need witnessing.
  Beliefs explain.

  Hold on, it seems the belief about\dots it's not clear why this should drop out of the answer to \qWhy{}.
\end{note}

\begin{note}
  Clear on \citeauthor{Longino:1978wv}.
  ...
  Distinction between objective and subjective points of view.
  \citeauthor{Longino:1978wv} bridges this.

  Delicate balance.

  Is it the case that we get 2)/b) without witnessing?

  Calculator case.
  Know whether.
  Some if were to reason, go through.

  Unclear.%
  \footnote{
    Not a criticism of \citeauthor{Longino:1978wv}.
    The paper is short, and focuses on the interaction of objective and subjective.
    Nicely summarises what's going on with \citeauthor{Audi:1986to}.
  }
\end{note}

\begin{note}
  Returning to~\citeauthor{Audi:1986to}, a little clearer.
  \begin{quote}
    The account I shall develop, then, will construe beliefs for reasons as responses to reasons, and not mere effects; as held in the light of reasons, and not merely explainable in terms of them; and as, in a special way, under the control of reasons.%
    \mbox{}\hfill\mbox{(\citeauthor[237]{Audi:1986to})}
  \end{quote}

  Seems to involve witnessing.

  Doxastic component narrows down parts which are answer \qWhy{}.
\end{note}

\begin{note}[Tolliver]
  Skip the account, but the pendulum case.
  ...
  \begin{quote}
    ...
    ``For all things, call them x, if x is a pendulum of length \emph{1} (call this ``Lx''), then x is a pendulum of period \emph{p} (``Px''), and (x) (if Px, then Lx).
    ...
    \footnote{No typos, the initial left double quotation mark is not closed.}
  \end{quote}
  ...
  What else is there other than the requirement for witnessing?
\end{note}


\begin{note}
  An additional \illu{0}\dots
  ...
  As with~\autoref{illu:gist:calc}, know whether \dots
  However, pressed the hint button.
  Didn't work through the constraints.
  In particular, did not notice presence of \(1\) in the top right cell of the bottom left square, and did not combine this with understanding that each cell in a square must be distinct.
\end{note}

\begin{note}[Summary]
  In both \illu{1}, two possible accounts of why.
  How the agent concluded, one account.
\end{note}


\paragraph*{Broader issue}
...
\begin{note}
  ...
  \begin{restatable}[Witnessing]{issue}{issueWitnessing}
    ...
    If a relation of support between \(\phi\) having value \(v\) and some pool of premises \(\Phi\) answers, or is part of the answer to \qWhy{} an agent concluded \(\phi\) has value \(v\), then the agent has witnessed reasoning from \(\Phi\) to \(\phi\) having value \(v\).
  \end{restatable}
...
\begin{note}
  Basic idea.
  Check.
  Prior to concluding \(\pv{\phi}{v}\), check on whether it makes sense, from agent's perspective, to conclude.
  ...
  However, see the idea.
  Particular kind of support.
  \zS{}.

  Insight, in various cases, no witnessing.
  \fc{0}.

  Calculator.
  Test to see whether or not it's functioning correctly.
  If fail to conclude sum, then calculator is no good.
  Conversely, calculator only if \fc{0}.
  ...
  \item
    ?Testimony
  \item
    not \fc{0} then not testimony
  \item
    Equiv.\ testimony then \fc{0}.
  \item
    Testimony is fine.
  \item
    \fc{0}
...
\paragraph*{The general pre-theoretic argument}
...
\begin{note}
  With the notion of a \requ{} in hand, we now define \zSN{0}:
  ...
  \Autoref{idea:zs} just is~\autoref{question:zs}, refined with the notion of a \requ{}, and coupled with the position resolution to~\autoref{question:zs}.
  ...
  Hence, we will may also abbreviate `\autoref{question:zs}' to `\qzS{-}', or more often `\qzS{}'.

  Going forward, or goal is to give an equivalent characterisation of \zS{}.
  Or, equivalently, an alternative characterisation of a negative resolution to \qzS{}.
...
\begin{note}
  \begin{idea}[\iZS{-} --- \iZS{}]
    ...
    An agent \vAgent{} has \emph{\ZS{-}} for a proposition-value pair \(\pv{\phi}{v}\) with respect to some pool of premises \(\Phi\) when concluding \(\pv{\phi}{v}\) from \(\Phi\) just in case:
    \begin{itemize}
...
\begin{note}
  The key distinction between \izS{} and \iZS{} is that the idea of an agent settling that they would conclude \(\pv{\psi}{v'}\) from \(\Psi\) for any \requ{} \(\pvp{\psi}{v'}{\Psi}\) is replaced by the agent concluding \(\pv{\psi}{v'}\) from \(\Psi\).
\end{note}

\begin{note}
  The relevance of this distinction is captured by the following proposition:
    \begin{proposition}[Equivalence between \qzS{}, \izS{}, and \iZS{}]
  ...
  Note, the equivalence between~\ref{prop:qzs-tick-equals-iCS:qzS} and~\ref{prop:qzs-tick-equals-iCS:qzS} is immediate, given that we introduced \zS{} as shorthand for a negative resolution to \qzS{}.
