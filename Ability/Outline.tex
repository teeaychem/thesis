...
\begin{note}
  ...
  \begin{question}[\csN{2}]
    \label{question:CS}
    For an agent \vAgent{}, after to concluding \(\pv{\phi}{v}\) from \(\Phi\):

    Is it the case that \(\phi\) having value \(v\) ensure that there is some proposition-value-premise pairing \(\pvp{\psi}{v'}{\Psi}\) such that (from \vAgent{}'s perspective) \vAgent{} \emph{may fail} to conclude \(\pv{\phi}{v}\) if they were to have first reasoned attempted to conclude \(\pv{\psi}{v'}\) from \(\Psi\)?
    \begin{itemize}
...
\begin{note}
  Plausible, also for premises.
  Need to be the case that would not have diverged from premises.

  Else, question.

  However, this is delicate.
  We're interested in a fixed epistemic state.
  And, if concluded then part of the epistemic state.
  For different value, would need to revise epistemic state.

  Hence, if the agent has concluded, then this is enough.

  Indeed, whether this holds in general is unclear.

  So, \csN{} is relative.
  Prior to concluding \(\pv{\phi}{v}\) was open.
  And, \(\pv{\psi}{v'}\) was also open.
\end{note}
...
\begin{note}
  \iCS{} states if an agent \csV{} for \(\pv{\phi}{v}\) from \(\Phi\), then any \requ{}, \(\pvp{\psi}{v'}{\Psi}\), of concluding \(\pv{\phi}{v}\) from \(\Phi\) has been satisfied in either of two ways:
...
\begin{note}
  \iCS{} is our understanding of \csN{}.
  Intuitively, satisfying clauses of \iCS{} is sufficient to answer~\autoref{question:CS}.
  For, grant that notion of a \requ{0} captures the relevant cases, \iCS{} requires concluding.

  Still, by no means done.

  There is an important question.
  Whether \iCS{} is the weakest condition sufficient to answer~\autoref{question:CS}.

  And, a clearer understanding of what \iCS{} amounts to.
\end{note}

\begin{note}
  Two sections.
  First, argue that no weaker principle is sufficient.
  Second, understanding \iCS{} in further detail.

  These two subsections are core parts of this document.
  See how important \iCS{} is, and observe important features of \iCS{}.
  However, \iCS{} is fine for a negative resolution.
  Indeed, this is why our target is tension and motivation for a positive resolution.
\end{note}


\subsubsection{\iCS{}, \iCSm{}, and the role of \iCS{} in the overall argument}
\label{overview:sec:iCS-iCSm-limitation-closure}

\begin{note}
  \iCS{}, concluding.
  \autoref{question:CS}, whether the agent may have failed to conclude differently.
  Query whether resolution to~\autoref{question:CS} only if agent has concluded.
...
\begin{note}
  \begin{idea}[\csmN{2} --- \iCSm{}]
    \label{idea:CpS:overview}
    An agent \csV{} for \(\pv{\phi}{v}\) when concluding \(\pv{\phi}{v}\) from \(\Phi\) \emph{only if}:
    \begin{enumerate}[label=\arabic*., ref=\named{CS\(^{-}\):\arabic*}]
    \item
      \label{idea:CpS:overview:requ}
      For any proposition-value pair \(\pv{\psi}{v'}\) which is a \requ{} of concluding \(\pv{\phi}{v}\) from \(\Phi\) either:
      \begin{enumerate}[label=\alph*., ref=\named{CS\(^{-}\):1.\alph*}]
      \item
...
\paragraph*{Two views on \iCS{} and \iCSp{}}

\begin{note}[Two views on \iCS{}]
  There are two ways to view \iCS{}.
  First, as a limitation on \csVImp{} and second as a closure condition on \csVImp{}.

  When viewing \iCS{} as a limitation on \csVImp{}, whether the agent has \csVed{} is at issue, and hence failure to have concluded \(\pv{\psi}{v'}\) from some \(\Psi\) shows that the agent has not \csVed{}.

  In full, the following conditional follows directly from \iCS{}:
  ...
  When viewing \iCS{} as a closure condition on \csVImp{}, we assume the agent \emph{has} \csVed{},%
  \footnote{
    Whether the agent has \csVed{} is \emph{not} at issue.
  }
  and \iCS{} establishes the agent has concluded \(\pv{\psi}{v'}\) from some \(\Psi\), for any \(\pv{\psi}{v'}\) that is a \requ{} of concluding \(\pv{\phi}{v}\) from \(\Phi\).
  ...
  Now, from the perspective of \emph{motivating} \iCS{}, our attention has focused on \iCS{} as a limitation.%
  \footnote{
    Recall, if an agent has not considered every place in which they may have left their keys, then the agent may not \csN{} that they have lost their keys.
    For, the agent may conclude their keys are in a place they have not yet considered.
    }
  However, from the perspective of developing tension via \iCS{}, our attention will be focused on \iCS{} as a closure condition.
\end{note}

\begin{note}
  If only get \iCSm{}, then problem.
\end{note}
...
\begin{note}
  ...
  Supposing we have some \requ{}, then two cases.
  Either the agent has concluded, or the agent has not yet concluded.
  This distinction gives us the two clauses.
  A tells us that concluding is sufficient, and B applies just in case the agent has not concluded.

  Various questions.
  Key is concluding.
  Both clauses appeal to this.
  Still, question is whether the agent would conclude.
  Why not the weaker proposition that would not fail to conclude.

  {
    So, suppose not possible to fail and not concluded.
    Then, there is something to check.
    The former tells me that from current perspective, the check will always be satisfied.
    However, check still remains.
    And, it remains the case that if were to fail check, then would give up \(\pv{\phi}{v}\) from \(\Phi\).
    Indeed, in this case, not only give up \(\pv{\phi}{v}\) from \(\Phi\), but also a revision of epistemic state.
    For, give up that not possible to fail.

    Might think this is problematic.
    Take the agent's epistemic state as fixed.
    Still, the observation is only that so long as a question remains, then possible to fail to conclude.

    Okay, push back.
    Suppose we're before concluding not possible to fail.
    Then, concluding is clearly a \requ{}.
    So, as in the present epistemic state, no conclusion, it follows that in the prior epistemic state no \csN{}.
    Those branches are still open.

    Now, this makes \csN{} quite strong.
    Branches persist.
    However, recall that a \requ{} is always relative to some proposition-value pair being concluded.
    It must be the case that the branch still holds up in the agent's current epistemic state.
  }
\end{note}

\begin{note}
  We offer a `squeezing' argument.
  ...
  \(p \rightarrow q\), \(q \rightarrow r\), \(r \rightarrow p\), then \(q \leftrightarrow r\).

  Subtlety, we're not interested in conditions for something to obtain.
  Rather, for something to be a resolution to the issue.
  Sufficient will not be a necessary condition for necessary condition to obtain in general, but will be for the necessary condition to resolve the relevant issue.
...
\begin{note}
  ...
  For, neither~\ref{idea:CS:overview:requ-sat:Past:alt} nor~\ref{idea:CSm:overview:requ-sat:Pres} would require the agent to conclude \(\pv{\psi}{v'}\) from \(\Psi\).

  Indeed, we will argue for the existence of cases of exactly the kind described.
  Hence, the role of~\ref{idea:CS:overview:requ-sat:Past} and~\ref{idea:CS:overview:requ-sat:Pres} over~\ref{idea:CS:overview:requ-sat:Past:alt} and~\ref{idea:CSm:overview:requ-sat:Pres} is not merely an issue of motivation, but also crucial to establishing tension.
\end{note}

\begin{note}[Argument]
  With the above observations in hand, we now turn to providing an argument for ~\ref{idea:CS:overview:requ-sat:Past} and~\ref{idea:CS:overview:requ-sat:Pres} over~\ref{idea:CS:overview:requ-sat:Past:alt} and~\ref{idea:CS:overview:requ-sat:Pres:alt}.
  ...
  Therefore, the agent also has the option of checking whether~\ref{idea:CS:overview:requ-sat:Past:alt} and~\ref{idea:CS:overview:requ-sat:Pres:alt} hold.
