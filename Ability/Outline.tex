...
\begin{note}
  ...
  \begin{restatable}[\zsN{2} --- \zs{}]{definition}{definitionZS}
    \label{def:zs}
    An agent \vAgent{} has \zs{} --- or \zsN{0} --- for a proposition-value pair \(\pv{\phi}{v}\) with respect to some pool of premises \(\Phi\) when concluding \(\pv{\phi}{v}\) from \(\Phi\) just in case:
    \begin{itemize}
    \item
  ...
  \Autoref{def:zs} just is~\autoref{question:zs} coupled with the position resolution to~\autoref{question:zs}.
...
\begin{note}
  ...
  \Autoref{prop:qzs-tick-equals-iCS:var} states that an agent satisfying \iCS{} is a necessary condition of the agent satisfying an property \(\chi\) which is sufficient to resolve~\autoref{question:zs}.
