...
{
  \color{red}
  Start with a some \illu{1}\dots

  Maps.

  Calculator.

  Not the case that concluded from understanding of (basic) arithmetic.
}
...
\begin{note}
  So, key interest is understanding concluding.

  Negative resolution, then in a variety of cases, separate concluding from witnessing.

  Two possible outcomes.

  \begin{itemize}
  \item
    In general, witnessing may be reduced to various different components.
  \item
    No unified phenomenon.
  \end{itemize}

  Which of these, beyond scope.
  Focus is on motivating this.

  Two threads.

  \begin{itemize}
  \item
    Instances, how to understand this idea.
  \item
    Interesting instances.
  \end{itemize}

  Second is of most interest.

  Grant concluding is just a term, then perhaps not too much interest in providing an account of the term which allows separation.

  Possible reduction in some cases, even if granting the possibility of this reduction, it serves no interesting role.
  No need to separate in order to understand agents.

  Why there is some interest in doing this.
  Broad target is witnessing a pre-theoretical constraint.
  Plausible pre-theoretical constraint.
  Abundant.
  Process.
  So, always some start point.
  Hence, always some premises.

  Broad strategy for pre-theoretical constraint:

  For every instance where reduction, alternative analysis where same benefits of reduction are obtained via analysis involving witnessing.

  `General pre-theoretic argument'.
  In short, always resources to construct such an analysis.

  Problem going against pre-theoretic constraint, related to the counterexample fallacy.
  In short, counterexample fallacy, counterexample to any specific proposal is not a counterexample to any particular analysis is not a counterexample to a style of analysis.
  Need to show problem with the style.
  A general recipe for generating counterexamples.

  If some pre-theoretical constraint, no good to come up with instances which do not necessarily obey the constraint.
  For, this does not show that it's worth the reduction.

  Of course, this does not guarantee that the theory is any good.
  However, it seems the pre-theoretical constraint is intuitive.

  So, a general recipe for generating bases for reduction.

  Basics of the counterexample fallacy, varied to cover alternative theories.
  Possibility of reduction, but also general account.
  Sufficiently general to place strain on the pre-theoretical constraint.

  That's the target, pressure on non-reduction.
  Motivation against taking witnessing as a pre-theoretical constraint.

  Then, future, broaden this.

  {
    \color{red}
    Should put in somewhere, about `has', allowing agent to `re-conclude'.
    Concluding involves witnessing or having witnessed.
    Then, section on the two options is also good, and kind of interesting.
  }
\end{note}

\begin{note}
  For a parallel, consider knowledge.
...
\begin{note}
  Positive resolution, limited, as only instances.
\end{note}


\begin{note}[Foregone-conclusions]
  So, \autoref{issue:Main}, whether concluding is the composition of some relation \(R\) and witnessing.
  Or, whether it is the case that in some cases an agent witnesses reasoning when concluding and in other cases the agent does not.

  Talk of concluding, to keep things straightforward.

  Key idea governing our use of the term `concluding' is re-assignment.
  If some composite, the re-assign concluding to non-witnessing components.
...
\begin{note}[Foregone-concluding]
  ...
  \begin{restatable}[Foregone-concluding]{idea}{ideaForegoneCing}
    ...
    \begin{itemize}
    \item
      `Concluding' \(\coloneq\) the combination of \(R\) and witnessing \(R\)
    \end{itemize}
    Then:
    \begin{itemize}
    \item
      `Concluding' \(\coloneq\) \(R\)
    \end{itemize}
    \vspace{-\baselineskip}
  \end{restatable}
