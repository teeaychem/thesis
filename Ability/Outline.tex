...
\begin{note}
  Basing relation.
  If concluded, then basing relation.
  Conversely is less clear.
  In particular, consider Audi.

  For this reason, we will limit our discussion of basing to a pair of related accounts.
  Swain and Tolliver.
\end{note}
...
\begin{note}[Audi]
  \begin{quote}
    ...
    \item
      \emph{S} believes that \emph{p} and that \emph{r};
    \item
      \emph{S} believes \emph{C} to hold between \emph{r} and \emph{p} (or believes something to the effect that \emph{r} bears \emph{C} to \emph{p});
    \item
      \emph{S}'s basis belief, that \emph{r}, and at least one connecting belief, i.e., his believing \emph{C} to hold between \emph{r} and \emph{p}, or his believing something to the effect that \emph{r} bears \emph{C} to \emph{p}, are part of what explains why \emph{S} believes \emph{p}, and, together, fully explain this;
    \item
      \emph{S} is non-inferentially disposed, independently of seeking reasons he has, had, or might have had, at or before \emph{r}, for believing \emph{p}, to attribute his belief that \emph{p} to the explaining beliefs specified in (3);
    \item
      those explaining beliefs do not (singly or together) accidentally sustain or produce \emph{S}'s belief that \emph{p} and
    \item
      those beliefs do not (singly or together) sustain or produce it via an alien intermediary.\newline
      \mbox{}\hfill\mbox{(\citeauthor[262]{Audi:1986to})}
    \end{enumerate}
  \end{quote}
  {
    \color{red}
    Cut of after 3, use as Audi states `\dots'
  }

  Interest is (3).
  Here, causation is not explicitly involved.
  However, in short, witness reasoning from \emph{r} to \emph{p}.
  Describing a rule of inference.

  Connecting belief, \emph{de dicto} or \emph{de re} (around 248)

  \begin{quote}
    If \emph{S} believes \emph{p} for a reason \emph{r}, then the associated basis and connecting beliefs are sufficient to explain why he believes \emph{p}; indeed, together they fully explain it, in the sense that citing them (given appropriate contextually presupposed information) is \emph{sufficient} to provide an understanding of why \emph{S} believes \emph{p}, even if some other reason state is also sufficient.
  \end{quote}

  Stated in terms of belief, but witnessing is still involved.

  Further question, do we get (2) without witnessing?
\end{note}
...
\begin{note}[Back to Swain]
  Have:
  \(P = 2\pi\times\sqrt{L}{g}\) iff \(L = g(\frac{P}{2\pi})^{2}\)
...
\begin{note}
  \begin{quote}
    ...
    \item
      ...
      \item
        A believes that the truth of p is evidence for the truth of q att,and
      \item
...
\begin{note}[Shifting]
  Started with \autoref{illu:gist:calc}.
  Here, check on calculator.
  More generally, testimony.

  However, slight difficulty.
  Isn't about own reasoning.

  Motivating \zS{}, focus is on own process.

  To keep parallel, variant case.
  Return to testimony later.
\end{note}
