  ...
  \begin{restatable}{question}{questionZS}
    ...
    \item
      ...
      \item
        ...
        \vAgent{} \emph{would not} conclude \(\pv{\phi}{v}\) if \vAgent{} were fail to conclude \(\pv{\psi}{v'}\) from \(\Psi\).%
        \footnote{
          This condition is expressed by a subjunctive conditional because we have no guarantee that an agent will attempt to conclude \(\pv{\psi}{v'}\) from \(\Psi\).
      ...
      \item
        ...
        \vAgent{} has the option of concluding \(\pv{\psi}{v'}\) from \(\Psi\).
      \item
...
\begin{note}
  ...
  Second, a negative resolution.
  Prior to concluding \(\pv{\phi}{v}\) from \(\Phi\) there is no such \(\pvp{\psi}{v'}{\Psi}\) as described.
  Note, there are two distinct ways in which the negative resolution may be obtained.
  \begin{enumerate}
  \item
    There is no such \(\pvp{\psi}{v'}{\Psi}\).
  \item
    The agent has settled that \(\pv{\psi}{v'}\) from \(\Psi\) for any such \(\pvp{\psi}{v'}{\Psi}\).
  \end{enumerate}
\end{note}
...
\begin{note}
  ...
  I have some doubts as to whether or not this metaphor really works, but it is at least far easier to talk of an agent having \zs{} for some proposition-value-premise pairing, than saying an agent has a negative resolution to~\autoref{question:zs} with respect to the proposition-value-premise pairing.
...
\begin{note}[Notion of a \requ{}]
  \begin{notion}[\requ{3}]
    ...
    \item
      ...
      \item
        ...
        %   If \(\pv{\phi}{v}\) is a member of \(\Psi\), then the agent
        \item
  ...
  \begin{itemize}
  % \item
  %   By~\ref{notion:overview:requ:pool:int}, the agent need not reason from \(\pv{\phi}{v}\) as a premise to conclude \(\pv{\psi}{v'}\).
  \item
...
\begin{note}[With \(\chi\)]
  ...
  Now, by~\ref{question:zs:may-fail}, \(\chi\) must ensure that from the agent's perspective, the agent would not fail to conclude \(\pv{\psi}{v'}\) from \(\Psi\).
...
\begin{note}
  Further, the basic strategy applied to case~\ref{iZm:arg:case:I} does not extend to case~\ref{iZm:arg:case:II}.
...
\begin{note}[Still a \requ{}]
  ...
  What matters is option and failure.

  Of course, would not fail.
  However, if reason and did fail, problem.

  So, from the present point of view, it is a \requ{}.
\end{note}
...
\begin{note}
  There is a subtle difference between the two cases.
  By assumption, \(\chi\), has not concluded.

  So with \(\chi\), two types of possible epistemic state if reason.
  ...
  By contrast, with concluding, the agent already has the relevant property.
  The agent has concluded \(\pv{\psi}{v'}\) from \(\Psi\).
  So, revision.
  But, assumed, conclusion is sufficient to rule out revision, without change to epistemic state.

  So, if reason, new epistemic state.
  It is the case that from present, would not.
  But, not yet in epistemic state.

  So, question.
  If reason, which type of epistemic state?
  But this is \qzs{}.
  \(\chi\) gives agent's perspective.
  However, as the agent has not concluded, this perspective is limited.
  So, insufficient.

  Form a different view, because no conclusion and option, check on any appeal to \(\chi\).

  If \(\chi\) does not entail concluding, then, there remains the possibility of surprise when reasoning from \(\Psi\).
\end{note}
...
\begin{note}
  \begin{itemize}
  \item
    If the agent were to fail to conclude \(\pv{\psi}{v'}\) from \(\Psi\), the agent would not conclude \(\pv{\phi}{v}\) from \(\Phi\).
  \item
    If \(\phi\) has value \(v\), then the agent has the option of concluding \(\pv{\psi}{v'}\) from \(\Psi\).
  \item
    Whether or not, test.
  \item
    \(\chi\), doesn't entail concluded.
  \item
    So, from the viewpoint of the agent, test of whether they satisfy \(\chi\) by attempting to conclude.
  \item
    If fail to conclude \(\pv{\psi}{v'}\) from \(\Psi\), would follow that the agent does not satisfy \(\chi\).
  \item
    So, it remains the case that failure to conclude would lead to a failure.
  \end{itemize}
...
\begin{note}[Importance of \csN{}]
  \csN{} is key.
  Argument for negative resolution to~\autoref{issue:Main} largely rests on \csN{}.
...
\begin{note}
  ...
  will lead to tension with a positive answer to~\autoref{issue:Main} just in case we manage to find an instance in which an agent \csN{} for \(\pv{\phi}{v}\) from \(\Phi\) without witnessing reasoning from \(\Psi\) to \(\pv{\psi}{v'}\) for some \requ{} \(\pvp{\psi}{v'}{\Psi}\) of concluding \(\pv{\phi}{v}\) from \(\Phi\).
...
\begin{note}
  ...
  If you go for positive answer, then ruled out, but then first option alone.
