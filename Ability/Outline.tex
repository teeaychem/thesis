...
\begin{note}
  The general type of scenario \autoref{illu:gist:calc} conforms to falls just outside the scope of the core argument of this document.
  ...
  Second, because I expect there will, for at least some time, be some mystery about what distinguishes this general type of scenario.%
  \footnote{
    Well, it's testimony.
    Oops.
...
\begin{note}
  \begin{scenario}[Quadratic roots]
    ...
    \item
      \label{illu:gist:roots:qf:4}
      \(x = 1\) or \(x = -\sfrac{1}{2}\) \hfill \ref{illu:gist:roots:qf:3}, Simplification
    \end{enumerate}
  ...
  Indeed, for present purposes, our interest is with steps \ref{illu:gist:roots:qf}, \ref{illu:gist:roots:subs}, and \ref{illu:gist:roots:qf-subs}.

  Intuitively, the agent concludes `\(x = 1\) or \(x = -\sfrac{1}{2}\) if \(2x^{2} - x - 1 = 0\)' in part from their understanding of arithmetic.
  And, specifically, in part from their understanding of the quadratic formula and how to use it.

  In other words, the quadratic formula is, intuitively, a premise of the agent's reasoning.
\end{note}

\begin{note}
  Still, in the same way the agent may have calculated \(23 \times 15 = 345\) without the aid of a calculator in \autoref{illu:gist:calc}, the agent may have concluded \(x = 1\) or \(x = -\sfrac{1}{2}\) if \(2x^{2} - x - 1 = 0\) without the aid of the quadratic formula in \autoref{illu:gist:roots}.

  For example, consider the following variant:
  \begin{quote}
  ...
  \item
    Either \(x = 1\) or \(x = -\sfrac{1}{2}\). \hfill \ref{illu:gist:roots:zero}, \ref{illu:gist:roots:case:a}, \ref{illu:gist:roots:case:b}
  \end{enumerate}
  \end{quote}
\end{note}

\begin{note}[Difference between the scenarios]
  The key difference between \scen{1} \ref{illu:gist:calc} and \ref{illu:gist:roots} is the presence and then absence of (roughly) testimony.

  With \autoref{illu:gist:roots}, the agent did not receive information that `\(x = 1\) or \(x = -\sfrac{1}{2}\) if \(2x^{2} - x - 1 = 0\)' from an external source.

  Intuitively, the agent \emph{concluded} `\(x = 1\) or \(x = -\sfrac{1}{2}\) if \(2x^{2} - x - 1 = 0\)' from pool of premises which includes the quadratic formula.
  Intuitively, the agent \emph{did not conclude} `\(x = 1\) or \(x = -\sfrac{1}{2}\) if \(2x^{2} - x - 1 = 0\)' from a pool of premises that do not involve the quadratic formula.
\end{note}
...
\begin{note}
  {
    \color{red}
    \emph{Only if}
    This does not restrict support to come from concluding.
  }
\end{note}
