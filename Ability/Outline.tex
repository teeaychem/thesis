...
\begin{note}[Ability]
  ...
  Hence, it seems clear that if one \csV{} for \(\langle \phi,v \rangle\) when concluding \(\langle \phi,v \rangle\) from \(\Phi\) and one's reasoning is a specific instance of a general ability, then there at least various \requ{1} associated with concluding \(\langle \phi,v \rangle\) from \(\Phi\).
...
\begin{note}
  ...
  A \cluster{0} is a collection of proposition-value-premise pairs such that every proposition-value-premise pair is a \requ{} of every other proposition-value-premise pair in the collection.

  Intuitively, every proposition-value-premise pair of the \cluster{0} is an independent check on every other proposition-value-premise pair of the \cluster{0}.

  \cluster{3} have a few interesting properties, and certain kinds of \cluster{1} have further interesting properties.

  Indeed, obtain~\autoref{goal:tension:requ:conclude} from \cluster{1} in general, and certain kinds of \cluster{1} will ease our argument for~\autoref{goal:tension:requ:no-reason}.
\end{note}
...
\begin{note}[Main idea]
  A \cluster{0} is a collection of proposition-value-premise pairs such that everyq proposition-value-premise pair is a \requ{} of every other proposition-value-premise pair in the collection.
\end{note}
