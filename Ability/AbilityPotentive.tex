...
\begin{note}[Idea for second pass]
  The core idea of the second pass is that the agent obtains information of relation of support in potentive event.
  Uses this to establish a pointer to the relation of support established by witnessing potential event.

  Something like loan.
  I take out a loan, and I get to use the money now.
  Money from the loan functions for various purposes as money that has been received from performing work.
  Some differences.
  Take a home or auto loan.
  Restricted use.
  Loaned support is for a particular conclusion.
  And, further restrictions.

  However, this gets the agent something now.
  So, instead of propositional support as in the case of G\&T, something like an epistemic loan.

  Gain support (of a kind) for conclusion, and this support is just the support that would be obtained by witnessing.
  This has the same core function of providing the agent with support for the conclusion, but the process of loaning may restrict the function of the support in certain ways --- for the moment I take it as intuitive that it does.

  Pushing the analogy a little more, see how this satisfies the constraint.
  Do not need money to take out a loan, need some security that there is an event in which the agent repays the loan.
  Likewise, the agent does not need support for taking out the epistemic loan, security is that there is an event in which the agent repays the loan.
  And, have such security, the general ability.
\end{note}

\begin{note}[Only an analogy]
  This is, for the moment, merely an analogy.
  Still, analogy that will help guide.

  Use terminology of a `loan' when following intuition.
  The technical term used is `epistemic debt'.
\end{note}

\begin{note}[Structure]
  Start out with the idea of accumulation of epistemic debt.
  Then, build on this ???
\end{note}


\begin{note}[Relations between the agents]
  Really, it's one's potential self that makes the promise, the current self redeems the promise, and a future self fulfils the promise.
\end{note}
