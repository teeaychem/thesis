...
\begin{note}[Propositional and doxastic support]
  ...
  \begin{quote}
    ...
    \item\label{dj:def:iiii} S's belief in p is appropriately connected to S's sufficient epistemic reasons to believe p.
    \end{enumerate*}
  \end{quote}
...
\begin{note}[P/D Example]
  ...
  \begin{tikzpicture}[box/.style={rectangle, rounded corners, draw=black, text width=15mm}]
    ...
    \node (Answer) at (7.25,1.25) [text width=6.5cm]
    {\textsf
  ...
  \(\frac{1}{2} \times \text{base} \times \text{(perpendicular) height} (\text{square units})\).

  Consider two students who have applied the formula, and calculated that \(\frac{1}{2} \times 5\text{(cm)} \times 3\text{(cm)} = 7.5(\text{cm}^{2})\).
  ...
  May compare to a third student who did not understand the formula but has it written down on a cheat sheet.
...
\begin{note}[Takeaway]
  ...
  Given assumption that the information is true, that the agent has propositional support matters because forming doxastic support does not require additional (sufficient epistemic) reasons.
\end{note}

\begin{note}[Important point for prop/dox def.]
  The observation that forming doxastic support does not require additional (sufficient epistemic) reasons follows independently of any particular account of what sufficient epistemic reasons are.
  For, condition~\ref{dj:def:i} states that the agent has propositional support to believe that p, which reduces to the agent having sufficient epistemic reasons to believe that p.
  Hence, if sufficient epistemic reasons relied on additional (epistemic) reasons introduced by the agent forming an attitude toward the proposition, the either the sufficient epistemic reasons identified by the agent having propositional support are not sufficient --- a contradiction --- or the definitions of propositional and doxastic support are circular, as propositional justification must reference doxastic justification in order to identify the additional epistemic reasons introduced by the agent forming doxastic support, and doxastic support in turn requires the agent to have propositional support and so on\dots

  So, as doxastic is the attitude, proposition is supported independently of whether or not the agent has (a doxastically support) attitude toward the proposition.\nolinebreak
  \footnote{
  ...
  Hence, as we're dealing with a potential event, then given the definitions of propositional and doxastic support \emph{and} given the truth of the information, then the agent has propositional support for the conclusion, no matter what sufficient epistemic reasons turn out to be.
\end{note}
...
\begin{note}[Concludes first step]
  ...
  Agent doesn't need any additional support (sufficient epistemic reasons).
\end{note}

\begin{note}[Second step]
  Continue to assume information is true, then the second step involves observing that the agent is in a good position to claim doxastic support for whatever sufficient epistemic reasons are.

  That is, complex doxastically support belief.
  Given information, then have propositional support for conclusion.
  For potentive event.
  Relies on general ability.
  Agent has good support for general ability.

  Here, substantive requirement on sufficient epistemic reasons.
  Don't need to be factive.
  As, possibility that the agent is mistaken about general ability.
  Here, the attitude is important.
  Belief, rather than knowledge.
  Plausible, else belief would also be factive.

  Simplify attitude.
  Belief that the agent has propositional support for any premises used in reasoning in line with general ability.
  Eliminate the conditional nature by a universal.
  Retain doxastic support for propositional support.

  Main case, it's the general ability to reason with the rules of chess.

  Question is whether the agent can extend this to conclusion.
  Doxastically supported belief that the agent has propositional support for conclusion.

  Here, the support for the conclusion the agent is appealing to is the propositional support.
  In turn, whatever premises would be used.

  Constraint of interest is whether the agent requires support for information to have such a doxastically supported belief.
\end{note}
...
{
  \begin{note}[A kind of `closure' principle]
  ...
  \begin{note}[Nothing more than acceptance for entailment?]
  ...
  \begin{note}[\WR{}]
    ...
    Agent is not merely claiming that there is \emph{some} propositional support.
    Rather, that the propositional support is sufficient for some positive doxastic attitude.
  \end{note}

  \begin{note}[A little more work]
    Propositional support is not quite sufficient.
    Propositional support for contrary propositions.

    For example: \cite{Bondy:2018ux}
    \begin{quote}
      S’s belief B is propositionally but not doxastically justiﬁed iff S holds \(B\) only on the basis of a set of reasons \(R1 = \{R1_{1}, \dots, R1_{n}\}\), and either:
      \begin{enumerate}[label=(\arabic*)]
      \item \(R1\) is not a good set of reasons for holding \(B\), and
      \item S has available to her another set of reasons \(R2 = \{R2_{1}, \dots, R2_{n}\}\) which are good reasons for holding B; or
      \item \(R1\) is a good set of reasons, and
      \item S arrives at \(B\) on the basis of \(R1\) in an improper way.\linebreak
      \mbox{}\hfill\mbox{(\citeyear[3813--3814]{Bondy:2018ux})}
    \end{enumerate}
  \end{quote}
    Avoid both of these considerations.
    For, information adds that witnessing would be successful.

    And, plausible that this extends to any other considerations which would prevent the agent from establishing doxastic support in line with the relevant basing relation.

    In other words, whatever is chosen for \(\geop{2}\) allows observation of this stuff.
  \end{note}
