...
\begin{note}
  We now turn to a pair of consequences of \autoref{assu:supp:independence}.
  The first draws out sufficient conditions for a \cprequ{}.
  The second observes an limitation that follows.
\end{note}
...
\begin{note}
  \begin{restatable}[\OCS{-}  --- \OCS{}]{proposition}{propOCS}
    \label{prem:ni}
    Let \(\phi\) and \(\psi\) be propositions, \(v\) and \(v'\) values, and \(S\) an agent.

    Suppose \(S\) is reasoning such that:

    \begin{enumerate}[ref=\named{\OCSacro{}:\arabic*}, series=OCS_counter]
    \item
      \label{nI:background}
      \begin{enumerate}[label=\alph*., ref=\named{\OCSacro{}1:\alph*}]
      \item
        \label{nI:background:step}
        Reasoning includes step for which \(\phi\) having value \(v\) is the conclusion.
      \item
    ...
    \begin{enumerate}[ref=\named{\OCSacro{}:\arabic*}, resume*=OCS_counter]
    \item
      \label{nI:inclusion}
      When making the step of~\ref{nI:background:step}, \(S\) is confident that:
      \begin{enumerate}[label=\alph*., ref=\named{\OCSacro{}2:\alph*}]
      \item
        \label{nI:inclusion:position}
        If \(S\) is not \misled{} when making the step, then it is possible for \(S\), given present context, to reason to \(\psi\) having value \(v'\) (without appealing to \(\phi\) having value \(v\)).
      \item
        \label{nI:inclusion:bound}
        If \(S\) is not \misled{} when making the step, then \(S\) would not be \misled{} were \(S\) to reason to \(\psi\) having value \(v'\).
      \end{enumerate}
    \end{enumerate}
    ...
    \item
      ...
      \item
        \label{nI:goingbyvalue:psi}
        \(\psi\) not having value \(v'\) is an \ep{} for \(S\) (both prior to and after making the step of~\ref{nI:background:step}).
      \end{enumerate}
    \end{enumerate}

    Then:
    \begin{enumerate}[ref=\named{\OCSacro{}:\arabic*}, resume*=OCS_counter]
    \item \(\psi\) having value \(v'\) is a \cprequ{} of the step of~\ref{nI:background:step}.
    \end{enumerate}
  \end{restatable}
\end{note}

\begin{note}
  \begin{restatable}[\nI{-}  --- \nI{}]{proposition}{propNI3}
    Suppose:
    \begin{enumerate}
    \item Clauses of \OCS{} hold.
    \item \(S\) is confident that \(S\) requires the result of the step of~\ref{nI:background:step} --- that \(\psi\) has value \(v\) --- only from \(\phi\)
    \end{enumerate}

    Then:
    \begin{enumerate}
    \item No claiming support for \(\phi\) having value \(v\).
    \end{enumerate}
    And, hence, so long as \(\phi\) having \(v\) persists, further reasoning that appeals to \(\phi\) having value \(v'\) as a premise is (also) not an instance of claiming support.
  \end{restatable}
\end{note}
...
\begin{note}[In more detail]
  Temporarly ordered.
  Restate looking at the `dependency relations'.


  Interest is in step of reasoning that obtains \(\phi\).
  \ref{nI:inclusion} brings \(\psi\) as a \prequ{} by establishing a form of interdependence between claimed support for \(\phi\) and claiming support for \(\phi\).
  Such that in order for agent to appeal to \(\phi\) having value \(v\) the agent must assume that \(\psi\) has value \(v'\).

  \ref{nI:going-by-value} requires reasoning about \prequ{}, as requires \(\phi\).

  \ref{nI:background} prevent agent from reasoning for \prequ{} that does not violate \ref{assu:supp:independence}.
\end{note}
...
\begin{note}
  If~\nIBackground{} hold, then the way of claiming support captured by~\ref{nI:going-by-value} is intuitively undercut.
  In the sense that \requ{}, so need to reason, but not possible to reason without \requ{}.
...
\paragraph{Complexity of clauses}
...
\paragraph{It's the move to \(\phi\)}
...
\paragraph{Clarifying important parts of \nI{}}
