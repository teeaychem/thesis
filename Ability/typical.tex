...
\begin{note}
  ...
  Therefore, this same thing extends, at least, to other \poP{1} the agent may reason from.
...
\begin{note}
  ...
  In particular, which proposition-value-\poP{} pairs are \tI{1} of the \tRep{1}.

  Still, keep in mind motivating idea.
  Something about present event which secures the truth of the progressive.

  So, proposition-value-\poP{} pairs are \tI{1} of the \tRep{1} just in case \itc{} must be true.
\end{note}
    ...
    \item
      ...
      \item
        ...
        \item[\emph{And}:]
          \begin{enumerate}
          \item
            After \(a'\), either concluding or some action \(a''\) such that concluding.
          \end{enumerate}
        \end{itemize}
      \end{itemize}
    \end{itemize}
...
\begin{note}
  So, strengthened.
  Here, there's got to be the possibility of reasoning, and for this to either amount to, or be extended to concluding.
\end{note}
...
%     \item[\emph{And}:]
%       \(\pvp{\psi}{v'}{\Psi}\) is an instance of \tRep{0}.
%     \item[\emph{And}:]
%       Available action such that \vAgent{} is reasoning from \(\Psi\) after performing \(a\).
%     \item[\emph{Then}:]
...
%     \begin{itemize}
%     \item
%       \(e\) is an event in which \vAgent{} is \tC{} \(\pv{\phi}{v}\) from \(\Phi\).
%     \item
%       \(\pvp{\psi}{v'}{\Psi}\) is an instance of \tRep{0}.
%     \item
%       Available action such that \vAgent{} is reasoning from \(\Psi\) after performing \(a\).
%     \end{itemize}
%     % Then, available action, so antecedent of \itc{} from \autoref{def:tC} holds.
%     % Therefore, the consequent holds.

%     % So, we get that the agent is concluding.
%     % Also needs to be the case that not concluding something incompatible.
%     Need to fix def of conclusions and \fc{1}.
%   \end{argument}
% \end{note}
