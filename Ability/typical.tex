...
\begin{note}
  \begin{proposition}[\typeAdj{2} \fc{1}]%
    ...
    \item[\emph{If}:]
      ...
      \end{enumerate}
    \item[\emph{Then}:]
      \(\pv{\psi}{v'}\) is a \fc{0} from \(\Psi\) for \vAgent{} through \(\ed{i}\).
    \end{itenum}
    \vspace{-\baselineskip}
  \end{proposition}
...
\begin{note}
  ...
  \begin{argument}{prop:tCV-fc}
    ...
    \end{enumerate}
    %
    Hence, the definition of \(\pv{\phi}{v}\) being a \fc{0} from \(\Psi\) through \(\ed{i}\) is satisfied (cf.~\autoref{def:fc}, \autopageref{def:fc}).
  \end{argument}
\end{note}
