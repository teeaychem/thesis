...
\begin{note}
  ...
  \begin{rdefinition}{def:tpro}{The \tprof{0}}%
    ...
    \begin{itemize}
    \item
      A collection \(\tproS{}\) of (maybe possible) events associated with \prop{0}-\val{0}-\pool{0} pairings is a \emph{\tpro{}} of a type \(T\) with respect to \(\ed{}\) and agent \vAgent{}.
    \end{itemize}
  ...
  \noindent%
  \autoref{def:tpro} defines a property of some collection of (maybe possible) events associated with \prop{0}-\val{0}-\pool{0} pairings.%
  \footnote{
    We do not require the agent to be reasoning by a particular type in the consequent of the conditional.
    I would guess for most \tpro{1} the reasoning is of the same \torNa{} as the antecedent.
    Still, I see no clear motivation for requiring this in the definition.
  }
  In other words, \autoref{def:tpro} does not define a collection of events and associated pairings.
  Rather, \autoref{def:tpro} defines when a term applies to any such collection, given also an event and an agent.

  In general, I have little to say about when some event and associated \prop{0}-\val{0} pairing is included in a \tpro{}.
  Part of the overall argument of this document as stated rests on particular events and \prop{0}-\val{0}-\pool{0} pairings being included in a \tpro{}, as we are in search of failures of \issueInclusion{}.
  However, the way we obtain failures of \issueInclusion{} is compatible with a variety of different perspectives on which events and associated \prop{0}-\val{0} pairings are included in a \tpro{}.

  Some intuition for the broad concept is given.
  Then some \illu{1} and an application follow.
  And, the application is designed to highlight various considerations I think are sufficient (though not necessary) to establish some event and \prop{0}-\val{0}-\pool{0} pairing is part of a \tpro{} by showing, rather than telling.
\end{note}
...
\begin{note}
  ...
  \noindent%
  Note, it is not possible to strengthen \autoref{prop:tpro-switch} to an if and \emph{only if}, as \tpro{1} are defined by a necessary condition on an agent \tCV{}.
\end{note}
