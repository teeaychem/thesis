...
\begin{note}
  \autoref{cha:typical:sec:g-s} type of reasoning by which an agent concludes.
  Means-end reasoning.
...
\begin{note}
  \begin{idea}[\dtCN{2}]
    ...
    \begin{itemize}
    \item
      \vAgent{} is \emph{\dtCV{}} \(\pv{\phi}{v}\) from \(\Phi\) by type of reasoning \(T\) in \(e\).
    \end{itemize}
...
\begin{note}[Law]
  ...
  Means-end reasoning, is very general.
  However, specific accounts of means-end reasoning are a little more interesting.
  For example, belief-desire versus intention.
  Temptation.
  No cases of temptation, but if there are cases, then problem.

  Law tells us about what the type amounts to in other cases.
\end{note}


\begin{note}[Dependence]
