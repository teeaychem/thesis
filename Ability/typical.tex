...
\begin{note}
  \begin{application}%
    ...
    \begin{itenum}
    \item[\emph{If}:]
      If \(\ed{\flat}\) is an event in which the agent is \tCV{} \pv{\propI{\rootsCon{}}}{\valI{True}} from \(\Phi\) by factorisation.
    \item[\emph{Then}:]
  ...
  \begin{dets}{app:sc1-typ}
    ...
    Then, we argue there is a \tpro{} in which each event happens by the agent doing some action in \(\edn{\flat}\).
    \medskip

    \noindent%
    Suppose \(\ed{\flat}\) is an event in which the agent is \tCV{} \pv{\propI{\rootsCon{}}}{\valI{True}} from \(\Phi\) by factorisation.

    Now, factorisation is a method.
    ...
    Indeed this is precisely what Step~\ref{illu:gist:roots:F:factor} of the \agents{} reasoning in \autoref{illu:gist:roots:F}.

    Hence, it is clear there must be assignments other than \(a \coloneq 6\), \(b \coloneq 1\), \(c \coloneq 1\) such that it is possible for the agent to conclude \(\rootsConEqGenV{a}{b}{c} = \rootsFacV{i}{j}{q}{r}\).
    For, if not there is no generality to the \agents{} reasoning.

    At issue is:
    \begin{itemize}
    \item
      Which values of \(a\), \(b\), and \(c\) are such that it is possible for the agent to conclude \(\rootsConEqGenV{a}{b}{c} = \rootsFacV{i}{j}{q}{r}\).
    \item
    ...
    \noindent%
    To see the values of \(a\), \(b\), and \(c\) in \(\mathbb{P}\) must be possible, we observe the relation between \(a\), \(b\), \(c\), \(i\), \(j\), \(q\), and \(r\) in the variant quadratic equations structurally parallel those of the equation the agent reasons about in Step~\ref{illu:gist:roots:F:factor}.
    ...
    \footnote{
      ...
      Though, as \tpro{1} are used to capture a necessary rather than sufficient condition for an agent to be \tCV{} there is no need to narrow the relevant type.
    }
    \smallskip

    \noindent%
    So, possible for the agent to conclude \(\rootsConEqGenV{n}{}{1} = \rootsFacV{i}{j}{1}{1}\).
    Now, for the agent to conclude \rootsConGenV{i}{(i - 1)}.
    Agent is concluding \rootsCon{}.
    So, it must be possible to complete the remaining steps.
    And, the arithmetic required is trivial.
    \smallskip

    \noindent%
    So, Step~\ref{illu:gist:roots:F:factor} given minimal variations of the quadratic equation and steps~\ref{illu:gist:roots:F:zero} to \ref{illu:gist:roots:F:factor:done} by simplicity of the reasoning.
    There are possible events in which the agent is concluding \pv{\propI{\rootsConGen{}}}{\valI{True}}.
    \medskip

    \noindent%
    Finally, events are the outcome of an action done by \vAgent{} in \(\edn{\flat}\).

    I take it to be clear the agent may pause and substitute \(6\) for \(n\).
    And, the description of \autoref{illu:gist:roots:F} is consistent with this.
