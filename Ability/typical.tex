...
\begin{note}
  ...
  \begin{rdefinition}{def:tpro}{\tprof{3}}%
    ...
    \begin{itemize}
    \item
      A collection \(\tproS{}\) of descriptions associated with \prop{0}-\val{0}-\pool{0} pairings is a \emph{\tpro{}} of type \(T\) with respect to a description \(\edo{}\) an agent \vAgent{}, \prop{0}-\val{0} pair \(\pv{\phi}{v}\), and \pool{} \(\Phi\).
    \end{itemize}
...
\begin{note}
  ...
  By defining \tpro{1} relative to descriptions the only 


  the role of a \tpro{} is to identify certain things which must also be true if 

  So, when evaluating whether some description and associated \prop{0}-\val{0}-\pool{0} pairing is part of a \tpro{} 
\end{note}

\begin{note}
  In general, I have little to say about when some description and associated \prop{0}-\val{0} pairing is included in a \tpro{}.
