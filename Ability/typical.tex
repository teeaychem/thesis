...
\begin{note}
  \autoref{cha:requs} detailed the way answers to \qWhy{} follow from the interaction between \se{1} and \progEx{} (\autoref{cha:events-progress}), \fc{1} (\autoref{cha:fcs}), and \fingfr{1} (\autoref{cha:ros}).

  And, to help keep things simple we condensed the relevant interaction with the idea of a \requ{}.
\end{note}

\begin{note}
  Further, at the close of \autoref{cha:requs} \autoref{prop:requ-WhyV-ces} (\autopageref{prop:requ-WhyV-ces}) used the idea of a \requ{} to state sufficient conditions for failures of \issueInclusion{} .

  So, our final task is to establish a way to identify \requ{1} such that \issueInclusion{} fails.
\end{note}

\begin{note}
  Given an \eiw[\(\ed{}\)]{0} an agent concludes \(\pv{\phi}{v}\) from \(\Phi\), \(\pv{\psi}{v'}\) being a \fc{} from \(\Psi\) for \vAgent{} through \(\ed{\flat}\) is a \requ{} of \(\ed{}\) just in case two conditions are satisfied (cf.\ \autoref{def:requ}, \autopageref{def:requ}):
  ...
  We term this idea that of an agent `\tCV{1}'.

  And, finally, in \autoref{cha:ces} we combine the idea of an agent \tCV{1} with \requ{1} in full to obtain counterexamples to \issueInclusion{}.
\end{note}

\begin{note}
  This chapter beings with initial motivation for the idea of an agent \tCV{1} and a sketch of the way the idea is developed.

  The following section then develops the idea of an agent \tCV{1} in detail.

  And, the final section links an agent \tCV{1} to \fc{1}.
\end{note}
...
\begin{note}
  ...
  The second section of this chapter introduces the idea of a \tprof{0}, and the third section uses the idea of a \tpro{0} to link an agent \tCN{} to \fc{1}.
\end{note}
