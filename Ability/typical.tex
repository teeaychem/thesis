...
\subsection{Types of reasoning by which an agent concludes}
\label{cha:typical:g-s}
...
\begin{note}
  ...
  Set aside these concerns, and focus on the details of means-end reasoning.
\end{note}

\begin{note}
  Means-end is general type.

  Desire-belief and intention models.

  \begin{sketch}[Belief-desire]
    Conclusion of means-end reasoning is from belief and desire.
  \end{sketch}

  \textquote{What it is important to notice about this model of practical reasoning is that it provides no distinctive role for an agent's future-directed intentions as inputs to such reasoning.} 17

  \begin{sketch}[Intention]
    Prior intention filters admissible desires.
  \end{sketch}

  Clear that cases are means-end reasoning, but what about the sub types?

  For example, perhaps Piglet would rather YYY but has the intention.
\end{note}
...
\begin{note}
  ...
  Event in progress.
  While the event was in progress, Alice was concluding XXX by means-end reasoning, and Piglet was concluding XXX by reasoning about means-end reasoning, and so on.
\end{note}

\begin{note}
  \autoref{cha:typical:g-s} type of reasoning by which an agent concludes.
  Means-end reasoning.
  \scen{3}, event in which the agent concludes.
  Interest, event in progress.

  Our interest is in giving a necessary condition for an agent concluding to be an instance of a type of concluding.

  Provide idea.
  Expand using dispositions.
  \illu{3} of idea.

  In \autoref{cha:typical:tCDef} build on the idea of \dtC{} to define the technical term \tC{}.
\end{note}
...
\begin{note}
  \begin{idea}[\dtCN{2}]
    ...
    \begin{itemize}
    \item
      \(e\) is an event in which \vAgent{} is \emph{\dtCV{}} \(\pv{\phi}{v}\) from \(\Phi\) by type of reasoning \(T\).
    \end{itemize}
    ...
    \begin{itemize}
    \item
      some collection of events \({\cal E}\), collection of \prop{1}, \val{1}, and \pool{1} \({\cal X}\):
    \item
      For every event \(e'\):
      \begin{enumerate}
      \item[\emph{If}:]
        \(e'\) is in \({\cal E}\):
      \item[\emph{Then}:]
        For some proposition \(\psi\), value \(v'\), and \pool{} \(\Psi\) in \({\cal  }X\):
        \begin{itemize}
        \item
  ...
  `An agent is \emph{\dtCV{}} \(\pv{\phi}{v}\) from \(\Phi\) by type of reasoning \(T\)' is understood as a predicate of an event.
  \autoref{idea:tR-law} states that this predicate is true of an event just in case there is a universally quantified conditional which is true.

  This universally quantified conditional is abstractly specified via two collections.
\end{note}

\begin{note}
  Intuitively, `depends' on `law'.
\end{note}
...
\begin{note}[Law]
  ...
  \footnote{
    ...
    \end{quote}
    \citeauthor{Hempel:1965aa}'s Deductive-Nomological account of scientific explanation (\autopageref{mention:Hempel:1}), \citeauthor{Boole:1854aa}'s laws of thought, and so on.
  }

  Interest of a law is that require certain things in certain situations.
  Idea is that given an circumstances, agent reasons in a certain way.
...
\begin{note}
  Law tells us about what the type amounts to in other cases.
...
\begin{note}
  ...
  Return to the two types.

  If basic type, then present in all events.
  If seems otherwise, then the additional parameter does not work.
...
\subsubsection[Dispositions]{Dispositions \hfill (Optional)}
...
\begin{note}
  ...
  \footnote{
    ...
    Though, in turn \citeauthor{Manley:2008aa}'s discussion is almost identical to~(\cite[\S2.1]{Fara:2006aa}).
    Perhaps the paragraphs go back further\dots
...
\begin{note}
  ...
  \begin{center}
    \begin{tabular}{R{.45\textwidth} L{.45\textwidth}}
      ...
      Proposition-value pair & \pool{2} \\
      \hline
      \pv{\propI{Give Eeyore a balloon}}{\valI{Do}} & \pv{\propI{Cheer up Eeyore}}{\valI{Want}}, \dots \\
      \pv{\propI{Flatter cicada}}{\valI{Do}} & \pv{\propI{Eat cicada}}{\valI{Want}}, \dots \\
      \pv{\propI{Respond `7 years and 6 months'}}{\valI{Do}} & \pv{\propI{Answer Dumpty's question}}{\valI{Want}}, \dots \\
    \end{tabular}
  \end{center}
