...
\begin{note}
  ...
  \begin{definition}[The \tprof{0}]
    \label{def:tpro}
    For a collection \(\tproS{}\) of events associated with \prop{0}-\val{0}-\pool{0} pairings:
...
\begin{note}
  \begin{application}%
    ...
    \item[\emph{Then}:]
      ...
      \begin{itemize}
      \item
        The agent may switch to \rootsConEqGen{} and be concluding \pv{\propI{\rootsConGen{}}}{\valI{True}} from \(\Phi'\), where \(m \times (m - 1) = n\).
      \end{itemize}
    \end{itenum}
    \vspace{-\baselineskip}
  \end{application}

  In other words, there is some \tpro{} which includes \dots

  \begin{dets}{app:sc1-typ}
    In contrast, \autoref{illu:gist:roots:F} is intuition.
    ...
    Note, whether the agent has any motivation to do is irrelevant, though is also compatible.
    \medskip

    \noindent%
    Concluding.

    The intuition is that the relevant reasoning for each quadratic equation is an instance of the reasoning the agent is doing.
    In particular,  \(- 1\) so must be the case that \((ix + 1)(jx = 1)\), and \(n = m(m-1)\) for some \(m\).

    So, however the agent approaches the problem, whatever they are doing in the actual case extends.

    To tighten this up, assume the agent will observe it must be that \((ix + 1)(jx = 1)\) and fall back to an exhaustive over values of \(i,j \in \mathbb{N}\) search after some time.
    This provides a solution.

    For example, \(20\).

    For, only values of interest are where \(i,j \leq 20\) and \(i \times j = 20\).

    \(1, 20\), however \(\pm 19x\)
    \(2, 10\), however \(\pm 8x\)
    \(3, x\), no \(k\) such that \(3 \times k = 20\)
    \(4, 5\), then \((5x + 1)(6x - 1)\) or \((6x + 1)(5x - 1)\).

    Other values follow the same pattern.

    So, so long as the agent will perform an exhaustive search if any other strategy fails, the agent will conclude \((5x + 1)(6x - 1)\).
  \end{dets}
\end{note}
