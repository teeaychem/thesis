...
\begin{note}
  Out goal is to construct a `lawful' condition for an instance of reasoning to be an instance of a type of reasoning.
\end{note}
...
\begin{note}
  ...
  \begin{itemize}
  \item
    We have only assumed reasoning amounts to reasoning to some proposition-value pair \(\pv{\phi}{v}\) from some \poP{} \(\Phi\).
    Therefore, nothing other than proposition-value pairs, \poP{1}, and events in which an agent reasons.
  \item
    Not events, as it may be the case that agent's reasoning is of type, even though there is no event.
...
\begin{note}
  \begin{sketch}[\tR{2}]
    ...
    \begin{itemize}
    \item
      \(e\) is an event in which \vAgent{} is reasoning to \(\pv{\phi}{v}\) from \(\Phi\) by \emph{\tR{0}} of type \(T\).
    \end{itemize}
...
\begin{note}
  \begin{restatable}[\rotor{2}]{definition}{defToRRep}
    ...
    \begin{itemize}
    \item
      \(T'\) is \emph{representative} of \(T\) for \vAgent{} with respect to an event \(e\).
    \end{itemize}
...
\begin{note}
  \begin{definition}[\tR{2}]
    ...
    \begin{itemize}[noitemsep]
    \item
      For some representative \(T'\) of \(T\) with respect to \(e\):
      \begin{itemize}[noitemsep]
      \item
        For every pairing \(\pvp{\psi}{v'}{\Psi}\) in \(T'\).
        \begin{itemize}[noitemsep]
  ...
  \autoref{def:cmptnc} is a partial definition.
  Necessary condition.

  The difference between \autoref{def:cmptnc} and \autoref{sketch:tR} is \rotor{}.
  Rather than all instances of type, only need representative.

  By assumption, \rotor{} is sufficient.
  So, if any then good to go.

  Indeed, \autoref{def:cmptnc} is compatible with the edge case in which there is something about the event itself which is sufficient.
  So, in abstraction, \autoref{def:cmptnc} is insubstantial.
  In order for \autoref{def:cmptnc} to do any work in showing reasoning is not of type, need interesting \rotor{1}.

  However, follows intuition.
  If find a failure for each representative, then problem.
  This is what \tR{} captures.
\end{note}
...
\begin{note}
  \begin{proposition}
    ...
    If \(e\) is an event in which \vAgent{} is \tC{} \(\pv{\phi}{v}\) from \(\Phi\)

    Unique representative \(T'\)

    \(\pvp{\psi}{v'}{\Psi}\) is instance of representative.

    Then, \fc{}.
  \end{proposition}

  \begin{argument}{prop:tC-fc}
    More or less immediate.
  \end{argument}
\end{note}

\begin{note}
  Wait, if I have this, then there's nothing more that I need.
  This is the key result.
  So, okay, the idea with \requ{} is to get essential representatives.
\end{note}
