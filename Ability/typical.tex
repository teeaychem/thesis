...
\begin{note}
  ...
  Conversely, if the agent did not factor, then the agent did not conclude \pv{\propM{\rootsCon{}}}{\valI{True}} from \(\Phi\).
...
\begin{note}
  ...
  So, given \autoref{def:tpro}, a \tpro{} \tproS{} of multiplication for powers includes events in which the agent begins to work on exercises \ref{mfp:a} through to \ref{mfp:h} and the relevant \prop{0}-\val{0}-\pool{0} pairings capture the appropriate answer to the exercise the agent is working on.
...
\begin{note}
  ...
  \noindent%
  For quick intuition, consider \autoref{app:sc1-typ} as a variant of \autoref{illu:tR:powers} in which the relevant exercises are generated by the agent (rather than being given to the agent).
  ...
  The details of \autoref{app:sc1-typ} highlight the way in which the instances of \(n\) amount to \emph{structurally similar} quadratic equations.
  ...
  Indeed, it not enough to highlight, e.g.\ \rootsConEqGenV{20}{}{1} is a quadratic formula and \rootsConGen{} is an expression of the relevant factors.
  For, \rootsConEqGenV{15}{}{1} is likewise a quadratic formula, but it seems unreasonable to expect the agent to be concluding \(x \in \{\sfrac{(\sqrt{61} - 1)}{30}, -\sfrac{(1 + \sqrt{61})}{30}\}\).%
  \footnote{
  ...
  \begin{dets}{app:sc1-typ}
    ...
    \noindent%
    To see the relevant \prop{0}-\val{0}-\pool{0} pairings are part of some \tpro{}, suppose \(\ed{\flat}\) is an event in which the agent is \tCV{} \pv{\propI{\rootsCon{}}}{\valI{True}} from \(\Phi\) by factorisation.
