...
\begin{note}
  Three \scen{0}.
  Each \scen{0} intuitively involve an agent which concludes something by a type of reasoning.
...
\begin{note}
  Intuitively, each \scen{0} involves an agent reasoning to a conclusion by \emph{means-end} reasoning.%
  \footnote{
...
\begin{note}
  ...
  For example, the \scen{1} intuitively involve reasoning \emph{about} means-end~reasoning.%
  \footnote{
    Taking each \scen{0} in turn:
...
\begin{note}
  ...
  By `lawful' universally quantified material conditional which is true.%
  \footnote{
  ...
  \noindent%
  For example, \citeauthor{Helmholtz:1977aa}:
  ...
  Key here is quantifier.
  \emph{Whenever} preconditions are alike in a certain respects, consequences that are alike in a certain other respect follow.
\end{note}

\begin{note}
  Note, laws understood as universally quantified material conditionals are not restricted to laws of nature.
  For example, consider the law of large numbers,%
  \footnote{
    Consider \cite{Seneta:2013aa}'s translation of \citeauthor{Poisson:1837aa}'s (\citeyear{Poisson:1837aa}) statement:
    \begin{quote}
      ...
      \mbox{ }\hfill\mbox{(\citeyear[9--10]{Seneta:2013aa})}
    \end{quote}
  }
  the law of truly large numbers,%
  \footnote{
    As stated by \citeauthor{Diaconis:1989aa}: \textquote{[W]hen enormous numbers of events and people and their interactions cumulate over time, almost any outrageous event is bound to occur} (\citeyear[853]{Diaconis:1989aa}).
  }
  \citeauthor{Boole:1854aa}'s laws of thought, and so on.

  It may be more accurate to speak of universally quantified material conditional which are true, but as `law' seems more-or-less synonymous in many cases, we adopt the shorthand.
\end{note}

\begin{note}
  Laws, conditions.
  So, if \tC{} is lawlike, then for any instance in which an agent is \tCV{}, there exists some law:

  \begin{idea}[\tCN{2} are lawful]
    \label{idea:tR-law}
    \cenLine{
...
\begin{note}
  Idea here is that this deductive.
...
\begin{note}
  ...
  \footnote{
    \citeauthor{Goodman:1983aa}, difficulty with counterfactuals and laws.
    (\citeyear[Ch.I,\S3]{Goodman:1983aa})%
  }

  Following , rewrite as a direct conditional and link to disposition:

  \begin{observation}[Basic observation]
    ...
    \item
      ...
      \begin{enumerate}
      \item[\emph{If}:]
        Some condition \(C\) in a collection of conditions \({\cal C}\) is the case.
      \item[\emph{Then}:]
        For some action \(M\) in a collection of actions \({\cal M}\):
        \begin{itemize}
        \item
          Object \(o\) does action \(M\).
        \end{itemize}
      \end{enumerate}
    \end{itemize}
    \vspace{-\baselineskip}
  ...
  \autoref{obs:disp:basic} is not an analysis of dispositions.
  Rather, it states a consequence of disposition:
  There is be some `law' such that conditions leads to manifestation.

  Outer conditional, whether or not the object has the disposition.
  Inner conditional, rewrites `simple conditional analysis' as a material conditional.
  If basic observation is derived from simple conditional analysis, then \(C\) are given by \(C\), but not sensitive to world of evaluation.%
  \footnote{
...
\begin{note}
  Stress, \autoref{obs:disp:basic} is not an analysis of dispositions.
  There are various counterexamples to instances of the simple conditional analysis.%
  \footnote{
    Masks ~(\cite{Johnston:1992aa}),~finks~(\cite{Martin:1994aa}),~mimics~ (\cite{Lewis:1997wg}), and so on.
  }
  However, these counterexamples are counterexamples to \emph{instances} of the simple conditional analysis.%
  \footnote{
  ...
  For, disposition, then disposition manifests circumstances.
\end{note}
...
\begin{note}
  \begin{observation}
    ...
    Partial grasp on \({\cal C}\) and \({\cal M}\) is sufficient to establish an object does not have disposition \(d\).
  \end{observation}

  \begin{motivation}{obs:disp:partial}
    Circumstance \(C\) and action \(M\).
    Fails, then by \autoref{obs:disp:basic}, object \(o\) does not have disposition \(d\).
  \end{motivation}
...
\begin{note}
  The parallels between \autoref{idea:tR-law} and dispositions does not suggest that for an agent to be \tC{} is for the agent to have a disposition.
...
\begin{note}
  ...
  \begin{center}
    \begin{tabular}{R{.45\textwidth} L{.45\textwidth}}
      \multicolumn{2}{c}{Proposition-value-\pool{} pairings in type `by modus ponens'} \\
      \hline\hline
  ...
  \begin{center}
    \begin{tabular}{R{.45\textwidth} L{.45\textwidth}}
      \multicolumn{2}{c}{Proposition-value-\pool{} pairings not in type `by modus ponens'} \\
      \hline\hline
      Proposition-value pair & \pool{2} \\
      \hline
...
\begin{note}
  ...
  \begin{center}
    \begin{tabular}{R{.45\textwidth} L{.45\textwidth}}
      \multicolumn{2}{c}{Proposition-value-\pool{} pairings in type `by means-end reasoning'} \\
      \hline\hline
      Proposition-value pair & \pool{2} \\
      \hline
  ...
  \begin{center}
    \begin{tabular}{R{.45\textwidth} L{.45\textwidth}}
      \multicolumn{2}{c}{Proposition-value-\pool{} pairings not in type `by means-end reasoning'} \\
      \hline\hline
      Proposition-value pair & \pool{2} \\
      \hline
      \pv{Q}{\val{True}} & \pv{P}{\val{True}}, \pv{\prop{If }P\prop{ then }Q}{\val{True}}, \dots \\
