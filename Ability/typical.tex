...
\begin{note}
  ...
  \item
    ...
    Refinement of key idea, link to \fc{1}.
  \end{TOCEnum}
\end{note}
...
\begin{note}
  ...
  \begin{idea}[\tCN{2}]
    ...
    \item
      ...
      \item
        ...
        \item[\emph{Then}:]
          For some \prop{0}-\val{} pair \(\pv{\psi}{v'}\) and \pool{0} \(\Psi\) in \(\mathcal{X}\):
          \begin{itemize}
          \item
  ...
  \noindent%
  Intuitively, \autoref{idea:tR-law} expresses the idea that the predicate `\vAgent{} is \tCV{} \(\pv{\phi}{v}\) from \(\Phi\) by type of reasoning \(T\)' applies to an event only if some `law' holds.%
  \footnote{
  ...
  \autoref{idea:tR-law} does not specify the content of the relevant law.
...
\begin{note}
  ...
  However, the events the law of \autoref{idea:tR-law} quantifies over need not happen, and the event at issue may be a single event.
\end{note}
...
\begin{note}
  ...
  \begin{argument}{obs:disp:basic}
    ...
    \footnote{
      We understand the \dBCA{} as specifying \(C\) and \(M\) relative to a context, and hence \(C'\) and \(M'\) are given with respect to some context.
      However, it does not follow that \(C\) and \(M\) need to take the relevant context as an argument.
      (\autoref{idea:tR-law} is also understood this way.)
    }%
    \(^{,}\)%
    \footnote{
  ...
  \end{argument}
  The parallel between the consequent of \autoref{obs:disp:basic} and \autoref{idea:tR-law} is clear by inspection.%
  \footnote{
    However, \autoref{obs:disp:basic} does not immediately suggest \tCV{} may be understood as a disposition.
...
\begin{note}
  With the parallel between the \dBCA{} and \autoref{idea:tR-law} in hand, briefly observe a number of issues for the \emph{simple} conditional analysis of dispositions do not apply to the \dBCA{}.
  ...
  By contrast, the \dBCA{} analysed a disposition \(d\) which did not contain \(C\) and \(M\), and hence allowed free choice of \(C\) and \(M\) in the corresponding counterfactual.

  Further, observe well-know counterexamples to the the simple conditional analysis require the choice of \(C\) and \(M\) being fixed.
  ...
  \footnote{
    \cite{Johnston:1992aa} provides counterexamples to a specific instance of the \dBCA{}.
  }
  ...
  Masks are a problem for the simple conditional analysis.
  For, the simple conditional analysis concerns the disposition to be digested when ingested, but there is little preventing someone from ingesting a sweet wrapped in plastic.
  However, masks are not a problem for the \dBCA{}.
  For, the \dBCA{} concerns `edible', and the conditional may be rejected as an adequate analysis as it fails to specify the appropriate conditions \(C'\) under which \(M'\) manifests.

  For sure, a masks may be counterexamples to \emph{instances} of the \dBCA{}, but in contrast to the simple conditional analysis, masks are not counterexamples to the (basic conditional) \emph{analysis}.%
  \footnote{
    ...
    \nocite{Manley:2007aa}
    In response~\citeauthor{Manley:2011aa} (\citeyear{Manley:2011aa}) seem to argue that counterexamples concern whether it is possible for the \dBCA{0} to function as an analysis of disposition \emph{ascriptions} --- not whether the \dBCA{0} is true (\citeyear[cf.][\S1.3]{Manley:2011aa}).
  }%
  \(^{,}\)%
  \footnote{
    Examples of \dBCA{1} are those cited by \citeauthor{Choi:2021wg} as endorsements of simple conditional analysis.
    ...
    \end{quote}
    And, `Entailment' is equivalent to the \dBCA{0}.
...
\begin{note}
  ...
  By \autoref{idea:tR-law}, \tCV{} only if some law.
...
\begin{note}
  ...
  Still, the collection may also be understood as collections of \tI{1} of the narrower type `multiplication by powers with respect to problems \ref{mfp:a}--\ref{mfp:h}'.
  In this respect, additional \prop{0}-\val{0}-\pool{0} pairs covering problems \ref{mfp:e} to \ref{mfp:h} may be included.
  Or, understood as collections of \tI{1} of the narrower type `multiplication by powers with respect to problems \ref{mfp:a}--\ref{mfp:d}'.
...
\begin{note}
  With a \torN{} in hand, we define what it is for an agent to be `\ptCV{0}':
...
\begin{note}
  Further, quantify over all \tI{1} of \torN{}.
  Hence, it seems unlikely for an agent to be \ptCV{0} by various \torN{1}.
\end{note}
...
\begin{note}
  ...
  Still, for any \torN{} \(T\), obtain further \torN{1} by taking sub-collections \(T'\) of \(T\).
  At issue the way in which an \ptCV{0} by \(T'\) relates to the agent \tCV{} by \(T\).
\end{note}
...
\begin{note}
  ...
  And, \autoref{illu:tR:powers} (\autopageref{illu:tR:powers}).
