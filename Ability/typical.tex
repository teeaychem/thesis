...
\begin{note}
  ...
  Roughly, the \tpro{} of a \torN{} is a collection of (possible) events in which an agent is concluding particular \prop{0}-\val{0} pairs from certain \pool{1}.
  ...
  And conversely, if there is some event in the \tpro{} in which the agent is not concluding the relevant \prop{0}-\val{0} pair from the \pool{0}, the agent is not concluding by the type.
...
\begin{note}
  ...
  The first section of this chapter introduces the idea of a \tprof{0}, and the second section uses the idea of a \tpro{0} to link an agent \tCN{} to \issueInclusion{}.
\end{note}
...
\begin{note}
  ...
  For, if the agent did not conclude the relevant \prop{0}-\val{0} pair by \tCV{} by some \torNa{} which corresponds to the \agents{} understanding, then various \prop{0}-\val{0} pairs of the relevant \pool{} would be irrelevant.
  ...
  Conversely, if the agent did not factor, then it seems the agent did not conclude \pv{\propM{\rootsCon{}}}{\valI{True}} from \(\Phi\).
  For, some aspect of the \agents{} understanding of factorisation was irrelevant to the way the agent concluded \pv{\propM{\rootsCon{}}}{\valI{True}} from \(\Phi\).%
  \footnote{
...
\begin{note}
  ...
  \item
    The relevant \tpro{} \(\tproS{}\) includes events in which an agent is doing a selection task with an associated \prop{0}-\val{0}-\pool{0} pairings which capture appropriate \evalN{1} of what do.
    E.g.:
    %
    \begin{center}
...
\begin{note}
  ...
  Given \autoref{def:tpro}/\autoref{prop:tpro-switch}, this means the agent would be concluding certain \prop{0}-\val{0} pairs from particular \pool{1} in select events.
  And, it seems the agent would be concluding \pv{\propI{a is 6}}{\valI{True}} from some \pool{} \(\Phi\) which includes \pv{\propM{2^{100} \cdot 3^{100} = a^{100}}}{\valI{True}} and captures the \agents{} understanding of multiplication for powers were the agent to abandon \ref{mfp:g} and work on~\ref{mfp:f}.%
  \footnote{
    Of course, \emph{verify} the agent would be concluding \pv{\propI{a is 6}}{\valI{True}} from \pv{\propM{2^{100} \cdot 3^{100} = a^{100}}}{\valI{True}} were the agent to abandon \ref{mfp:g} and work on~\ref{mfp:f} is difficult.
  ...
  So, a \tpro{} \tproS{} of multiplication for powers includes events in which the agent begins to work on exercises \ref{mfp:a} through to \ref{mfp:h} and the relevant \prop{0}-\val{0}-\pool{0} pairings capture the appropriate answer to the exercise the agent is working on.
  Hence, if the agent would fail to be concluding \pv{\propI{a is 6}}{\valI{True}} from \(\Phi\) when working on~\ref{mfp:f}, then it seems the agent does not have an adequate understanding of multiplication for powers to be concluding \emph{by} an understanding of multiplication for powers.%
  \footnote{
...
\begin{note}
  \begin{application}%
    ...
    \item[\emph{Then}:]
      ...
      \begin{itemize}
      \item
        There are \prop{0}-\val{0}-\pool{0} pairing, for each \(n\) in \(\rootsConAltSet{}\), such that:
        \begin{itemize}
        \item
        ...
        \end{itemize}
      \item
        Each event happens by the agent doing some action in \(\edn{}\) when agent is concluding.
      \end{itemize}
    \end{itenum}
    \vspace{-\baselineskip}
  \end{application}

  Plausibly it need not be the case that the is a \tpro{} which includes every instance of factorisation.
  For example\dots

  Details of \autoref{app:sc1-typ} motivate the particular five.

  \begin{dets}{app:sc1-typ}
    We first argue there relevant \prop{0}-\val{0}-\pool{0} pairings are part of some \tpro{}.
    Then, we argue there is a \tpro{} in which each event happens by the agent doing some action in \(\edn{}\).
    \medskip
    ...
    Agent is concluding.
    Hence, concludes.
    Structurally similar reasoning.
    Sequence of steps.
    Steps \ref{illu:gist:roots:F:zero} to \ref{illu:gist:roots:F:factor:done} are not particularly interesting.
    Interest is with Step~\ref{illu:gist:roots:F:factor}, when the agent factors.

    \(n\) in \(\rootsConAltSet{}\) are structurally similar, and so figure out.
    For, factorisation is a method.
    It does not depend on any particular instance of \(a\), \(b\), and \(c\).
    Then there is something about the particular value.%
    \footnote{
      Note, this motivation is compatible with strengthening the type of reasoning to factorisation with respect to quadratic formulas of a specific form.
      Still, specific form is still factorisation.
    }
    Must be possible events in which the agent concludes.
    And, as the agent is concluding, get this before event.
    Hence, events in which conclusion is in progress.


    Structural similarity, four observations:
    \begin{itemize}
    \item
      A quadratic equation is of the form \rootsConEqGenV{a}{b}{c}, may be rewritten to be of the form: \((ix + q)(jx - r)\).
    \item
      The \(c\) term is obtained by multiplying \(q\) and \(r\).
      For each \(n\) the \(c\) term is \(-1\).
      So, \((ix + q)(jx - r)\) may be simplified to \((ix + 1)(jx - 1)\).%
      \footnote{
        Expanded, either \(q = 1\) and \(r = -1\) or \(q = -1\) and \(r = 1\).
        As we have no yet fixed \(i\) and \(j\), there is no difference between these two cases.
      }
    \item
      Given \((ix + 1)(jx - 1)\), the relevant \(b\) term is \((j - i)\).
      And, given \(b = 1\), it must be the case that \(i = (j - 1)\).
    \item
      The \(a\) terms is \(n\), it must be the case that \(j \times (j - 1) = n\).
    \end{itemize}
    %
    Now, the agent may not reason so abstractly.

    The point of this analysis is to highlight there is no structural difference between figuring out \rootsConEqExV{6}{3}{2} and, e.g., \rootsConEqExV{20}{5}{4}.
    These two quadratic equations and corresponding factorisations amount to variations on the same structural constraint between \(a\), \(b\) and \(c\) with respect to the quadratic equation and \(i\), \(j\), \(q\), and \(r\) with respect to the factorisation.

    So, if the agent is reasoning by factorisation, the \agents{} reasoning should at least extend to apply to the instances relevant to the \prop{0}-\val{0}-\pool{0} pairings in \(\mathbb{P}\).

    Again, the low bound on \(n\) is to ensure the agent has no difficulty doing the arithmetic required.
    \medskip

    \noindent
    Action may do.
    Obtaining any variation is a matter of substitution.
    Nothing more is required, and the agent may then work on variation in same way.
    To deny this, not possible for the agent to substitute and work.
    However, \autoref{illu:gist:roots:F} only captures reasoning.
    So, get these events.
  \end{dets}
\end{note}
...
\begin{note}
  Given \tprof{}, the link between and agent \tCV{} and \fc{1} is simple.
