...
\subsection{A link between \qWhy{} and \qWhyV{}}
...
\begin{note}
  ...
  \linkW{} states the truth of the \itc{} of \qWhyV{} is sufficient to explain, in part, why an agent concluded \(\pv{\phi}{v}\) from \(\Phi\).
...
\begin{note}
  Interest in both \qWhy{} and \qHow{} is `explanatory reasons', summarised by \citeauthor{Hieronymi:2011aa} (\citeyear{Hieronymi:2011aa}) as:
  % 
  \begin{quote}
  ...
  \end{quote}
  % 
  \citeauthor{Hieronymi:2011aa}, the extreme heat or the faulty construction is a reason why the engine failed (\citeyear[409]{Hieronymi:2011aa}).
\end{note}
...
\begin{note}
  ...
  \end{quote}
  %
  Conditional is a lawlike constraint.
  A gamble, as in \autoref{illu:coinT} is not a but if the conditional is true the \ros{} enables us to understand why the phenomenon occurred.%
  \footnote{
    \nocite{Tichy:1976tp}%
    In the literature on subjunctive conditionals, the conditionals which specify the gamble are `laws' (\cite{Chisholm:1955aa,Lewis:1979vm,Veltman:2005tj}) or `lump together' the facts regarding the coin and relative wealth of agents \(A\) and \(B\) (\cite{Kratzer:1981aa,Kratzer:1989aa}).
  }
\end{note}

\begin{note}
  We present an analysis of a \scen{1} to motivate \autoref{link:why:support:pvpp}.
\end{note}
...
\begin{note}
  ...
  Pritcher calmly tells Fox the are a fellow member of the Democratic Underground Party at the end of the \scen{}.
  However, Fox has already drawn the conclusion when Fox asks `Who are you?' via the sequence of countersigning.

  Consider the sub-event in which Pritcher responds to `Miran is early this year' with `No earlier than last year'.
  ...
  %
  \begin{quote}
    \pv{\propI{What Pritcher said was an appropriate response to Fox's sign}}{\valI{True}}
  \end{quote}
  %
  Suppose the \ros{0} fails to hold.
  Then, {\color{red} \dots}


  Then, the sequence of countersigning fails.
...
\begin{note}
  ...
  \begin{observation}%
    ...
    There are some difficult cases for \autoref{link:why:support:pvpp}.
  \end{observation}

  \begin{motivation}{obs:qWhyV:rosNNecAns}
    Consider a \scen{0} in which an agent concludes \(23 \times 15 = 345\) from their understanding of arithmetic.
    The agent starts work on the problem, but part way though concludes it would be good to brew a cup of tea.

    It may well be the case that the agent would have been too tired to conclude the theorem without the aid of the coffee.
    If so, then just prior to the conclusion that it would be good to brew a cup of tea, {\color{red} conditional is true}.
  \end{motivation}
  ...
  Two things here.
  Whether the conditional really is true.%
  \footnote{
    Note, important that conditional is included in event.
    Of course, if you think the \ros{} does capture why, even if before, this is fine, as link is only sufficient.
  }
\end{note}
