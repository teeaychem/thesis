...
\begin{note}
  ...
  \item
    \wit{3} for \ros{1} \hfill \autoref{cha:var:ros:W}
  \item
...
\begin{note}
  ...
  When we talk of \wit{1} we talk in terms of `having a \wit{0}'.
...
\begin{note}
  As indicated in \autoref{cha:var:sec:wiggling}, \autoref{sec:vars} will use \ros{1} and \wit{3} to develop variations of \qWhy{}, \qHow{}, and \issueInclusion{} from \autoref{cha:introduction}.
\end{note}
...
\begin{note}
  As forecast in \autoref{cha:var:sec:wiggling}, we eliminate `how' in favour of whether an agent has a \wit{1} for \ros{}.
  ...
  Specifically, our variation of \qHow{} will seek answers to how an agent concluded \(\pv{\phi}{v}\) from \(\Phi\) in terms of events which are \wit{1} to \ros{1} which answer, in part, \qWhyVnP{}.
  ...
  Indeed, as the primary role of the variation to \qHow{} is to capture an intuitive constraint on answers to \qWhyVnP{}, the variation will amount to the constraint that the agent has a \wit{0} for any \ros{0} which answers \qWhyVnP{}.
\end{note}
...
\begin{note}
  Still, recall from \autoref{prop:cws}, that if an agent concludes \(\pv{\phi}{v}\) from \(\Phi\) then the agent has a \wit{} for the \ros{0} between \(\pv{\phi}{v}\) and \(\Phi\).
  Hence, the \wit{0} to the \ros{0} will, in part, answer the variation to \qHow{}.
  And, as the \wit{0} for a \ros{0} between \(\pv{\phi}{v}\) from \(\Phi\) just is the event in which the agent concludes \(\pv{\phi}{v}\) from \(\Phi\), we assume that the event at least serves 
 ...
 Following:
  ...
  \begin{restatable}[]{proposition}{propVariationsAndInclusion}
    ...
    \begin{argument}
      ...
      So, there is some \(e\) such that \(e\) is a \wit{0} for the \ros{} between \(\pv{\psi}{v'}\) and \(\Psi\).
    \end{argument}
  \end{restatable}
