...
\begin{note}
  ...
  And, to do so we will provide a recipe for generating counterexamples to \issueInclusion{}.
...
\begin{note}
  The goal is to develop a sufficiently precise variation of \issueInclusion{}.
  To do so, variations to \qWhy{} and \qHow{} and links between initial questions and variants.

  So,

  \begin{itemize}
  \item
    Variant to \qWhyV{} such that answers to \qWhyV{} are answers to \qWhy{}.
  \item
    Variant to \qHow{} such that answers to \qHow{} are answers to \qHowV{}.
  \end{itemize}

  Then, \issueConstraint{}, this means, constrain answers to variant to \qWhyV{} by answers to \qHowV{}.

  Result is variation to \issueInclusion{}.

  Counterexamples to \issueInclusion{} then
  \begin{itemize}
  \item
    Links fails
  \item
    Or, \issueInclusion{} fails.
  \end{itemize}

  In this respect, the variations are secondary.
  Need four things:
...
\begin{note}
  We begin by introducing \ros{1}.

  These will be key.

  Then, go \qWhy{}, \qHow{}, and \issueInclusion{}.
\end{note}
