...
\begin{note}
  \autoref{illu:gist:roots:F} is the following \scen{0}:
...
\begin{note}
  In order for a \ros{} between \pv{\rootsCon{}}{\valI{True}} and \(\Phi\) to answer \qWhyV{} we need to identify a \se{} of interest.
  ...
  \item
    ...
    \(\ed{\flat}\) is such that \(e\) under description `the agent concludes \pv{\rootsCon{}}{\valI{True}} from \(\Phi\)' is in progress.
    \begin{itemize}
     \item
       By inspection.
       For, it seems clear the agent is concluding \pv{\rootsCon{}}{\valI{True}} from \(\Phi\) when the agent does Step~\ref{illu:gist:roots:F:factor}.
    \end{itemize}
  \item
    \label{illu:gist:roots:F:qWhyV:ent}
    No matter any detail implicitly added to `the agent concludes \pv{\rootsCon{}}{\valI{True}} from \(\Phi\)' in \(d\), \(d\) is true \emph{only if} `the agent concludes \pv{\rootsCon{}}{\valI{True}} from \(\Phi\)' is true of \(e\).
    \begin{itemize}
    \item
      For, in order to ask \qWhyV{}, \(e\) under \(d\) is an event in which the agent concludes \pv{\rootsCon{}}{\valI{True}} from \(\Phi\).
    \end{itemize}
  \item
    ...
    \item
      By inspection.
      For, it seems clear that if the agent concludes \pv{\rootsCon{}}{\valI{True}} from \(\Phi\) then this happens as a result of factoring \(2x^{2} - x - 1 = 0\).
  ...
  \end{enumerate}
  %
  Given \ref{illu:gist:roots:F:qWhyV:prog}, \ref{illu:gist:roots:F:qWhyV:ent}, and \ref{illu:gist:roots:F:qWhyV:itc} \(\ed{\flat}\) is an \se{} of \(\ed{}\) by \autoref{def:se} (\autopageref{def:se}).
\end{note}


\subparagraph*{Connecting a \ros{} to the \se{}}
\label{qWhyV:ex:con}


\begin{note}
  So, we have an event \(\ed{}\) in which the agent concludes \pv{\rootsCon{}}{\valI{True}} from \(\Phi\) and an event \(\ed{\flat}\) which is a \se{} of \(\ed{}\).

  Our final task is to show:
  \begin{itenum}
  \item[\emph{If}:]
    A \ros{0} between  \pv{\rootsCon{}}{\valI{True}} and \(\Phi\) fails to hold through \(\ed{\flat}\).
  \item[\emph{Then}:]
    \(\ed{\flat}\) is not an event such that \(\ed{}\) is in progress.
  \end{itenum}
  %
  ...
  \begin{itemize}
  \item
    \(\ed{\flat}\) is an \se{0} of \(\ed{}\) such that:
    \begin{itenum}
    \item[\emph{If}:]
      A \ros{0} between\pv{\rootsCon{}}{\valI{True}} and \(\Phi\) fails to hold from \agpe{} through \(\ed{\flat}\).
    \item[\emph{Then}:]
      \(\ed{\flat}\) is not an event such that \(\ed{}\) is in progress.
    \end{itenum}
  \end{itemize}
  %
  Hence, the \ros{0} between\pv{\rootsCon{}}{\valI{True}} and \(\Phi\) answers \qWhyV{} as applied to \autoref{illu:gist:roots:F}.
\end{note}
