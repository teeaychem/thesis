...
\begin{note}
  The broad question is `why' \(e\) happens over any other event \(e'\).

  For example, roll a die.
  Lands on odd or even.
  However, assuming the die is fair, then there's nothing to be said for odd or even.
  There is no explanation `why' the die rolled odd.
  Other than that's what happened.

  However, if the die is bias, then explain `why' the rolled odd.
  The bias.
  If no bias, then would not have anything to say over odd or even.
\end{note}
...
\begin{note}
  ...
  \noindent%
  \qWhyV{}, think of conclusion like die.
  Agent may conclude.
  However, some things indicate.
  So things, absence, means nothing which favour over any other event.

  Though, \qWhyV{} is slightly broader.
  With the die, no event over any other.
  With conclusions, the agent still concludes.
  Still a \ros{} `follows from', and this does some work.

  So, two cases.
  Either no conclusion, as it happened, or nothing which favours.

  {
    \color{red}
    Key interest is \(e\) over any other event which may have happened.
    Consider sub-event prior to conclusion.
    If no \ros{} then there is nothing which favours \(e\) over any other event.

    So, to get that \(e\) develops, need a \ros{}.
  }
\end{note}
...
\begin{note}
  This approach is delicate.

  Assumes it is possible for the same event to occur regardless of whether some \prop{0}-\val{0}-\pool{0} pair is a \ros{}.
  For, else any \ros{} which holds for some sub-event is sufficient.

  Assume \ros{} holds at \(e^{\flat}\).
  Then, no \ros{} distinct event.
  And, as two events have distinct sub-events, distinct.

  Sufficiently plausible.
  Only method available for \ros{} is \supportI{} and \supportII{}.
  \supportI{}, then fine.
  \supportII{}.
  \fc{}.
  Action, concluding.
  ...
  What distinguishes \(e\) from any other event in which the agent concludes \(\pv{\phi}{v}\) from \(\Phi\)?

  For, if \emph{any} distinction between two events is sufficient, then 
  ...
  Note, \qWhyV{} is a different question if the consequent is strengthened to:
  \begin{itemize}
  \item \(e^{\flat}\) is not, or does not develop into, an event in which \vAgent{} concludes \(\pv{\phi}{v}\) from \(\Phi\).
  \end{itemize}
  For, at issue would be which \ros{} are necessary for \(e^{\flat}\) be or develop into an event in which the agent concludes \(\pv{\phi}{v}\) from \(\Phi\).

  Of course, if a \ros{} is necessary for \(e^{\flat}\) be or develop into an event in which the agent concludes \(\pv{\phi}{v}\) from \(\Phi\) and \(e^{\flat}\) is or develops into \(e\), then \(e^{\flat}\) be or develop into an event in which the agent concludes \(\pv{\phi}{v}\) from \(\Phi\).
  Hence, any answer to \qWhyV{} with the strengthened consequent is answer to \qWhyV{} as stated.

  Still:
  \begin{enumerate}
  \item
    Our interest is with \(e\).
  \item
    Without some additional constraints on what it is for an agent to conclude \(\pv{\phi}{v}\) from \(\Phi\), there is no way to deny an event may develop into an event in which an agent concludes \(\pv{\phi}{v}\) from \(\Phi\).
  \end{enumerate}
\end{note}
...
\begin{note}
  ...
  \qWhyV{} considers a prior event, and then looks to see whether there was something prior which secured.
  And, these are answers.
  We're not interested in whether it's an event in which the agent concludes.
  But, in whether throughout event in which the agent concludes, events were developing.

  So, fix an event, and see whether there's something which means that isn't or does not develop.
  If does not develop, then the conclusion is `unexpected'.

  Without this being a \fc{}, nothing substantial to say about why something happened, for nothing prior.
  I.e. something like a random walk.
\end{note}
...
\begin{note}
  \begin{observation}[Overdetermination]%
    \label{obs:qWhyV:od}%
    It may be the case that conclusion is overdetermined, if so, neither \ros{} answers \qWhyV{}.
  \end{observation}

  \begin{motivation}{obs:qWhyV:od}
    Way \qWhyV{} is defined.
  \end{motivation}

  From \autoref{obs:qWhyV:od}, \qWhyV{} is not a substitute for \qWhy{}.
  However, we do not hold \qWhyV{} is a substitute for \qWhy{}.
  Link is one way.

  And, so long as cases without overdetermination, things are fine.
  Hence, simpler definition.
  This wins over more complex.

  In the more complex case, we're not looking at \ros{}, but clusters of \ros{}.
\end{note}
