...
\begin{note}
  \qWhy{} questions why \(e\) is an event in which an agent concludes \(\pv{\phi}{v}\) from \(\Phi\) rather than an event in which the agent cleans the lint from their pocket, etc.
\end{note}
...
% \begin{note}
%   \begin{idea}[Progressive explanation]
%     \label{obs:qWhyEIP:bb}%
%     Given \(e_{d}\) is an event in which \vAgent{} concludes \(\pv{\phi}{v}\) from \(\Phi\):

%     \begin{itenum}
%     \item[\emph{If}:]
%       \(e^{\flat}_{d^{\flat}}\) is a \se{0} of \(e\) such that \(e_{d}\) is in progress.
%     \item[\emph{Then:}]
%       Whatever features of \(d^{\flat}\) entail \(e_{d}\) is in progress answer `why' \(e_{d}\) is an event in which \vAgent{} concludes \(\pv{\phi}{v}\) from \(\Phi\), with respect to the sense of `why' of \qWhy{}.
%     \end{itenum}
...
\begin{note}
  \begin{condition}[\ros{3} and progressive explanation]%
    ...
    \item[\emph{If}:]
      \(e^{\flat}_{d^{\flat}}\) is a \se{0} of \(e\) such that \(e_{d}\) is in progress.
    \item[\emph{And}:]
      \(e^{\flat}_{d^{\flat}}\) is a \se{0} of \(e\) such that \(e_{d}\) is in progress \emph{only if} a \ros{} between \(\pv{\psi}{v'}\) and \(\Psi\) holds throughout \(e^{\flat}_{d^{\flat}}\).
    \item[\emph{Then:}]
  ...
  \noindent%
  If \(e_{d}\) is an event in which an agent concludes \(\pv{\phi}{v}\) from \(\Phi\).
  Then, if there is some \se{} in which the agent is concluding \(\pv{\phi}{v}\) from \(\Phi\), and the agent is concluding \(\pv{\phi}{v}\) from \(\Phi\) \emph{only if} a \ros{} between \(\pv{\psi}{v'}\) and \(\Psi\) holds while the agent is concluding \(\pv{\phi}{v}\) from \(\Phi\), then the \ros{} between \(\pv{\psi}{v'}\) and \(\Psi\) answers \qWhy{}.

  Intuitive motivation is:
  \se{} when concluding.
  Therefore, as a result of concluding.
  And, as the agent is concluding, this favours event in which agent concludes over an event in which the agent does not conclude.

  Intuitively, concluding.
  Some stuff is happening.
  This captures `why' I conclude, so long as what happens when concluding is tied to conclusion.


  And, \ros{} between \(\pv{\psi}{v'}\) and \(\Psi\) is sufficiently entwined so that it is not possible to characterise the event as an event in which concluding without there being a \ros{} between \(\pv{\psi}{v'}\) and \(\Psi\) from the \agpe{}.

  \begin{motivation}{obs:qWhyEIP}
    Consider an event \(e_{d}\) in which \vAgent{} concludes \(\pv{\phi}{v}\) from \(\Phi\).
    And, assume \(e^{\flat}_{d^{\flat}}\) is a \se{0} of \(e\) such that \(e_{d}\) is in progress.

    \autoref{assu:p:ex}
    And, as in progress, \(e^{\flat}_{d^{\flat}}\) over any event for which a description incompatible with \(d^{\flat}\) holds.
  ...
  The motivation for \autoref{obs:qWhyEIP} is close to an argument (granting the relevant assumptions).
  However, sense of `why' by which \qWhy{} has been understood is fixed by intuition.
\end{note}
...
\begin{note}
  ...
  \noindent%
  And, a link between \qWhy{} and \qWhyV{} follows in parallel from our motivation for the sufficient condition for answers to \qWhy{}:
  ...
  % 
  % \noindent%
  % Expanded, \qWhyV{} considers whether an \se{0} does not develop into a given event in which the agent concludes if a \ros{} failed to hold.
  % And, answers to \qWhyV{} are \ros{1} such that the \ros{} failing to hold is sufficient to ensure the \se{0} does not develop into an event in which the agent concludes.
...
\begin{note}
  ...
  \begin{argument}{prop:ros-always-answer}
    ...
    We need to show there is some \se{0} \(e^{\flat}\) of \(e\) such that:
    %
    \begin{itenum}
    \item[\emph{If}:]
    ...
    \end{itenum}
    %
    By \supportI{}, a \ros{} holds between \(\pv{\phi}{v}\) and \(\Phi\), for the agent, when the agent pairs \(\phi\) with \(v\) as a \se{0} of an event in which the agent concludes \(\pv{\phi}{v}\) from \(\Phi\).
...
\begin{note}
  ...
  Intuitively it is not be the case that the \ros{} and the conclusion to get some tea answers why \emph{the agent} concluded some theorem is true from some \pool{}.%
    ...
    \item
      the \ros{} holds throughout some \se{0} of the event in which the agent concludes, and
    \item
      absence of the \ros{} for the \se{0} leads to an event in which the agent does not conclude \(\pv{\phi}{v}\) from \(\Phi\).
    \end{enumerate*}

    Hence, \qWhy{} is somewhat stringent.
    For example, a \ros{} between \pv{\propI{Brew a cup of coffee}}{\valI{Do}} and some \pool{} \(\Psi'\) does not answer \qWhyV{} as the agent concluded to get tea, rather than coffee (and even if the choice between tea and coffee was a figurative coin flip).
    And, the \ros{} between \pv{\propI{Brew a cup of tea}}{\valI{Do}} and \(\Psi\) fails to answer \qWhyV{} if the agent may have concluded \(\pv{\phi}{v}\) from \(\Phi\) without the aid of the cup of tea.
\end{note}
...
\begin{note}
  ...
  Indeed, the consequent follows from Condition~\ref{cond:nH:cPVP} alone.
...
\begin{note}
  ...
  \begin{question}{questionHowV}{\qHowV{}}%
    ...
    \begin{itemize}
    \item
      Which events \(e^{\sharp}\) are such that both XX and YY are true:
      \begin{itemize}
      \item
        \(e^{\sharp}\) before or at the same time as \(e\).
      \item
...
\begin{note}
  ...
  \begin{argument}{prop:support-and-witnessing}
    ...
    \item
      By \issueInclusion{}, an event in which the agent concludes \(\pv{\psi}{v'}\) from \(\Psi\) answers \qHow{}.
    \item
      By \linkH{}, the event in which the agent concludes \(\pv{\psi}{v'}\) from \(\Psi\) answers \qHowV{}.
    \end{itemize}
    \vspace{-\baselineskip}
  \end{argument}
\end{note}
