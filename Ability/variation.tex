...
\begin{note}
  Second, it is plausible that there are cases in which \ros{} without \wit{0}.
  \ros{} holds between \(\pv{\phi}{v}\) and \(\Phi\) due to \(\pv{\phi}{v}\) from \(\Phi\) being a \emph{\fc{0}}.
...
\begin{note}
  ...
  And, as looking for counterexamples, only need to convince you that there are true instances without \wit{0}.
\end{note}
...
\begin{note}
  ...
  Following \ros{1} as answers to \qWhyVnP{}, \qHowV{} concerns \wit{1} for \ros{1}.
...
\begin{note}
  ...
  View having a \wit{0} as a necessary constraint.
  ...
  And, if cause then has a \wit{0}.

  Rules, then \wit{0} to rules.

  Contrast, exercising disposition.
  In this case, exercise of the disposition results in a \wit{0}.

  So long as account requires having a \wit{0}, then variant to \qHow{} captures way in which account is of interest.
\end{note}
...
\begin{note}
  ...
  Further, as agent concludes, the agent has a \wit{0} for the \ros{} between \(\pv{\phi}{v}\) and \(\Phi\).
...
\begin{note}
  So, via \fc{1} obtain instances of a \ros{1} holding from an \agpe{} without the agent having a \wit{0} for the \ros{0}.
...
\begin{note}
  ...
  Here, we return to the prospect of embedding \ros{1} within \ros{1} as seen in~\autoref{cha:var:support:Emb}.
