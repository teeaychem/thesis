...
\begin{note}
  \begin{rdefinition}{def:requ}{A \requ{0}}%
    % \autoref{def:requ}
    For any \eiw[\(\ed{}\)]{0} \vAgent{} concludes \(\pv{\phi}{v}\) from \(\Phi\):
    %
    \begin{itemize}
    \item
...
\begin{note}
  ...
  \begin{argument}{prop:requ-WhyV}
    ...
    A \ros{0} between \(\pv{\psi}{v'}\) and \(\Psi\) holds for the agent through \(\ed{\flat}\).
  \end{argument}
\end{note}
...
\begin{note}
  ...
  \noindent%
  Note, \pv{\rootsCon{}}{\valI{True}} being a \fc{} from \(\Phi\) through \(\ed{\flat}\) being a \requ{} of \(\ed{}\) captures the way a \ros{} between \pv{\rootsCon{}}{\valI{True}} and the \pool{} \(\Phi\) answers \qWhy{}.
...
\begin{note}
  ...
  For example, in \autoref{obs:LucasRequ} a \ros{} between \pv{\propM{L_{8} = 47}}{\valI{True}} and \(\Phi\) answers \qWhy{} the agent concluded \pv{\propM{L_{[:9]} = [2, \dots, 76]}}{\valI{True}} from \(\Phi\).
  ...
  Likewise, in \autoref{illu:gist:roots:F} a \ros{} between \pv{\rootsCon{}}{\valI{True}} and \(\Phi\) answers \qWhy{} the agent concluded \pv{\rootsCon{}}{\valI{True}} from \(\Phi\).
  However, when the agent concludes \pv{\rootsCon{}}{\valI{True}} from \(\Phi\) the agent has a \wit{} for the \ros{} between \pv{\rootsCon{}}{\valI{True}} and \(\Phi\).
  %
  So, to find a counterexample to \issueInclusion{} we need to ensure the agent does not have a \wit{0} for the relevant \ros{0}:
...
\begin{note}
  ...
  And, in turn \requ{1} for which the agent does not have a \wit{} for the corresponding \ros{}.
\end{note}
...
\begin{note}
  ...
  For, by \autoref{prop:requ-WhyV}, a \ros{} between \(\pv{\psi}{v'}\) and \(\Psi\) answers to \qWhy{} when \(\pv{\psi}{v'}\) being a \fc{} from \(\Psi\) is a \requ{} of some \se{} of the \eiw{0} the agent concludes \(\pv{\phi}{v}\) from \(\Phi\).
