...
\begin{note}
  This chapter collects together key ideas from previous chapters and establishes a path to counterexamples to \issueInclusion{} by providing sufficient conditions for answers to \qWhy{}.

  Specifically, the interaction between \se{1} and \progEx{} (\autoref{cha:events-progress}), \fc{1} (\autoref{cha:fcs}), and \fingfr{1} (\autoref{cha:ros}) is condensed into the idea of a `\requ{}'.
\end{note}

\begin{note}
  This chapter beings by defining the idea of a \requ{} and in turn shows the way \requ{1} help identify answers to \qWhy{}.

  With an understanding of \requ{1} in hand we then provide two detailed applications of the idea, both compatible with \issueInclusion{} --- i.e.\ we apply the definition of a \requ{} to a pair of \scen{} and identify answers to \qWhy{} asked with respect to those \scen{1}.

  Finally, we highlight the kind of \requ{} required to establish the failure of \issueInclusion{}.
\end{note}
...
\begin{note}
  ...
  \begin{itemize}
  \item
    A conclusion of \(\pv{\phi}{v}\) from \(\Phi\) is in progress through \(\ed{\flat}\), and if the conclusion in progress happens, it happens as a result of \(\ed{\flat}\).
  \item
...
\begin{note}
  ...
  For, the details of \autoref{appl:qWhyV-s1} simply expand on \autoref{app:requ-scen1} with the observations made in the argument for \autoref{prop:requ-WhyV}.
\end{note}
...
\begin{note}
  ...
  \begin{scenario}[Lucas numbers]%
    ...
    \footnote{
      \nocite{OEIS-Foundation-Inc.:aa}
      Starting at \(2\), see \hyperlink{cite.OEIS-Foundation-Inc.:aa}{OEIS sequence A000032} for more details.
    }
    %
    \[
      L_{n} = \left\{
  ...
  \begin{dets}{obs:LucasRequ}
    ...
    Implicit in the description of \autoref{scen:LucasNums} is that the agent has not calculated the first nine Lucas numbers prior to \(\ed{}\).
    ...
    Fix \(\edn{\flat}\) as the event when the agent concludes \pv{\propM{L_{7} = 29}}{\valI{True}}, and take \(\edo{\flat}\) as a natural description of \(\edn{\flat}\).
    \smallskip
    ...
    And, as \(\edo{\flat}\) as a natural description of \(\edn{\flat}\), it is clear an \eiw{0} the agent concludes \pv{\propM{L_{[:9]} = [2, \dots, 76]}}{\valI{True}} from \(\Phi\) is in progress.
    \smallskip
    ...
    Therefore, as there is some action available to the agent such that the agent continues to be concluding \pv{\propM{L_{[:9]} = [2, \dots, 76]}}{\valI{True}} from \(\Phi\), the same action also involves the agent concluding \pv{\propM{L_{8} = 47}}{\valI{True}} from \(\Phi\).
  \end{dets}
...
\begin{note}
  ...
  \begin{rproposition}{prop:requ-WhyV-ces}{\requ{3} and \issueInclusion{}}%
    \vspace{-\baselineskip}
    \begin{itenum}
    \item[\emph{If}:]
...
\begin{note}
  ...
  Key to the failure of \issueInclusion{} is the existence of \requ{1} \emph{such that} \issueInclusion{} fails to hold.
