...
\begin{note}[Checking conditions]
  Condition~{\color{red} background} is provided by the scenario.
  ...
  Condition~{\color{red} inclusion} is obtained by reflection on ability.
\end{note}
...
\begin{note}
  ...
  First, from ability.
  Second, from `ability'.
  {\color{red} Goal is to break {\color{red} inclusion:bound}}
\end{note}
...
\begin{note}
  ...
  \item Instead, agent is appealing to those premises and steps in the same way they would do when witnessing \emph{and this doesn't require appeal to general ability}.
    ...
    \end{itemize}
  \item So, ability as a whole carries the \crequ{}, but appeal to distinct components does not.
...
\begin{note}[Setting expectations]
  ...
  We will say more on this in section~{\color{red} sec:establishing-tension}.
...
\begin{note}[Working through the details]
  {\color{red} background} \dots



  {\color{red} inclusion} are satisfied in the scenarios of interest.

  Letter, but more importantly the spirit.

  Spirit goes back to \ideaCS{}.
  Saw that \FCS{} follows from \ideaCS{}.
  Whether to reason about `antecedent check'.

  In other words, \adB{} does not lead to similar conflict with \ideaCS{}.

  Hence, the `in principle' defence that \adB{} need not conflict with \nI{} rests on showing that the agent may conclude that they may conclude \(\psi\) has value \(v'\) when steps of reasoning are \adB{} with respect to ability.
...
\section{Incompatibility of \LCS{}, \gsi{}, and \adA{}}
