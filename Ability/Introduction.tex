...
\begin{note}
  ...
  Specifically, our interest is with whether a constraint on answers to \emph{why} the way an \eiw{0} the agent concludes happens in terms of answers to \emph{how} the way an \eiw{0} the agent concludes happens.

  I argue the constraint does not hold.
\end{note}

\begin{note}
  The role of this chapter is to introduce the constraint on answers to \emph{why} in terms of answers to \emph{how} and sketch the in outline the argument to follow.
\end{note}


\begin{note}
  We begin with a pair of \eiw{1} an agent concludes happens.
  In turn, abstraction used to reason about \eiw{1} an agent concludes happens are introduced, and applied to an additional \scen{0}.

  With an abstract understanding of \eiw{1} an agent concludes happens, we introduce, detail, and motivate the why question, the how question, and the constraint on answers to the why question in terms of answers to the how question.

  With an understanding of the constraint in hand, the method for obtaining counterexamples to the constraint is sketched.
\end{note}

\section{\eiw{3} an agent concludes}
\label{sec:eiw2-an-agent}
...
\begin{note}
  ...
  \begin{rscenario}{illu:gist:roots:F}{Quadratic roots --- Factorisation}%
...
\begin{note}
  ...
  These abstractions are stated with the help of \scen{1} \ref{illu:gist:roots:F} and \ref{illu:gist:roots:QF} and are then applied to an additional \scen{}.
\end{note}
...
\begin{note}
  So, an \eiw{0} an agent concludes involves an agent \evaling{} some \prop{0} \(\phi\) to have \val{0} \(v\) from some \pool{} \(\Phi\).
...
\begin{note}
  \qWhy{} fixes an event \(e\), and queries why it is an \eiw{0} some agent concludes \(\pv{\phi}{v}\) from \(\Phi\) rather than an \eiw{0} some other thing happens --- why \(e\) is not an \eiw{0} something incompatible with the conclusion \(\pv{\phi}{v}\) for \(\Phi\) happens.
...
\begin{note}
  ...
  For, \mainCard{} was shuffled to \mainCardPosX{}, hence there is some \eiw{0} \mainCard{} being shuffled to \mainCardPos{} results in \(e\).
...
\begin{note}
  ...
  Hence, so long as the die lands on 6 as a result of bias, then clauses \ref{idea:why:result}, \ref{idea:why:favour}, and \ref{idea:why:feat} of \autoref{idea:why} are satisfied by an \eiw[\(e'\)]{0} bias influences the rolling of a die.
  In particular:
...
\begin{note}
  In short, the relevant \fofr{0} answers \qWhy{} as it is part of what it is for the conclusion to be in progress, and so to be favoured over any other thing.
...
\begin{note}
  ...
  For example, given is an \eiw[\(e\)]{0} a deck of cards is shuffled, \mainCard{} is \mainCardPos{}, and the shuffler is engaging in sleight of hand, \qHow{} queries the way the shuffler performed sleight of hand, which cards where moved where, etc.
...
\subsection{\issueInclusion{}: A constraint on answers to \qWhy{} in terms of answers to \qHow{}}
...
\begin{note}
  ...
  With \autoref{scen:countS} our interest is with \fingfb{\pv{\propI{\signConB{}}}{\valI{True}}}{some \pool{} \(\Psi\)}.
...
\begin{note}
  ...
  \footnote{
    ...
    Though, this is of little interest as we are talking about a case where the agent concluded \(\pv{\phi}{v}\) from \(\Phi\).
...
\begin{note}
  ...
  \begin{motivation}{obs:iIceRestriction}
    ...
    \begin{enumerate}[label=\arabic*., ref=\arabic*]
    \item
      \qWhy{} concerns an \eiw{0} an agent concludes.
      So, it must be the case that for some \prop{0}-\val{0} pair \(\pv{\phi}{v}\) and \pool{} \(\Phi\) we have an \eiw{0} an agent concludes \(\pv{\phi}{v}\) from \(\Phi\).
    \item
...
\begin{note}
  ...
  \end{quote}
  %
  In other words, the goal of this document is to show any \scen{0} which satisfies a collection of features entails \issueInclusion{} does not hold.
...
\begin{note}
  ...
  \item
    ...
    So, I doubt the agent may be concluding \pv{\propM{\rootsCon{}}}{\valI{True}} from their understanding of factorisation \emph{unless} \fofb{\pv{\propM{\rootsConEqExV{12}{4}{3}}}{\valI{True}}}{a \pool{} which includes the \agents{} understanding of factorisation}.
  \end{itemize}
  %
...
\begin{note}
  This chapter introduced the way we understand \eiw{1} an agent concludes, questions \qWhy{} and \qHow{}, and the constraint \issueInclusion{}.
