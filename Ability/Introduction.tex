...
\begin{note}
  ...
  More could be said about the way in which the agent concluded \(23 \cdot 15 = 345\) from the calculator, but these details won't be of too much interest.%
  \footnote{
...
\begin{note}
  ...
  So, say the agent concluded `\(23 \cdot 15 = 543\)' has value `\(\text{True}\)' from a \poP{} of premises which includes the calculator.
\end{note}

\begin{note}
  Abstracting from almost all the details of \autoref{illu:gist:calc}, then we may say that an agent concluded some proposition-value pair from some \poP{} of premises.
  % We assume an event in which an agent concludes always amounts to a conclusion that some proposition has some value from some \poP{} of premises.
\end{note}

\begin{note}
  Our interest is understanding the way in which an event in which some agent concludes some proposition \(\phi\) has some value \(v\) happened.

  In this respect, our interest is primarily with the intuition that the agent concluded `\(23 \cdot 15 = 543\)' has value `\(\text{True}\)' \emph{from} the calculator.
  ...
  The conclusion was `from', `due to', `because of' \dots etc., the relevant \poP{} of premises.
...
\begin{note}
  ...
  Hence, given the conclusion a relation holds between `Four legs good, two legs bad', `\(\text{True}\)' and the existence of Snowball's explanation, for each bird.
\end{note}
...
\begin{note}
  Our interest is understanding the way in which an event in which an agent concludes some proposition has some value happened.
...
\begin{note}
  ...
  % It seems, respectively, the calculator and Snowball's explanation are sufficient to understand the way in which the agent concluded the relevant proposition has the specified value.
  %
...
\begin{note}
  ...
  So, answers to \qHow{} when applied to \scen{3}~\ref{illu:gist:calc}~and~\ref{scen:animalism},
  % ,~and~\ref{scen:sudoku:intro},
  at least, are the events captured by the descriptions given.
  ...
  % Events of \scen{3}~\ref{illu:gist:calc}~and~\ref{scen:animalism} seem to provide an account of the way in which conclusion was obtained from \poP{} of premises.
\end{note}
...
\begin{note}
  ...
  \begin{restatable}[\issueInclusion{}]{constraint}{issueInclusionFirst}
    ...
    \begin{itemize}
    \item
      Some relation between proposition, value and \poP{} of premises is, in part, an answer to \qWhy{}.
    \end{itemize}
    \emph{Only if:}
    \begin{itemize}
    \item
...
\begin{note}
  ...
  In principle, relation between any proposition, value, and \poP{} of premises may answer \qWhy{}.
  ...
  \item
    ...
    A relation may hold between `\(23 \cdot 15 = 345\)', `\(\text{True}\)' and whatever \poP{} of premises would be associated with the agent calculating \(23 \cdot 15 = 345\) from their understanding of arithmetic.
    However, there is no event in which the agent concluded `\(23 \cdot 15 = 345\)', has value `\(\text{True}\)' by performing arithmetic, and hence there is no answer to \qHow{}.
  \item
...
\begin{note}
  ...
  Absence of event is sufficient.%
  \footnote{
  ...
  Whatever connections relations to events, need some witness.

  Of course, \qHow{} may narrow down range on answers to \qWhy{}.
  Specifies, in part what the relation amounts to.
  Still, granting \qWhy{} stated independently of \issueInclusion{}, \issueInclusion{} amounts to a substantive constraint.
\end{note}
...
\begin{note}
  \qWhy{} and \qHow{} are questions about the way in which an event in which an agent concludes some proposition has some value happened.

  \qWhy{} seeks `explanatory reasons'.
  \citeauthor{Hieronymi:2011aa} (\citeyear{Hieronymi:2011aa}) summaries such reasons as:
  ...
  In short, explanatory reasons are things we cite to explain other things.

  In the case of \qWhy{}, the relevant explanatory reason is a relation between a proposition-value pair and a \poP{} of premises.
  The relation between `\(23 \cdot 15 = 345\)', `\(\text{True}\)' and the calculator is a reason why the agent has concluded `\(23 \cdot 15 = 345\)' has value `\(\text{True}\)'.
  If such a relation failed to hold, it would not be the case that the agent has concluded `\(23 \cdot 15 = 345\)' has value `\(\text{True}\)'.
\end{note}
...
\begin{note}
  ...
  So, at issue is why the agent did what they did, and the explanatory reason for this state of affairs involves the agent's reason.
...
\begin{note}
  With respect to relations between proposition-value pairs and \poP{1} of premises, a sensible candidate for the agent's reasons is the \poP{0} of premises.
  ...
  \item
    The reason for which the agent concluded the solution was correct was the initial Sudoku grid and the agent's understanding of Sudoku.
  \end{itemize}

  Granting a relation between a proposition-value pair and a \poP{1} of premises provides an explanatory reason, we may understand the explanatory reason in terms of an agent's reasons by identifying the \poP{1} of premises as the agent's reasons.%
  \footnote{
...
\begin{note}
  Broadening scope, agent's reasons reduce to some psychological facts of an agent.%
  \footnote{
  ...
  And, relation between proposition-value pair and \poP{} of premises follows from some psychological facts of the agent.

  Calculator, but strictly recognition, and likewise the bird's perception that Snowball's explanation exists.

  Something about agent, get conclusion.%
  \footnote{
    Note, we have talked in terms of an agent's reasons as corresponding to some \poP{} of premises.
    Hence, an agent's reason is also some psychological fact.
    However, I am taking for granted only the idea that the relation between some proposition-value pair and some \poP{} of premises follow from some psychological fact(s) of the agent.

    Hence, it seems to me that what I am taking for granting is compatible with views such as those of~(\cite{Dancy:2000aa}) and~(\cite{Alvarez:2013aa}), in which the state of affairs corresponding to content of belief may be the agent's reason.

    For, we may understand premises as states of affairs.
...
\begin{note}
  ...
  Something gives relation between proposition-value and \poP{} of premises.
  Do other psychological facts give other relations.
  ...
  So, explanatory relation, when give the agent's reason, this amounts to causal explanation.
  The agent's reason, whatever it turns out to be, stands in a causal relation to the action.

  So, we have a constrain on reasons in terms of what happened.
  Parallel to \issueInclusion{}.

  \qHow{} looks for events.
  This is not explicitly causal, but causal entails event.

  So, to get a relation, then seems causation.
\end{note}
...
\begin{note}
  ...
  For \citeauthor{Broome:2013aa} active reasoning just is a particular kind of causal relation between conclusions and \poP{} of premises.

  Hence,  no other relation between some proposition, value and \poP{} of premises is required to understanding the way in which an event in which an agent concludes some proposition has some value happened.%
  \footnote{
    ...
    Likewise, \citeauthor{Davidson:1963aa} focuses on causal explanation, and does not rule out that there may be cases in a reason explains an action by given the agent's reason for doing what they did in addition to other considerations (which may be causally involved).
  }
\end{note}

\begin{note}
  Motivation from \citeauthor{Davidson:1963aa} and \citeauthor{Broome:2013aa}.
  Analogues to \issueInclusion{} in terms of causation.
  However, abstracting a little, the role of causation in \citeauthor{Davidson:1963aa} and \citeauthor{Broome:2013aa}'s account is of interest only to the extent that the relevant causal relations suggest that relation between proposition-value pair and \poP{} of premises which holds when the agent concludes is sufficient.

  Final illustration, \citeauthor{Hieronymi:2011aa} captures this general idea without (at least explicitly) turning to causation:
...
\begin{note}
  ...
  Motivated \issueInclusion{} by a pair of \scen{1}, and then more generally in terms of reasons.
\end{note}

\begin{note}
  Our goal is to provide a recipe for generating counterexample to \issueInclusion{}.

  Relations between proposition-value pair and \poP{} of premises such that there is no event in which the agent concludes the proposition-value pair from the \poP{} of premises.
\end{note}

\begin{note}
  Interest here is common sense.

  With \autoref{scen:animalism}, follows from \issueInclusion{} that the content of Snowball's explanation is irrelevant.
  ...
  Parallel reasoning entails the agent's understanding of arithmetic is irrelevant with respect to \autoref{illu:gist:calc}.
  However, I find this less clear.
  ...
  Though, whether this is the case will depend on whether some additional details hold of~\autoref{illu:gist:calc}.
\end{note}

\begin{note}
  No sketch of the recipe.
  Turns on working out the details.

  Indeed, substance of document is largely definitions.
  Construction will be brittle.
  However, once given, the motivation for the construction extends.
  There's just no easy way to reach without initial construction.
\end{note}

\begin{note}
  First part, groundwork.
  Clarify understanding of a conclusion, and an event in which an agent concludes.
  Then, variations on questions and constraint.
  Function is to refine to make precise the way in which counterexamples work.

  Second part, ingredients for the recipe.
  Three ideas.
  \fc{}, \tR{}, and \requ{}.

  Third part, mixing these ingredients.
\end{note}
...
%   The \poP{} of premises includes the initial Sudoku grid and the rules of Sudoku.
