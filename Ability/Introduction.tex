...
\begin{note}
  ...
  Our understanding of \autoref{scen:calc} is be different if the agent concluded \propM{\gistCalcEq{}} has value \valI{True} from the their understanding of arithmetic, or if the agent concluded \propM{\gistCalcEq{}} has value \valI{True} from some other person.
...
\begin{note}
  \scen{3}~\ref{scen:calc}~and~\ref{scen:animalism} seem compatible with \issueInclusion{}:%
  \footnote{
  ...
  \item
    ...
    Still, as there is no event in which the birds conclude \propI{Four legs good, two legs bad} has value \valI{True} from the content of Snowball's explanation, no such \ros{} may answer \qWhy{}, given \issueInclusion{} holds.
  \end{itemize}
\end{note}
...
\begin{note}
  ...
  And, it seems no way to capture other \ros{1} in terms of rationalisations.
...
\begin{note}
  ...
  And, we motivated the constraint in part by a pair of \scen{1} (\scen{1}~\ref{scen:calc}~and~\ref{scen:animalism}) and then more generally in terms of reasons.
...
%   \footnote{
%     Roughly, if it were the agent failed to conclude \propM{\gistCalcEq{}} in \autoref{scen:calc}, then there would be conflict between the agent's understanding of arithmetic and the testimony of the calculator.
