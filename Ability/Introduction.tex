...
\begin{note}
  Abstracting from almost all the details of \autoref{illu:gist:calc}, then we may say that an agent concluded some proposition has some value from some \poP{} of premises.
  ...
  For ease, we abbreviate `\poP{} of premises' to `\poP{}'.
\end{note}
...
\begin{note}
  ...
  The conclusion was `from', `due to', `because of' \dots etc., the relevant \poP{}.
...
\begin{note}
  ...
  \begin{restatable}[\qWhy{}]{question}{questionWhyBasic}
    ...
    \item
      ...
      \begin{itemize}
      \item
        Which relations between some proposition, value, and \poP{} explain \emph{why} \(e\) is such that \vAgent{} concluded \(\phi\) has value \(v\)?
      \end{itemize}
    \end{itemize}
    \vspace{-\baselineskip}
  \end{restatable}
...
\begin{note}
  \qWhy{} seeks an understanding of the way in which an agent concluded \(\phi\) has value \(v\) in terms of relations between propositions, values, and \poP{1}.
  ...
  \footnote{
    ...
    Functions as a premise, given the understanding of \poP{1}.
...
\begin{note}
  ...
  % Reasoned from \poP{} to conclusion.
  % Events of \scen{3}~\ref{illu:gist:calc}~and~\ref{scen:animalism} seem to provide an account of the way in which conclusion was obtained from \poP{}.
\end{note}
...
\begin{note}
  ...
  \begin{restatable}[\issueInclusion{}]{constraint}{issueInclusionFirst}
    ...
    \begin{itemize}
    \item
      Some relation between proposition, value, and \poP{} is, in part, an answer to \qWhy{}.
    \end{itemize}
    \emph{Only if:}
    \begin{itemize}
    \item
      An event in which the agent concludes the proposition has the value from the \poP{} is, in part, \qHow{}.
    \end{itemize}
    \vspace{-\baselineskip}
  \end{restatable}

  In short, for any relation between a proposition, value, and \poP{} which grants some understanding of the way in which an agent concluded \(\phi\) has value \(v\), there is an event such that the agent has concluded the proposition has some value from the \poP{}.
\end{note}

\begin{note}
  I consider \issueInclusion{} intuitive.
  In principle, relation between any proposition, value, and \poP{} may answer \qWhy{}.
  ...
  \item
    ...
    And, while a relation may hold between `\(23 \cdot 15 = 345\)', `\(\text{True}\)' and whatever \poP{} would be associated with the agent calculating \(23 \cdot 15 = 345\) from their understanding of Arithmetic.
    There is no event in which the agent concluded `\(23 \cdot 15 = 345\)', has value `\(\text{True}\)' by performing arithmetic.
    So, there is no answer to \qHow{} with respect to this \poP{}, and given \issueInclusion{}, it is not possible for the relation to answer \qWhy{}.
  \item
    Parallel reasoning applies to~\autoref{scen:animalism}.

    The bird's conclusion of `Four legs good, two legs bad' has value `\(\text{True}\)' from the existence of Snowball's explanation answers \qHow{}, and the relation between the proposition, value, and \poP{} answers \qWhy{}, in line with \issueInclusion{}.
  ...
  \footnote{
    Note, \qHow{} does not explicitly require the relevant event to be the event in which the agent concludes \(\phi\) has value \(v\) from the \poP{}.
...
\begin{note}
  ...
  With \qWhy{}, the relevant explanatory reason is a relation between a proposition, value, and a \poP{}.
...
\begin{note}
  ...
  % }
  And, with respect to conclusions, a sensible candidate for an agent's reasons are \poP{0}.
  ...
  \footnote{
    ...
    \end{quote}
    However, our interest is with the \emph{relation} between a proposition, value, and \poP{}.
  }
  ...
  So, it seems relations between propositions, values, and \poP{1} may be understood as \citeauthor{Davidson:1963aa}ian rationalisations.
  Or, conversely, one may grant a relation between a proposition, value, and a \poP{1} provides an explanatory reason, and understand the explanatory reason in terms of an agent's reasons by identifying the \poP{1} as the agent's reasons.%
  \footnote{
...
\begin{note}
  ...
  If we grant relations between a proposition, value, and \poP{1} are rationalisations, and rationalisations are causal explanations, and \poP{1} capture an agent's reasons then \issueInclusion{} follows.

  For, suppose some relation between a proposition, value, and \poP{} which answers \qWhy{}.
  By assumption, this relation amounts to a rationalisation.
  And, by assumption rationalisations as causal explanations.
  Hence, there must be some causal relation holding between a \poP{1} and the agent's conclusion that the proposition has the value.
  And, grating there is a causal relation between the agent's conclusion that the proposition has the value and a \poP{1}, then there is some event in which the agent concludes the proposition has the value from the \poP{1}.
...
\begin{note}
  ...
  \citeauthor{Broome:2019aa}, however, holds that causation of a conclusion-attitude by premise-attitudes is sufficient for a process to be active reasoning, and so we do not need to consider anything other than the causal relation to understand the way in which the event happened.%
  \footnote{
...
\begin{note}
  ...
  However, abstracting a little, the role of causation in \citeauthor{Davidson:1963aa} and \citeauthor{Broome:2013aa}'s account is of interest only to the extent that the relevant causal relations ensure the relation between a proposition, value, and \poP{} which holds between the conclusion and the \poP{} is sufficient.
  ...
  Therefore, if a relation between a proposition, value, and \poP{} answers \qWhy{} and this relation is understood in terms of the agent taking the \poP{} to settle the question of whether to conclude the proposition has the value, we must have, by \citeauthor{Hieronymi:2011aa}'s account, an answer to \qHow{} in line with \issueInclusion{}.
\end{note}

\begin{note}
  Key idea.
  \issueInclusion{} is motivated by how an agent concludes being sufficient to understand why.
  Specifying the \poP{} gets us a reason, and this reason seems to be sufficient.
...
\begin{note}
  ...
  However, I do not think that a relation between the proposition, value, and \poP{} exhaust the relevant psychological facts.
  In particular, I think that in various cases additional relations between distinct propositions, values and \poP{1} answer \qWhy{} without there being an event in which the agent concluded the propositions have values from \poP{1}.
\end{note}
...
\begin{note}
  ...
  \item
    ...
    The counterexamples occur when each idea applies to a \scen{0} in which an agent concludes some proposition has some value from some \poP{}.
  ...
  \item
    ...
    Finally, we provide a few sample \scen{1} where \issueInclusion{} fails, and consider any leftover issues from the recipe.
  ...
  Instead, the recipe is based on the way in which a number of ideas come together when we understand the way in which some agent concludes some proposition \(\phi\) has some value \(v\) happened.
\end{note}
...
%     In the \scen{1} of interest, this hypothetical --- or in some cases possible --- conflict will strictly be between the agent's reasoning from \poP{1} to conclusions.
%   }
% \end{note}
...
%   The \poP{} includes the initial Sudoku grid and the rules of Sudoku.
