...
\begin{note}
  \color{blue}
  Specific \prop{3} and \val{1} are written in a \textsf{sans-serif} font, and arbitrary \prop{} and \val{} are written using lower case Greek letters and variations on the Latin letter \(v\), respectively.
...
\begin{note}
  ...
  And, we use a \ros{} between \pv{\propM{\rootsCon{}}}{\valI{True}} and a \pool{} which captures the \agents{} understanding of factorisation to captures whatever it is that ensures the agent is concluding.
  ...
  In this respect, a \ros{} between \pv{\propM{\rootsCon{}}}{\valI{True}} and a \pool{} which captures the \agents{} understanding of \emph{the quadratic equation} may hold at the second step of reasoning in \autoref{illu:gist:roots:F}.
...
\begin{note}
  ...
  And, when Fox \evals{} \propI{\signConA{}} as having \valI{True}, a \ros{} holds between \pv{\propI{\signConA{}}}{\valI{True}}, and \(\Phi\).%
  \footnote{
  ...
  Further, after Pritcher's opening `I come from Miran', Fox (sub-)concludes \pv{\propI{What Pritcher said was a sign}}{\valI{True}} and some \pool{} \(\Psi\).
  And, of interest is with the \ros{} between \pv{\propI{What Pritcher said was a sign}}{\valI{True}} and some \pool{} \(\Psi\).
  ...
  \begin{itenum}
  \item[\emph{If}:]
    A \ros{} between \pv{\propI{What Pritcher said was a sign}}{\valI{True}} and some \pool{} \(\Psi\) fails to hold.
  \item[\emph{Then}:]
...
\begin{note}
  ...
  \begin{question}{questionWhy}{\qWhy{}}
    Given \(e\) is an \eiw{0} \vAgent{} concludes \(\phi\) has \val{0} \(v\) from \(\Phi\):
    \begin{itemize}
    \item
      Which \ros{1} are included in (partial) explanations of \emph{why} \(e\) is an \eiw{0} \vAgent{} concludes \(\phi\) has \val{0} \(v\) from \(\Phi\) (rather than some other event)?
    \end{itemize}
  ...
  \begin{question}{questionHow}{\qHow{}}
    ...
    \begin{itemize}
    \item
      Which past or present events (partially) explain \emph{how} \(e\) is an \eiw{0} \vAgent{} concludes \(\phi\) has \val{0} \(v\) (rather than some other event)?
    \end{itemize}
...
\begin{note}
  \qWhy{} fixes an event, and queries why the event is an \eiw{0} an agent concludes \(\phi\) has value \(v\) from \(\Phi\) rather than an \eiw{0} something incompatible with the conclusion \(\phi\) has \val{0} \(v\) for \(\Phi\) happens.
  For example, an \eiw{0} the agent concludes \(\pv{\phi'}{v}\) from \(\Phi\), \(\pv{\phi}{v}\) from \(\Phi'\), takes their dog for a walk, runs a bath, etc.

  \qHow{}, by contrast, queries how the event is an \eiw{0} an agent concludes \(\phi\) has \val{0} \(v\) from \(\Phi\) came about.
\end{note}
...
\begin{note}
  Now, \qWhy{}, specifically, seeks answers to why \(e\) is an \eiw{0} an agent concludes \(\phi\) has \val{0} \(v\) from \(\Phi\) in terms of \ros{1}.
...
\begin{note}
  ...
  In short, given \(e'\) is the \eiw{0} Fox concludes \propI{What Pritcher said was a sign} has \val{0} \valI{True} from \(\Psi\) and \(e\) is the \eiw{0} Fox concludes \propI{\signConA{}} has value \valI{True} from \(\Phi\), and the \ros{} between \propI{What Pritcher said was a sign}, \valI{True} and \(\Psi\):
...
\begin{note}
  ...
  \begin{constraint}{consInclusion}{\issueInclusion{}}
    ...
    \item[\emph{Then}:]
      An \eiw{0} the agent concludes \(\psi\) has value \(v'\) from \(\Psi\) answers \qHow{}.
    \end{itenum}
    \vspace{-\baselineskip}
  \end{constraint}
  ...
  \footnote{
    Note, \qHow{} does not explicitly require the relevant event to be the \eiw{0} the agent concludes \(\phi\) has value \(v\) from \(\Phi\).
    Hence, a previous \eiw{0} the agent concludes \(\psi\) has value \(v\) from \(\Psi\) may answer \qHow{}.
...
\begin{note}
  ...
  For, it seems implausible that the \agents{} understanding of the quadratic formula favoured an \eiw{0} the agent concluded \propM{\rootsCon{}} has value \valI{True} from their understanding of factorisation over any other event, as the agent did not conclude by the quadratic formula.
...
\begin{note}
  ...
  \begin{enumerate}
  \item
    An \eiw{0} an agent concludes \(\phi\) has value \(v\) from \(\Phi\).
  \item
    ...
    \item
      \label{reciF:nHow}
      There is no prior (or present) \eiw{0} the agent concludes \(\psi\) has value \(v'\) from \(\Psi\).
    \end{enumerate}
  \end{enumerate}
  %
  ...
  \item
    \qWhy{} applies to \eiw{1} an agent concludes \(\phi\) has value \(v\) from \(\Phi\).

    Now, if \(\psi\) is the same as \(\phi\), \(v'\) is the same as \(v\), and \(\Psi\) is the same as \(\Phi\) then the \eiw{0} the agent concludes \(\phi\) has value \(v\) from \(\Phi\) is an \eiw{0} the agent concludes \(\psi\) has value \(v'\) from \(\Psi\).
    And, the \eiw{0} the agent concludes \(\phi\) has value \(v\) from \(\Phi\) answers \qHow{}.
    Hence, an \eiw{0} the agent concludes \(\psi\) has value \(v'\) from \(\Psi\) answers \qHow{}.
  \item
    Finally, \qHow{} is compatible with \scen{1} where some prior \eiw{0} the agent concludes \(\psi\) has value \(v'\) from \(\Psi\) answers \qHow{}.
    Whether or not there are \scen{1} with this feature is something we set aside.
    Still, a counterexample to \issueInclusion{} is not guarantee if there is some prior \eiw{0} the agent concludes \(\psi\) has value \(v'\) from \(\Psi\).
  \end{enumerate}
  \vspace{-\baselineskip}
  \end{motivation}
\end{note}
...
\begin{note}
  ...
  Of some interest, then, is whether \issueInclusion{} actually fails --- whether there really are events where a \ros{} between \(\psi\), \(v'\), and \(\Psi\) answers \qWhy{} without an \eiw{0} the agent concludes \(\psi\) has value \(v'\) from \(\Psi\).
...
\begin{note}
  ...
  A \ros{} answers \qWhy{} when the \ros{} explains why an \eiw{0} an agent concludes \(\phi\) has \val{0} \(v\) from \(\Phi\) happens, rather than some other event.
...
\begin{note}
  Consider an \eiw{0} an agent concludes some \prop{0} \(\phi\) has \val{} \(v\) from some \pool{} \(\Phi\).
  Further, suppose there is an \eiw{0} the agent is \emph{concluding} \(\phi\) has \val{} \(v\) from \(\Phi\).
  ...
  \item
    ...
    When the agent figures out \rootsConEqFac{}, the agent is concluding \propI{\rootsCon{}} has \val{0} \valI{True} from their understanding of factorisation.
    And, the eventual conclusion is a result of figuring out \rootsConEqFac{}.
    So, a \ros{} between \pv{\propI{\rootsCon{}}}{\valI{True}}, and a \pool{} which captures the \agents{} understanding of factorisation answers \qWhy{}, as the \agents{} eventual conclusion is favoured over any \eiw{0} the agent does not conclude \pv{\propI{\rootsCon{}}}{\valI{True}} from the relevant \pool{}.
  \end{itemize}
  %
  ...
  In this respect, the \ros{} between \(\psi\), \(v'\), and \(\Psi\) is part of what it is for an an \eiw{0} the agent concludes \(\phi\) has \val{0} \(v\) from \(\Phi\) to be favoured over some other event.
  %
  \begin{itemize}
  \item
    ...
    So, I doubt it is possible the agent is concluding \pv{\propI{\rootsCon{}}}{\valI{True}} from their understanding of factorisation \emph{unless} a \ros{} (also) holds between \pv{\propI{\rootsConEqExV{12}{4}{3}}}{\valI{True}} and a \pool{} which captures the \agents{} understanding of factorisation.
  \end{itemize}
  %
  ...
  And, as the \eiw{0} the agent is concluding \(\phi\) has \val{0} \(v\) from \(\Phi\) favours a conclusion of \(\phi\) having \val{0} \(v\) from \(\Phi\), and the conclusion is the result of the concluding, the \ros{} between \(\psi\), \(v'\), and \(\Psi\) answers \qWhy{}.
