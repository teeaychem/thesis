...
\begin{note}
  ...
  \illu{2} answers to each in turn:
\end{note}
...
\begin{note}
  ...
  In principle, relation between any \prop{0}, \val{0}, and \pool{} may answer \qWhy{}.
  ...
  \item
    ...
    The bird's conclusion of `Four legs good, two legs bad' has value `\(\text{True}\)' from the existence of Snowball's explanation answers \qHow{}, and the relation between the \prop{0}, \val{0}, and \pool{} answers \qWhy{}, in line with \issueInclusion{}.
...
\begin{note}
  ...
  With \qWhy{}, the relevant explanatory reason is a relation between a \prop{0}, \val{0}, and a \pool{}.
...
\begin{note}
  Rationalisation for \citeauthor{Davidson:1963aa} covers all actions.
  Conclusions are the result of a kind of action.%
  % \footnote{
  ...
  \footnote{
    ...
    \end{quote}
    However, our interest is with the \emph{relation} between a \prop{0}, \val{0}, and \pool{}.
  }
  ...
  So, it seems relations between \prop{1}, \val{1}, and \pool{1} may be understood as \citeauthor{Davidson:1963aa}ian rationalisations.
  Or, conversely, one may grant a relation between a \prop{0}, \val{0}, and a \pool{1} provides an explanatory reason, and understand the explanatory reason in terms of an agent's reasons by identifying the \pool{1} as the agent's reasons.%
  \footnote{
...
\begin{note}
  The main argument of made in \citetitle{Davidson:1963aa} is:
  %
  \begin{quote}
  ...
  \end{quote}
  %
  If we grant relations between a \prop{0}, \val{0}, and \pool{1} are rationalisations, and rationalisations are causal explanations, and \pool{1} capture an agent's reasons then \issueInclusion{} seems to follow, at least when considering the \agents{} reasons.
...
\begin{note}
  \color{red}
  Important things with \citeauthor{Davidson:1963aa}.
  \begin{enumerate}
  \item
    Reason
  \item
    Relation between reason and action.
  \item
    Assumption that reason explains the action by giving the agent's reason.
  \end{enumerate}

  So, restricted to giving the agent's reason.
  In the cases I construct, I don't clearly give the agent's reason.

  Liberties.
  \prop{0}-\val{0}-\pool{0} pairs are plausibly broader.
  And, it's not the case that \qWhy{} is restricted by the agent's reason.

  However, `\agpe{}'.
  Is it the case that similar argument holds?
  Suspect this leads to some tension with \citeauthor{Davidson:1963aa}'s account.
\end{note}


\begin{note}
  \citeauthor{Broome:2013aa}'s account of active reasoning explicitly links the same idea to conclusions:
  \begin{quote}
...
\begin{note}
  ...
  However, abstracting a little, the role of causation in \citeauthor{Davidson:1963aa} and \citeauthor{Broome:2013aa}'s account is of interest only to the extent that the relevant causal relations ensure the relation between a \prop{0}, \val{0}, and \pool{} which holds between the conclusion and the \pool{} is sufficient.
