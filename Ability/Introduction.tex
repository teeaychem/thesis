...
\begin{note}
  ...
  Likewise, in the case of \autoref{scen:countS} we considered a \ros{} between \propI{What Pritcher said was a sign}, \valI{True} and some \pool{} \(\Psi\) with respect to Fox and noted that if the \ros{} fails to hold, Fox does not conclude Pritcher is a Party member.
  In this respect, the \ros{} favours an event in which Fox concludes \propI{\signConA{}} has value \valI{True} from some \pool{} \(\Phi\), and Fox makes the relevant conclusion.
  ...
  \footnote{
    ...
    For, example, a \ros{} between \propI{What Pritcher said was a sign}, \valI{True} and some \pool{} \(\Psi\) plausibly fails to answer \qWhy{} applies to Fox's conclusion of \propI{What Pritcher said was a sign} has value \valI{True} from some \pool{}.
    For, it is implausible the relevant \ros{} holds prior to Fox's conclusion.
    Though, the \ros{} favours Fox's (overall) conclusion that \propI{\signConA{}} has value \valI{True} from \(\Phi\).
  }
\end{note}
...
\begin{note}
  ...
  When the agent figures out \((2x + 1)(x - 1) = 0\), the agent is concluding \propI{\rootsCon{}} has \val{0} \valI{True} from their understanding of factorisation, and the eventual conclusion is a result of figuring out \((2x + 1)(x - 1) = 0\).
  ...
  \end{itemize}
  %
  Now, it may be the case the agent is concluding \(\phi\) has \val{0} \(v\) from \(\Phi\) \emph{only if} a \ros{} between \(\psi\), \(v'\), and \(\Psi\) also holds.
  %
  \begin{itemize}
  \item
  ...
  \end{itemize}
  %
  So, a \ros{} between \(\psi\), \(v'\), and \(\Psi\) fails to hold, the agent is not concluding \(\phi\) has \val{0} \(v\) from \(\Phi\).
