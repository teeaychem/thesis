...
\begin{note}[The Pianist]
  ...
  Considering reasoning based on preferences \emph{at given time}.
  ...
  The pianist's reasoning is unwavering throughout the span of time in which the pianist has the option of drinking the wine; they should drink the wine in order to calm their nerves.
  So, if the pianist does not drink the wine, the pianist does acts in contrast to their own preferences.
\end{note}

\begin{note}[Reasoning and desire]
  Core conjecture:
  \begin{conjecture}\label{conj:resolve-issue-act}
    Any instance of rational action performed by an agent is the result of the agent resolving the issue of how to act.
    Where:
    \begin{enumerate}
    \item An act is an admissible resolution for how to act only if claimed support for preference for some proposition and expectation that act would bring about proposition, and
    \item An act is a resolution only if no other candidate resolution for which the agent has a stronger preference.
    \end{enumerate}
  \end{conjecture}

  The core of \autoref{conj:resolve-issue-act} is that rational action involves reasoning at various stages.
  First, preference for some proposition.
  Second, act would bring about proposition.
  Third, that no act would bring about a preferred proposition.
...
\begin{note}[Plan]
  So, understand the issue.
  We being by looking at \ESU{}.
  Then, turn to \EAS{}.
\end{note}

\begin{note}
  Suppose \ESU{} is true.
  Then, either action deviates from what the agent reasons to.
  Or, there is something else going on in the agent's reasoning.

  The first option is bad --- conflicts with \autoref{conj:resolve-issue-act}.
  The second option, on the other hand, has been explored.
\end{note}
...
\begin{note}[Intention and \ESU{}]
  ...
  However, because blocked, the pianist does not have the option of acting on that preference.
  Rather, act in a way that is compatible with intention.
  And, binary choice, so not drinking.\nolinebreak
  \footnote{
    Expand.
    Not a full glass, then the agent may drink a partial glass.
    Cascade down.
    Whole, most, half, fraction, none.
  ...
  For, the role of intention in the role of \citeauthor{Bratman:2007ab} account is to constrain possible resolutions for how to act.
  An intention to not drink renders drinking a glass of wine an inadmissible resolution for how to act ({\color{red} REFERENCE}).

  Further, given \ESU{}, it seems any similar theory must propose some way in which the agent's reasoning is constrained.
  For, given~\autoref{conj:resolve-issue-act} an agent acts by resolving the issue of how to act, which requires an agent to reason to a preference for some proposition and an expectation that the act would bring about that proposition.
  And, in the case of the pianist, the reasoning that the pianist performs establishes a preference for a glass of when presented with the option of drinking a glass of wine.
\end{note}

\begin{note}[Moving on from \ESU{}]
  I think constraints on possible resolutions for how to act are plausible, and that agent's do render certain resolutions for how to act inadmissible.

  {\color{red}
  \hozline{}
  }
  However, remains the case that if the pianist were to reason as in time before and after, the pianist would go for wine.
  For, \ref{conj:resolve-issue-act} and \ESU{}.
  From \ref{conj:resolve-issue-act} reasoning.
  And from \ESU{} must be the case that agent reasons to abstaining.\nolinebreak
  \footnote{
    Might suggest that this is due to giving priority to agent's preferences at time of reasoning.
    However, this doesn't follow.
    Haven't made this assumption.
    Compatible with scenario that agent discounts present preference for expected future preference and still reasons to wine.
    Though, nothing hangs on resisting this point.
  }

  In turn, block explanation because preference for abstaining.
  Even if pianist has preference, not in a position to reason to that preference.

  Suggestion to be offered is that such an explanation is possible given \EAS{}.

  Two conjectures.
\end{note}

\begin{note}[Two additional conjectures]
  First step.
  ...
  Still, the core idea is simple: Claimed support for some preference toward a proposition given more information is worth more than claimed support for some preference toward a proposition given less information because more information typically increases the reduces the likelihood that the claimed support is either mistaken or misled.\nolinebreak
  \footnote{
...
\begin{note}[Back to the pianist]
  ...
  But does the agent have the ability to reason?
  On the one hand, seems plausible that premises and steps of reasoning are there --- really does prefer, and these combine to abstaining.
  On the other hand, understandable for the agent to express that they lack the ability to whatever it is that they *really* prefer because of the nerves.
  Plausible that if the agent tried, then they'd fail --- that's the problem with nerves.
  Yet, also plausible to say that one has ability, but that witnessing is blocked by nerves.
  I have the ability to ... but you can't expect me to witness this when I'm under pressure.

  Key idea here is that two different blocks on ability.
  First, lack of resources.
  Say different preference, or conviction that they would not even make it to the stage.
  Second, interference.
  So, putting together steps, not discarding premises, etc.

  Former, no witnessing event because something is missing.
  Latter, no witnessing event because there's some interference with the resources.

  Whether or not this a compelling objection, and response is up to you.
  Need this claim that the agent has the ability, in the relevant sense.
  And, requires a complex relation to witnessing event.

  With the exception of this section, assume something more straightforward.
  Working through resources available.
  Counterpart objection is that those resources may be required for something else, or that the agent may be interrupted.
  But this is weaker than above.
  Possible event.
  Constraints may prevent witnessing, but not due to resources required for the event to take place.
  So, resources \emph{and} would be no interference.

  Further, need a defence of \EAS{} first.
  If there's no good argument for \EAS{}, then, in line with the above, reject such an approach in favour of any approach in line with \ESU{}.

  Thing is, we've used the term `ability', but all that \EAS{} really needs is premises and steps.

  Ability is difficult.

  Strays close to ideal adviser.
  But this isn't right.
  In such case, preference to act as ideal adviser would.
  Ad-hoc to require that adviser is only relevant if preferences align.
