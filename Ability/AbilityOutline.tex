...
\begin{note}[Redo of section]
  Seen \uRa{} and \WR{}.
  Turn to \AR{}.
  Introduce a constraint on claiming support.

  Provide account of constraint.
  Motivate.
  Show how relates to \AR{}.
\end{note}
...
\begin{note}[Inertia]
  ...
  \nI{} is about when an agent may claim support for some proposition \(\psi\) from some support claimed for some other proposition \(\phi\) --- an agent may not do so if the agent claims support for \(\psi\) for otherwise the actual value of \(\phi\) is different from the supported value of \(\phi\).
  ...
  if that's a Rolex, then S isn't wearing the watch they purchased last weekend.
  (Works because counterfeits looks the same, etc, and seems one would retract support --- so misleading.)
  Second example, a little less clear.
  If you're looking at a Nicolas Poussin, then the gallery hasn't purchased a fake.
  Less clear, but here, background of a gallery.

  Both of these, entailments.
  Add on.
  S has been given a raise.
  The gallery has a new director (as the old director refused to purchase paintings of the French Baroque style).

  Misleading support is not, in general, a problem.
  Roll a die, claim support for bias, misleading --- and clear that it may be misleading, but fine.
\end{note}

\begin{note}[For \nI{}]
  Still, as suggested by examples, the role of misleading support in premise~\nI{} does lead to a problem.

  Brief explanation, to be expanded on in chapter~\ref{cha:inertia}.
\end{note}
...
\begin{note}[Conditional B]
  ...
  Unlike \mcA{}, don't need premise stating agent needs support.
