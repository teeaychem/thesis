...
\begin{note}[Dependencies do not inherit support]
  ...
  \begin{proposition}[\nI{-}  --- \nI{}]\label{prem:ni}
    ...
    \item has received information that support claimed for \(\phi\) is misleading if \(\psi\) is not (also) the case.
    \end{enumerate}
...
\begin{note}[Quick explanation of \nI{}]
  ...
  \nI{} focuses on value of \(\psi\) from \(\phi\) not being misleading.
  Intuitively, the issue is that then as \(\xi\) requires \(\psi\), the agent does not have a way of getting to \(\xi\) only from support claimed for \(\phi\).
  ...
  Don't have an account where support is misleading.
  So, does not establish that from support agent has for \(\phi\), there's support for \(\xi\).
  Rather, so long as not misleading.
  Still, can't hold that may be misleading and that the reasoning to \(\xi\) works out.
\end{note}
