...
\begin{note}
  \begin{restatable}[Claiming support Nai Strong]{assumption}{assuExpectNaiStrong}\label{assu:expect-nai-strong}
    Reasoning that recognised defeaters do not obtain may not depend on claimed support which involves expectation that the defeater does not obtain.
  \end{restatable}
...
\begin{note}
  \begin{restatable}[Claiming support persists]{proposition}{propExpectNai}\label{assu:expect-nai}
    Conclusion of reasoning is not an instance of claiming support if require expectation from premises appealed to that \(\phi\) has value \(v\) in order to conclude that \(\phi\) has value \(v\) (i.e.\ no consideration of expectation).
  \end{restatable}

  {
    \color{red}
    This follows from requirement that there is some reasoning, alone.
    However,~\autoref{assu:expect-nai-strong} adds additional substance to this.
  }
  ...
  And,~\autoref{assu:expect-nai-strong} adds in that reasoning against when unknown is not enough.
\end{note}
...
\begin{note}
  Out goal is to show that \nI{} follows our understanding of claimed support --- that is, from the assumptions made regarding claiming support in~\autoref{sec:abil-access-supp}.

  In particular, the argument for \nI{} rests on two propositions which follow from the assumptions made regarding claimed support.
  ...
  \autoref{prop:info-to-expectation} is a sufficient condition for \(\psi\) having value \(v'\) to be an expectation for claimed support for \(\phi\).

  \autoref{assu:expect-nai} states that no claimed support if reasoning that concludes with \(\psi\) having value \(v'\) requires expectation that \(\psi\) has value \(v'\).

  {
    \color{red}
    Restate generation motivation for these propositions.
  }
\end{note}
...
\begin{note}[Plan for the section]
  \color{red}
  Outline of how the text will go.
\end{note}
...
\begin{note}[\ref{nI:claimed-support}]
  ...
  E.g., conditional as seen in above illustrations.
\end{note}

\paragraph{\ref{nI:going-by-value}}

\begin{note}
  Intuitively, All this does is ensure that the agent appeals to claimed support for \(\phi\).

  However, some subtlety.

  Perhaps appeal don't require \(\phi\) having \(v\).
  So, explicit.
