...
\begin{note}
  \begin{restatable}[\twodefeaters{-} --- \twodefeaters{}]{idea}{ideaDefeatersForCS}\label{prop:defs-for-CS}
...
\begin{note}[Quick intuition]
  The basic idea behind \autoref{prop:defs-for-CS} is that of \emph{undercuts} undercutting defeaters.
...
{
  \color{red}
  This is a definition of a possible defeater.
  That's all it is.
  Though, it's a little more precise.
  It's a possible defeater that's really involved in the reasoning.
}
...
\begin{note}
  \begin{restatable}[\requ{1} of claimed support]{definition}{defRequisite}\label{def:requisite}
    ...
    \begin{itemize}
    \item Claiming support includes some part with consequence of \(\psi\) having value \(v'\).
    \item If \(\psi\) were not to have value \(v'\) then some part of reasoning gets broken.
    \end{itemize}
  \end{restatable}
\end{note}
...
\begin{note}
  The subjunctive here does, generally speaking, highlight a possible defeater.
  However, there's nothing problematic in general about a possible defeater.

  For, recognise that only prove X if prove Y.
  Yet, no problem with going ahead and proving Y.

  \begin{illustration}
    Or, better, answer in textbook.
  ...
  \begin{illustration}
    ...
    Then, if appeal to former, the latter is a requisite.

    Need not be appealed to.
    Still, get a problem.
  \end{illustration}

  However, this does not prevent going through the reasoning to the answer.

  The general problem with the subjunctive alone is that while it does should that \(\lnot\psi\) would be enough to retract or revise the reasoning, it does not do enough to highlight that there may be a problem with the structure of the agent's reasoning as it stands.

  Applies to reasoning as a whole, but plausibly also any part.
  It is possible that \mom{}.
  However, there is no clear move from this to structural concern without strong assumptions.

  So, what this definition adds is that not only is this something that would be problematic.
  Further, the reasoning is such that the possibility needs to be excluded.



  Some part of the reasoning is such that it would be defeated if \(\psi\) were not to have value \(v'\).

  This alone is not necessarily a problem.
  For example, reasoning by cases.

  Here, different cases for different possible defeaters.
  



  The subjunctive here requires some care.

  We have fixed a particular way of reasoning.
  
\end{note}

\begin{note}
  The regular condition is interesting.
  It is only about what is the case.
  So, it does not follow that changes compatible with \(\phi\) are included here.
  This is important, as we are only interested in what is happening relative to this claimed support.
...
\begin{note}
  \begin{restatable}[]{proposition}{propReasondefRequisite}\label{prop:reason-requ}
    {
      \color{red} I don't think I need this.
    }
...
\begin{note}[\eiS{}]
  ...
  \begin{restatable}[Claiming support persists]{idea}{ideaEIS}\label{idea:eiS}
...
\begin{note}[Proposition]
    \begin{restatable}[\ESU{} and \adB{}]{idea}{propNoESUandADB}\label{mcA:WR-and-denied-claim}
...
\begin{note}
  ...
  \end{quote}
  \nI{} is a variation of this.
  In particular, less general as requires various clauses, but stronger as does not require assumption that the agent does not reason --- instead that there is no possible reasoning.

  Same motivating intuition.

  Agent needs to have indication of why even if \mom{}.
  The problem will be that not possible to conclude that \(\psi\) has value \(v'\) from a line of reasoning that entertains the possibility that \(\psi\) does not have value \(v'\).
...
\begin{note}[Key proposition]
  In particular, the argument for \nI{} rests on {\color{red} propositions} which follow from the assumptions made regarding claimed support.
  ...
  With this in mind.
  Claimed support such that compatible with possibility that \(\psi\) does not have value \(v'\).
  Hence, reasoning fails as instance of claiming support if fails under assumption that \(\psi\) does not have value \(v'\) then problem.

  {
    \color{red} Claiming support for \(\psi\) having value \(v'\) is a \requ{} of claimed support for \(\phi\) having value \(v\).

    Then, this gets expanded, to be such that \(\psi\) having value \(v'\) is a consequence of this.
  }

  Recognise possibility, but account of why even if that turns out to be the case.
  From~\autoref{assu:supp:independence} so reasoning that takes care of such a possibility.

  So, reasoning described is not compatible with such a possibility.
  Hence, as nothing else, failure of claiming support.
\end{note}
...
\begin{note}
  First, establish a major and a minor point:
  \begin{enumerate}
  \item If \(\psi\) not having value \(v'\) then no going to \(\phi\) from claimed support that \(\phi\) has value \(v\).
  \item If \(\phi\) then \(\psi\).
  \end{enumerate}
  Then, bring these together to get problem.
\end{note}
...
\begin{note}
  Don't yet have a clear issue.
  Sufficient, but possible contingency.
  It doesn't really matter that \(\psi\) having value \(v'\) follows from the move.
  In other words, have shown that \(\psi\) is there in the background, but have not yet shown that move to \(\phi\) is not compatible.

  In order to establish this, second conditional.
  This move is such that it requires ruling out possibility of \(\psi\) not having value \(v'\), roughly.
\end{note}

\begin{note}[B]
  Possible that \(\psi\) doesn't have value \(v'\).
  Then, possibility that claimed support for \(\psi\) having value \(v'\) is \misled{}.
  As a result, possibility that either \mom{} with respect to claimed support for \(\phi\) having value \(v\).
  However, move to \(\phi\) brings that this is not the case.
  So, would not move to \(\phi\).

  The point is that here we don't get \(\psi\) following, but rather stuff about claiming support for \(\psi\).
  {
    \color{red}
    This isn't quite right.
    The problem here is that need that \(\phi\) has value \(v\), and this doesn't hold up when \(\psi\) does not have value \(v'\).

    Although, maybe this is helpful.
    The problem is, the only thing that really matters is that the agent needs to move to \(\phi\) having value \(v\).
    And, it seems enough that \(\psi\) having value \(v'\) is a non-accidental consequence of this.
    But then the point here is that the above observation is in part an account of why \(\psi\) having value \(v'\) is a non-accidental consequence.
  }
\end{note}

\begin{note}
  So, not only is \(\psi\) bought along, but not going to get \(\phi\) without this being involved.

  May think only need B.
  I think this is mostly correct.
  There is a slight difficulty.
  Strictly speaking, B only gets perspective on claimed support for \(\psi\) having value \(v'\).
  A adds observation that given this, also get \(\psi\) has value \(v'\).
  {
    \color{red}
    This line of though is correct, but needs more care.
    For, the task is to show not only is the claimed support for \(\psi\) having value \(v'\) a \requ{}, but that it also leads to \(\psi\) having value \(v'\) as a \requ{}.

    I.e.\ it is correct to think that this all traces back to the interdependence between claiming support, but without observing these consequences, there's no clear issue.

    
  }

  So, correct that B is the most important part.
  But A is still an important observation.
\end{note}
