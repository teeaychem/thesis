...
\begin{note}[\nI{}]
  ...
  \begin{proposition}[\nI{-}  --- \nI{}]\label{prem:ni}
    ...
    \item\label{nI:inclusion} \(S\) is confident that the claimed support for \(\phi\) having value \(v\) is mistaken or misled if \(S\) is not (given present context) in a position to claim support that \(\psi\) has value \(v'\) (without appealing to \(\phi\) having value \(v\)).\nolinebreak
      ...
      \emph{And, if \(\phi\) is good then \(\psi\) would also be good.}
...
\begin{note}[Different from value dependence]
  Attention on claimed support.
  ...
  However, it's not immediate that these implications are clear to the agent.

  Put another way, the following does not hold:\nolinebreak
  \footnote{
    This is also a way in which the issue here are distinct from those concerning knowledge.
  }
  \[CS(\phi) \rightarrow CS(\psi)\]
  Hence,~\ref{nI:received-info} gets:
  \[CS(\phi \rightarrow \psi)\]
  And~\ref{nI:inclusion} gets:
  \[CS(\phi) \rightarrow CS(\psi)\]
\end{note}
...
\begin{note}
  \begin{itemize}
  \item \RBV{} for \(\phi\).
  \item So, assumption that the agent is in a position.
  \item Reasoning goes through, but assumption remains.
  \item 
  \end{itemize}
\end{note}
