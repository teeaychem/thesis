...
\begin{note}[Exhaustive]
  ...
  There remains a puzzle about what reference to potential events consists of.
  However, initial task is to establish that this is a puzzle that needs to be resolved.

  The exhaustivity comes from whether or not reference is (in part) determined by modal.
  If no modal, then need to refer to it being the case that there is potential.
  If in part, then we end up referring to the (potential) witnessing event.

  Example.
  Lewis' modal realism. (Counterpart theory because it's no possible for me to exist in some other world.)
  Reference to possible worlds.
  Contrast, essentialism.
  Here, I get to reference something that holds in this world.

  (There are papers on this that I could reference here.)
...
\begin{note}[\WR{} alternative]
  ...
  Hence, given \label{either-AR-or-WR}, if problem with \AR{} then \WR{} and not \ESU{}.
\end{note}
