...
\begin{note}
  ...
  Quick illustration in terms of \dd{} \dr{} distinction.
...
\begin{note}[Table]
  \begin{figure}[H]
    ...
    \begin{tblr}{abovesep=8pt, belowsep=8pt, width=0.95\textwidth, colspec={Q[c,m]|Q[c,m]|Q[1.8,c,m]|Q[1.8,c,m]}}
      ...
      & Derived & ?  & ? \\
    \end{tblr}
    \caption{Distinction matrix for interpretations of the \aben{}. \\ Rows are interpretations of ability, columns are type of reasoning regarding ability.}
  \end{figure}
\end{note}
...
\begin{note}[Focus]
  ...
  However, it's also the case that the `final' bit of reasoning is more or less the \aben{}.

  So, in a sense both ways of reasoning depend on these two things.

  The only difference is the particular for the \aben{} would take.
...
\begin{note}[Difference to reflection]
  ...
  So, it is not clear that there's a way to derive instances of the \aben{} from principles which motivate reflection.
\end{note}
...
\begin{note}
  ...
  For, if this is the case, then it's not clear that the agent needs to worry about the \aben{}.
...
\begin{note}
  ...
  This leaves us with two options for understanding the \aben{}.
\end{note}
