...
\subsubsection{Background for claiming support and reasoning}
...
\begin{note}[Understanding of claiming support]
  ...
  If claimed support, then agent considers that claimed support is not defeated.
  `Considered' is a placeholder.
  Strong or weak.
...
\begin{note}[Sufficient condition]
  ...
  \begin{proposition}[\bP{-} --- \bP{}]\label{prem:bP}
    ...
    (Given that the agent considers that the claims to support for premises and steps used are undefeated when drawing conclusion.)
  \end{proposition}
...
\begin{note}
  ...
  \begin{itemize}
  \item Claimed support may (discovered to be) be \emph{mislead} by suggesting that a proposition has some value that it does not (in fact) have. And,
...
\begin{note}[Subjectively sound]
  ...
  \begin{proposition}
    \textbf{(Claimed support is considered neither misleading nor mistaken)}
  ...
  However, not required that the claimed support is not (in fact) misleading nor mistaken, only that the agent \emph{considers} that the claimed support is not misleading or mistaken.
  ...
  As noted above, `considers' is a placeholder.
  A claim to support requires some resistance to possible defeaters, but how resistant the agent needs to be is up to you.
...
\begin{note}[???]
  ...
  However, agent considers that no defeaters, so may term this `\emph{subjectively} sound' reasoning.
...
\subsubsection{Interest with claiming support and reasoning}
