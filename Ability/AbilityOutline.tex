...
\begin{note}
  \begin{restatable}[Reasoning about expectations]{assumption}{assuCSNoExp}\label{assu:CS-nai}
    Reasoning.
...
\begin{note}
  ...
  \footnote{
    Perhaps Harman footnote here?
    Difference is that with Harman there counter-evidence.
  }
\end{note}
...
\begin{note}
  ...
  \item\label{Dretske:No-C:cond:no-k-then-ep} If \(S\) does not know that \(\phi\) is true, then \(\phi\) being false is an epistemic possibility for \(S\).
  \item\label{Dretske:No-C:cond:ep-then-no-k} If \(\phi\) being false is an epistemic possibility for \(S\), then \(S\) does not know that \(\phi\) is true.
  \end{enumerate}

  Term (\ref{Dretske:No-C:cond:no-k-then-ep}) and (\ref{Dretske:No-C:cond:ep-then-no-k})  `weak factivity'.\nolinebreak
  \footnote{
  ...
  The following is a reconstruction of (\citeyear[Cf.][1011]{Dretske:1970to}).\nolinebreak
  \footnote{
  ...
  \item\label{Dretske:No-C:ep-not-psi-and-phi} \(\psi\) being false while \(\phi\) is true is not an epistemic possibility for \(S\).%
  ...
  Reasoning leads to closure of knowledge (under known entailment), given (\ref{Dretske:No-C:cond:no-k-then-ep}) and (\ref{Dretske:No-C:cond:ep-then-no-k}).
  For, we have shown that --- given (\ref{Dretske:No-C:cond:no-k-then-ep}) and (\ref{Dretske:No-C:cond:ep-then-no-k}) --- (\ref{Dretske:No-C:not-k-phi}) follows from (\ref{Dretske:No-C:k-entail}) and (\ref{Dretske:No-C:dunno-psi}).
  Hence:
  \begin{itemize}
  \item If \(S\) knows that \(\phi\) entails \(\psi\) and \(S\) does not know that \(\psi\) is true, then \(S\) does not know that \(\phi\) is true.%
  ...
  \begin{itemize}
  \item If \(S\) knows that \(\phi\) entails \(\psi\), then if \(S\) knows that \(\phi\) is true then \(S\) knows that \(\psi\) is true.%
    \mbox{} \hfill \(K_{S}(\phi \rightarrow \psi) \rightarrow (K_{S}\phi \rightarrow K_{S}\psi)\)
  \end{itemize}
\end{note}

\begin{note}
  So, reject (\ref{Dretske:No-C:cond:no-k-then-ep}) or (\ref{Dretske:No-C:cond:ep-then-no-k}).
  \citeauthor{Dretske:1970to} rejects (\ref{Dretske:No-C:cond:ep-then-no-k}).
\end{note}

\begin{note}
  (\ref{Dretske:No-C:cond:ep-then-no-k}) parallels assumption regarding claiming support.

    \begin{enumerate}[label=K\Alph*\('\)., ref=(K\Alph*\('\))]
  \setcounter{enumi}{1}
  \item\label{Dretske:No-C:cond:ep-then-no-k-cont} If \(S\) knows that \(\phi\) is true, then \(\phi\) being false is not an epistemic possibility for \(S\).
  \end{enumerate}

  So, \citeauthor{Dretske:1970to} allows epistemic possibility, while claiming support requires agent reasons about such possibilities.

  Further, issue parallels reconstruction of \citeauthor{Dretske:1970to}.
\end{note}

\begin{note}
  So, the concern is that assumptions on claiming support lead to structurally similar problem of closure.
  Reject for knowledge, reject for claiming support.

  And, in particular, the parallel to (\ref{Dretske:No-C:cond:ep-then-no-k}).

  Of course, this is not a reduction.
  Yet, it is equally the case that rejection of closure for knowledge is not limited to a rejection of closure more generally.
  \citeauthor{Dretske:1970to}'s point is that closure does not hold for many operators.
  (E.g.\ explanation, mistake, etc.)

  So, have drawn out some understanding of how assumptions combine.
  However, also observation that is sufficient for pause.

  Not enough to note that claiming support is distinct from knowledge.
\end{note}

\begin{note}
  Recall assumption \autoref{assu:R-closure}:

  \assuRClosure*

  Here, denied a closure principle.
  However, end up with closure principle for \requ{}.

  \begin{itemize}
  \item If P is \requ{} and P implies Q, then \(Q\) is \requ{}.
  \end{itemize}

  Reject either \autoref{assu:R-closure} or this close and things are fine.
\end{note}

\begin{note}
  Indeed, \citeauthor{Dretske:1970to} goes with assumption, and doesn't really consider parallel to second.
\end{note}

\begin{note}
  \citeauthor{Dretske:1970to} closure and relevant alternatives.
...
\begin{note}
  Added by the closure principle is that possibility is alternative, and when claiming support reason about all alternatives.

  So, to extent that knowledge persists for \citeauthor{Dretske:1970to}, things are different when claiming support.
  This is not to deny weaker attitude.
  Indeed, consider stronger requirement that \citeauthor{Dretske:1970to} offers.
  Know only under those alternatives.\nolinebreak
  \footnote{
    Analogous to Harman in response to the dogmatism paradox.
  }
  So, something odd, at least.
  Intuition is that failure to claim support, while \citeauthor{Dretske:1970to}'s intuition (and perhaps your intuition also) is that the agent knows.

  \citeauthor{Dretske:1970to} rejects for knowledge, but we assume for claiming support.
  Yet, the difference is not so significant, as without factivity there's no clear lower bound on reasoning involved.
  It's only that we require reasoning.
\end{note}

\begin{note}
  Seems that the issue with closure is not simply closure, but closure when mixed with factivity, and rather than viewing \citeauthor{Dretske:1970to} as a rejection of closure, it is rather primarily a restriction on factivity, which implies restriction on closure as closure follows from factivity.

  Intuitively, seems that there is some reasoning that mule is an irrelevant alternative.
\end{note}
...
\begin{note}
  ...
  With factivity, required to rule out.
  However, claiming, seems enough.
  However, these don't seem to commit the agent.
\end{note}

\begin{note}
   `What if there are reports about the zoo authorities cleverly disguising animals to look like other animals?
