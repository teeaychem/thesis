...
\begin{note}[Summarising illustrations]
  ...
  Hence, with respect to ability, if an agent appeals to their ability to demonstrate that \(\phi\) is the case and that \(\phi\) must be the case in order for the agent to have the ability to demonstrate that \(\phi\)  when claiming support for \(\phi\), then the premises and steps involved in a full account of reasoning with these two key claims must be adequate to claim support that \(\phi\) is the case.
\end{note}
...
\begin{note}[Alternative]
  ...
  Key is that if~\EAS{} is true, then the agent is not required to use having the ability to reason as a premise.
...
\begin{note}[\EAS{} illustration]
  ...
  \item\label{EAS:ex:box:1} If I have ability to calculate the area of a box, then I have the ability to demonstrate that a box with dimensions \(19\text{cm}\) by \(7\text{cm}\) has area \(133\text{cm}^{2}\).
  \end{enumerate}
  ...
  Still, (I claim support that) I have the ability to calculate the area of a box, therefore:
  \begin{enumerate}[label=\arabic*., ref=(\arabic*), resume]
  \item\label{EAS:ex:box:2} I have the ability to demonstrate that a box with dimensions \(19\text{cm}\) by \(7\text{cm}\) has area \(133\text{cm}^{2}\).
  \end{enumerate}
  From~\ref{EAS:ex:box:2} it follows that:
  \begin{enumerate}[label=\arabic*., ref=(\arabic*), resume]
  \item\label{EAS:ex:box:3} A box with dimensions \(19\text{cm}\) by \(7\text{cm}\) has area \(133\text{cm}^{2}\).
  \end{enumerate}
  \EAS{} holds that, when I claim support for~\ref{EAS:ex:box:3} from~\ref{EAS:ex:box:2}, I may appeal to dimensions and formula, though as I do not witness the ability, I do not use the premise and step.
  For, if~\ref{EAS:ex:box:2} is the case then it is possible for me to witness reasoning in which I demonstrate that~\ref{EAS:ex:box:3} is the case, and it is the premises and steps of reasoning used in such reasoning that establishes~\ref{EAS:ex:box:3} is the case.
...
\begin{note}[More detail]
  ...
  The details matter, and we postpone detailing this argument to section~\ref{sec:broad-argum-overv}.
...
\begin{note}
  ...
  \begin{itemize}
  \item we will outline alternative reasoning patterns from~\ref{EAS:ex:box:1} to~\ref{EAS:ex:box:3}.
    Narrow focus.
    {
      \color{red}
  ...
  \item we will consider parallels between abilities and dispositions.
    The parallel will provide some additional intuition for why an agent may appeal to premises and steps that have no been used, and help clarify why we focus on ability.
  \item we will consider a related proposition argued from by \citeauthor{Moretti:2019wx}.
    The comparison will highlight the contrast between support and claimed support.
    {
      \color{red}
      In addition, the comparison will suggest an interpretation of \EAS{}.
    }
  \end{itemize}
  Finally, we close with a handful of broad remarks concerning ability.
\end{note}

\begin{note}[Alternatives]
  Alternatively, one may hold that appealing to having the ability noted in \ref{EAS:ex:box:2} is sufficient to claim support for \ref{EAS:ex:box:3}.
  But this is not the only strategy.
  For, though the syntactic form of \ref{EAS:ex:box:1} is a conditional, it does not necessarily follow that the semantic content of~\ref{EAS:ex:box:1} is a (also) conditional and~\ref{EAS:ex:box:1} may be interpreted to explicitly state that~\ref{EAS:ex:box:2} is an ability that an agent may have.
  For example:
  \begin{enumerate}[label=\arabic*., ref=(\arabic*), resume]
  \item\label{EAS:ex:box:4} The ability to demonstrate that a box with dimensions \(19\text{cm}\) by \(7\text{cm}\) has area \(133\text{cm}^{2}\) is an ability an agent may have and it is an ability an agent has if they have ability to calculate the area of a box.
  \end{enumerate}
  Hence, \ref{EAS:ex:box:1} is interpreted so that \ref{EAS:ex:box:2} is accessible without endorsing the antecedent of \ref{EAS:ex:box:1}.
  \ref{EAS:ex:box:4} states that there is some ability that it is possible for an agent to have, and in addition provides sufficient conditions for having the relevant ability.
  Further, the important part of \ref{EAS:ex:box:4} is the former conjunct:
  \begin{enumerate}[label=\arabic*., ref=(\arabic*), resume]
  \item\label{EAS:ex:box:5} The ability to demonstrate that a box with dimensions \(19\text{cm}\) by \(7\text{cm}\) has area \(133\text{cm}^{2}\) is an ability an agent may have.
  \end{enumerate}
  And, \ref{EAS:ex:box:3} follows from \ref{EAS:ex:box:5} by the observation that it is not possible to demonstrate that \(\phi\) if \(\phi\) is not the case --- there is no need for the agent to appeal to witnessing their ability.
  ...
  For, it then follows that you know that \ref{EAS:ex:box:2} is the case, and therefore you know that \ref{EAS:ex:box:3} is the case.
  Again there is no need for me to appeal to witnessing an ability.
\end{note}

\begin{note}[Box]
  The specific details of each alternative reasoning pattern are of little interest to us.
  Rather, the existence of alternative reasoning patterns is important.
...
\begin{note}[Counterexample]
  However, suppose that you don't know how to calculate the area of a box.
  Instead, you have looked through my notes, and have applied my formula for calculating the area of a box.
  We may now restate the semantic content of~\ref{EAS:ex:box:1} as follows:
  \begin{itemize}
  ...
  \end{itemize}
  The formula indicates that I have the ability to do something, but you offer no information as to whether my characterisation of the ability is correct or incorrect. Rewriting, I claim support that the general ability \(\gamma\) is the ability to calculate the area of a box, and in turn that that \(\varsigma\) is the specific ability to demonstrate that a box with dimensions \(19\text{cm}\) by \(7\text{cm}\) has area \(133\text{cm}^{2}\).\nolinebreak \footnote{ Consider in reverse. I am often attributed abilities that I deny I have. I do not have the ability to see without glasses, but I do have the ability to memorise the configuration of an optician's eye chart. } However, it is now not possible to draw the consequence that a box with dimensions \(19\text{cm}\) by \(7\text{cm}\) has area \(133\text{cm}^{2}\) from a source other than my specific ability. If \(\varsigma\) is some other ability, then~\ref{EAS:ex:box:3} does not follow, witnessing the relevant ability would not demonstrate that a box with
  ...
  What is of interest is that~\ref{EAS:ex:box:3} is obtained from~\ref{EAS:ex:box:2}.
  And, in turn, \emph{how}~\ref{EAS:ex:box:3} is obtained from~\ref{EAS:ex:box:2}.
\end{note}

\begin{note}[Point]
  Here, the key is that the agent reasons to and from.
  Scenarios may be a little narrow, but functionally these don't need to be a single argument.
  Hence, combination of fairly natural issues.
  Arguably, capture a core puzzle about ability.
  Don't need all instances for general, and in turn allows one to extend to specific instances that one hasn't used to claim general.
\end{note}

\begin{note}[Narrowed, now  \ESU{} and \EAS{}]
  Here, key point is that ability is a key premise.
  So, relevant interpretation.
  On from \ESU{} and another from \EAS{}.
\end{note}

\begin{note}[Moving from point]
  Question is about relevant interpretation of the specific ability.

  With this is hand, see \EAS{} and move to related conclusion to help clarify.
\end{note}
...
\begin{note}[Parallel]
  Above, argued that the agent is required to claim support for and reason from a (specific) ability.
  With respect to \EAS{}, it's the event of claiming support that's important.

  Parallel with dispositions.
  Focus on the role of an event.
  Parallel will be in terms of claiming support.
  Then, diverge with respect to reasoning from.
\end{note}
...
\begin{note}[Parallel between dispositions and ability]
  ...
  To illustrate, \citeauthor{Mandelkern:2017aa} trace the Conditional Analysis of ability  to \textcite{Hume:1748tp} and \cite{Moore:1912te}, among others:
  \begin{quote}
...
\begin{note}[\citeauthor{Moretti:2019wx}]
  \citeauthor{Moretti:2019wx} argue for a relation conclusion:
  A belief held by an agent may be based on premises that the agent did not use when forming the belief.
  The following is a fragment of the general principle relating propositional justification to well-grounded belief (alternatively doxastistcally justified belief) \citeauthor{Moretti:2019wx} argue for.
  ...
  \begin{quote}
    ...
    \item Q is propositionally justified for S in virtue of P1, P2, \(\dots\), Pn being justifiedly true from her perspective because S justifiedly believes P1, P2, \(\dots\), Pn, and in virtue of her being aware that Q is an inductive or deductive consequence of P1, P2, \(\dots\), Pn jointly, and
    \item S carries out a \emph{plain} inference from P1, P2, \(\dots\), Pn to Q.
    \end{enumerate*}
    ...
    (3) S's belief that Q is well-grounded.\nolinebreak
  ...
  An instance of plain (non-enthymematic) may take the intermediary step that \nagent{6} is taller than \nagent{5} before abstracting from \nagent{6}, while an instance of enthymematic reasoning consists of the (single) premise and conclusion noted.\nolinebreak
  \footnote{Cf.\ (\citeyear[87--89]{Moretti:2019wx}) for examples given by \citeauthor{Moretti:2019wx}.}

  Our interest with \citeauthor{Moretti:2019wx}'s proposal is that \ref{LT:2.3} provides an account of the basing relation involving the premises from \ref{LT:1.3}.
  If an agent reasons enthymematically, then the agent's belief may be based on those premises that the agent would use in the corresponding plain inference.
  (\citeyear[Cf.][86--87]{Moretti:2019wx})

  Here, then, we have a proposal on which an agent's belief is supported by premises and steps that an agent has not used.
  Still, in order to establish tension with \ESU{} we would need to show that the agent's belief is not only supported by unused premises and steps, but also that the agent may appeal to those unused premises and steps when coming to believe the conclusion of a relevant enthymematic inference.
  And, it is not clear to me that the required proposition is immediate.
  For, it is consistent with \citeauthor{Moretti:2019wx}'s proposal that the agent need only to appeal premises and steps of the enthymematic inference.
  ...
  Conversely, (in isolation) neither \ESU{} nor \EAS{} imply any constraints on the structure of propositional support.
\end{note}

\begin{note}
  By contrast, \EAS{} the agent is appealing to premises and steps that support the conclusion of reasoning.
  
\end{note}

\begin{note}
  Consider \citeauthor{Moretti:2019wx} proposal from the viewpoint of representationalist theories as characterised by \citeauthor{Neta:2019aa}.
  For an agent to form a conclusion based


  So, grants \emph{de re} awareness, maybe.
  In contrast, \EAS{} 


  Distinguish between the reason and how the reason is represented.
  Enthymematic representation may be considered sufficient.\nolinebreak
  \footnote{
    \color{red}
    There is also simple response which is to deny the combination of (1.3) and (2.3), and consider the enthymematic inferences are complete in the cases in which the agent forms a well founded belief.
    For example, reliabilist.
    Start out with complex reasoning, but over time compact.
    E.g.\ \(p \rightarrow q\) and \(q \rightarrow r\).
    \(p \rightarrow r\).
    This belief is formed as an immediate response to syntax given appropriate context (i.e.\ it does not hold when investigating non-classical logics).
    There are various ways in which this may be considered enthymematic, and will in part depend on the relevant proof system or application of a meta-theorem.
    However, from reliabilist viewpoint it's sufficient that the process is reliable.
  }
\end{note}

\begin{note}
  \color{red}
  Here, our argument against \ESU{} is different.
  We will argue that there are cases when reasoning with ability which involve an agent `directly' appealing to premises and steps.
  Hence, at issue is not whether relevant propositional support reduces to premises and steps.
  This may be true which appeal to having ability is sufficient to adequately represent.
  Rather, claiming support itself involves this.
\end{note}

\begin{note}[See difference, but principle is interesting]
  We have focused on the conclusion, and how this differs.
  Still, the argument \citeauthor{Moretti:2019wx} make is also of interest.
  Note in the definition, propositional justification.
  propositional justification is distinct from claimed support.
  Idea, then, is that ability in these cases is viewed in terms of a projection of the agent's propositional support.

  So, agent's ability is understood as guaranteeing proposition support for various things.
  Then, information provided is just a better description of that propositional support.

  So, this gives natural \ESU{} understanding.
  Contrasting \EAS{} understanding maintains that doxastic support is key.
\end{note}

\begin{note}[Segue]
  For now, we close the present section with a handful of remarks concerning ability and~\EAS{}.
\end{note}
...
\begin{note}[Actual support]
  ...
  Doesn't show that the agent has support.
  {
    ...
    Basically, this means that~\EAS{} isn't too strong.
  }
