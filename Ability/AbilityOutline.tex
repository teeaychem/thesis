...
\begin{note}
  \begin{enumerate}
  \item If you have the ability to reason with basic algebra and surds, then you have the ability to verify the following equation.
  \end{enumerate}

  \begin{figure}[H]
    \[\frac{(3 + \sqrt{3})^{2} + \sqrt{6}^{2} - (2\sqrt{3})^{2}}{2(3 + \sqrt{3})\sqrt{6}} = \frac{1}{\sqrt{2}}\]
    \caption{A mildly intimidating equation}
  \end{figure}
...
\begin{note}[Difficulty]
  ...
  A question remains as to whether you also satisfy property of having the ability to reason with basic algebra and surds, but on the de dicto reading of ability, this question --- like the question about the details of ability --- is only of derivative interest.

  Some reasoning with ability is like this.
  Plausible that it's the reasoning you performed.
  And, granting the de dicto reading, confidence that someone has the ability to reason with basic algebra and surds is likely stronger than confidence that oneself has the same ability.

  However, not all reasoning with ability may be captured by the de dicto reading of ability.
...
\begin{note}
  ...
  In contrast to de dicto, interest in how ability works.
...
\begin{note}[More examples, and narrowing]
  Paper is about de re reading of ability.
...
\begin{note}
  ...
  Rather, in \WR{} understanding, agent appeals to premises and steps of reasoning that they would access were they to witness ability.
...
\begin{note}[Structure]
  So, given conflict, takes some argument.
  Initial focus, `negative' argument.
  \emph{De re} from examples like those given above.
  \AR{} and \WR{}.
  Conflict between simple picture and \WR{}.
  Relatively strong argument in favour of \AR{}.
  Conflict between \AR{} and `simple' limitation of support.

  Either simple picture goes, or simple limitation goes.
  Reject simple picture.
  Ability may be seen as an exception to something which is true for the most part.

  Interest in what is gained.
  So, positive argument, focusing on novel insights from \WR{}.
\end{note}
