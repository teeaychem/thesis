...
\begin{note}[Example with a constant]
  Example.
...
\begin{note}[Put to use: nonstandard models]
  Example.
...
{
  \color{blue}
  Most important difference to similar restrictions is that I'm only denying a way of claiming support.
}
...
\begin{note}[\nI{}]
  ...
  \begin{proposition}[\nI{-}  --- \nI{}]\label{prem:ni}
    For any agent \(S\).
    Suppose:
    \begin{enumerate}[ref=(\textsf{ELC}:\arabic*), series=nI_counter]
    ...
    \end{enumerate}
    And, suppose \(S\) is confident that:
    \begin{enumerate}[ref=(\textsf{ELC}:\arabic*), resume*=nI_counter]
    \item\label{nI:inclusion}
      \begin{enumerate}[label=\alph*., ref=(\textsf{ELC}:3\alph*)]
      \item\label{nI:inclusion:position} Their claimed support for \(\phi\) having value \(v\) is mistaken or misled if \(S\) is not (given present context) in a position to claim support that \(\psi\) has value \(v'\) (without appealing to \(\phi\) having value \(v\)).\nolinebreak
      \footnote{
        Agent considers.
    ...
    \item\label{nI:inclusion:bound} That they would not be mistaken or misled when claiming support for \(\psi\) if their claimed support for \(\phi\) is not mistaken or misled. (when not going by value.)
      \end{enumerate}
      % (This is to help with the argument.)
      % [Also, it isn't quite a biconditional.]
      % (Also, also, this now motivates including the details on inclusion.)
    \end{enumerate}
    Then:
    \begin{enumerate}[ref=(\textsf{ELC}:\arabic*), resume*=nI_counter]
    \item\label{nI:going-by-value} \(S\) may not claim support for \(\psi\) having value \(v'\) by appeal (only) to:
      \begin{enumerate}[label=\alph*., ref=(\textsf{ELC}:4\alph*)]
...
\begin{note}[What \nI{} amounts to]
  ...
  Still, because expectation, agent can't use claimed support to claim support that \(\psi\) has value \(v'\).
...
\begin{note}[So named because\dots]
...
\begin{note}[Overview]
  ...
  Our primary goal here is to clarify each clause to the extent that the remaining task in our argument for \nI{} is to explain why~\ref{nI:going-by-value} follows from \ref{nI:claimed-support},~\ref{nI:received-info} and~\ref{nI:inclusion}.
  ...
  \indent\indent The first two clauses (\ref{nI:claimed-support} and~\ref{nI:received-info}) will be combined as these are relatively straightforward background conditions.
...
\begin{note}[Often, but not always]
  ...
  And, we will assume that there is no tension between the claimed support for \(\phi\) and the claimed support for if \(\phi\) then \(\psi\).
\end{note}
...
\begin{note}[Main idea]
  ...
  Overall role of inclusion is to identify a relation between the claimed support for \(\phi\) and claiming support for \(\psi\).
  Roughly, support claimed for \(\phi\) is only good if there's some way to claim support for \(\psi\) (that's not \RBV{}).
  In turn, lead to circularity when reasoning as described in~\ref{nI:going-by-value}.
\end{note}

\begin{note}[Two clauses]
  Two clauses:
  \ref{nI:inclusion:position} and~\ref{nI:inclusion:bound}.
  \ref{nI:inclusion:position} ensures that the agent is confident that it's possible to claim support.
  \ref{nI:inclusion:bound} ensures that the agent thinks that claiming support is bound.

  \ref{nI:inclusion} is here to ensure that claiming support for \(\psi\) is a possible defeater for \(\phi\).
  Something connecting claimed support for \(\phi\) and \(\psi\).
  Talk about this in some detail below.

  Seen examples above.
\end{note}

\begin{note}[\ref{nI:inclusion} and~\ref{nI:received-info}]
  \ref{nI:received-info} establishes connexion.

  Of course, already some connexion from~\ref{nI:inclusion}.

  Start by explaining why this is different.
\end{note}

\begin{note}[\ref{nI:inclusion} and~\ref{nI:received-info}]
  To illustrate, difference between~\ref{nI:inclusion} and~\ref{nI:received-info}.
  From~\ref{nI:received-info} get that \(\phi \rightarrow \psi\).
  So, restriction on value.
  This has implications for claimed support.
  In particular, if \(\phi \ne v'\) then \(\phi \ne v\).
  So, agent recognises that they may be misled.

  However, while this connects claimed support for \(\phi\) and claiming support for \(\psi\), the connexion is weak.
  \ref{nI:inclusion} strengthens.
  \ref{nI:inclusion:position} and~\ref{nI:inclusion:bound} go significantly beyond.
  First,~\ref{nI:inclusion:position} with way of claiming support.
  Second,~\ref{nI:inclusion:bound} with stronger from~\ref{nI:received-info}.

  So, we certainly interesting in something more than \(\phi \rightarrow \psi\) alone.

  It is the case that~\ref{nI:received-info} shows that something is bad if \(\psi\) doesn't have value.
  However, possible that agent doesn't think \(\phi\) relates to \(\psi\).
  `Mere' consequence, so to speak.
\end{note}
...
\begin{note}[Key role of \ref{nI:inclusion:position}]
  Most of the issue are going to arise from \ref{nI:inclusion:bound}.
  However,\ref{nI:inclusion:position} is important as \ref{nI:inclusion:bound} is a counterfactual.
  It only states how good support would be in the agent claimed support.
  So, it's quite possible that the agent has claimed support for \(\phi\), yet, there's really no worry about the value of \(\psi\) because the claimed support for \(\phi\) would need to be enhanced in some way in order to also claim support for \(\psi\).

  To illustrate, starting out with mathematics.
  Claiming support for some sum.
  Well, fine.
  Now, learn the relationship between multiplication and addition.
  Here, things are more complex.
  Recognise a problem with multiplication if addition doesn't go through.

  So, because agent is in a position, the relationship is as needed.
\end{note}
...
\begin{note}
  \color{red} Two things about \RBV{}.

  \begin{itemize}
  \item Requires expectations to hold.
  \item Is more or less \ur{}, so doesn't allow the agent to claim support from some other `basis'
  \end{itemize}
\end{note}

\begin{note}[Claimed support by \RBV{}]
  This is a very important point.
  Claiming support by \RBV{} ties support for \(\psi\) to support for \(\phi\), primarily.
  This is because the conditional doesn't provide any useful information.

  I've used testimony a lot, but it's useful again here.
  Anything got via testimony is only as good as one's trust in the testifier with respect to the subject.

  If someone lies, things go bad.
  If a book contains various errors, things go bad.
...
\begin{note}[Different idea of claiming support in comparison to \citeauthor{Wright:2011wn}]
