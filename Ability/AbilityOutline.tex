...
\begin{note}
  ...
  Here, conditional seems to be true.
  ...
  Here, some background is need, as the impossibility result may rely on some other branch of mathematics.
  ...
  There are some nearby conditions that aren't required here.
  For example, regardless of the agent's confidence.
  Or, expansion of resources, in a kind of coherenist style.
  Or, requiring that the agent uses the resources.
  Ignore the first as interested in recognises problem.
  The second two are stronger, and do vary with views about response to potential defeaters.
  Here, don't need the strength.

  For our purposes, cases where \ref{nI:inclusion} does not (necessarily) raise issue with claimed support for \(\phi\).
  Looking forward, issue will be with a way of claiming support for \(\psi\) given \ref{nI:claimed-support} and~\ref{nI:received-info}.
\end{note}
