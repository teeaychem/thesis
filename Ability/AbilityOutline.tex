...
\begin{note}
  \begin{restatable}[Claiming support Nai Strong]{assumption}{assuDoNotRequireProp}\label{assu:expect-nai-strong}
  ...
  \begin{proof}
    This assumption expands on \eiS{} in a fairly minimal way.
...
\begin{note}[Expanding on~\ref{nI:inclusion}]
  ...
  \autoref{assu:expect-nai-strong} with respect to second point.
\end{note}
...
\begin{note}
  ...
  \end{enumerate}
  Hence, \ref{assu:expect-nai-strong} applies.
  More broadly, \eiS{}.
...
\begin{note}
  ...
  Recall, add some strength to this.

  Not possible to discharge expectation by appealing to claimed support.
...
\paragraph{\ref{nI:claimed-support}, \ref{nI:psi-is-new}, and~\ref{nI:going-by-value}}

\subparagraph{\ref{nI:claimed-support} and~\ref{nI:psi-is-new}}
...
\begin{note}
  ...
  Rather, our goal is to show that not possible to move to \(\phi\) without also \(\psi\).
  The primary result from \ref{nI:inclusion} is that claiming support for \(\psi\) having value \(v'\) is an expectation of the claimed support for \(\phi\) having value \(v\).
  In turn, this interacts with move to \(\phi\) having value \(v\).
  ...
  Seems more basic and stronger.
  Problem is possibility that \(\psi\) is not the case.
  But, only claim support given \(\psi\).
  Hence, not claiming support.

  This relies on \(\phi\).
  Above suggestion would not necessarily do so.
  Hence, above seems preferable, but requires more than assumptions offer, and it also seems that motivating \nI{} without such an argument would be informative for developing such an argument.
\end{note}
