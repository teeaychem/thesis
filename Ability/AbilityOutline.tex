...
\begin{note}[Bootstrapping]
  ...
  \begin{quote}
    \begin{illustration}\label{ill:gas-gauge}
      \emph{The Gas Gauge}. The gas gauge in Roxanne’s car is reliable, though she has no evidence about its reliability.
...
\begin{note}[No feedback]
  \begin{quote}\phantlabel{wnf}
    ...
    \item\label{W:NF:ii} \(C\) is inferred from \(L_{1}-L_{n}\) (and possibly some of \(P_{1}-P_{m}\)) by an argument whose justificatory power depends on making \(C\) at least \(x\) probable,\nolinebreak
      \footnote{
        There is a possible ambiguity here.
...
\begin{note}
  To begin, let us identify the relevant premises and lemmas, and conclusion of~\autoref{ill:gas-gauge}.

  \begin{enumerate}
  \item The gauge is reliable. (assumption)
  \item The gauge reads full.
  \item So, the tank is full.
  \item Hence, the gauge is functioning correctly on this occasion.
  \end{enumerate}

  Have assumption because in the original, Roxanne skips over this as an explicit step.
  ...
  So, premise, full, lemma, tank is full, preliminary conclusion, the gauge is functioning correctly on this occasion.
  Combine sufficient occasions for conclusion that the gauge is reliable.
  This is~\ref{W:NF:i}

  Problem identified by \wnf{}:
  Needs to be the case that the gauge is reliable --- this is a premise.
  And, without this premise, argument for lemma breaks down.
  Hence, we get~\ref{W:NF:ii}.

  And, don't get to check the whether the gauge is reading correctly without the status of the tank.
  ...
  \begin{itemize}
  \item Lemmas, that the tank is full, drawn from the trust that the gauge is full.
  \item Reading full doesn't (alone) make reliability without the lemma that the gauge is reliable.
  \item C is , drawn from the lemma and proposition.
  \end{itemize}

  So, need gauge to be reliable to move from the state of the tank to the reliability of the gauge.
\end{note}

\begin{note}[Probability]
  But this is fine.
  Probability is formal, so this places constraints on some way of evaluating the relevant propositions.
\end{note}

\begin{note}
  Parallel in terms of undercutting defeater.

  Quick parallel, very similar to counterexample problem.
...
\begin{note}
  Here, C is \(\psi\).
  \(L_{1}-L_{n}\) are \(\phi\) and the implication from \(\phi\) to \(\psi\) (in contrast to the previous discussion of \citeauthor{Wright:2011wn}'s templates, we don't need to assume an entailment).

  Require C to be at least x probable.

  Here's where there's some connexion.
  Observation is that \nI{} holds in part due to requiring that \(\psi\) holds in order to claim support for \(\psi\), leading to a violation of \eiS{}.

  Snag is that we don't get candidates for \(P_{1}-P_{m}\) from \nI{}.

  Still, this is okay.
  Consider case of claimed inductive support.
  Not going to go to no counterexamples without theory, so it looks as though the last condition is alright too.
\end{note}

\begin{note}[Extend to \citeauthor{Wright:2011wn}]
  For sure, same quick response to \citeauthor{Wright:2011wn}.
...
\begin{note}[Difficulty]
  The difficulty is with the argument.

  The kind of reasoning highlighted by~\ref{nI:going-by-value} doesn't rely on making \(\psi\) at least \(x\) probable.

  Require \(\psi\) to be the case when \RBV{}.
  However, \RBV{} does not require \(\psi\) to be the case independent from the move to the value of \(\psi\).

  Here, recall \autoref{ill:CE:testimony}.
  Don't require that the theory is adequate in order to move from testimony to adequacy of theory.

  Indeed, this is why we haven't framed \nI{} in terms of circularity.
  The problem identified by \nI{} has nothing to do with claimed support for \(\psi\) prior to proposed reasoning.
  Rather, it's because the result of such reasoning violates \eiS{}.
\end{note}
