...
\begin{note}[Causal theories of basing]
  Indeed,~\ESU{} seems to be implied by causal theories of basing.
...
\begin{note}[\ESU{} does not imply CTB]
  Important, however, is that \ESU{} does not imply any causal theory of basing.
  For, \ESU{} does not imply any theory of basing.
  Rather, \ESU{} is about when an agent may claim support, and does not touch on how the support is based.
\end{note}
...
\begin{note}[Representationalism]
  ...
  \end{quote}
  {\color{red}
    \citeauthor{Moser:1989tv} may be seen as an instance of this, in which the causal relation establishes why the representation is part of the reason why the agent acts (forms a belief).
  }
  ...
  Still, the relation between representationalist accounts and~\ESU{} may be more visible.
...
% \begin{note}[Where \ESU{} is okay]
%   I think~\ESU{} and~\gESU{} are fine from an aetiological perspective --- account for how or why it came to be that the agent claimed support --- but question whether this restricts what the agent appeals to.
% \end{note}
...
\begin{note}[Alternative]
  ...
  The alternative, in line with~\ESU{}, would be for the agent to use the proposition that they have the ability.

  To illustrate, suppose you inform \emph{S} that:
  \begin{enumerate}[label=\arabic*., ref=(\arabic*)]
  \item\label{AES:ex:box:1} If I have ability to calculate the area of a box, then I have the ability to demonstrate that a box with dimensions \(19\text{cm}\) by \(7\text{cm}\) has area \(133\text{cm}^{2}\).
  \end{enumerate}
  I claim support that I have the ability to calculate the area of a box, therefore:
  \begin{enumerate}[label=\arabic*., ref=(\arabic*), resume]
  \item\label{AES:ex:box:2} I have the ability to demonstrate that a box with dimensions \(19\text{cm}\) by \(7\text{cm}\) has area \(133\text{cm}^{2}\).
  \end{enumerate}
  From XXX it follows that:
  \begin{enumerate}[label=\arabic*., ref=(\arabic*), resume]
  \item\label{AES:ex:box:3} A box with dimensions \(19\text{cm}\) by \(7\text{cm}\) has area \(133\text{cm}^{2}\).
  \end{enumerate}

  If you had informed me of~\ref{AES:ex:box:2} then I may have reasoned that \dots
  Rewrite~\ref{AES:ex:box:1} to explicitly state that~\ref{AES:ex:box:2} is an ability that an agent may have.
  For example:
  \begin{enumerate}
  \item The ability to demonstrate that a box with dimensions \(19\text{cm}\) by \(7\text{cm}\) has area \(133\text{cm}^{2}\) is an ability I have if I have ability to calculate the area of a box.
  \end{enumerate}
  So, If I know that you know that I have the ability to calculate the area of a box, then there is no doubt that~\ref{AES:ex:box:2} is an ability that an agent may have, and therefore~\ref{AES:ex:box:3} holds because it would not be possible for an agent to have such an ability if~\ref{AES:ex:box:3} were not the case.
  In short, there is nothing for the conditional to do.
  ...
  \end{itemize}
  The formula indicates that I have the ability to do something, but you offer no information as to whether my characterisation of the ability is correct or incorrect.
  Rewriting, I claim support that the general ability \(\gamma\) is the ability to calculate the area of a box, and in turn that that \(\varsigma\) is the specific ability to demonstrate that a box with dimensions \(19\text{cm}\) by \(7\text{cm}\) has area \(133\text{cm}^{2}\).\nolinebreak
  \footnote{
    Consider in reverse.
    I am often attributed abilities that I deny I have.
    I do not have the ability to see without glasses, but I do have the ability to memorise the configuration of an optician's eye chart.
  }
  However, it is now not possible to draw the consequence that a box with dimensions \(19\text{cm}\) by \(7\text{cm}\) has area \(133\text{cm}^{2}\) from a source other than my specific ability.
  If \(\varsigma\) is some other ability, then~\ref{AES:ex:box:3} does not follow, witnessing the relevant ability would not demonstrate that a box with dimensions \(19\text{cm}\) by \(7\text{cm}\) has area \(133\text{cm}^{2}\), and so a box with dimensions \(19\text{cm}\) by \(7\text{cm}\) may (from my epistemic perspective) have some other area.

  What is of interest is that~\ref{AES:ex:box:3} is obtained from~\ref{AES:ex:box:2}.
  And, in turn, \emph{how}~\ref{AES:ex:box:3} is obtained from~\ref{AES:ex:box:2}.

  \EAS{} holds that, when I claim support for~\ref{AES:ex:box:3} from~\ref{AES:ex:box:2}, I may appeal to dimensions and formula, though as I do not witness the ability, I do not use the premise and step.
  For, if~\ref{AES:ex:box:2} is the case then it is possible for me to witness reasoning in which I demonstrate that~\ref{AES:ex:box:3} is the case, and it is the premises and steps of reasoning used in such reasoning that establishes~\ref{AES:ex:box:3} is the case.
...
\begin{note}[Parallel]
  A parallel may help illustrate both the idea behind \EAS{} and why \EAS{} is of some interest.

  Consider dispositions.
\end{note}

\begin{note}[Simple truth conditions]
  Start with simple truth conditions.
  ...
  \footnote{
    Footnote to some examples, maybe SEP.
  }

  Key parallel here is that something needs to happen.
  So, like ability, there is something about the object which means that there is a possible event in which it\dots
  We don't care too much about what ability is, but note that one gets a quick account of ability to adding in volition.
  \emph{Wow, this is exactly what happens in some of the analyses}.
  There's also morphology: solu\emph{able}.
\end{note}

\begin{note}[Claiming support]
  Truth conditions provide us with a way to claim support.

  Past events as a guide.
  I've dropped these glasses before, there's nothing distinct, so this event will happen.

  Project to a possible event, but this is nothing too interesting.
  Use some law, and so I have claimed support for disposition.

  Weaken this a little by taking some novel object with sufficient features in common.

  Now take a novel act and a object pair.
  Something you don't have much experience with.
  Personally, I have\dots

  Empty fountain pen, submerged in water.
  Claim support that it's going to float.
  Because, store for ink, empty so air.
  Cap on, so air trapped.
  Both, close to airtight, else run the risk of spilt in.
  Finally, wasn't so expensive, so no heavy components.
  Therefore, when placed in water, the air won't be replaced with water.
  ...
  I suppose it is the case that C, and see if `it' Ms.

\end{note}

\begin{note}
  Ability may be thought of in the same way.
  I have the ability to demonstrate that the pen will float when placed in water.
  Truth conditions involve an event.

  Parallel reasoning.

  However, this is where things get difficult.
  For, it doesn't follow that the fountain pen is floating in water, while it does follow that the pen will float in water.

  {
    \color{red}
    This seems a nice example.
    It's clear that there's a difference in the strength of statements.
    And, that I haven't demonstrated that the pen will float in water.
    However, it also seems plausible that I have the ability to demonstrate.
    Indeed, what would be involved in witnessing the ability is sketched out by the reasoning is used to claim support for the disposition.
    Still, it falls short of a demonstration.
  }
\end{note}
...
\begin{note}
  ...
  The primary argument for \EAS{} is that \ESU{}, certain instances of reasoning with information about ability, and a principle concerning when an agent is permitted to claim support are in tension.
...
\begin{note}[Parallel to dispositions]
  \color{red}
  Parallel to dispositions.
  Conditional analysis.
  This gives truth conditions, but figure these out independently.
  Issue is whether when reasoning about dispositions one may appeal to the disposition, or whether one considers conditional expansion.
  Glass is fragile, so it will break if dropped.
  Here, one may do a quick mental simulation, following \ESU{}, the sufficient elements of the event are `used'.
  Or, appeal to disposition itself.
  {
    \color{green}
    This seems to be an really helpful analogy.
  }
\end{note}
...
\begin{note}
  ...
  Enthymematic representation is sufficient.\nolinebreak
  \footnote{
    \color{red}
...
\begin{note}[Ability]
  ...
  If~\EAS{} then a novel perspective on limited agents.
  When and why limited agent may claim support for result of reasoning.
  Here, conflict with appeal to~\ESU{} it its unrestricted form.
  ...
  Even if arguments for~\EAS{} fail to convince, not much has been said on ability to reason, and first steps are useful.

  Third, use of~\ESU{}.
  Potential alternative conclusions to arguments that appeal to~\ESU{} as a premise.
  Revise premises for arguments in which~\ESU{} is a conclusion.
