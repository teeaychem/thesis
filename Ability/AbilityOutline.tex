...
\begin{note}
  \begin{proposition}[Two defeaters for claimed support]\label{prop:defs-for-CS}
...
\begin{note}[\eiS{}]
  ...
  \begin{proposition}[\eiS{-} --- \eiS{}]\label{prop:supp:independence}
    Claimed support for \(\phi\) having value \(v\) indicates that \(\phi\) has value \(v\)
    % independently of the value of \(\phi\) --- or
    regardless of whether the claimed support for \(\phi\) having value \(v\) is \mom{}.
\nolinebreak
    \footnote{
  ...
  \eiS{} follows from Propositions~\ref{prop:CSNMORM} and~\nfcs{}.
...
\begin{note}
  So, we assume that \ref{nI:claimed-support},~\ref{nI:received-info}, and~\ref{nI:inclusion} all obtain.

  Now,~\ref{nI:going-by-value}.

  Agent may not claim support for \(\psi\) having value \(v'\) by appeal (only) to:
  \begin{quote}
    \begin{enumerate}[label=\named{\nItext{}4:\alph*}]
...
\begin{note}
  \begin{definition}[Dependencies of claimed support]\label{def:CS-depends}
    Claimed support for \(\phi\) having value \(v\) \emph{depends} on \(\psi\) having value \(v'\) just in case the agent would consider the claimed support for \(\phi\) having value \(v\) to be either \mom{} if \(\psi\) does not have value \(v'\).
  \end{definition}

  There's nothing too special about a dependency.
  In particular, there is no requirement that an agent has claimed support for any dependency.
  ...
  Again, a dependency isn't anything special.
...
\begin{note}
  We begin by assuming that~\ref{nI:claimed-support},~\ref{nI:received-info}, and~\ref{nI:inclusion} all obtain.
\end{note}

\begin{note}
  \ref{nI:claimed-support} and~\ref{nI:received-info} are straightforward.

  Restated:
  \begin{quote}
    \begin{enumerate}[label=\named{\nItext{}:\arabic*}]
    \item \(S\) has claimed support that some proposition \(\phi\) has value \(v\).
    \end{enumerate}
  \end{quote}
  And:
  \begin{quote}
    \begin{enumerate}[label=\named{\nItext{}:\arabic*}]
      \setcounter{enumi}{1}
    \item \(S\) has claimed support that if proposition \(\phi\) has value \(v\) then proposition \(\psi\) has value \(v'\).
    \end{enumerate}
  \end{quote}
  Neither~\ref{nI:claimed-support} nor~\ref{nI:received-info} are particularly important for the details of the argument.
  Rather, the two clauses provide background for the reasoning of \ref{nI:going-by-value}.
\end{note}
