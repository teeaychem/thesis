...
\begin{note}
  \begin{definition}[\AR{}]\label{A:s}\mbox{ }
    An \aben{} (\emph{S} has the ability to \emph{V} that \(\phi\) entails that \(\phi\) is the case) holds because:
    \begin{enumerate}[label=\textsf{A}\arabic*., ref=(\textsf{A}\arabic*)]
    \item\label{A:s:1} \emph{S} has the ability to \emph{V} that \(\phi\) is or reduces to some (potentially complex) property \emph{P} of \emph{S}
    \item\label{A:s:2} \emph{P} entails that \(\phi\) is the case, and
    \item\label{A:s:3} \emph{P} is not analysed in terms of events.
    \end{enumerate}
  \end{definition}

  \AR{} holds that having the ability to \emph{V} that \(\phi\) is a property of an agent.
  ...
  However,~\ref{A:s:2} and~\ref{A:s:3} constrain the property of interest.
  ...
  From a different perspective,~\ref{A:s:3} ensures that the property of interest is akin to having a heart, possessing ¥500, being of a certain age, and so on\dots

to illustrate focus on the idea of reduction.
  ...
  While these suggestions reduce ability to complex properties, \AR{} also admits of the possibility that the ability to \emph{V} that \(\phi\) is a basic property which does not admit of further analysis.
  If so, then it seems that the \aben{} must also be taken as basic.
  I lack any suggestion for how to understand \AR{} if the property is indeed basic, but there is no need to rule out this option.\nolinebreak
  \footnote{
...
\begin{note}[\WR{} def.]
  \begin{definition}[\WR{}]
    An \aben{} (\emph{S} has the ability to \emph{V} that \(\phi\) entails that \(\phi\) is the case) holds because:
    \begin{enumerate}
    \item \emph{S} has the ability to \emph{V} that \(\phi\) \emph{if and only if} there is a potential event in which \emph{S} witnesses the act of \emph{V}ing that \(\phi\), and
    \item \(\phi\) must be the case in order for there to be a potential event in which \emph{S} witnesses the act of \emph{V}ing that \(\phi\).
    \end{enumerate}
  \end{definition}
...
\subsubsection{Incompatibility of \ESU{}, \gsi{}, and \WR{}}
