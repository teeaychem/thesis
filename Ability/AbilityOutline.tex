...
\begin{note}[Reasoning and desire]
  ...
  \begin{conjecture}\label{conj:resolve-issue-act}
    ...
    Where:
    \begin{enumerate}
    \item An act is possible resolution for how to act only if preference for some proposition and expectation that act would bring about proposition, and
...
\begin{note}[Intention and \ESU{}]
  ...
  Key here is that intention does not interact with the pianists preferences.
  ...
  So, \citeauthor{Bratman:2007ab} is a nice example of how to resolve weakness of will given relation between reasoning and action and \ESU{}.
  I think, in broad strokes, phenomena fit this kind of theory.
  However, not clear to me that all phenomena fit such a theory.
  Need something that limits the agent's reasoning.
  This is the role of \citeauthor{Bratman:2007ab}'s intentions.
  And, any similar theory working with constraints needs to find some way to limit.

  For, given~\autoref{conj:resolve-issue-act} preference.
  At time of resolving how to act, no greater preference.
  Still, agent could discount preference for wine or increase preference for abstaining.
  Or, resolve in advance an refuse to re-resolve.
\end{note}
...
\begin{note}[Moving on from \ESU{}]
  ...
  Wrong limitation to focus on in some cases.
  Issue is limitation that nerves impose on agent's reasoning.

  Two conjectures, then interest in \EAS{}.
\end{note}
...
\begin{note}[Two additional conjectures]
  ...
  \begin{conjecture}\label{conj:pref-vs-reasoning}
    Whether, or to what degree, an agent has a preference for some proposition may differ from whether, or to what degree, the agent claims support that they have a preference for the proposition.
  \end{conjecture}

  Simple case to illustrate with lack of information.
  ...
  Also with limited processing.
  Prefer one television over another.
  However, lighting in the store.
  If tested both in identical conditions, preference would be reversed.

  True, narrow proposition.
  However, point is purchasing, and this information is used to claim support for preference.
\end{note}

\begin{note}[Final conjecture]
  Final conjecture, this time about the pianist.

  \begin{conjecture}\label{conj:true-pref-pianist}
    That what the pianist desires does not change.
  \end{conjecture}

  \ref{conj:true-pref-pianist} applies \ref{conj:pref-vs-reasoning} to specific scenario.
  If \ref{conj:pref-vs-reasoning} then pianist's reasoning doesn't necessarily.
  Hence, What the pianist desires remains constant.
  It is only what the pianist reasons to that changes.
  ...
  Thing is, we've used the term `ability', but all that \AES{} really needs is premises and steps.
