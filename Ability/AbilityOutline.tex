...
\begin{note}
  \begin{definition}[\AR{}]\label{A:s}\mbox{ }
    ...
    \item\label{A:s:2} \emph{P} entails that \(\phi\) is the case (intuitively, because the agent could not have \emph{P} without \(\phi\) already being the case).
    \end{enumerate}
  \end{definition}

  {
    \color{red}
    \AR{} holds that an agent applies the \aben{} by holding the ability to \emph{V} that \(\phi\) is a property of an agent.\nolinebreak
    \footnote{
