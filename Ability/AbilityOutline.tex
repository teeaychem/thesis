...
\begin{note}[\nI{}]
  ...
  \begin{proposition}[\nI{-}  --- \nI{}]\label{prem:ni}
    ...
    \item\label{nI:inclusion} \(S\) is confident that the claimed support for \(\phi\) having value \(v\) is mistaken or misled if \(S\) is not (given relevant context) in a position to claim support that \(\psi\) has value \(v'\) without appealing to \(\phi\) having value \(v\).\nolinebreak
      \footnote{
        Agent considers.
...
\begin{note}
  ...
  {
    \color{red}
    The gist is that \nI{} captures an intuitive constraint that agent is not in a position to claim support for some proposition \(\psi\) by information that \(\phi\) entails \(\psi\) if failure to establish support for \(\psi\) independently of the value of \(\phi\) would reveal problem with the support claim for \(\phi\).
  }
...
\begin{note}
  ...
  Saw this with Wally.
  If proposition that the individual I identified is Wally is true, then follows that the proposition that the individual is wearing round glasses is also true.
  Alternatively, from the same antecedent it follows that the proposition that the individual is wearing square glasses is false.
...
\begin{note}
  From \ref{nI:received-info} we have \(\lnot\psi\) as a potential defeater for the claimed support for \(\phi\), as \(\phi\) is not the case if \(\psi\) is not the case.
  \ref{nI:inclusion} adds that not having the resources to claim support for \(\psi\) is also a potential defeater.

  Note, these are two different things.
  Recognise \(\lnot\psi\) as a potential defeater, but clear that one does not have the resources to claim support.
  For example, testimony from layperson.\nolinebreak
  \footnote{
    More to say about testimony, either below or in Chapter~\ref{cha:inertia}.
  }
  Here, things would go badly for the expert.
  However, without the relevant expertise, there's no sense in which the agent would establish that the expert is wrong if they were to attempt and fail.
  One would expect to fail.

  So, there is a distinction as \ref{nI:received-info} does not entail \ref{nI:inclusion}.
  From \ref{nI:claimed-support}, it follows that \(\psi\) not having value \(v'\) would be a defeater.
  ...
  To illustrate, two experts.
  Stated that a theorem is true, expect that I have the resources.
  Well, I have the paper with the proofs to work through.
  Here, conditional is intuitively true.
  If we are peers and it is not possible to follow the proof, then it's not good to follow your testimony.\nolinebreak
  \footnote{
  ...
  Focus point is establishing this particular type of potential defeater.
  Assume that \(S\) does not have the resources to claim support.
  Then, establish that the claimed support is either mistaken or misled.

  \ref{nI:inclusion} does not place any constraints on why the conditional is true.

  Simple, inclusion as relation between support.
  So, resources used to claim \(\phi\) should also allow for \(\psi\).
  If so, then potential defeater on tools that you don't establish an expectation against when reasoning by value.

  Common collection of resources.
  So, first player prevents loss at chess.
  If so, should show that there is now way for second player to secure a victory.
  Point here is that retracing the argument with a focus on the second player should be sufficient.
  If not, the something has gone wrong with initial argument.

  Here a mistake-mistake type of cases.
  For, if fail to show now way for a second player to secure a victory, then haven't necessarily shown that there is a way for the second player to secure a victory.
  May be some step is see as mistaken because of overlooked move.
  ...
  Still, may need additional resources.
  Wally is like this.
  Didn't need to observe glasses to claim support.
  ...
  If shown that ... then flaw in impossibility result.
  Here, need to read through proof of result, but there's got to be some flaw and this follows without denying the result.

  {
    \color{red}
    Note, in the illustration, move from claimed support, to reasoning with \(\phi\) having value \(v\).
    I don't get to reason from the premise that I have claimed support.
    Similarly, I don't get to use the factive inference with a high degree of confidence.\nolinebreak
    \footnote{
      There is a corresponding inference, \(\phi\) is likely to be the case, but here we go back to value.
    }
  }

  Note to ability, however ignore these for now as independent motivation.


  Okay, with the examples given, see that this is not a vacuous conditional.
  There are some instances where it is true.
  Still, do not overstate.
  There are various ways in which this fails.
  ...
  Two things.
  Move to \(\phi\), and then what this allows the agent to do.
  ...
  \ref{nI:inclusion} focuses on whether claimed support is any good, and hence on the move.
  Expectation regarding claiming support for \(\psi\) given that the agent is in position to make the move.
  In turn, potential defeater, but expectation so this doesn't establish a problem.
\end{note}

\begin{note}[Move on 3]
