...
\paragraph{Wright on transmission of warrant (failure)}

\begin{note}[Transmission of warrant]
  Mentioned above some links.
  Main link is to \citeauthor{Wright:2011wn}'s work on transmission failure.
\end{note}
...
\begin{note}[First question]
  ...
  Let's now return to the illustrations concerning counterexamples from the start of this section.
\end{note}

% \begin{note}[Example, 1]
%   So, misidentification.
%   Geoguesser.
%   All the visual cues.
%   So, visual cues, then location --- conditional hides a lot of stuff.
%   And, of course, if as the result then something has gone wrong.
%   And, other condition holds up too.

%   Problem.
%   Is a different location.
%   Didn't know that other country used the same markings.

%   Closest thing here is that two similar places.
%   So, if doubt that these features are unique to area, then doubt the significance?

%   Seems not.
%   Open to possibility, don't have all the details.
%   Do have some doubt for this.
%   But, claimed support, still.

%   Thing here is that it's okay for claimed support to be \mom{}.
%   I still have good support that the features are unique.

%   And, to strengthen would be very problematic.
%   For, then, \wrt{} would apply to any case of inductive support.
% \end{note}
