...
\begin{note}
  \citeauthor{Dretske:1970to}'s presentation focuses on knowledge.
\end{note}

\begin{note}
  The following issue arises, assuming that the agent knows that P entails Q:
  \begin{quote}
  ...
  Final step requires a third conditional:

  \begin{itemize}
  \item If S knows that P, then P not being the case is not an epistemic possibility for S.
  \end{itemize}

  Term this `weak factivity'.\nolinebreak
  \footnote{
    Contrast to `factivity':
    \begin{itemize}
    \item If S knows that P, then P.
    \end{itemize}
    This may be read in at least two different ways.
    First, relation between epistemic state of the agent and state of the world.
    Second, how things appear from epistemic state.

    Strong and weak.
    For, weak factivity is only about epistemic possibility, and it is not immediate that failure to be an epistemic possibility is sufficient to obtain P.
  }
  Reasoning leads to closure of knowledge (under known entailment):
  \begin{itemize}
  \item If S knows that P, and knows that P entails Q, then S knows that Q.
  \end{itemize}
  For:
  \begin{itemize}
  \item \(K(P \rightarrow Q) \rightarrow (\lnot KQ \rightarrow \lnot KP)\)
  \item \(K(P \rightarrow Q) \rightarrow (KP \rightarrow KQ)\)
  \item \((K(P \rightarrow Q) \land KP) \rightarrow KQ\)
  \end{itemize}
\end{note}

\begin{note}
  Now, `weak factivity' does not hold.
  However, parallel concerning epistemic possibility.
\end{note}
...
\begin{note}
  \begin{illustration}
    ...
    \begin{enumerate}[label=\arabic*., ref=(I\ref{illu:CS:wheres-wally}.\arabic*)]
    \item Zebra.
    \item Not a cleverly disguised mule.
    \end{enumerate}
  \end{illustration}

  Lack of reasoning that does not rely on claimed support for zebra.
\end{note}
...
\begin{note}
  ...
  Closure:
  \begin{itemize}
  \item If P is not an epistemic possibility, then anything entailed by P is not an epistemic possibility.
  \end{itemize}
