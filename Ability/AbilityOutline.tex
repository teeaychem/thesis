...
\begin{note}[Support]
  ...
  \begin{proposition}[\eiS{-} --- \eiS{}]\label{prop:supp:independence}
    If agent claims support for some proposition \(\phi\), then the support claimed indicates value of \(\phi\) regardless of what the value of \(\phi\) is.\nolinebreak
    \footnote{
  ...
  \end{proposition}
  Intuitively, if claimed support, then always prepend `even if \(\phi\) is not the case, this suggests \dots'.\nolinebreak
  \footnote{
    Note, different from learning that \(\phi\) is not the case.
  }
  ...
  No argument, taken as basic.
  Focus of support.

  No link to attitudes.

  Hard to say how restrictive this is.
...
\begin{note}[\nI{-}]
  ...
  \begin{proposition}[\nI{-}  --- \nI{}]\label{prem:ni}
    ...
    \end{enumerate}
    Then, the agent does not have the option of claiming support for \(\psi\) if:
    \begin{enumerate}
    \item The agent appeals to value of \(\phi\) and uses 2 to get value of \(\psi\)
    \item If not \(\psi\), then mistake.

      If misled for \(\phi\), then mistake for \(\phi\).
    % \item Claim to support for \(\xi\), requires value of \(\psi\).
    \end{enumerate}

    The converse is quite strong, if not mistaken, then not misled.
    This says, the support claimed is not mistaken, then it does establish the value of the proposition.



    Go via value.
    Possibility of being misled, and not \(\psi\) would require a mistake.
