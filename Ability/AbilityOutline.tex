...
\paragraph{A pair of general illustrations}

\subparagraph{Testimony 1}

\begin{note}
  \begin{illustration}\label{illu:CS:test:basic}
  ...
  I take the reasoning of~\autoref{illu:CS:test:basic} to be intuitively problematic, and I am somewhat confident that you will have seen some variant of the reasoning in relation to circularity before.

  Indeed, the goal for the moment is to explain why the two ideas expressed in relation to claiming support (\nfcs{} and \eiS{}) highlight a way in which the reasoning is problematic, and this way in which the reasoning is problematic is not clearly an instance of circularity.
\end{note}

\begin{note}
  Of course, the reasoning of \autoref{illu:CS:test:basic} seems problematic without constraints on the purpose.
  However, our interest is with claiming support, and so to begin, let us assume that the reasoning of \autoref{illu:CS:test:basic} is viewed from the perspective of claiming support.
...
\begin{note}[The problem]
  ...
  And, if \nagent{11}'s statement did not amount to an instance of testimony then the agent seems to lack a line of reasoning that indicates that \nagent{11} is trustworthy when speaking on matters regarding their personal character.
\end{note}
...
\begin{note}
  Now, granting that the above identifies a problem, the immediate question is:
...
\begin{note}
  ...
  For example, consider what \citeauthor{Sgaravatti:2013wu} terms the `Justification Account' of circularity.\nolinebreak
    ...
    Such may be the case, and plausibly is, but to the extent that the instance of claiming support is an instance of forming a justified belief, the problem highlighted by appeal to \ideaCSA{} nor \ideaCSB{} relied only on entertaining the possibility that \nagent{11} is not trustworthy.
    It does not necessarily follow that it was necessary for the agent to have claimed support for \nagent{11} being trustworthy to claim support for the premises --- no matter how plausible this may be.\nolinebreak
    \footnote{
...
\begin{note}
  Still, rather than toy with proposals, let's investigate further by granting the agent with a way of claiming support for the initial premise of the reasoning.
...
\begin{note}[Truth telling 2]
  \begin{illustration}\label{illu:CS:test:with-CS-for-premise}
    \begin{enumerate}[label=\arabic*., ref=(\arabic*)]
    \item I was assured that \nagent{11} will be honest with me.
...
\begin{note}
  ...
  For \ref{sg:JA}, more difficult.
  Different statement, seems fine.
  So, two options.
  Either did not meet standards of justified belief before discovering what \nagent{11} would say.
  Problem, as this seems fine in a lot of cases.
  Or, retraction of justification.
  Plausible.
  However, at issue is not whether circularity is present, but whether problem identified by \ideaCSA{} and \ideaCSB{} is sufficiently distinct.
  % Make this explicit with the following addition.

  % \begin{itemize}
  % \item Testimony about matters regarding personal character is only if trustworthy on matters regarding their personal character.
  % \end{itemize}
\end{note}
...
\begin{note}
  ...
  \begin{quote}
    the conclusion be such that evidence against it would (to at least some degree) undermine the kind of justification you purport to have for the premises.\nolinebreak
    \mbox{}\hfill\mbox{(\citeyear[359]{Pryor:2004ws})}
  \end{quote}

  Builds in limitation that is of interest when claiming support.
  However, subtle difference.
  Assumes evidence against.
  With ideas, just interested in the possibility.

  Objection:

  \begin{quote}
    (1) The cat sees the mouse.
    (2) If the cat sees the mouse, then there are some cases of seeing.
    (3) So there are some cases of seeing.
  \end{quote}

  Setting aside whether this is fine.

  Bad, given proposal, as if no cases of seeing, then the cat is not seeing.

  However, ideas are about entertaining possibility.

  And, no clear trouble.
  ...
  Point is, different concern.

  Now, return to initial.
  Entertain possibility.
  In this case, possibility, and the mere possibility alone seems enough.
  For, might be the case, and if so then would be bad.
\end{note}

\subparagraph{Testimony 2}

\begin{note}
  Particular instance of testimony.
  However, the general pattern seems okay.

  \begin{illustration}
    \begin{itemize}
    \item S testified that p is the case.
    \item Any agent and proposition, agent testified that proposition is the case only if proposition is the case.
    \item S testified that p is the case only if p is the case.
    \item So p is the case.
    \end{itemize}
  \end{illustration}
  ...
  However, claiming support.

  Possibility of \mistaken{}.
  Well, misheard, speaking sarcastically.
  Possibility of \misled{}.
  That \(p\) is not the case.
  And, if \(p\) is not the case then \(S\) does not know that \(p\).

  \(p\) is neither a \requ{} nor an \expec{}.
  For, \(p\) is clear throughout, and only get \(p\) at the end.
  So, falls outside scope of assumptions.

  Yet, within ideas that motivate assumptions.

  Intuition compatible with ideas.


  Quick response here is that testimony surpasses claiming support.

  Suppose claiming support, though.
  Account of why compatible.

  Quick sketch.

  Sufficient conditions for claiming support that S testified that do not rest on the specific content of testimony.

  S is an expert on \dots
  \(p\) is \dots

  So, looks good.
  Though, this requires assumptions about \(p\).

  You may disagree with the plausibility of this quick sketch.
  The goal is not to motivate a view on (reasoning about) testimony.
  Rather, highlight that there is no immediate conflict.
...
\paragraph{More interesting illustrations}

\begin{note}
  Previous illustrations were about reasoning.

  In the following, turn to \autoref{prop:CS-nai}. 
\end{note}
...
\begin{note}[Spot the difference]
  \begin{illustration}\label{illu:cs:spot-the-diff}
     Spot the difference
    \begin{enumerate}
    \item Look through pictures, no counting.
    \item Claim support for all differences.
    \item Okay, so found seven.
    \item Yeah, found seven.
    \end{enumerate}
  \end{illustration}
  ...
  Problem here is going to all the differences.
  However, this works, falls short of claiming support.
...
\subparagraph{Truth functional completeness}
...
\begin{note}
  ...
  Before going further, structure of this.
  Structure is similar to \autoref{illu:cs:spot-the-diff}, but contrast:
  Providing information about what also follows from the agent has claimed support for --- or what must also be the case.

  Again, problem here is that \requ{}.
...
\begin{note}[Logic proof]
  Recall \autoref{illu:CS:tfc} (again).
  \begin{illustration}
  ...
  The point here is that \nI{} expands the basic observation about the difficulty to a case where we go indirectly via how the agent has claimed support.
\end{note}
