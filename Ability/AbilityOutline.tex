...
\begin{note}[Notes on \nI{}]
  ...
  Quick intuition is the `even if\dots' test with `mistake'.
  Even if the support the agent has claimed is mistaken, the agent is in a good position for general ability.
  However, this seems to block drawing any consequences from this.
  For, there's no extension from the support claimed for \(\phi\) to the support claimed for \(\psi\).

  Get that there is a commitment to \(\psi\) having particular value.
  However, far from clear that there's much to be said regardless of whether \(\psi\) has value or if there is extension from what holds from support claimed for \(\phi\) to \(\psi\).
  ...
  Key here is mistaken support, really.
  Some subtleties, though.
    

  Basic idea, \(\psi\) is not the case, then not only would \(\phi\) not be the case, but would defeat some support claimed for \(\psi\).
...
\begin{note}[Second illustration of \nI{}]
  ...
  If \nI{}, then support is not from Poussin.
...
\begin{note}[Expanding of knowledge scenarios]
  No problem with going by value.
  And, even if misleading.

  Nor is there a problem to reasoning to \(\psi\).
  Problem is in claiming support for \(\psi\).
\end{note}
...
\begin{note}[Starting out]
  Have two conditions.
  Support, which may be misleading.
  Information that so long as \(\phi\) is not misleading, then \(\psi\).

  Then, going to support for \(\psi\)
\end{note}
...
\begin{note}[Different from\dots]
  Focus is on support.
  A pair of related things.
\end{note}

\begin{note}[No feedback/Bootstrapping]
  No feedback by Weisberg.

  \begin{quote}
    No Feedback If (i) B1 )B n are inferred from A1 )Am , and (ii) C is inferred from B1 )B n (and possibly some of A1 )Am ) by an argument whose justiﬁcatory power depends on making C at least x probable, and (iii) A1 )A m do not make C at least x probable without the help of B1 )Bn , then the argument for C is defeated.
  \end{quote}
  Suppose line of argument is.
  Support for \(\phi\), so \(\phi\), hence not misled.
  Does this require \(\psi\) being somewhat probable?

  This part is unclear.
  For, it's not clear that the move from support to \(\phi\) requires \(\psi\).
  This is the kind of move that seems fine.

  It's not clear that the possibility of a defeater is an issue, so long as there's no further information that that it is a defeater.
  It seems things change if there's some claim that \(\psi\) doesn't hold.
  Then the agent would need to discard defeater in order to get to \(\phi\).
  Still, that's not the kind of information of interest.

  Many cases where possibly misleading but this doesn't stop reasoning with value.

  So, it seems the second condition of this doesn't quite apply.
\end{note}

\begin{note}[Bootstrapping?]
  First observation is that agent doesn't have access to support is not misleading.
  So, agent isn't in a position to detach \(\psi\).

  This does not seem correct.
  Move to \(\phi\) having value.
  From this, support is not misleading.
  Hence, \(\psi\).
  May seem problematic.
  Here, property of support that agent does not have access to.
  Still, the observation is not that the agent establishes the property, but that given the agent's perspective, the support is not misleading.
  Goal is not that \(\psi\) holds, but that form support claimed, claim support for \(\psi\).

  This is not to say the sketched is fine, but that it's not obvious.
\end{note}

\begin{note}[Wright on transmission failure]
  Here, there's some possible fault in the reasoning process.

  Transmission failure is about something preventing transmission.
  Here, the idea is that of an absence.
  It's not that the agent has made some assumption that blocks anything, rather it's that there's no support that's been gained.

  Again, it's not clear that there is any problem with support gained for \(\phi\) given relation.
  No clear reason why the agent would have needed to assume \(\psi\) when gaining support.
