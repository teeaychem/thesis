...
\begin{note}
  \begin{proposition}[\twoDefeaters{-} --- \twoDefeaters{}]\label{prop:defs-for-CS}
    There are at least two ways in which a claim to support may be defeated.
      ...
      \item Claimed support may (discovered to be) be \emph{mistaken} by relying on factors that do not indicate the value of the proposition.
        {\color{red} Involves appeal to some proposition \(\phi\) having value \(v\) which does not have value \(v\).}
      \end{itemize}
      \vspace{-\baselineskip}
  \end{proposition}
...
\begin{note}[Subjectively sound]
  ...
  \begin{proposition}[Adequacy of claimed support]\label{prop:CSNMORM}
    If agent claims support for some proposition, then the agent has some expectation that possible defeaters (of the two types of noted) do not obtain.\nolinebreak
    \footnote{
...
\begin{note}
  \color{red}
  \large
  Switch \nfcs{} with \autoref{prop:CSNMORM}.

  Point is, always possibility, but agent \emph{expects} that these things don't obtain.
\end{note}

\begin{note}[Agents are fallible]
  The missing piece of \autoref{prop:CSNMORM} is an account of what `deem' amounts to.
  The following proposition states an assumption, which allows a (general) functional characterisation of `deem'.

  \begin{proposition}[\nfcs{--} -- \nfcs{}]\label{prop:supp:nfactive}
    ...
    In other words, claimed support that \(\phi\) having value \(v\) does not entail \(\phi\) has value \(v\).

    Hence, an agent may never move from claiming support for \(\phi\) having value \(v\) to \(\phi\) having value \(v\) without assuming that the claimed support is \nmom{}.
  \end{proposition}
...
\begin{note}[\eiS{}]
  ...
  \begin{restatable}[\eiS{-} --- \eiS{}]{proposition}{eisProp}\label{prop:supp:independence}
    Claimed support for \(\phi\) having value \(v\) indicates that \(\phi\) has value \(v\) independently of whether the claimed support for \(\phi\) having value \(v\) is \mom{}.
  \end{restatable}
  {
    \color{red}
    Old footnotes\nolinebreak
...
\begin{note}[Argument for \eiS{}?]
  What I need here is a clear account of why the claimed support for \(\phi\) having value \(v\) fails if it requires that \(\phi\) has value \(v\).
  ...
  \begin{enumerate}
  \item Well, the problem is not \(\psi\) having value \(v'\), but the agent not being \mom{}.
  \item So, with \nI{} the issue comes down to only holding up if the agent is not \misled{}.
  \item The issue isn't that the agent requires that \(\psi\) has value \(v'\) itself.
  \item Rather, the issue is that this in turn means that the agent recognises that their claimed support cannot be \mistaken{}.
  \item So, recognition that there's an epistemic possibility that I'm mistaken, but the claimed support only indicates if I am not mistaken.
  \item No, this still extends to all propositions.
  \item \dots
...

  {
    ...
    \large
    \eiS{} is different.
    What I want is possibility, and as a result of this, claimed support survives even if \mom{}.
    That's a different proposition, but motivated by \eiS{}.
    Still, I have sufficient motivation for this additional proposition.
  }

  {
    Why does this not make sense as something immediate?
    Not possible to hold not mistaken.
    Not possible for claimed support only given \(\phi\).
    Because, \dots

    Well, easier to go, possibility of \mom{}.
    Therefore, claimed support holds up even if.
    But this is the thing that seems to over-generate.
    ...
    not possible if reasoning involves appeal to value of propositio.

    this is the form the thing should take.

    why?
    possibility of \mom{}
    if value then need to exclude prop not having value.
    ...
    Look, this is just what claimed support is.
    For what follows a simple way out of the issues will be to deny interest in claimed support as I understand it.
    I don not find this plausible, but it's got to be like this because not talking about reasoning in general.
    And, claimed support is a `strong attitude' in the sense that it accounts for these possibilities.

    First is weak, CS only does so much.
    Second is strong, in sense that need this to be the case.
    It's a sort of squeezing thing.
  }

  No, there's two ideas that I'm mixing together here.
  \eiS{} as stated is immediate from \nfcs{}.
  For, it is not possible for the agent to rule out that claimed support is \nmom{}, so it must be the case that the claimed support indicates regardless of whether it is in fact \mom{}.

  However, the other idea is that if does not indicate regardless, then no claimed support.
  And for some reason this seems harder to argue for.
  Though it really should just be the contraposition.

  Well, the first idea is basically expressing the impossibility of the agent holding that their claimed support is \nmom{}.

  The second idea is slightly different.
  As it's expressing the idea that reliance on \(\phi\) is enough to discount whatever the thing is as claimed support.

  \begin{itemize}
  \item Not possible for claimed support for \(\phi\) having value \(v\) to depend on \(\phi\) having value \(v\).
  \end{itemize}
  I.e.\
  \begin{itemize}
  ...
  \end{itemize}
  This last formulation gets to what I want, given the appropriate reading of `requires'.
  As, in the case of \nI{}, the issue I get is that claimed support for \(\psi\) having value \(v'\) requires that \(\psi\) has value \(v'\).

  There's tension, as possibility of being \misled{}.
  So, hence, the claimed support is incompatible with the (epistemic) possibility that the claimed support is \misled{}.

  Problem, still, is that this point seems to apply to any proposition having some value.
  Though, that's kind of what I have had in mind all along\dots
  At the same time, it seems far stronger than what I need.

  I only need this with respect to the proposition that the agent has claimed support for.
  
\end{note}

\begin{note}
  \eiS{} follows from~\autoref{prop:CSNMORM} and~\nfcs{}.

  From~\autoref{prop:CSNMORM}, deemed that claimed support is \nmom{}.
  From~\nfcs{}, always possibility.

  Suppose depends on value of \(\phi\).
  Still, from \nfcs{}, (epistemic) possibility that value of \(\phi\) differs.
  If differs, then claim to support is misleading.
  By~\autoref{prop:CSNMORM} agent deems not mistaken or misled, and so deems that possible defeaters do not obtain.
...
\paragraph{\ESU{} in the literature}
...
\begin{note}[Memory]
  ...
  \autoref{ill:ad:proof:mem} seems a straightforward case of claiming support.\nolinebreak
    \footnote{
      ...
      For, \ref{nI:claimed-support} and \ref{nI:received-info} may be satisfied by taking \(\phi\) to be the proof and \(\psi\) to be theorem-hood.
...
\begin{note}[Background to Limitation]
  ...
  The primary content of~\ref{nI:going-by-value} is an account of the particular way of claiming support, and the statement that \emph{that} way of claiming support is undercut given~\nIOTT{}.
...
\begin{note}[An instance of \adA{}]
  ...
  So, using claimed support for conditional and claimed support for antecedent to claim support for conclusion.

  Hence, the type of reasoning here is an instance of \adA{}.
  Indeed, the way in which the agent claims support conforms to the \hyperref[abstract-adA]{basic (abstract) instance} of \adA{} present in~\autoref{sec:ability-ads-adc}.
\end{note}
...
\begin{note}
  ...
  Our task is now to demonstrate why~\ref{nI:going-by-value} is the case when \ref{nI:claimed-support},~\ref{nI:received-info}, and~\ref{nI:inclusion} all obtain.
...
\begin{note}
  ...
  \item Then, the question is whether \adA{} requires using ability is this way.
  \item Finally, why \nI{} doesn't apply to \adB{}.
  \end{itemize}
...
\begin{note}
...

  {
    ...
    Issue is that \adA{} suggests that it's possible that the agent doesn't care about what they're referring to.
    Everything proceeds following form.
    This isn't right.
    If only form, then the agent doesn't link up results to how things actually are.
    The point with \adA{} is that it proceeds by value in these cases because these are conditionals about how things are.
  }
...
\begin{note}
  ...
  So, \RBV{} is an instance of \adA{}, generally.
  And, because of the construction of the scenarios, the case of \adA{} we're interested in is an instance of \RBV{}.
\end{note}
