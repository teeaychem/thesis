...
\begin{note}
  \begin{restatable}[\eiS{-} --- \eiS{}]{assumption}{assuEIS}\label{assu:supp:independence}
    ...
    \begin{itemize}
    \item Reasoning about recognised \requ{0}.
    \item Reasoning about unrecognised \requ{0}.
    \end{itemize}
    \vspace{-\baselineskip}
  \end{restatable}
...
\begin{note}
  ...
  \begin{quote}
    ...
    If S does not know whether or not Q is true, then for all he knows it might be false. If Q is false, however, then P must also be false. Hence, for all S knows, P may be false. Therefore, S does not know that P is true.\nolinebreak
...
\begin{note}
  There is no direct link with respect to claiming support.
  For, given \ideaCSA{} and \autoref{assu:supp:nfactive}, this reasons does not apply.
  Possibility that false does not prevent claiming support.

  However, indirect link.
  Possibility.
  In the case of knowledge, entertain the possibility that \(\lnot\psi\) when \(K\phi\) then a problem.
\end{note}

\begin{note}
  Possible to extend, though.
  If claimed support for knowledge, then remove the factivity.
\end{note}
...
\begin{note}
  Seems that agent needs to apply regularities.
\end{note}
