...
\begin{note}
  \begin{illustration}\label{ill:ad:proof}
    \begin{enumerate}
    \item\label{ill:Eproof:def} Syntactic proof is sufficient to establish theorem.
    \item\label{ill:Eproof:exi} Exists a syntactic proof.
    \item\label{ill:Eproof:thm} Theorem.
    \end{enumerate}
  \end{illustration}

  Claimed support for existence of a proof, say some previous proof.
  So, apply to~\ref{ill:Eproof:def} and `detach' \ref{ill:Eproof:thm}.
...
\begin{note}
  \begin{definition}[\adA{}]
    Suppose:
    \begin{enumerate}[label=\arabic*., ref=(\textsf{S}\arabic*), series=adA_counter]
    ...
    \begin{enumerate}[label=\arabic*., ref=(\textsf{S}\arabic*), resume*=adA_counter]
    \item\label{def:adA:psi} Claims support for \(\psi\) having value \(v'\) by claimed support for \(\phi\) having value \(v\).
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{definition}
\end{note}
...
\begin{note}
  \begin{definition}[\adB{}]
    Suppose:
    \begin{enumerate}[label=\arabic*., ref=(\textsf{I}\arabic*), series=adB_counter]
    % \item Agent has claimed support that \(\psi\) has value \(v'\) when \(\phi\) has value \(v\).\nolinebreak
      ...
      % }
    \item Claimed support for \(\phi\) ensures there is some (distinct) collection of propositions \(\gamma_{1},\dots,\gamma_{k}\), such that possible to claim support for \(\psi\) by \(\gamma_{1},\dots,\gamma_{k}\).
    \end{enumerate}
    Then, agent claims support for \(\psi\) having value \(v'\) by `\adB{}' from \(\phi\) to \(\psi\) via \(\gamma_{1},\dots,\gamma_{k}\) when:
    \begin{enumerate}[label=\arabic*., ref=(\textsf{I}\arabic*), resume*=adB_counter]
...
\begin{note}
  ...
  Appeal to the premises and steps of the proof.
  Intuitively, applies to ability in the same way.
  However, we will delay details until we've seen a few more illustrations.
\end{note}

\begin{note}[Descriptive]
  \adA{} and \adB{} are descriptive from the perspective of claimed support.
  Don't say that 1 and 2 are sufficient for 3 in the case of \adA{}.
  Nor that there are cases for \adB{}.
\end{note}

\begin{note}[Important]
  \adA{} traces a chain of support, claimed support for \(\phi\) goes to \(\psi\).

  \adB{} as defined does not detail how the agent has claimed support for \(\gamma_{1},\dots,\gamma_{k}\).
