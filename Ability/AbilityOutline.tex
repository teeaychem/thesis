...
\begin{note}
  \ref{nI:inclusion} introduces potential defeater of a certain kind.
  ...
  Here, mistaken.
  This implies that the agent has made a mistake.
  However, does not follow that problem if the agent is misled.

  The argument sketched above, and detailed below, applies regardless of mistaken or misled.
  ...
  Note, the defeater focuses on the claimed support for \(\phi\), not whether \(\phi\) has value \(v\).
  In some cases, potential defeater for \(\phi\) having value \(v\) may be of this kind.
  Still, not all.

  Important case is knowledge.
  ...
  Still, possible to separate \nI{} into two distinct conditions.
  One for being mistaken and one for being misled.
  For us, misled is the most important.
  And, arguably the harder to establish.
  For, this requires that failure is sufficient to indicate that \(\phi\) does not the relevant value.
  This was the case with Wally.

  Still, the argument to be given does not depend on the particular relation between the support claimed for \(\phi\) and the potential to claim support for \(\psi\).
\end{note}
