...
\begin{note}[\gsi{}++]
  ...
  \begin{proposition}[\textsf{|gs-I\space·\space H|}]
    ...
    Then, \emph{S} may claim support for having the specific ability \(\sigma\) by reasoning that they have the property of being able to \emph{V} that \(\phi\).
  \end{proposition}
...
\begin{note}[\gsi{}++ applied : \WR{}]
  ...
  FORTRAN 77 does not allow recursion, a function may not call an instance of itself.\nolinebreak
  \footnote{
  ...
  By contrast, Python would be fine.

  So, Python has the general ability to express recursion, therefore Python may implement the recursive factorial algorithm.
...
\begin{note}[Summary]
  In this section, argue that \uRa{} conflicts with \WR{}.
  ...
  So, included in appeal to potential witnessing event is that sufficient resources are available.
  Recall, mere possibility isn't sufficient for the \aben{}.
...
\begin{note}[Moving to incompatibility]
  Incompatibly with \uRa{} will be from common point of appeal to sufficient resources.
  To this we now turn.
\end{note}
