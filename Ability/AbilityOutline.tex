...
\begin{note}[Representationalism]
  Different, representationalist.
  basing C on R involves the agent's representing R as justifying C.

  \citeauthor{Neta:2019aa}'s broad characterisation of representationalist theories of basing:
  \begin{quote}
  ...
  Here, the key is representation.
  If not represented, then not used.
...
\begin{note}[Responding to reasons]
  ...
  Thing with \citeauthor{Lord:2018aa} is that for an action to be rational goes beyond claiming support.
  However, grant that claiming support is involved.
  From this, plausibly derive~\uRa{}.

  \citeauthor{Lord:2018aa} argues for the following definition of reacting for normative reasons
  \begin{quote}
...
\begin{note}[Alternative]
  ...
  Key is that if~\rC{} is true, then the agent is not required to use having the ability to reason as a premise, the agent may instead appeal to the premises (and steps) that would be used when witnessing the ability.
  ...
  The alternative, in line with~\uRa{}, would be to use that the agent has the ability.
\end{note}
