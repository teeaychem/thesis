...
\begin{note}
  Well, perhaps formal and non-formal.
  Where, formal is logical consequence.
\end{note}
...
\begin{note}[Two cases]
  \begin{itemize}
  \item `Formal' and \ur{}: nonstandard models
  \item `Non-formal' and \nr{}: time
  \end{itemize}
\end{note}
...
\begin{note}[Example with an existential]
  ...
  \end{illustration}
    ...
    \vspace{-\baselineskip}
    \begin{enumerate}[label=\(\protect\iEDogd\)\space\arabic*., ref=(\(\protect\iDogd\)\space\arabic*), align=left, leftmargin=*]
      % \item[\(\protect\iDogd\)\space P.] The dog is \RIPa{} and \RIPb{}.
    ...
    \item\label{ill:iDogd:E:sep-app} \(\text{\RIPa{-} \& \RIPb{} dog}(i) \rightarrow ((\text{\RIPa{-} dog}(x) \land \text{\RIPb{-} dog})(i))\)
