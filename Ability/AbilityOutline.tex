...
\begin{note}[Minimal constraints]
  Minimal constraints on what it is to claim support.
  ...
  \bP{} states a sufficient condition for when an agent may claim support for a conclusions of reasoning from premises, steps used.
  We take~\bP{} as basic.
  The purpose of taking~\bP{} as basic is to fix a basic understanding of when an agent may claim support in more-or-less standard instances of reasoning.
  {
    \color{red}
  ...
  For example, perhaps some reasoning required to claim that eyesight used to read ruler is good, or perhaps claim of support because appeal to eyesight --- e.g.\ reliable process.
\end{note}

\begin{note}[Support and value]
  Claim support for some proposition.
  And, more precisely, that the proposition has some value.

  Examples.
  Testimony, so claim support that \emph{p} is true.
  Unreliable, so claim support that \emph{p} improbable.
  Religious text, so claim support that \emph{p} ought to be the case.
  Producer, so claim support that album is desirable.

  If prefer, reduce all these examples to more complex propositions and binary value of true or false.
  Permissive here.
  Core idea is that claim of support is that things are a certain way.
  Proposition, what the thing is.
  Value, the way it is.

  And, clarify reasoning, tracing value through propositions/establishing that proposition has value.
...
\begin{note}[Subjectively sound]
  Instead, for claim support, subjectively sound.
...
\begin{note}
  Claim to support doesn't ensure that proposition has value, plausibly even if agent is successful.

  Two important idea, both of which will be used.
  Claim to support may be \emph{misleading}.
  Claim to support may be \emph{mistaken}.
...
\begin{note}[Misleading]
  ...
  Hence, claimed support may be misleading.
...
\begin{note}[Subjectively sound II]
  \begin{proposition}[Claimed support is considered neither misleading nor mistaken]
    {
      \color{red}
      |\textsf{CS\(\rightarrow\)M\(\downarrow\)M}|
    }
    If agent claims support for some proposition, then from the perspective of the agent, the agent considers that the claimed support is not misleading nor mistaken.
  \end{proposition}

  Idea of proposition is that necessary condition on claiming support is that agent considers the claimed support as indicative of what the value of the proposition is.
  Not misleading, so value.
  Not mistaken, so indicative.

  However, not required that the claimed support is not (in fact) misleading nor mistaken, only that the agent considers that the claimed support is not misleading or mistaken.

  Proposition adds that the agent \emph{considers} that the claimed support is not misleading or mistaken.
  Stronger than distinct claim that the agent does not consider that the claimed support is misleading or mistaken, as presence of consideration.
  Latter, but absence of consideration.
  Former, some positive account of why the claimed support is not misleading or mistaken.
  ...
  Well, I am not sure what such consideration amounts to, but I also don't think it matters a great deal.
  I will introduce a (hypothetical) test --- the `even if\dots' test --- to clarify what I mean by such considerations and so long as an agent may provide an adequate responses to the test, the agent will be in a position to claim support.

  {
    \color{red}
  Broadly, if clear that mistaken or misled, then claimed support is defeated.
  No longer hold support relation.
  The `even if\dots' test highlights that if claimed support, then agent is in position to dismiss potential defeaters as `merely' potential.
  }
...
\begin{note}[Even if: mistaken]
  ...
  Fault:
  Clock, with a second hand that appears to be stuck.
  Important, only interested in the hour.
  Even if the second has isn't moving because the clock is malfunctioning \dots still time.
  Bad, it may be the time stated on the clock, but a broken clock is a bad guide to the time.
  However, model of clock second hand often fail, then no particular problem.
...
\begin{note}[Even if: more]
  Primary observation from these examples is that in positive cases considerations are provided.
  In the failure cases, no considerations.
...
\begin{note}[\eiS{}]
  ...
  \begin{proposition}[\eiS{-} --- \eiS{}]\label{prop:supp:independence}
    If agent claims support for some proposition, \(\phi\), then the claimed support indicates value of \(\phi\) independently of the value of \(\phi\) (or support).\nolinebreak
    \footnote{
  ...
  \end{proposition}
  Include clause regarding support as having distinguished claiming from having.
  If collapse, then this may be omitted.

  Qualification --- things may well need to be a certain way for an agent to claim, or to be in a position to, claim support.
  The force of the corollary is that agent does not depend on \(\phi\) being the case in order to claim support for \(\phi\).
