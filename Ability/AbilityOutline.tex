...
\begin{note}[Sufficient condition]
  ...
  \begin{idea}[\USE{-} --- \USE{}]\label{prem:bP}\label{prop:USE}
...
\begin{note}[Value proposition]
  ...
  \begin{assumption}[Claimed support is for the value of a proposition]\label{prop:csifvoap}
...
\begin{note}
    \begin{itemize}
  \item \emph{p} is true.
  \item \emph{p} improbable.
  \item \emph{p} ought to be the case.
  \item \emph{p} is desirable.
  \end{itemize}
\end{note}
...
\begin{note}[Reasoning proposition]
  \begin{assumption}[Claiming support is the result of reasoning]\label{assu:CS-culmination-of-R}
...
\subsection{Claiming support and `defeaters'}
...
\begin{note}
  \begin{definition}[\mistaken{-} and \misled{}]\label{def:MoM}
...
\begin{note}
  \begin{idea}[\twodefeaters{-} --- \twodefeaters{}]\label{prop:defs-for-CS}
    \mistaken{-} and \misled{} are defeaters for claimed support in the sense that:

    Were the agent to learn that the claim to support was misleading or mistaken, then the agent would not hold that claimed support is problematic.\nolinebreak
    \footnote{
...
\begin{note}[Agents are fallible]
  ...
  \begin{restatable}[\nfcs{--} -- \nfcs{}]{assumption}{nfcsProp}\label{prop:supp:nfactive}
...
\begin{note}
  ...
  \begin{restatable}[\eiS{-} --- \eiS{}]{assumption}{eisProp}\label{prop:supp:independence}
    Claimed support for \(\phi\) having value \(v\) involves:
    \begin{itemize}
    \item Reasoning that recognised defeaters do not obtain.
    \item Reasoning that unrecognised defeaters do not obtain.
    \end{itemize}
    \vspace{-\baselineskip}
  \end{restatable}

  \eiS{} is rather uninformative.
  Still, a little more than~\autoref{idea:eiS} as reasoning.
  Indeed, primary purpose of \eiS{} is that we may expand on what the relevant reasoning involves.
  ...
  Also, fine with additional requirements, such as investigating unrecognised defeaters.
  However, do not assume this.
\end{note}

\begin{note}
  \begin{assumption}\label{assu:consider-recognised-defeaters}
    If in position to deal with defeater, then got to deal with it --- rule out possibility to greatest extent possible. {\color{red} This is how I get the need to do the reasoning with respect to \nI{}.}
    {
      \color{blue}
      This needs to be weaker.
      I mean, no consideration is enough to get me that pure expectation is a problem.
      And, all I really want from this is that agent doesn't get to simply claim support that as something was an expectation, they're good to go.

      So, maybe observe expectation problem from \eiS{}.
      Then, add in this assumption.

      Well, for \nI{} then problem is that there's no way to work with \(\phi\) without the relevant expectation, given the interdependence there.
      I think I should leave this as a possible object that I don't have an argument for.

      I could have, recognising expectation of previously claimed support is not enough to be a consideration.
      Some effort --- in particular, observing expectation is not enough.
      Claimed support without expectation may be adequate.
      No, what I really need for \nI{} is that there's no way to go without expectation.
  ...
  As stated, agent relative.
  Fine with something stronger.

  However, keep \nfcs{} in mind.
  It need not be the case that the agent completely rules out possibility of defeater obtaining --- i.e.\ of being \mom{}.
...
\begin{note}
  Following is an immediate consequence of \autoref{assu:consider-recognised-defeaters}:

  \begin{proposition}\label{prop:CS-only-if-consider-recognised-defeaters}
    If recognised defeater and does not rule out to greatest extent possible, not an instance of claiming support.
  \end{proposition}
\end{note}
...
\begin{note}
  We will appeal to \autoref{assu:consider-recognised-defeaters}, and in particular~\autoref{prop:CS-only-if-consider-recognised-defeaters}, but this assumption is not our focus.

  Our interest is with unrecognised defeaters, and consequence of \autoref{assu:consider-recognised-defeaters} with respect to these.
\end{note}

\begin{note}[Expectation, assumption]
  Start with a definition:

  \begin{restatable}[Expectation with respect to claimed support]{definition}{expectationDef}\label{def:expectation}
...
\begin{note}
  {
    \color{red}
    Expectation is somewhat subtle.
    ...
    Instead, expectation is a general catch all.
    Expects that future information would not defeat claimed support.
  }
...
\begin{note}
  \begin{proposition}
    If claimed support for \(\phi\) and then receive information about \(\psi\), then expectation of claimed support that \(\psi\).
  \end{proposition}
\end{note}


\begin{note}
  \begin{assumption}\label{assu:CS-persists}
    Claimed support persists given introduction of novel possible defeaters.
...
\begin{note}
  \begin{proposition}\label{assu:expect-nai}
    Conclusion of reasoning is not an instance of claiming support if require expectation from premises appealed to that \(\phi\) has value \(v\) in order to conclude that \(\phi\) has value \(v\).
  \end{proposition}

  {
    \color{red} This follows from \autoref{assu:consider-recognised-defeaters}.
    For, then haven't considered to full extent possible.
  }

  Basic issue is that when claimed support for \(\phi\), did not consider \(\psi\).
  \autoref{assu:expect-nai} does not state that the agent is required to give up claimed support.
...
\subsection{Additional notes on claimed support}
\label{sec:refl-claim-supp}
...
\begin{note}
  ...
  Plausible that this follows from \ref{prop:supp:independence}.
  However, tricky.
...
\begin{note}
  High probability is fine, as this is just good indication even if not true.
\end{note}

\subsubsection{Intuition}

\begin{note}
  The only thing I really rule out here is claiming support for phi only in the circumstances that phi is actually the case.
  Two key assumptions are the cleanest general principles for why I think this is the case.
\end{note}
...
\begin{note}[Important]
  \begin{itemize}
  \item Strictly speaking, the problem is not \(\phi\) having value \(v\), but the agent not being \mom{}.
  \item The issue isn't that the agent requires that \(\phi\) has value \(v\) itself.
  \item Rather, consequence of not being \misled{}
  \end{itemize}

  This is with respect to \(\phi\).
  \eiS{} also extends to other propositions.
\end{note}

\begin{note}[Consequences]
  \begin{enumerate}
  \item Always possible to be \mom{}
  \item Problem is rule out possibility of being \mom{}.
  \item In particular, if claimed support for \(\phi\) requires \(\phi\), as this rules out being \misled{}.
  \item Assuming \(\phi\) to claim support for \(\psi\), as long as I'm okay with \(\phi\) not being the case, which in turn amounts to claimed support for \(\phi\).
  \item I guess that's an argument for this general position, then.
  \item Get something of a regress.
  \item Stop this by granting claimed support.
  \item Point is that if there is some \(\phi\) such that requires \(\phi\) has value \(v\), then agent pieces this together with something that indicates that \(\phi\) has value \(v\) even if it does not.
  \item Ah, but the escape is expectation.
  \item So it doesn't have to be claimed support.
  \item Claiming support is from other propositions, expectation is not, at least in principle.
  \item Only constraint is that expectation is weaker than claimed support, so that an agent expects something does not grant claimed support.
  \item Can't appeal to expectation to claim support for expectation.
  \item So, either claimed support or expectation.
  \end{enumerate}
\end{note}
...
\begin{note}[Quick clarification on \eiS{}]
  A quick clarification may be helpful.
...
\begin{note}
  ...
  It may be the case that no agent would be in a position to claim support that the speed of light is constant if the speed of light were not constant, but in claiming support an agent must expect that possible defeaters do not obtain, e.g.\ that the laws of nature are constant, and that no mistakes have been made when observing relevant phenomena.
\end{note}

\begin{note}[Argument for \eiS{}?]
  {
    Worry is over-generation.
    Claimed support does not rely on what the `actual' value of anything is.
    Fine to have assumptions.
    This does not extend to all premises, only the conclusion and some part of the reasoning that leads up to claiming support.

    So, it is not possible for it to be the case that claimed support only if proposition has that value.

    Well, \mom{}.
    If require, then not compatible with being misled.
    Right, because \mom{} then must be compatible with these possibilities.
    I can just assume this for now.
  }
\end{note}
...
\begin{note}[The `Even if\dots' test]
  ...
  \begin{enumerate}
  \item[(ML)] Even if \(\phi\) does not have value my claimed support indicates, I expect it to be the case that\dots
...
\begin{note}[Even if: more]
  Primary observation from these examples is that in positive cases provided responses indicate that some response to possibility of being \mom{} is available to the agent.
...
\begin{note}
  Key consequence of~\eiS{}.
\end{note}

\begin{note}
  \begin{proposition}[\eiSC{-} --- \eiSC{}]\label{prop:eiS:consq}
    If result of reasoning to \(\phi\) having value \(v\) is such that agent considers that reasoning fails if \(\phi\) does not have value \(v\), then reasoning is not an instance of claiming support.
  \end{proposition}
...
\begin{note}
  Does not extend to all propositions.
  \eiSC{} holds with respect to \(\phi\) as \misled{} is a possible defeater in cases of claiming support.
  In order to expand to premises and steps of reasoning, then need \mistaken{} to apply to all.
  However, mistaken is existential.
  So, only get \emph{some}.
\end{note}
...
\begin{note}[Truth telling]
  ...
  No, because if X is not trustworthy, then it may be the case that~\ref{ex:tt:test} was an instance where X was not to be trusted.
...
\begin{note}
  ...
  Problem with \eiSC{}.
  Don't know \(\psi\) then don't know \(\phi\).
  So, no claiming support.
...
\begin{note}[Generalising point for K example]
  ...
  This is ruled out by~\autoref{assu:expect-nai}.
...
\begin{note}
  \autoref{prop:eiS:consq} is fundamental to the thesis.
...
\section{Two (conflicting) propositions regarding claimed support}
...
\begin{note}
  \begin{definition}[\adA{}]\label{def:adA}
...
\begin{note}
  \begin{definition}[\adB{}]\label{def:adB}
    Fix an agent and suppose:
...
\begin{note}[Proposition]
  \begin{proposition}[\adB{} is incompatible with~\ESU{}]\label{mcA:WR-and-denied-claim}
...
\begin{note}[\nI{}]
  \begin{proposition}[\nI{-}  --- \nI{}]\label{prem:ni}
    Suppose \(\phi\) is any, and \(\psi\) is such that value regardless of claiming support:
    ...
    \item {\color{red} Reasoning did not consider \(\psi\). ZZZ}
    \end{enumerate}
    And, suppose:
...
\begin{note}[What \nI{} amounts to]
  ...
  Hence, claimed support for \(\psi\) having value \(v'\) is \nmom{}.
  So, given background, \(\phi\) having value \(v\) requires \(\psi\) having value \(v'\).

  Problem, as claimed support for \(\psi\) having value \(v'\) `requires' \(\psi\) having value \(v\).
  Hence, if agent considers possibility that they're misled, then tension.
  For, \(\psi\), but `requiring' something agent is misled about.
  Problem because if am misled, then claimed support involves appeal to something which is no the case.
  Claimed support would be mistaken.
...
\begin{note}[Overview]
  ...
  Start with~\ref{nI:claimed-support} as this is relatively straightforward.
...
\paragraph{\ref{nI:claimed-support}}

\begin{note}[\ref{nI:claimed-support}]
  Restated:
  \begin{quote}
    \begin{enumerate}
    \item[\ref{nI:claimed-support}] \nIClauseClaimedSupport{}
    \end{enumerate}
  \end{quote}

  \ref{nI:claimed-support} identifies claimed support that some proposition \(\phi\) has value \(v\).

  If the agent satisfies clauses \ref{nI:claimed-support}, then the agent often has the option of reasoning to \(\psi\) having value \(v'\) by combining \(\phi\) having value \(v\) with further information.
  In particular, conditional as seen in above illustrations.
\end{note}
...
\begin{note}[Background to Limitation]
  ...
  The argument for why the way of claiming support is undercut will be given in~\autoref{sec:argument-ni}.
  However, we will argue for a proposition that we will in turn appeal to when arguing for~\nI{}.
\end{note}
...
\begin{note}[Core idea]
  ...
  Note, in contrast to the observation made above with respect to~\ref{nI:claimed-support} it is \emph{claiming support for \(\psi\) having value \(v'\)} that is a possible defeater for the claimed support that \(\phi\) has value \(v\), rather than \(\psi\) not having value \(v'\).
...
\begin{note}
  The previous sections provided a discussion of each clause of~\nI{}.

  Our task is now to demonstrate why~\ref{nI:going-by-value} is the case when \ref{nI:claimed-support} and~\ref{nI:inclusion} obtain.
  ...
  In particular, why~\nI{} applies to ability paired with \adA{} (\S\ref{sec:ni-ability:adA}) but not when paired with \adB{} (\S\ref{sec:ni-ability:adB}).
\end{note}

\paragraph*{Argument preview}

\begin{note}
  So, we assume that \ref{nI:claimed-support} and~\ref{nI:inclusion} obtain.

  Claimed support for \(\phi\) having value \(v\).
  Interdependence between claimed support for \(\phi\) having value \(v\) and claiming support for \(\psi\) having value \(v'\).

  Now,~\ref{nI:going-by-value}.
  \(\phi\) having value \(v\).

  If the two conditions, then unsuccessful.
  Why unsuccessful given assumptions.

  \begin{itemize}
  \item If \(\phi\) having value \(v\) is involved, then violation of \eiS{}.
  \end{itemize}

  Hence, assume antecedent and argument for consequent.

  Recall:
  \begin{quote}
    \eisProp*
  \end{quote}
  Basic constraint on claimed support.
  Motivated by \nfcs{}.
\end{note}

\begin{note}
  Failure for claimed support, though.
  Relatively minimal about what claimed support amounts to.
  Handful of ideas in \autoref{sec:claimed-support}.
  Two important are:
  \nfcs{} and \eiS{}.

  From \nfcs{}, (epistemic) possibility that any instance of claimed support is \mom{}.
  From \eiS{}, failure of claimed support if claimed support for \(\phi\) having value \(v\) requires \(\phi\) having value \(v\).

  So, focus of argument is primarily on why~\ref{nI:inclusion} leads to violation of~\eiS{} with respect to \(\psi\) having value \(v'\).
  Throughout the argument, \nfcs{}.
\end{note}

\paragraph*{Argument proper}

\begin{note}
  \begin{definition}[Entanglement]\label{def:CS-depends}
  ...
  \begin{restatable}[]{proposition}{RBVinheritance}\label{prop:RBV:inheritance}
...
\begin{note}[Two conditionals]
  Goal is to argue for \nI{}.

  Break this down into two main conditionals that hold given the three suppositions of \nI{}.

  \begin{enumerate}
  \item If appeal to \(\phi\) having value \(v\) from claimed support of \ref{nI:claimed-support}, then any reasoning that proceeds from \(\phi\) having value \(v\) depends on \(\psi\) having value \(v'\).
  \item If reasoning from \(\phi\) having value \(v\) requires that \(\psi\) has value \(v'\), then any claimed support as the result of such reasoning depends on \(\psi\) having value \(v'\).
  \end{enumerate}

  Claimed support for \(\psi\) having value \(v'\) depends on \(\psi\) has value \(v'\).
  Therefore does not indicate the value of \(\psi\) regardless of whether the claimed support is \mom{}.
  For, if claimed support is misled then \(\psi\) does not have value \(v'\) and hence dependency is broken.
\end{note}

\begin{note}[First conditional]
  Note here to above details on \ref{nI:going-by-value} where I should have noted that this kind of dependency isn't problematic in general, and maybe restate key point.

  Testimony, depends on you being truthful.
  Anything which follows is contingent on this.
\end{note}
...
\begin{note}[Problematic]
  \color{red} Moved as a way to illustrate \eiS{}.
  \begin{illustration}
...
\begin{note}[First question]
  ...
  However, claimed support for \(\phi\) being \mom{} reduces to either the claimed support indicating \(\phi\) has some value it does not have (misled) or the claimed support relies on factors that do not indicate the value of the \(\phi\) (mistaken) --- Cf.~\twodefeaters{}.
...
\begin{note}
  ...
  \begin{assumption}\label{assu:detach-to-claim}
