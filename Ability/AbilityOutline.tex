...
\begin{note}[\nI{}]
  ...
  \begin{proposition}[\nI{-}  --- \nI{}]\label{prem:ni}
    ...
    \item\label{nI:claimed-support} \(S\) has claimed support that some proposition \(\phi\) has value \(v\).
    ...
    \item\label{nI:inclusion} \(S\) considers that the support claimed for \(\phi\) having value \(v\) is mistaken or misled if \(S\) is not in a position to claim support that \(\psi\) has value \(v'\) --- without appealing to \(\phi\) having value \(v\).
    \end{enumerate}
    Then:
    \begin{enumerate}[ref=(\textsf{NI}:\arabic*), resume*=nI_counter]
...
\begin{note}
  ...
  The intuition behind~\nI{} is that an agent is never permitted to claim support for propositions that may reveal problem with claimed support for entailing proposition by appealing to claimed support for entailing proposition if

  claimed support for some proposition does permit an agent to claim support for some implied proposition if inability to claim support for the other proposition would highlight some problem with the claim support for the implying proposition.

  A researcher is never permitted to claim support that some data is an anomaly on the basis of theory if anomaly would show that claimed support for theory is problematic.
  If theory is good, then account for anomaly without appealing to truth of theory.
  ...
  {
    \color{red}
    Eleven differences in \emph{next} pair of images.
    Perhaps works better with colour of jumper, stripes or whatever.
  }

  To illustrate, spot the difference.
  Exhaustive search.
  Instance of \ref{nI:claimed-support}, the proposition that I have found all of the differences between the two pictures is true.
  On the back of the page it states there are eleven differences.
  Instance of \ref{nI:claimed-support}, if it is true that I have found all of the differences between the two pictures, then I have found eleven differences between the two pictures.

  Here, simple deduction.
  If I have found all the differences, then I have found eleven differences.
  Therefore, claim support for having found eleven differences.

  The third condition~\ref{nI:inclusion} queries whether claimed support for finding all differences has a certain property.
  Would I be mistaken or misled when claiming exhaustive search for all differences if I am not in a position to claim support for eleven differences (without appealing to having found all the differences)?
  I am in a position to re-search.
  So, suppose I am not in a position to claim support for eleven differences.
  Perhaps because I found fewer.
  Then, misleading.
  Perhaps I found more.
  Then mistake.
  Given information from the back of the page, there may be some problem with claimed support for having found all.

  Note, this does not suggest that I should revise claim to support.
  There may be some problem, but it does not state there is a problem.

  Nor does it say that I may not reuse the same support --- though I did not count, may be that memory is sufficiently good that I can recall each difference and claim support.

  Still, the fourth condition~\ref{nI:going-by-value} denies that I may claim support by simple deduction.
  Issue is that reasoning by value will implicitly or explicitly hold that I am in a position to establish \(\psi\).
  Therefore, prior to reasoning to value of \(\psi\), I have already assumed what I set out to establish.

  For, when moving from claimed support to value, hold that support is good, but then by 3, this means that I already hold that I am in a position.

  Again, no suggested problem with claimed support for all differences with current picture.
  Potential defeater, but exhaustive search is sufficient to expect that no difference has been missed.
...
{
  \color{red}
  This should be cut down and offered as an alternative to \RBV{}.
}
