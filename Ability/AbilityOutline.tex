...
\begin{note}[\eiS{}]
  ...
  \begin{proposition}[\eiS{-} --- \eiS{}]\label{prop:supp:independence}
    Claimed support is taken to indicate the value of \(\phi\) independently of the value of \(\phi\) or whether the claimed support is \mom{}.
\nolinebreak
    \footnote{
    ...
    {
      \color{red}
      Maybe `Regardless of value', with the mnemonic that you `rov' someone regardless of who they are.
    }
  \end{proposition}
...
\begin{note}[Bootstrapping]
  ...
  Consider the gas guage problem.

  Way to formulate this as an issue for \eiS{} is to hold that the agent doesn't get to reason about the gas guage without taking it to be reliable.
  Indeed, this seems obvious.
  Problem is that it looks as though the reasoning only goes through if the agent is \nmom{} about the reliability of the guage.
  ...
  So, question is whether agent can move from guage to gas even if \mom{}.
  ...
  There's no requirement from \eiS{} alone that the link is going to be broken if the guage isn't (actually) reliable.
\end{note}
...
\begin{note}[Transmission of warrant]
  
\end{note}

\begin{note}
  Basic ideas go back (at least) to the Proper Execution Principle of~\textcite{Wright:1991vn}, and in particular the information-dependence template of~\textcite{Wright:2000tq} and (\citeyear{Wright:2003aa}).\nolinebreak
  \footnote{See also~\textcite{Wright:1986ug,Wright:2002uk} and \textcite{Wright:2004uo}.}

  The idea of information dependence is simple.
  \begin{quote}
...
\begin{note}
  ...
  {% to delete?
    Still, looking at the \emph{form} of \nI{} and \citeauthor{Wright:2000tq} information-dependence template, there is a difference.
  }
  ...
  And, by contrast, the information-dependence template is concerned with preconditions for claiming warrant (or support) for a proposition that `might' be used to claim support.

  How agent moves from \(\phi\) and \(\phi \rightarrow \psi\) to \(\psi\).
...
\begin{note}[The revised template]
  The information-dependence template is intuitive, but has some downsides.

  Foremost, we would like additional clarity with respect to collateral information.
  External preconditions for the effectiveness of your method is somewhat opaque.

  Though, the more significant downside is that the template over-generates.\nolinebreak
  \footnote{For more information, see \textcite[90 \& fn.19]{Wright:2011wn} and then (\citeyear[76--77 \& fn.4]{Wright:2003aa}) --- in that order.}
  This doesn't prevent a comparison, as such, it might lead one to consider the comparison disingenuous.
  ...
  In relation to the information-dependence template discussed above:
...
\begin{note}[Applied to a case]
  ...
  \end{itemize}
  Hence, \wrt{} identifies a failure of transmission in much the same way as we saw above with respect to the information-dependence template.
\end{note}
...
\begin{note}[Entailment]
  ...
  In other words, whether an agent presupposes whenever ~\ref{nI:inclusion} obtains.
  And, conversely, whether ~\ref{nI:inclusion} obtains given a presupposition.
\end{note}

\begin{note}[Gist of why these are different]
  Well, the latter is certainly not the case.
  Presupposition, but not~\ref{nI:inclusion}.

  Testimony.
  No other way.
  But, doesn't mean in position.

  The former.
  More difficult as always going to be some presupposition. (REF FOR THIS.)
  So, non-trivial.
  Counterexample.
  Don't presuppose this.
\end{note}

\begin{note}[Moving to illustrations]
  Expand these two observations by working through the illustrations we gave after the initial statement of \nI{}.
\end{note}

\begin{note}[Difference by illustrations]
  Theory to no counterexample.

  So, claiming support for theory is subjectively compatible with C holding.
  Further, the existence of a counterexample entails C.

  Further, C needs to be incompatible with some presupposition of the cognitive project of obtaining a warrant for the adequacy of the theory.

  Question is whether there's anything plausible for C.

  Here, things are kind of interesting.
  Fair to assume that the agent considered the possibility of counterexamples.
  And, need C to be entailed by a counterexample.
  So, work through this, and it needs to be the case that the agent has some presupposition which means that certain counterexamples don't obtain.

  This is very close to the issue identified.
  However, for \citeauthor{Wright:2011wn} this needs to occur as a presupposition for claiming support/warrant for the theory.

  This seems implausible.
  The issue is that going for all counterexamples.
  So, need all possible counterexamples to be incompatible with some presupposition.
  The point is that the theory gains inductive support.
  And, it's hard to see how this is going to be possible if there's a presupposition.
  This is puzzling.

  It's not to deny that a researcher may have this problem.
  Inductive support is hard to figure out.
  Still, we've been light on the details.
  Hence, applies to theories quite generally.
\end{note}
...
\begin{note}[Second illustration]
  Of course, might be on the edge regarding the first illustration.
  So, here observe that it applies to the second too.


  Here, we can strengthen the issue by considering the second illustration.
  In this case we have a novice.

  Suppose the researcher has a list of all presuppositions.
  Now, seems that the novice could provide researcher with sufficient details.
  And, researcher checks that if things don't go the way they expect, things are bad.

  This is a development of the same intuition.
  So, shouldn't expect distinct results.
\end{note}
...
\begin{note}[Summary on difference]
