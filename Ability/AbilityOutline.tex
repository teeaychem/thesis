...
\subsection{Ability \adA{} and \adB{}}
...
\paragraph*{Initial illustration}
...
\begin{note}[Memory]
  \begin{illustration}\label{ill:ad:proof:mem}
    \begin{enumerate}
    \item \ref{ill:Eproof:def} is an immediate consequence of how (syntactic) theorem-hood is defined.
    ...
    \item So, there exists a syntactic proof of \formula{\forall x Px \rightarrow \lnot \exists x \lnot P x} (using a sound first-order system)
...

  {
    ...
    \footnote{
      ...
      Is it possible to get \ref{ill:Eproof:thm} without \ref{ill:Eproof:exi}?
      It's not obvious, as \ref{ill:Eproof:def} functions as a definition.
      So, it looks as though the agent needs to appeal to \ref{ill:Eproof:exi} in some way.

      Well, I think the interdependence doesn't hold up with respect to memory.
      In particular, \ref{nI:inclusion:bound}.
      It's not clear why being \nmom{} about memory requires that one would be \nmom{} when proving.
      Right, there's nothing about memory itself which seems to ensure the relevant property holds.
      Hence, don't get a violation of \eiS{} because it's not the case that one needs to assume \(\psi\) in order to get \(\phi\) by memory.

      The point with ability is that appeal to general is only good if it applies to the specific case.

      The related point here is that it seems that in order to appeal to memory, one already needs to think that what one remembers holds up.
      But that's not right.
      For, requires that memory is factive.
    }
  }

  Key is that claimed support goes via however claimed support for \ref{ill:Eproof:exi}.
  So, memory, testimony, etc.
\end{note}
...
\begin{note}[Before\dots]
  ...
  % \begin{itemize}
  % \item Sound first-order proof system.
  % \item Application of the rules of the system to particular formulae such the proof begins with no assumptions and the last line of the proof is not part of any assumption made during the course of the proof and the proof.
  % \end{itemize}

  So, given claimed support that the first-order system is sound, the construction of the proof allows one to claim support for the existence of a proof.

  Or, rephrased, given claimed support that the first-order system is sound, one may claim support that a proof exists because they have constructed a proof.
\end{note}

\begin{note}
  Still, appeal to construction of the proof is complex.
  ...
  Sparing the details, claimed support that~\autoref{fig:syntx-prf} is a syntactic proof of \formula{\forall x Px \rightarrow \lnot \exists x \lnot P x} from the construction of~\autoref{fig:syntx-prf} is a matter of claiming support for each step of the construction.

  Indeed, to spare the details in general, let us instead talk of some collection of propositions and steps of reasoning.
  Claiming support that a proof exists from the some construction in the way under discussion is an instance of reasoning from details of the construction to the conclusion that a proof exists.
  Hence, as an instance of reasoning involves certain premises and steps of reasoning.
  And, whatever these turn out to be, the proceed from the construction of the proof rather than from some other source such as memory, testimony, and so on.
\end{note}

\begin{note}
  In other words, one may claim support that a proof of \formula{\forall x Px \rightarrow \lnot \exists x \lnot P x} exists (primarily) \emph{because} of their reasoning from some collection of premises and steps of reasoning concerning the construction to the existence of a proof of \formula{\forall x Px \rightarrow \lnot \exists x \lnot P x}.
\end{note}

\begin{note}[Return to \ref{ill:ad:proof:eve}]
  Now let us return to the reasoning of~\autoref{ill:ad:proof:eve}.

  Consider steps~\ref{ill:ad:proof:eve:app} and~\ref{ill:ad:proof:eve:pos}.

  The idea behind these steps is that the agent is appealing to the equivalent premises and steps of reasoning as those discussed with respect to the proof given in~\autoref{fig:syntx-prf}.

  As talking about the past, make the point a little easier by highlighting those premises and steps the agent used.
  That previous instance of reasoning, and what was involved with that.
\end{note}

\begin{note}
  In addition, from \ref{ill:Eproof:def} understand that a proof is sufficient to establish.

  So, go from all the stuff that makes up the proof.\nolinebreak
  \footnote{
  ...
  Of course, the agent need not remember.
  Still, memory of proving means that there was some instance.
  It seems memory is only good to the extent that this proving was any good.

  {
    \color{red} Note: no conflict with \ESU{}.
    For, did use to claim support at some time in the past.
    Incompatible with a stronger variant of \ESU{}, though.
  }
\end{note}

\begin{note}[Because\dots]
  Summarise with distinct `because\dots' statements.
  \begin{itemize}
  \item Because there is some proof.
  \item Because rules of syntactic system combine to provide a proof.
  \end{itemize}
\end{note}

\begin{note}
  This may seem suspicious.
  The agent may remember proving, but not remember the details of the proof.

  As we'll see, this is incompatible with \ESU{}.
  However, that's not good enough to dismiss at the moment.

  Nor is the goal here to motivate this way of claiming support directly.
  Goal is to give a sufficiently clear account of the idea.
  We won't argue directly.
  Rather, indirect by arguing that this must be a way of claiming support given other constraints.

  It is that argument to which we will turn after clarifying these two ways of reasoning.
\end{note}
...
\begin{note}
  \begin{definition}[\adA{}]\label{def:adA}
    Suppose:
    \begin{enumerate}[label=\textsf{S:\arabic*}., ref=\named{\textsf{S}:\arabic*}, series=adA_counter]
    % \item\label{def:adA:p-to-p} Agent has claimed support that \(\psi\) has value \(v'\) when \(\phi\) has value \(v\).
    \item\label{def:adA:phi} Claimed support for \(\phi\) having value \(v\).
    \end{enumerate}
    Then, agent claims support for \(\psi\) having value \(v'\) by `\adA{}' from \(\phi\) having value \(v\) when:
    \begin{enumerate}[label=\textsf{S:\arabic*}., ref=\named{\textsf{S}:\arabic*}, resume*=adA_counter]
    \item\label{def:adA:psi} Claims support for \(\psi\) having value \(v'\) by \(\phi\) having value \(v\) (obtained from claimed support).
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{definition}
\end{note}
...
\begin{note}
  ...
  \begin{itemize}
  \item If \(\phi\) has value \(v\) then \(\psi\) has value \(v'\).
  \item And, as I have claimed support for \(\phi\) having value \(v\), \(\phi\) has value \(v\), from which is follows that \(\psi\) has value \(v'\).
  \item Hence, claim support that \(\psi\) has value \(v'\).
  \end{itemize}

  Note, \(\phi\) having value \(v\).
  Conditional is useful as need to satisfy the antecedent.
  Agent isn't appealing to some part of \(\phi\) or some other consequence of \(\phi\) having value \(v\), etc.

  Still, the agent need not claim support for a conditional.
  Consider \(A \text{ and } B\) to \(A\).
  Either conditional or a step.
\end{note}

\begin{note}
  \begin{definition}[\adB{}]\label{def:adB}
    Suppose:
    \begin{enumerate}[label=\textsf{I:\arabic*}., ref=\named{\textsf{I}:\arabic*}, series=adB_counter]
    ...
    \begin{enumerate}[label=\textsf{I}:\arabic*., ref=\named{\textsf{I}:\arabic*}, resume*=adB_counter]
