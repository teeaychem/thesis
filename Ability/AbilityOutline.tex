...
\begin{note}[Alternative]
  ...
  Our goal is to motivate an exception to \ESU{}:
  ...
  \EAS{}, when I claim for \emph{that the box has a certain area}, I appeal to dimensions and formula, though as I do not witness the ability, I do not use the premise and step.

  This is by no means obvious.
  A plausible view is that there is an entailment form the proposition \emph{that I have the ability to demonstrate that the box has the certain area} to \emph{that the box has a certain area}, and (in line with \ESU{}) I use the entailment.
...
\begin{note}[Tension, information]
  The tension arises when an agent receives (limited) information which we term `\gsi{-}'
  \begin{definition}[\gsi{}]
    ...
    \end{quote}
    Where \emph{S} is some agent and it is either implicitly or explicitly stated that the specific ability to \(\varsigma\) is instance of the general ability to \(\psi\).
  \end{definition}
...
\begin{note}[Not direct]
  \color{red}
  \Gsi{} contrasts to \dsI{-} --- information which has the semantics that the agent has some specific ability:
  \begin{definition}[\dsI{}]
    \Gsi{-} is information with the semantic content that:
    \begin{quote}
      \emph{S} has the ability to \(\varsigma\).
    \end{quote}
    Where \emph{S} is some agent.
  \end{definition}

  With respect to~\ref{qe:cond}:
  ...
  However, I do not have the ability to read French, and so I do not have the ability to read The Count of Monte Cristo without a translation.

  Hence, the agent is required to obtain~\ref{qe:cons} from~\ref{qe:cond} by endorsing the antecedent --- that they have the (general) ability to reason with the rules of chess --- and so it may not be the case that the agent has the specific ability.
\end{note}
...
\begin{note}[Both things are important]
  ...
  The role o the \aben{} is to highlight that the agent is in a position to claim support for some further proposition if they claim support for specific ability.
  Hence, scenarios combine claiming support \emph{for} specific ability and claiming support \emph{from} specific ability.
  So, partial function characterisation of the claimed support for ability.
  Note, however, that there is a distinction between how an agent reasons about ability, and what ability is.
  Our characterisation is limited to how an agent reasons about ability.
  Indeed, the two components of the scenarios of interest may be seen as capturing particular instances of reasoning from and reasoning to specific ability within some broader instance of reasoning.

  Focus is on interpretation of ability.
  Outline two general schematic interpretations of ability (and argue that these are exhaustive).
  ...
  Argument against \ESU{} and argument for \EAS{}
  Still, interpretation with general applicability is not restricted to instances in which it is forced.
\end{note}
...
\begin{note}[Where the focus is]
  \gsi{} and the \aben{} are constraints which do not hold in all cases of reasoning with specific ability.
  ...
  This does not entail that there is buried treasure in the garden, and testimony or the metal detector may be claimed as support for the ability.
...
\begin{note}[Possible restrictions]
  The important aspect of~\eA{} is that there are cases in which the agent may appeal to ability to obtain support.
  However, it is consistent with~\eA{} that there are cases in which the details of the cases outlined are satisfied, but where kind of support is unsuitable for certain purposes.
...
\begin{note}[Segue]
  \begin{itemize}
  \item Summarise type of scenario.
  \item See where the interest is, ability.
  \item Now, two interpretations.
  \item Outline schematics with respect to the \aben{}.
  \item Then, apply to \gsi{}.
  \end{itemize}
\end{note}
