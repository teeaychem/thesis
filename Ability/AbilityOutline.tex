...
\subsection{Claimed support}
\label{sec:claimed-support-1}

\subsubsection{Two types of defeater}
\label{sec:two-tpyes-defeater}

\begin{note}
  \color{red}
  Point here is that I'm just talking about types of defeaters, so I don't need to introduce claimed support yet.
\end{note}

\begin{note}
  \begin{proposition}[\twodefeaters{-} --- \twodefeaters{}]\label{prop:defs-for-CS}
    There are at least two ways in which some conclusion of reasoning may be defeated.
      \begin{itemize}
      \item The conclusion may (discovered to be) be \emph{misled} by indicating that a proposition has some value that it does not (in fact) have. And,
      \item The conclusion may (discovered to be) be \emph{mistaken} by relying on factors that do not indicate the value of the proposition.
...
\subsubsection{Two assumptions to characterise claimed support}
\label{sec:two-assumpt-regard}

\begin{note}
  State two assumptions.
  The provide a characterisation of claimed support.

  In addition, these assumptions will be appealed to in a core part of the argument (\nI{}).
  Some care, though.
  These are necessary for the argument given to follow.
  However, sufficient to construct an argument.
  It is not clear to me that these are required by \nI{}.

  So, some question about whether the spirit of the argument holds up without these.
  However, 
\end{note}

\paragraph{First assumption}

\begin{note}[Agents are fallible]
  \twodefeaters{} is an account of defeaters.
  However, does not state how these two types of defeaters relate to claiming support.

  The following assumption\dots
...
\begin{note}[\eiS{}]
  ...
  Of course, an immediate consequence of \eiS{} is that there is no requirement that claimed support for some proposition \(\phi\) is \nmom{}.
\end{note}
...
\begin{note}[Expectation]
  So expectation that possible defeaters do not obtain.
  This can still be quite weak.
\end{note}
...
\begin{note}[Expectation, assumption]
  ...
  \begin{restatable}[Expectation with respect to claimed support]{definition}{expectationDef}\label{def:expectation}
    If agent claims support for some proposition and not possible to rule out possible defeaters (of the two types of noted), then the agent \emph{expects} that those possible defeaters do not obtain.\nolinebreak
    \footnote{
...
\begin{note}[Generalising point for K example]
  ...
  \begin{corollary}
    ...
    \item\label{corr:cond:pq} \(\psi\) having value \(v'\) when \(\phi\) has value \(v\).
    \end{enumerate}
    If:
    \begin{enumerate}[resume]
    \item Clause~\ref{corr:cond:pq} also requires that \(\phi\) does not \(v\) when \(\psi\) does not have value \(v'\), and
    \item The agent does not appeal to any other premises other than claimed support for~\ref{corr:cond:p} and~\ref{corr:cond:pq} and some form of conditional detachment.
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{corollary}
  ...
  With this kind of information, there's no way to tell.
  Probability of P is quite high.
  Probability of Q given P is quite high.
  Tells us nothing about the probability of Q in general.
  If Q remains likely given not P, then great.
  If not, then not great.
  Point is, unclear.

  So, contraposition fails, but we get the same problem.
  This only tells us about \(\phi\).
  Still, it may be sufficient to hold that given \(\phi\), probability is so low that \(\psi\) is still okay.

  This extends to the proposition.
  Possible to add stuff in.
\end{note}
