...
\begin{note}
  ...
  \begin{itemize}[noitemsep]
  \item
    Intuition governing \citeauthor{Landman:1992wh}'s account \autoref{cha:sec:fcs-def:progressive-landman}.
  \item
    Background for and details of \citeauthor{Landman:1992wh}'s account \autoref{cha:fcs:sec:Landman:details}.
  \item
    Algorithmic (re)construction of \citeauthor{Landman:1992wh}'s account \autoref{cha:sec:fcs-def:progressive-landman:alg}
  \end{itemize}
\end{note}
...
\begin{note}[Omen]
  ...
  \item
    Algorithm.
  \item
    General role of the algorithm.
  \item
    Minor change to \citeauthor{Landman:1992wh} to allow for a continuation tree (as opposed to branch).
  \item
    Difficulty.
  \item
    Reasonable options and alternative.
  \end{itemize}
\end{note}
...
\begin{note}
  Observation that an agent may be \(\alpha\)ing though the agent does not proceed to \(\alpha\).
  ...
  For, as with Max crossing the road, it seems that Cyril would have gone on to say `\(4\)' followed by `\(2\)', etc., if Cyril has not been interrupted by the bird flying into the classroom.

  For ease of reference, and in line with notation, term event \(e\) and world \(w\).
  \(e\) is an event such that Cyril is stating the first ten digits of \(\pi\).
  However, \(e\) stops after Cyril states the two digits of \(\pi\) in \(w\).

  In a possible world \(u\) close to \(w\), \(e\) develops to the event \(g\) such that \(g\) is an event in which Cyril states the first three digits of \(\pi\).

  \AlgGetPStops{} observes that \(e\) is an event in \(u\) and does not advance \(e\) to \(g\) in \(u'\).
  To trace development of \(e\) to \(g\), \AlgAC{}.

  Interest is now \(g\) in \(u\).
  For, if \(g\) stops in \(u\), \AlgGetPStops{}.

  Indeed, just as Max is hit by a bus, a different bird flies in the classroom and interrupts Cyril.

  Hence, search for \(g\) in some possible world \(u'\) such that \(g\) develops to the event \(g'\) in \(u'\) such that \(g'\) is an event in which Cyril states (at least) the first four digits of \(\pi\).

  Term this process `\emph{drift}'.
\end{note}

\begin{note}
  \AlgFindBranches{} parallels \citeauthor{Landman:1992wh}'s general approach to drift.
  However, there is a significant difference in detail:
  \citeauthor{Landman:1992wh} captures drift by considering possible worlds which are both close to \(u\) and elements of \(\lRwe{w}{e}\).
  \AlgFindBranches{}, by contrast, captures drift by considering possible worlds that are close to \(u\) and events which are elements of \(\mRweSym{}\).%
  \footnote{
...
\begin{note}
 \autoref{PrAl:find-branches:close}.
...
\begin{note}
  ...
  \begin{enumerate}[label=\alph*., ref=(\alph*), noitemsep]
  \item
    Possible worlds such that \(e\) continues.
  \item
...
\begin{note}
  ...
  Start with \(e\) in \(w\), and drift to \(g\) in \(u\).

  Consider possible worlds, but possible worlds relative to the possible world drifted to.
  Partially solves two problems:
  ...
  So long as event continues relative to development, then continue drift.

  Second, whether development is `reasonable'.
  Possible world where Max is not hit by the bus because birds do not exist.
  Start at close worlds to \(w\), then progressive will not be true due to such possibilities.
...
\begin{note}
  ...
  Partial, because between worlds alone does not quite drift.
  For, close worlds, doesn't matter how continuation happens in the world.
...
\begin{note}
  ...
  So, it seems the worlds in which Cyril says `\(n\)' for \(0 \leq n \leq 9\) are all equally close to \(w\).
  Therefore, there is a close world in which Cyril says `\(4\)'.
  Likewise, \(u\).
  Therefore, close world where Cyril says `\(2\)'.
  ...
  However, consider the last instance of the drift.
...
\begin{note}
  ...
  It is not clear how this happens, but equally it's not clear how close world is after tenth digit correct.
\end{note}
...
\begin{note}
  ...
  Go back to Cyril.
  Cyril ends up not choosing randomly.
  However, from \(w\), if Cyril does not choose randomly, then this is because already had the information.
  Result of a distinct event.
  So, \(g\) is not a stage of \(e\).

  Idea, if more plausible initial conditions for the result of drift, then not part of the continuation tree.
  Conversely, continuation tree only if no more plausible way for event to develop.

  Now, so long as there is still path through possible worlds such that guessing, then no better from \(w\).
  ...
  \begin{definition}[Reasonable instance of event]
    ...
    \item
      ...
      \begin{enumerate}
      \item
        There is some world close to \(w\) in which \(g^{+}\) happens and \(g^{+}\) is a stage of \(e\).
      \end{enumerate}
    \end{enumerate}
    \vspace{-\baselineskip}
  \end{definition}
      ...
      \begin{quote}
        ...
        \item[\emph{If}:]
          ...
          \item[\emph{And}:]
            \(\alpha\) is true of \(g^{-}\)
          \end{enumerate}
        \item[\emph{Then}:] There is some world close to \(w\) in which \(g^{+}\) happens and \(g^{+}\) is a stage of \(e\).
        \end{enumerate}
      \end{quote}
  ...
  First, continue going through close worlds such that Rebecca crosses the minefield.
  Indeed, true if random path.
  For, prior to stepping on mine, consider possible world, and so long as there is a path, Rebecca may step forward without stepping on a mine.
...
\subsubsection{Reasonable stops}
...
\begin{note}
  ...
  \item
    Branches (and hence stops) then add branches to tree (\autoref{PrAl:dev-tree:mergebranches}) and make recursive call (\autoref{PrAl:dev-tree:recurse}).
  \end{itemize}
