...
\begin{note}
  ...
  \begin{algorithm}[H]
    ...
    \SetAlgoLined
    \DontPrintSemicolon
    \Input{\( \langle g,u \rangle_{i}\), an indexed event-world pairing}
    \KwResult{Continuations of event \(g\) in world \(u\)}
    \Begin{
      \(\text{Continuations} \longleftarrow \emptyset\)\;
      \If{\(g\) continues in \(u\)}{
        \(g_{m} \longleftarrow \text{max stage of }g\text{ in }u\)\;
        \(t_{0} \longleftarrow \text{time of }g\text{ in }u\)\;
        \(t_{m} \longleftarrow \text{time of }g_{m}\text{ in }u\)\;
        \(g_{x} \longleftarrow g\)\;
        \For{\(0 < j \leq m\)}
        {
          \(g_{i} \longleftarrow \text{development of }g_{x}\text{ at time }t_{j}\)\;
          \If{\(g_{i}\) is a stage of \(g\)}
          {
            \(\text{Continuations} \longleftarrow \text{Continuations } \cup \langle \langle g_{x},u \rangle_{i}, \langle g_{j},u \rangle_{i} \rangle\)\;
            \(g_{x} \longleftarrow g_{j}\)\;
          }
        }
      }
...
\begin{note}
  ...
  \begin{algorithm}[H]
    \SetAlgoLined
    \DontPrintSemicolon
    \Input{Continuations, set of pairings}
    \KwResult{Stopping points for each continuation}
    \Begin{
      \(\text{Stops} \longleftarrow \emptyset\)\;
      \For{\(\langle \langle e,w \rangle_{i}, \langle g,u \rangle_{i+1} \rangle \in \text{Continuations}\)}
      {
        \If{\(\langle g,u \rangle_{i+1}\text{ stops in }u\)}
        {
          \(g' \longleftarrow\text{ the maximal continuation of }g\text{ in }u\)\;
          Stops \(\cup \{\langle \langle e,w \rangle_{i}, \langle g',u \rangle_{i+1} \rangle\}\)\;
        }
      }
      \Return{\(\text{Stops}\)}
    }
...
\begin{note}
  ...
  \begin{algorithm}[H]
    ...
    \DontPrintSemicolon
    \Input{Continuations, set of pairings}
    \KwResult{Possible Stopping points for each continuation}
    \Begin{
      \(\text{Stops} \longleftarrow \emptyset\)\;
      \(\text{CloseWorlds} \longleftarrow \{u' \mid \text{Close}\}\)\;
      \For{\(\langle \langle e,w \rangle_{i}, \langle g,u \rangle_{i+1} \rangle \in \text{Continuations}\)}
      {
        \For{\(u'\) in CloseWorlds}
        {
          {
            \If{\(\langle g,u \rangle_{i+1}\text{ stops in }u\)}
            {
...
\begin{note}
  ...
  No shift in index, as the function identifies the same event in some other possible world.
    ...
    \SetAlgoLined
    \DontPrintSemicolon
    \Input{\(\langle \langle f,v \rangle_{n}, \langle g,u \rangle_{n+1}\rangle, e, w\)}
    \KwResult{Branches for event}
    \Begin{
      ...
      \Return{\(\{\langle \langle f,v \rangle_{n}, \langle g,u' \rangle_{n+1}\rangle \mid u' \in \text{ReasonableWorlds}\}\)}
    }
...
\begin{note}
  ...
  \begin{algorithm}[H]
    ...
    \DontPrintSemicolon
    \Input{%
      \(\text{Tree}\), a set of pairings, indexed\;
      \phantom{\Input{}}\(e\), initial event\;
      \phantom{\Input{}}\(w\), initial world\;
      \phantom{\Input{}}\(n\), index\;
    }
    \KwResult{Tree expanded so that there are no further stopping events}
    \Begin{
      \(\text{NewContinuations} \longleftarrow \emptyset\)\;
      \For{\(\langle \langle f,v \rangle_{n - 1}, \langle g,u \rangle_{n}\rangle\) in Tree}
      {
        \(\text{Continuations} \longleftarrow \AlgAC{}(\langle g,u \rangle_{n})\)\;
        \(\text{NewContinuations} \longleftarrow \text{NewContinuations} \cup \text{Continuations}\)\;
      }

      \(\text{Tree} \longleftarrow \text{Tree} \cup \text{NewContinuations}\)\;
      \lnl{PrAl:dev-tree:Stops:Land}%
      \textcolor{comment}{\texttt{//} \(\text{Stops} \longleftarrow \AlgGetStops{}(\text{NewContinuations})\)}\;
      \lnl{PrAl:dev-tree:Stops:Me}
      \(\text{Stops} \longleftarrow \AlgGetPStops{}(\text{NewContinuations})\)\;
      ...
      {
        \(\text{NewBranches} \longleftarrow \emptyset\)\;
        \For{\(\langle \langle g,u \rangle_{n}, \langle h,u \rangle_{n+1}\rangle\) in Stops}
        {
          \(\text{Branches} \longleftarrow \AlgFindBranches{}(\langle \langle g,u \rangle_{n}, \langle h,u \rangle_{n+1}\rangle, e, w)\)\;
          \(\text{NewBranches} \longleftarrow \text{NewBranches} \cup \text{Branches}\)\;
          }
          \eIf{NewBranches is empty}{
            \textbf{return} Tree\;
          }
          {
            Tree = Tree \(\cup\) NewBranches\;
            \AlgDevelopTree{}(Tree, \(e,w, n+1\))\;
          }
        }
      }
