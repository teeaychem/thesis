...
\begin{note}
  ...
  And, if the agent starts to read the clock the agent is clearly concluding \pv{\propI{It's XX:XX}}{\valI{True}} from some \pool{} which includes the \agents{} perception of the clock.
  ...
  Clause~\ref{def:fc:result} rules out \pv{\propI{It's XX:XX}}{\valI{True}} being a \fc{} from some \pool{} which includes the \agents{} perception of the clock, so long as the agent has not looked at the clock.
  However, Clause~\ref{def:fc:result} allows \pv{\propI{\rootsCon{}}}{\valI{True}} to be a \fc{} from some \pool{} which includes the \agents{} understanding of factorisation in \autoref{illu:gist:roots:F}, given the \agents{} understanding of factorisation is fixed prior to the event of \autoref{illu:gist:roots:F}.%
  \footnote{
