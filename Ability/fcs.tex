...
\begin{note}
  Additional function.
    \begin{quote}
      \textsc{Get-Possible-Continuations}(\(\langle \langle e,w \rangle_{i}, \langle g,u \rangle_{i+1}  \rangle\))\newline

      let \(\Diamond\)Continuations be a new empty set\newline
      let CloseWorlds = \(\{u' \mid u' \text{ is among the closest world to } u\}\)\newline

      \textbf{for} \(u'\) in CloseWorlds\newline
      \phantom{mm} \textbf{if} \(g \text{ is a stage of } h \text{ in } u'\)

      \phantom{mmmm} \(\Diamond\)Continuations = \(\Diamond\)Continuations \(\cup\) \(\langle h,u' \rangle_{i+1}\)\newline
    \textbf{return} \(\{\langle \langle g,u \rangle_{i+1}, \langle h,u \rangle_{i+1} \rangle \rangle \mid \langle h,u \rangle_{i+1} \rangle \in \Diamond\text{Continuations}\}\)
  \end{quote}
...
\begin{note}
  ...
  \begin{quote}
    ...
    \phantom{mmmm} Stops \(\cup\) \(\{\langle g',u \rangle_{i+1} \mid g'\) is the maximal continuation of \(g\) in \(u\}\)\newline
...
\begin{note}
  ...
  \begin{quote}
    ...
    \phantom{mm} let \(\Diamond\)Continuations = \textsc{Get-Possible-Continuations}(\(\langle \langle f,v \rangle_{n - 1}, \langle g,u \rangle_{n}\rangle\))\newline
    \phantom{mm} CurrentTree = CurrentTree \(\cup\) \(\Diamond\)Continuations\newline
    \phantom{mm} let Stops = \textsc{Get-Stops}(\(\Diamond\)Continuations)\newline
