...
\begin{note}
  ...
  Hence, it seems that in all the \closeW{1} to \(w\) Cyril may be interrupted before saying the first ten digits of \(\pi\).
...
\begin{note}
  ...
  So, while stating, Cyril receives information.
...
\begin{note}[Our approach]
  Our proposal roughly, is restrict continuations in the following way:
  \begin{quote}
    \begin{itemize}
    \item[\emph{If}:]
      \begin{enumerate}[label=\alph*., ref=(\alph*)]
      \item
        \label{myR:sketch:a}
        \(g\) in \(u\) is a \drAdj{} continuation of an event \(e\) in world \(w\).
      \end{enumerate}
    \item[\emph{Then}:]
      \begin{enumerate}[label=\alph*., ref=(\alph*), resume]
      \item
        \label{myR:sketch:b}
        In some \closeW{0} to \(w\) where \(g\) develops, the way in which \(g\) continues to develop in \(u\) is as a continuation of \(e\).
      \end{enumerate}
    \end{itemize}
  \end{quote}
  ...
  Or, conversely, \(g\) in \(u\) is part of the continuation tree of \(e\) only if \(g\) in \(u\) is among the most plausible ways for \(e\) to develop from \(w\).
\end{note}
...
\begin{note}
  Let us briefly clarify two parts of the proposal.
  \begin{itemize}
  \item
    our focus on the development of \(g\) rather than \(g\) itself.
  \item
    `way' in which an event develops.
  \end{itemize}
\end{note}

\begin{note}
  Our interest in the development of \(g\) rather than \(g\) itself follows from our understanding of why an instance of the progressive is true.
  ...
  Now, perfective correlate need not be true of \(g\) in \(u\).
  Though, the perfective correlate may be true of some continuation \(g^{+}\) of \(g\) in \(u\).
  Further, it may be the case that though \(g\) in \(u\) is \drAdj{}, the continuation \(g^{+}\) is \ndrAdj{}.
  To illustrate consider again Cyril.
  Cyril started by guessing randomly, but tenth digit was not a random guess.
  So, when moved to world, random, and only as event develops in world does Cyril know.

  Hence, at issue is not whether the event \(g\) is \drAdj{}, but whether \(g\) as \(g\) develops in \(u\) is \drAdj{}.%
  \footnote{
...
\begin{note}
  The motivation for considering the way in which an event is that \(e\) may be that the continuation of \(g\) include \(e\) and hence it may be trivially true that the continuation of \(g\) is a continuation of \(e\).
  ...
  Yet, as the initial event \(e\) included Cyril saying `\(3\)' and `\(1\)', \(e\) must be part of any event captured by \ref{R:event-problem:full}.
  So, it trivially true that in some \closeW{0} in which \ref{R:event-problem:full} happens, \ref{R:event-problem:full} is a continuation of \(e\).%
  \footnote{
    Likewise for Max crossing the road.
  On my understanding, to cross the road, start on one side of the road and end up on the other side of the road.

  However, not always the case.
  Consider X is climbing to the top of the mountain.
  Here, we don't need the whole event.
  }
  ...
  Still, as I am unsure of whether~\autoref{assu:event:way} holds in general, our full proposal will works with sub-events rather than the way in which an event develops.
\end{note}
...
\begin{note}
  \begin{definition}[\drAdj{2} instance of event]
    ...
    \begin{enumerate}
    \item
      For all \(\langle g^{\sharp},u \rangle \in \AlgAC{\langle g,u \rangle}\):
      \begin{enumerate}
      \item
        \(g^{+} \in \{ g^{+} \mid g^{+} \text{ is the restriction of }g\text{ to some subinterval of }g^{\sharp} \}\):
      \begin{enumerate}
      \item
        In the collection \closeW{0} to \(w\) such that \(g^{+}\) happens, \(g^{+}\) is a stage of \(g\).
      \end{enumerate}
    \end{enumerate}
  \end{enumerate}
  \vspace{-\baselineskip}
  ...
  \AlgAC{} is designed to capture the continuations of \(g\), which by definition include \(e\), hence, in addition, we consider the set of sub-events that capture the way in which \(g\) develops, granting~\autoref{assu:event:way}.

  Universal quantification over the sub-events of the continuation of \(g\) is motivated by observing that the continuation of \(g\) in \(u\) will be \drAdj{0} just in case there is no sub-event of the continuation of \(g\) which is \ndrAdj{0}.
  Hence, we test each sub-event to determine whether the sub-event is \ndrAdj{0}.

  The test then follows the intuitive account of our proposal.
  We consider the \closeW{1} to \(w\) such that \(g^{+}\) happens, and ensure that \(g^{+}\) happens as a result of \(e\).
\end{note}
...
\begin{note}
  ...
  \footnote{
    \citeauthor{Landman:1992wh} explicitly holds that the actual world may not be a `reasonable' world.
  ...
  If find out that someone way ready to inform Cyril of the remaining digits of \(\pi\) if Cyril made initial guesses, then seems the progressive is false.
  Indeed, the relevant evaluation of closeness may differ considerably on the information available to the agent when considering some event in progress.

  To illustrate, consider the following \scen{0} from \citeauthor{Engelberg:1999vi} (\citeyear{Engelberg:1999vi})%
  \footnote{
  ...
  Consider the two parts of \drift{}.
  ...
  Inclined to think that Rebecca is not destined to walk into a mine.

  Second.
  As progresses, continuations are result of walking.
  Likewise true, for absence of additional information, it seems Rebecca has no choice but to walk across the minefield.
  ...
  Therefore, so long as there is a \closeW{0} in which Rebecca does not walk into a mine, then it seems the relevant possibilities included in the continuation tree will be such that Rebecca walks to the other side (phew!).
\end{note}
...
\begin{note}
  ...
  In short, allow an event \(e\) to develop by \drifting{0} through possible worlds in which \(e\) happens, but restrict \drift{0} to `\drAdj{0}' events, events such that the way in which the event happens is plausibly as a continuation of \(e\) rather than \dots
