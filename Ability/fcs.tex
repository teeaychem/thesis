...

{
  ...
  \item
    ...
    \item
      ...
      \begin{itemize}
      \item
        \ros{} holds without \emph{partial} \wit{} so long as don't need novel information.
      \end{itemize}
    \end{itemize}
  \end{itemize}

  Then, this proposition becomes deductive.
  For, if \ros{} fails to hold, then some necessary condition.
  However, the necessary condition entails that a partial \wit{} is necessary for the \ros{}.
  But, then by \supportII{} this is not the case.

  Motivate the variant to \supportII{} by:
  \begin{enumerate}
  \item
    Propositional justification.
  \end{enumerate}
  There's also intuitive motivation for the no novel information constraint.
  For, we're looking for something about support from the \agpe{}.
  If need novel information, then this requires some change in the \agpe{}.

  Benefit here is that this weighty assumption is made when things are still being introduced.
