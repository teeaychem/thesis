...
\begin{note}
  \begin{restatable}[Potential event]{definition}{definitionPEvent}
    ...
    \item
      ...
      \item
        \label{def:PE:action}
        There is some action \(a\) avaiable to the agent.
      \item
        \label{def:PE:prog}
        \(\text{Prog}(e, \alpha)\) would be true of the event \(e\) in which \vAgent{} does \(a\).
      \end{enumerate}
    \end{enumerate}
    Where \(\text{Prog}(e, \alpha)\) stands for the progressive from of \(\alpha\) when evaluated with respect to \(e\).%
    \footnote{
      I.e.\ \(\text{Prog}(e, \alpha)\) is true just in case event \(e\) is an event of \vAgent{} \(\alpha\)ing.
    }
  \end{restatable}

  So, there is some action \(a\), and if agent were to \(a\), then agent would be \(\alpha\)ing.

  In the case of concluding, there is some action \(a\), such that if \(a\), then concluding \(\pv{\phi}{v}\) from \(\Phi\).

  Truth conditions for the progressive capture the sense in which there is a `potential' event.

  Definition relies on assumption regarding progressive.
  \begin{assumption}
    \label{assu:prog-modal-shift}
    \(\text{Prog}(e, \alpha)\) is true only if there is some possible event \(e'\) such that \(e'\) is a continuation of \(e\) and \(\text{Past}(e',\alpha)\) is true.
  \end{assumption}

  \autoref{assu:prog-modal-shift}, shift evaluation to some possible event in which something is true.
  Past, as easier to parse.
  Agent is concluding, concludes, concluded.
  Easier, concluded.

  \autoref{assu:prog-modal-shift}, common understanding of modals holds for the progressive.
  Or, if you prefer, a true instance of the progressive entails `bare' possibility modal which entails completion of the action.
  ...
  Assumption is common for modals, and likewise for the progressive.%
  \footnote{
    See, for example:
    (\cite{Bennett:1972uw}),
    (\cite{Dowty:1979vq}),
  ...
  \footnote{
    Different on this point, imperfective paradox.
  ...
  \end{quote}
  Is it possible that Mary wiped out the Roman army?
  Maybe!
  At least, no less possible that Kangaroos without tails.
\end{note}

\begin{note}
  So, we get the modal, but the progressive restricts modal.
  Ensures potential event.
  Sufficient.
  Entailment, so also get weaker event, but the progressive needs to be true, and this is what restricts the relevant possibility.
\end{note}
...
\subsection{\citeauthor{Landman:1992wh}'s account of the progressive}
