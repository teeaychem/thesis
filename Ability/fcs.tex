...
\begin{note}
  The idea of a \fc{0} is central to the way we characterise \ros{1}, and the way we generate counterexamples to \issueInclusion{}.
...
\begin{note}[\fc{2} definition]
  \begin{definition}[\fc{3}]%
    ...
    \item
      ...
      \item
        \label{def:fc:act}
        The \eiw[\(\edn{i +}\)]{0} \vAgent{} does \(a_{i}\) is an \eiw{0} \vAgent{} is concluding \(\pv{\phi}{v}\) from \(\Phi\).
        %
      \item
        \label{def:fc:result}
...
\begin{note}
  ...
  \begin{dets}{obs:var:factor:fc}
    ...
    And, let \(\ed{i}\) capture \(\edn{i}\) is an \eiw{0} the agent is concluding \pv{\rootsCon{}}{\valI{True}} from \(\Phi\).%
    \footnote{
    ...
    Therefore, by \assuPP{} (\autopageref{assu:PP}) there is a possible \eiw{0} the agent concludes \pv{\rootsCon{}}{\valI{True}} from \(\Phi\).
    And, as \(\edn{\flat}\) is the \eiw{0} the agent figures out \rootsConEqFac{}, it is clear \(\ed{i}\) is not an \eiw{0} the agent concludes \pv{\rootsCon{}}{\valI{True}} from \(\Phi\).
    Hence, in order for it to there to be a possible \eiw{0} the concludes \pv{\rootsCon{}}{\valI{True}} from \(\Phi\), there must be some action the agent may do such that the agent is concluding \pv{\rootsCon{}}{\valI{True}} from \(\Phi\) when they do the action.
