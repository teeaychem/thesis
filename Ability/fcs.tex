...
\begin{note}
  ...
  Root, inital event world pair, and then branch from event world pair to some distinct event world pair when index changes.
  %
  \footnote{
...
\begin{note}
  ...
  \begin{algorithm}[H]
    ...
    \Begin{
      ...
      \textcolor{comment}{\texttt{//} \(\text{CloseWorlds} \longleftarrow \text{CloseWorlds} \cap R(e,w)\)}\;
...
\begin{note}
  ...
  For, \citeauthor{Landman:1992wh} identifies branches by considering possible worlds which are both close to \(u\) and elements of \(R(e,w)\).
  \AlgFindBranches{}, by contrast, identifies branches by considering possible worlds that are close to \(u\) and events which are elements of \(R'(e,w)\).%
  \footnote{
    Two changes to \AlgFindBranches{} may be made obtain \citeauthor{Landman:1992wh}'s account.
    First, uncomment Line~\ref{PrAl:find-branches:R}, to restrict the possible worlds considered to those that are element of \(R(e,w)\).
    Second, comment Lines~\ref{PrAl:find-branches:Rprime:check} and~\ref{PrAl:find-branches:Rprime:check:end} to remove the restriction of events to elements of \(R'(e,w)\).
  }
  ...
  Consider \citeauthor{Landman:1992wh}'s \(R(e,w)\).
  Provide variation \(R'(e,w)\).
\end{note}
...
\begin{note}
  ...
  \begin{scenario}
    ...
    Cyril has been asked to recite the first ten digits (of the decimal expansion) of \(\pi\).
    Cyril knows the first ten digits of \(\pi\).
    Cyril utters `\(3\)' followed by `\(1\)'.
    However, interrupted a bird flying into the classroom through the open window.
  \end{scenario}
  Intuitively, as Cyril utters `\(1\)' it is true that:
  \begin{enumerate}
  \item
    Cyril is reciting the first ten digits of \(\pi\).
  \end{enumerate}
  ...
  For example, closest world where Cyril is not interrupted by the bird, Cyril utters `\(4\)' and \emph{then} the bird flies through the window.
  ...
  After drifted from initial world \(w\), so avoided first bus, may drift further from \(w\) to avoid second bus.
...
\begin{note}
  ...
  \autoref{scen:prog:Cyril:know:not} mirrors \autoref{scen:prog:Cyril:know} with the exception that Cyril does \emph{not} know the first ten digits of \(\pi\).

  Cyril guess at random.
  As before gets \(3\) and \(1\) correct and is then interrupted.
  However, as Cyril utters `\(1\)' it is intuitively \emph{not} true that:
  \begin{enumerate}
  \item
    Cyril is reciting the first ten digits of \(\pi\).
  \end{enumerate}
  No close world in which Cyril utters the first ten digits, and likewise there is no close world in which Cyril knows the first ten digits.
  However, possible to drift to such a world.
  For given random choice, worlds in which Cyril utters any of the digits is equally close.
  So, in a close world Cyril utters `\(4\)', and we may continue drifting through possible worlds until Cyril utters the first ten digits of \(\pi\).%
  \footnote{
  ...
  So, each instance is fine, close world.
  However, combination leads to event being sustained in an implausible way.

  Failure not because there is no possibility, but because things required to secure possibility stray too far from initial event.
\end{note}

\begin{note}
  Second part, not a `reasonable' progression.

  Just as things separate from event may stop event in progress, things may sustain an event.

  Role of \(R'(e,w)\) is to capture what is plausible.
  However, way in which \(R(e,w)\) functions is unclear.
  Purpose of \(R'(e,w)\) is to provide incremental improvement.
  In short, `\emph{regulated drift}'.

  Paraphrased, drift is local.
  To event as it has developed.
  Regulated drift is global.
  Development relative to initial event.
\end{note}
...
\begin{note}
  I assume way in which drift through possible worlds is clear.
  Focus on regulation.

  Start with \citeauthor{Landman:1992wh}'s \(R(e,w)\).
  Then, our \(R'(e,w)\).
\end{note}
...
\begin{note}
  ...
  \begin{quote}
    ...
    \mbox{ }\hfill\mbox{(\citeyear[26]{Landman:1992wh})}
  \end{quote}
  Consider a possible world where \(e\) continues, and query whether \(e\) may have continued in \(v\) given what is `internal' to \(e\).

  Phrased otherwise, \(\text{CloseWorlds}\) captures closeness between worlds, while \(R(e,w)\) captures closeness between events.

  Perhaps uttering `\(4\)' is reasonable, but as we progress through `\(1\)', `\(5\)', `\(9\)', `\(2\)', \dots it is, intuitively, quite unreasonable.
  ...
  In this respect, `reasonable chance'.

  However, not just reasonable change.
  For, consider the bus.
  Change of avoiding buses is likewise low.

  Hence, `internal'.
  \citeauthor{Landman:1992wh} doesn't elaborate.
  (And is criticised for this\dots)%
  \footnote{
    More general point.
...
\begin{note}
  Suggestion is to appeal to closeness in the converse direction.
  From \(w\), in some close worlds where \(g\) happens, \(g\) is a stage of \(e\).

  Broad picture.
  Drifting allows trace event through worlds.
  Developing event.
  Path of drift.
  Fix that we get to the event, query whether there is a strictly more plausible path.
  I.e.\ something else sufficiently unrelated to \(e\) happens in \(w\) to get \(g\).
  If not, then seems okay.
  ...
  \begin{definition}[Reasonable continuation of event]
    ...
    \begin{itemize}
    \item
      \(g \in Re(w,e)\). \hfill (\(g\) is a \emph{reasonable continuation} of an event \(e\) in world \(w\).)
    \end{itemize}
  ...
  Up to \(w\) is also important.
  For, \(w\) may be sustaining \(e\) in a strange way already.
  For example, Cyril has been hypnotised or something.%
  \footnote{
  ...
  Also, universal.
  \(R'(e,w)\) isn't suitable for \citeauthor{Landman:1992wh}'s account of the progressive.
...
\begin{note}
  ...
  With respect to \(R'(e,w)\) no.
  For, requires \(g\), which is only obtained by drifting through worlds.

  With respect to \citeauthor{Landman:1992wh}'s \(R(e,w)\), less clear.
