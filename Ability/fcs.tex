...
\begin{note}[\fc{2} definition]
  \begin{definition}[\fc{3}]%
    ...
    \begin{itemize}
    \item
      \(\pv{\phi}{v}\) is a \emph{\fc{0}} from \(\Phi\) for \vAgent{} throughout an event \(e_{d}\).
    \end{itemize}
    ...
    \begin{itemize}
    \item
      For any sub-event \(e^{\flat}\) of \(e_{d}\) there is some action \(a\) \vAgent{} may immediately do such that both \ref{def:fc:act} and \ref{def:fc:result} are true:
      % 
  ...
  The sense in which the relevant conclusion is forgone follows from \ref{def:fc:act} and \ref{def:fc:result}.
