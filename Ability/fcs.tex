...
\begin{note}
      ...
      \item
        ...
        \item
          ...
          \item
            ...
            \footnote{
              I.e.\ \vAgent{} is concluding \(\pv{\phi}{v}\) from \(\Phi\) after doing \(a'\).
            }
          \item
  ...
  Though, neither needs to be the case.
...

  {
    ...
    \item
      Action, concluding.
    \item
      If reasoning, then action.
    \end{itemize}

    So, to get these two things to align, I can adjust \tR{} so that the \(a'\) action comes after reasoning from.
  ...
  % Either attempt to conclude, or investigate without concluding.
  % Investigate without concluding.
  % This assumes some choice, but maybe.
  % In some sense a \fc{}.
  % For, there's no way for the agent to fail to conclude.
  % However, block the (b) condition from holding.

  The key idea, is that no matter how things go, there's nothing which stops \(\pv{\phi}{v}\) following from \(\Phi\).
...
\begin{note}
  \begin{proposition}
    If \(\pv{\phi}{v}\) from \(\Phi\) is a \fc{} then no \pevent{} in which agent concludes something proposition-value pair incompatible with concluding \(\pv{\phi}{v}\) from \(\Phi\).
  \end{proposition}

  So, \fc{}, this means reasoning from \(\Phi\), strengthen to concluding.
  However, if something incompatible, then this means that we don't get concluding.
  For, then after reasoning, it can't be the case that concluding \(\pv{\phi}{v}\).
\end{note}

\begin{note}
  Not possible concluding both \(\pv{\phi}{v}\) and \(\pv{\phi}{\overline{v}}\).

  Argument for this.
  Develops into an event in which agent concludes.
  But, one or the other.
  Not possible to conclude both at the same time.
  For, basic sense in which \(\phi\) has value \(v\).

  So, really, though, this should be stronger.
  At issue is the progressive.
  But this is going to fail even when the agent is not concluding.
  The difficulty is, specifying the conditions under which this is the case.

  Happenstance is not sufficient.
  For, the progressive does not rule out all possibilities.
\end{note}
