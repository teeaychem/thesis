...
\begin{note}
  ...
  \begin{dets}{obs:var:factor:fc}
    ...
    And, as \(\edn{\flat}\) is the event in which the agent figures out \rootsConEqFac{}, it is clear \(\ed{i}\) is not an event in which the agent concludes \pv{\rootsCon{}}{\valI{True}} from \(\Phi\).
...
\section[Limitations of the definition]{Limitations of the definition \hfill (Optional)}


\paragraph*{Practical \fc{1}}


\begin{note}
  A slight aside.
  The abstraction to \prop{0}-\val{0} pairs is designed to allow values other than \valI{True}.
  In particular, practical cases.

  Practical cases are interesting, as multiple incompatible conclusions.
  For example, game of tic-tac-toe.
  Not possible to play multiple cells at the same time, however any cell.%
  \footnote{
    Truth cases plausibly exist, but rest on paradoxes.
    For example, liar sentence, two \fc{1}.
  }

  However, while multiple incompatible conclusions, less clear there are multiple incompatible \fc{1}.
  For, need it to be the case that the agent is concluding.
  Hence, something establishes before the agent concludes.

  \begin{scenario}
    Two agents, \tttX{} and \tttO{}, are playing a game of tic-tac-toe.
    The game progresses to the state depicted below.
  ...
  Clear \tttO{} may play either top-mid, top-right, bottom-right, or bottom-mid.
  Further, there is no chance for \tttO{} to win if \tttO{} plays either top-right or bottom-right, but there is a chance if \tttO{} plays either top-mid or bottom-mid.
  And, any reasoning about the top positions may be mirrored with respect to the bottom positions.

  So, plausible nothing but a metaphorical coin toss distinguishes \pv{\propI{Top-mid}}{\valI{Play}} from \pv{\propI{Bottom-mid}}{\valI{Play}}.
  However, by this observation, there is no event in which the agent is concluding \pv{\propI{Top-mid}}{\valI{Play}} or \pv{\propI{Bottom-mid}}{\valI{Play}}.
  For, prior to either conclusion, nothing favours playing top-mid over bottom-mid and vice-versa.

  This is not to say \pv{\propI{Top-mid}}{\valI{Play}} may not be a \fc{}.
  For example, if \tttO{} has a strong preference for playing anywhere other than the bottom row, unless doing so results in a sure win, then \fc{}.
  {\color{blue} Here, trivial \fc{} as with application.}

  However, from a \pool{} which captures \tttO{}'s preference, \pv{\propI{Bottom-mid}}{\valI{Play}} is not a \fc{}.

  If not, unclear playing.
  Something distinguishes symmetric positions.
  But, bad description to say via tic-tac-toe.

  Of course, maybe something additional which constrains.
  However, \fc{} from restricted \pool{}.
  If not possible to distinguish understanding of tic-tac-toe from additional constraints, then bad description.

  Symmetry is built into the rules.
\end{note}
...
\begin{note}
  ...
  However, \autoref{scen:jojo:c} is plausibly not a \fc{} from any relevant \pool{} as multiple translations are possible.
  For example:
  %
  \begin{center}
    \bgroup
  ...
  Still, in some cases a unique translation is possible.
  For example, 「ソフト＆ウェット」 is translated as 'Soft and Wet'.
  And, given sufficient background context --- a number of other The Artist Formerly Known As Prince references --- and general knowledge, any other translation (e.g.\ 'software and wetware') is easily ruled out.
\end{note}
