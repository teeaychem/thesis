...
\begin{note}
  ...
  Then, \AlgAC{} finds the smallest \(g^{+}\) such that \(g^{+}\) is a continuation of \(g\) in \(u\), and adds \(\langle g,u \rangle_{i}, \langle g^{+},u \rangle_{i} \rangle\) to the set of continuations.
  The set of continuations is now \(\{\langle g,u \rangle_{i}, \langle g^{+},u \rangle_{i} \rangle\}\).
  Hence, the set of continuations is a tree, though \(g^{+}\) may develop further in \(u\) (and by extension \(g\) may also delevelop).
  Hence, \AlgAC{} finds the smallest \(g^{++}\) such that \(g^{++}\) is a continuation of \(g^{+}\) in \(u\), and adds \(\langle g^{+},u \rangle_{i}, \langle g^{++},u \rangle_{i} \rangle\) to the set of continuations.
  Hence, the set of continuations is now \(\{\langle \langle g,u \rangle_{i}, \langle g^{+},u \rangle_{i} \rangle, \langle \langle g^{+},u \rangle_{i}, \langle g^{++},u \rangle_{i} \rangle\}\).
  This process repeats until there are no further continuations of \(g\) in \(u\).
\end{note}
...
\begin{note}
  ...
  Here, \AlgAC{} takes liberty in assuming both
  \begin{enumerate}
  \item There is always a maximal stage for an event,%
    \footnote{
      Note the maximal stage of an event is not necessarily a stopping point of the event.
      The purpose of this liberty is to ensure \AlgAC{} terminates.
    }
    and
  \item Events are indexed by time
  \end{enumerate}
  Still, granting these assumption, the construction of \(\text{Continuations}\) is simple.
  We start at the time of \(g\), and progress through time up until the time of the maximal stage of \(g\).
  At each point in time, we check to see if the development of \(g\) is significant enough to be a stage of \(g\), and if so we include a pairing from the previous stage of \(g\) (tracked by \(g_{x}\)) to the current stage of \(g\).

  Both liberties may be objected to.
  However, \AlgAC{} is of minor importance we take them without much hesitation.
\end{note}
