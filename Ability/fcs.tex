...
\begin{note}[\fc{2} definition]
  ...
  For, of interest is that the agent do something and as a result be concluding \(\pv{\phi}{v}\) from \(\Phi\) throughout the event.
  And, depending on your view of actions, there may be no single action available to the agent throughout the relevant event.
...
\begin{note}
  \begin{application}[A \fc{} of \autoref{illu:gist:roots:F}]%
    ...
    \begin{itemize}
    \item
      For any description \(\edo{\flat}\) true of \(\edn{\flat}\) which includes `the agent is \propM{\rootsCon{}} has \val{0} \valI{True} from \(\Phi\)'.
      \begin{itemize}
      \item
        \(\edo{\flat}\) entails \pv{\rootsCon{}}{\valI{True}} is a \fc{} from \(\Phi\) throughout \(\ed{\flat}\).
      \end{itemize}
    \end{itemize}
  ...
  \begin{dets}{obs:var:factor:fc}
    ...
    So, clause~\autoref{def:fc:ai} and~\ref{def:fc:act} of \autoref{def:fc} is satisfied.
    \medskip
    ...
    \noindent%
    Finally, \(\edn{\flat 1}, \dots, \edn{\flat k}\) was an arbitrary partition of \(\ed{\flat}\) into sub-events and we have shown clauses \ref{def:fc:ai},~\ref{def:fc:act} and~\ref{def:fc:result} are satisfied with respect to any sub-event of the partition, it follows \pv{\rootsCon{}}{\valI{True}} is a \fc{} from \(\Phi\) throughout \(\ed{\flat}\).
  \end{dets}
\end{note}
...
\begin{note}
  ...
  Hence, so long as the agent does not conclude \(\pv{\phi}{v}\) from \(\Phi\) during \(\edn{\flat}\) then throughout \(\edn{\flat}\) there must be some action such that the agent continues to be concluding \(\pv{\phi}{v}\) from \(\Phi\).
...
\begin{note}
  The pair of \autoref{obs:se-inst} and \autoref{obs:var:factor:fc} show that \pv{\rootsCon{}}{\valI{True}} being a \fc{} from \(\Phi\) answers `why' (in the sense of \qWhy{}) the agent concluded \pv{\rootsCon{}}{\valI{True}} from \(\Phi\).

  \autoref{cha:var} makes this connexion in detail, though first we links \ros{1} to \fc{1} to obtain `direct' answers to \qWhy{} \autoref{cha:ros}.
\end{note}
...
\begin{note}
  \illu{1}.
  Pattern.
  Set up the background details.
  Highlight \prop{0}-\val{0} pair of interest.
  Observe the \prop{0}-\val{0} pair is, intuitively, a \fc{}.
\end{note}
