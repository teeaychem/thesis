...
\begin{note}
  \begin{proposition}
    If \requ{}, then \(X\) positive answer to \qzS{} only if \(X\) depends on potential witnessing event.
    \begin{argument}
      ...
      }
      {
        ...
        In \emph{this} presentation, parallel to \citeauthor{Carroll:1895uj}.v
      }
    \end{argument}
  \end{proposition}
  ...
  \begin{corollary}
    Positive answer to \qzs{} not a (typical) proposition-value pair.
  \end{corollary}
...
\begin{note}
  \begin{proposition}
    Positive answer to \qzS{} only if potential witnessing event.
    \begin{argument}
      Primarily by focusing on the observation that concluding is an act.
      \qzS{} asks whether agent would conclude.
      Key point is that concluding is an act.
      So, would conclude, would perform the act.
      Hence, event in which this happens.
      Nothing more than this is really required.
    \end{argument}
  \end{proposition}
\end{note}
...
\begin{note}
  Suppose we add that past witnessing entails potential witnessing event.

  Still the same problem.
  Appealing some something where present reasoning still has the possibility to dead-end.

  Failure to secure potential event.

  So, idea here is that have witnessed is a premise, and entails.
