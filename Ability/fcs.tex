...
\begin{note}[\fc{2} definition]
  ...
  However, \pv{\prop{It's 5 o'clock}}{\val{True}} is not a \fc{} for the agent, as without use of the clock's display, the agent would only conclude it's some time in the afternoon.
\end{note}
...
\begin{note}
  ...
  Specifically, from \citeauthor{Russell:1905aa}'s analysis it follows \prop{Pegasus is winged} is false.
...
\begin{note}[Chess II]
  ...
  \footnote{
    ...
    \footnote{
      ...
      I gave up after fifteen minutes or so and will not return.
    }
\end{note}
...
\begin{note}
  ...
  \item
    \label{ill:fcs:kw:cA}
    \pv{\prop{If \emph{S} knows whether \emph{p} and \emph{S} believes that \emph{p}, then \emph{p}}}{\val{True}}
  \item
    \label{ill:fcs:kw:cB}
    \pv{\prop{If \emph{S} knows whether \emph{p} and \emph{S} believes that not-\emph{p}, then not-\emph{p}}}{\val{True}}
  \end{enumerate}
\end{quote}
...
\begin{note}
  ...
  \begin{argument}{prop:fcs-only-if-pot-support}
    ...
    Hence, in whatever sensthe event is \potential{0}, a \ros{} between \(\pv{\psi}{v'}\) and \(\Psi\) is likewise \potential{0}.
  \end{argument}
\end{note}
...
\begin{note}
  ...
  \autoref{prop:pot-support-onlyIf-support}.
...
\begin{note}
  \autoref{prop:fcs-only-if-support} is key.
  For, given \autoref{prop:fcs-only-if-support}:
  ...
  \begin{argument}{prop:fc-wit}
    \ref{prop:fc-wit:fc} entails \ref{prop:fc-wit:ros} by \autoref{prop:fcs-only-if-support}.
...
\begin{note}
  \autoref{prop:fcs-only-if-support} and \autoref{prop:fc-wit} are the end of this chapter.

  We close with a broader perspective.
\end{note}
  ...
  \item
    ...
    \item
      ...
      \item
        ...
        Throughout event in which \vAgent{} concludes \(\pv{\chi}{v''}\) from \(X\), there is some action \(c\) s.t.\ \vAgent{} is concluding \(\pv{\psi}{v'}\) from \(\Psi\) after \(c\) is done.
      \end{enumerate}
    \end{itemize}
  \end{enumerate}
