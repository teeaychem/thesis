...
\begin{note}[\fc{2} definition]
  ...
  \noindent%
  The core idea of a \fc{} is captured by the availablility of an action such that \ref{def:fc:act} and \ref{def:fc:result}.

  For, \ref{def:fc:result}, the agent is concluding \(\pv{\phi}{v}\) from \(\Phi\).
  By \assuPP{}, there is a possible event in which the agent concludes \(\pv{\phi}{v}\) from \(\Phi\).

  Event in progress captures in part the sense in which the conclusion is `foregone'.
  For, something prior to conclusion is sufficient to ensure an event in which the agent concludes is in progress.
  The remaining part of the sense in which the conclusion is `foregone' is captured by \ref{def:fc:result}.
  For, when the agent concludes, do not require any novel \evals{}.
...
\begin{note}
  puzzle is designed so that, once read, you have all the information required to solve the puzzle, given the appropriate background.
  In this respect, if you start an attempt, you are concluding.
\end{note}

\begin{note}
  We limit ourselves to a single example from \citeauthor{Dudeney:1995aa}'s \citetitle{Dudeney:1995aa}.
  However, contains roughly 500 or so examples.%
  \footnote{
...
\begin{note}
  Talk of \emph{je ne sais quoi} is quite vague.
  To give a slightly more precise example, we present and work through a chess puzzle from \citeauthor{Emms:2000aa}' \citetitle{Emms:2000aa}.
\end{note}
