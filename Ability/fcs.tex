...
\begin{note}
  \fc{3} are defined via an event in which an agent is concluding.
  And, given \assuPP{}, an event in which an agent is concluding entails the existence of a (possible) event in which the agent concludes.
  So, definition similar to \autoref{def:fc} may be given which uses an event in which an agent concludes, rather than an event in which an agent is concluding.
...
\begin{note}
  ...
  An agent is concluding just in case an event in which the agent concludes is in progress.
  Hence, sense of possibility with which an event in which the agent concludes is possible is constrained by an understanding of events in progress.
  ...
  For example, it is unlikely there is an event in which the agent is \emph{concluding} \pv{\propI{It is not possible for White to checkmate in a single move}}{\valI{True}} from \(\Phi\).
\end{note}
...
\begin{note}
  ...
  \noindent%
  Intuitively, \autoref{scen:jojo:c} is a \fc{}, so long as the agent is fairly good at translation.
  Hence, something about the agent ensures they may make the translation.
