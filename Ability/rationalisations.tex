...
\begin{note}
  ...
  \begin{question}{questionWhyR}{\qWhyR{}}
    ...
    \begin{itemize}
    \item
      Which rationalisations explain \emph{why} \(e\) is an \eiw{0} \vAgent{} does \(a\)?
    \end{itemize}
  ...
  \begin{question}{questionHowR}{\qHowR{}}
    ...
    \begin{itemize}
    \item
      Which past or present events explain \emph{how} \(e\) is an \eiw{0} \vAgent{} does \(a\)?
    \end{itemize}
    \vspace{-1.5\baselineskip}
  \end{question}
  ...
  In contrast, \qWhy{} seeks an understanding of the way an event developed such that an agent concludes \(\phi\) has value \(v\) from \(\Phi\) in terms of \fingfr{1}.
  Likewise, \qHowR{} seeks an understanding of the way an event developed such that an agent did some action in terms of what happened, while \qHow{} is a restriction of \qHowR{} to \eiw{1} the relevant action is `concludes \(\phi\) has value \(v\) from \(\Phi\)'.
\end{note}
...
\begin{note}
  ...
  \end{itemize}
  % 
  Given this conditional, \fingfr{1} which answer \qWhy{} are constrained by answers to \qHowR{}.
...
\begin{note}
  Intuitively, certain \fingfr{1} correspond to rationalisations.
  ...
  \item
    \begin{itenum}
    \item[\emph{If}:]
      \(e\) is an \eiw{0} an agent concludes \(\phi\) has value \(v\) from \(\Phi\).
    \item[\emph{And}:]
  ...
  \begin{observation}
    ...
    In particular, there is no way link \fingfb{\(\pv{\psi}{v'}\)}{\(\Psi\)} to a rationalisation if either \(\psi\) is distinct from \(\phi\) or \(v\) is distinct from \(v'\).
  \end{observation}
  ...
  \begin{motivation}{obs:whRdifficult}%
    ...
    By assumption, the \fingfr{0} answers \qWhy{}, and so to capture the explanatory relation between the \fingfr{} and the \agents{} conclusion of \(\pv{\phi}{v}\) from \(\Phi\), \(\phi\) or \(v'\) must be referenced by \(R\).
  ...
  However, as a rationalisation is, at best, restricted to a specific \fingfr{1}, rationalisations are not suitable for capturing \fingfr{1} in general.
  ...
  \end{itemize}
  % 
  Then a more carefully developed version of \autoref{obs:whRdifficult} entails it is not possible for \fingfb{\(\pv{\psi}{v'}\)}{\(\Psi\)} to answer \qWhy{} (where \(\psi\) is distinct from \(\phi\) or \(v'\) is distinct from \(v\)).
  ...
  And, our interest with \issueInclusion{} is restricted to the \eiw{0} an agent concludes \(\phi\) has value \(v\) from \(\Phi\).
...
\begin{note}
  ...
  \begin{itemize}
  \item
    \emph{Why} is \(e\) is an \eiw{0} \vAgent{} concludes \(\phi\) has value \(v\)?
  \item
