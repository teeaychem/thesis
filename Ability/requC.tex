...
\begin{note}
  ...
  \begin{definition}[A \requ{0}]
    ...
    \begin{itemize}
    \item
      \(\pvp{\psi}{v'}{\Psi}\) is a \emph{\requ{}} of \(e\) under (an incomplete) description \(d\) being an event in which \vAgent{} is concluding \(\pv{\phi}{v}\) from \(\Phi\).
    \end{itemize}
...
\begin{note}
  The definition of a \requ{} appeals to an (incomplete) description \(d\) of an event in order to capture relation between agent concluding and \fc{}.
  As the \itc{} is a material conditional, trivially true.

  Indeed, same as \qWhyV{}.

  Whether there is some feature of the event capture such that it is not possible for agent concluding while not a \fc{}.
...
\begin{note}
  ...
  \begin{argument}{prop:hinge}
    \autoref{prop:hinge} is immediate from \autoref{prop:tC-and-fc} (\autopageref{prop:tC-and-fc}) and the definition of a \requ{} (\autoref{def:requ}, \autopageref{def:requ}).
