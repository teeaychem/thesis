...
\begin{note}
  \requ{3} entails conditional holds with respect  to event taking place, we state in both direct and contrapositive forms:
...
\begin{note}
  \begin{proposition}
    ...
    \item[\emph{If}:]
      ...
      \item
        ...
        \(\pvp{\psi}{v'}{\Psi}\) is representative of type \(T\).
      \end{enumerate}
    \item[\emph{And}:]
      \begin{enumerate}[label=\alph*., ref=(\alph*), resume*=propHingeSer]
      \item
        \label{prop:hinge:typical}
        \(e\) is event in which \vAgent{} is \sR[concluding]{} of type \(T\) \(\pv{\phi}{v}\) from \(\Phi\).
      \end{enumerate}
    \item[\emph{Then}:]
      \begin{enumerate}[label=\alph*., ref=(\alph*), resume*=propHingeSer]
      \item
  ...
  In other words, in order for concluding to be of some type, necessary that other instances of the type are \fc{1}.

  \begin{argument}{prop:hinge}
    Our argument for \autoref{prop:hinge} establishes the equivalent conditional:%
    \footnote{
      The conditional is of the form:%
      \hfill%
      \(\text{If } (A \text{ and } B) \text{ then } C\).

      By Import-Export, the conditional is equivalent to:%
      \hfill%
      \(\text{If } A \text{ then } (\text{If } B \text{ then } C)\).

      And, by contraposition the conditional is equivalent to:%
      \hfill%
      \(\text{If } A \text{ then } (\text{If }\text{\emph{not}-}C \text{ then } \text{\emph{not}-}B)\).

      Finally, by a second application of Import-Export we have:%
      \hfill%
      \(\text{If } (A \text{ and }\text{\emph{not}-}C) \text{ then } \text{\emph{not}-}B)\).
    }
    \begin{quote}
      If \ref{prop:hinge:rep} holds and \ref{prop:hinge:requ} does not hold, then \ref{prop:hinge:typical} does not hold.
    \end{quote}

    We assume \(a\), \(b\) and suppose for a contradiction \(c\) does not hold.

    Fix \(T\) as representative of some type, and suppose \(\pvp{\psi}{v'}{\Psi}\) is representative of type \(T\).
