...
\begin{note}
  ...
  So, I am \requSens{0} to whether or not reasoning is \tR{0}.
...
\begin{note}
  ...
  \requ{3} may be complex when \requ{} is distinct from what the agent is doing.
  However, in the case where coincide, trivial.

  Still, somewhat surprising.
  For, if concluding, then \fc{0}.
...
\begin{note}
  ...
  The central proposition of this section is \autoref{prop:hinge} which, in short, states that certain (non-trivial) proposition-value-\poP{} pairings are entailed by an agent concluding \(\pv{\phi}{v}\) from \(\Phi\) by \sTR{}.
...
\begin{note}
  ...
  Second point, various examples of proposition-value-\poP{} pairings which fail to be \requ{1}.
...
\begin{note}
  \begin{proposition}
    ...
    \item[\emph{If}:]
      ...
      \item
        ...
        \(e\) is an event in which \vAgent{} is reasoning \(\pv{\phi}{v}\) from \(\Phi\) by \sTR{} of type \(T\).
      \end{enumerate}
    \item[\emph{And}:]
  ...
  Or:
  In order for concluding to be of some type, necessary that other instances of the type are \fc{1}.
...
\begin{note}
  ...
  Have seen \sTR{} is strictly stronger.
  (\autoref{prop:sTRStronger}, \autopageref{prop:sTRStronger}).
  However, opportunity.
\end{note}

\begin{note}
  This is the motivation for \requ{1}.
