...
\begin{note}
  ...
  \begin{definition}[A \requ{0}]
    ...
    \begin{itemize}
    \item
      There is some (incomplete) description \(d\) such that:
      \begin{itemize}
      \item
        \(d\) is true of \(e\).
      \item
...
\begin{note}
  The definition of a \requ{} appeals to an (incomplete) description \(d\) of an event.%
  \footnote{
    Similar to our use of an (incomplete) description to state \qWhyV{} on \autopageref{questionWhyV}.
  }

  \itc{}, so either not concluding or \fc{}.

  However, with (incomplete) description ignore either.
  Something about the event secures requirement.

  When apply, exclude from description whether or not the agent is concluding \(\pv{\phi}{v}\) from \(\Phi\).

  So, whether there is some feature of the event capture such that it is not possible for agent concluding while not a \fc{}.

  Purpose of this is not to secure \requ{} by alternative events, but to ensure way in which get \requ{} does not rest on something trivial.
  Of course, it may be the case that \(d\) is narrow, and hence \itc{} is trivially true.
  However, never assume this.

  This technicality aside, the definition of a \requ{} is minimal.
  Note, in particular, it does not follow from \(\pvp{\phi}{v}{\Phi}\) being a \requ{} of an agent concluding \(\pv{\phi}{v}\) from \(\Phi\) that either:
  \begin{itemize}
  \item
...
\begin{note}
  ...
  % \end{enumerate*}
  Given the description of \autoref{illu:lost-key} it seems:
  \begin{itemize}
  \item
  ...
  \requ{2}.
  However, only given the description.
...
\begin{note}
  \begin{proposition}[\tC{2} and \requ{1}]
    ...
    \begin{itenum}
    \item[\emph{If}:]
      Clauses~\ref{prop:hinge:A:rep},~\ref{prop:hinge:A:tI},~and~\ref{prop:hinge:A:novel} jointly hold for any event \(e'\) for which \(d\) is true:
  ...
  \begin{argument}{prop:hinge}
    Consider an arbitrary event \(e^{a}\) such that Clauses~\ref{prop:hinge:A:rep},~\ref{prop:hinge:A:tI},~and~\ref{prop:hinge:A:novel} jointly hold.
    Suppose \tCV{}.
    Then, by \autoref{prop:tC-and-fc}, \fc{}.

    Therefore, \(d\) is true of \(e\), then by \autoref{def:requ}, \requ{}.
  \end{argument}
  ...
  Now, to get \tRep{}, plausibly the case that \(d\) is narrow.
  However, at issue is not whether there are other events for which \(d\) is true, but the way \requ{} follows.
  (As with the way this is introduced.)
\end{note}
