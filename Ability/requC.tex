...
\begin{note}
  ...
  The final section of this chapter, \autoref{cha:requs:tR}, motivates the existence of \requ{1} via \tC{}.
\end{note}
...
\begin{note}
  ...
  So, I am {\color{blue} keen} to whether or not reasoning is \tC{0}.
  Hence, any doubt during \autoref{scen:squish} that reasoning is \tC{0} would lead to preventing the event from developing further.
\end{note}
...
\begin{note}
  Regardless of \agpe{my}, then if \requ{} fails to hold, it is unclear whether \tC{} by reasoning.
  ...
  Without \fc{1}, then is nothing to suggest that \tC{} by reasoning.
\end{note}
...
\begin{note}
  ...
  In turn, we observe this puts pressure on \emph{maintaining} that an agent is concluding \(\pv{\phi}{v}\) from \(\Phi\) by \tC{} while \emph{denying} that other instances of the type of reasoning are \requ{1}.
