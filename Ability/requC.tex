...
\begin{note}
  ...
  \begin{restatable}[A \requ{0}]{definition}{defRequ}
    ...
    \begin{itemize}
    \item
      The following conditional is true, throughout \(e\):
      \begin{itemize}
      \item[\emph{If}:]
  ...
  \requ{3} capture a single idea which we state in both direct and contrapositive forms:
  \begin{itemize}
  \item
...
\begin{note}
  The role of \ninf{} is implicit in understanding the way in which .

  For, intuitively, if \(\pv{\psi}{v'}\) is not a \fc{} from \(\Psi\), then the agent stops the event from developing into an event in which the agent concludes \(\pv{\phi}{v}\) from \(\Phi\).%
  \footnote{
...
\begin{note}
  ...
  \begin{proposition}
    ...
    \begin{itemize}
    \item
      \(\pvp{\phi}{v}{\Phi}\) is not \emph{necessarily} a \requ{} of \vAgent{} concluding \(\pv{\phi}{v}\) from \(\Phi\).
    \end{itemize}
    \vspace{-\baselineskip}
  \end{proposition}
...
\begin{note}
  ...
  However, interest is in cases in which the agent is concluding.
  ...
  \item
    Agent is/would be \requSens{0} to whether or not \(\pv{\psi}{v'}\) from \(\Psi\) is a \fc{}.
  \item
    Agent would exert \ninf{2} over event due to \requSens{0}.
  \end{itemize}
...
\begin{note}
  In this respect, Hume.
  Possible for b to happen without a.
  Therefore, in what sense is a the cause of b.
  Only conjunction.%
  \footnote{
    Specifically, \citeauthor{Schaffer:2010vq}.
    ...
    For, \citeauthor{Schaffer:2010vq}'s demon is getting a belief arbitrarily.
    While, basing involves something non-arbitrary.
    Of interest is not whether or not the belief \emph{seems} to be properly based.

    Though, this is not to say basing relation avoids worries.
    With \citeauthor{Schaffer:2010vq}, concern about any instance of the basing relation.
    Which, is what we end up with.

    And, other demons for specific theories.
    With doxastic, regular demon changes how things are.
    And, with causal, Humean demon.
  }

  Similar.
  Causation, link between two things, here three different components.

  Argument concerns understanding of event.
  In particular, whether an agent's reasoning is \sR{0}.

  Hume gets no generality.
  I think this is bad enough.

  Goal is to argue for this.
\end{note}
...
\begin{note}
  ...
  \item
    ...
    Catch mistakes.
  \item
    \ninf{2} stop after catching mistakes.
  \end{itemize}
\end{note}
...
\subsubsection{Being \requSens{0}}

\begin{note}
  Previous section, \fc{1}.
  Specific with respect to \illu{1}.
  However, generalises.
\end{note}

\begin{note}
  Turn to being \requSens{0}:

  If not \fc{} then not concluding.
\end{note}

\begin{note}
  Hume and \citeauthor{Schaffer:2010vq}, scepticism.

  What is at issue is akin to scepticism.

  Existence of \requ{1}, specifically \requSens{0}.
  So, in order for this to fail, need there to be a systematic way in which being \requSens{0} fails.
\end{note}
...
\begin{note}
  \begin{proposition}
    \label{prop:hinge}
    If not \requSens{0}, then no \sR{}.
  \end{proposition}

  \begin{argument}{prop:hinge}
    Okay, want to show, not \sR{} then stop.

    So, suppose not \sR{}.
    Then, this means that something goes wrong at some point in the reasoning.
    So, this means there is a step that does not generalise.

    Right, so there's something that ends up not holding for other instances of the type.

    Isolate this.
    As agent is not \requSens{0}, then, this step, use this.
    But, this means, that for any proposed instance of the type, the agent is okay with using.
    But, then something wrong with \fc{1}.

    Oooooh, wow.
    This is wild.
    This is really neat.

    And, if not \fc{}, then not of type.
    Hence, not of \sR{}.
  \end{argument}
...
\begin{note}
  ...
  finding the problematic step.
  However, in cases and out cases.
  This is all being \requSens{0} amounts to.
...
\begin{note}
  Grant that \fc{} and \requSens{0}.
  Still, needs to be the case that \ninf{}.

  Well, this tails from the previous proposition.
  For, if no \ninf{}, then continue.
  Hence, problems with \fc{1}.
\end{note}
