...
\begin{note}
  ...
  Abstractly, there are cases in which there is nothing which prevents an agent from destroying evidence that is necessary for an agent to reach some conclusion prior to forming the conclusion.
  And, there are cases in which an agent may chose to appeal to additional resources beyond \(\Phi\) in order to conclude \(\pv{\phi}{v}\).
\end{note}
...
Idea here.
With lost keys, something which conflicts.

So the idea is to have some rule.
And, no conclusion due to what else this would entail.

\begin{note}
  \begin{illustration}[Goldbach conjecture]
    Goldbach conjecture:

    every number that is a sum of two primes can be written as a sum of “as many primes as one wants”.

    Know the conjecture has been verified \emph{up to} \(4 \times 10^{14}\) (\cite[cf.][]{Richstein:2001aa}).
  \end{illustration}

  Whether conjecture is true or not, numbers too big to think about while riding the train.
\end{note}
...
\begin{note}
  \begin{restatable}[Key card]{illustration}{scenarioKeycard}
    Going to work.
    Need keycard.
    Driving along.
    A few minutes.
    Some traffic.
    Start estimating arrival time.

    Conclusion, arrive in roughly half an hour or so.
  \end{restatable}

  If conclude did not pack, stop whatever is happening.

  However, no chance of reaching this conclusion.
  Careful with packing, recall each thing.
\end{note}

\begin{note}
  Main point.
  Don't need to think about keycard.

  Just, if not, then problem.

  And, this happens.
  Do turn back.
  Do recall failing to pack item for a trip.
\end{note}

\begin{note}
  Uh, this is not clear.
  Could just be random prompting.

  But, whether or not \fc{}, that's the issue.
\end{note}

\begin{note}
  Here, it need not be the case that arrive in half an hour, nor that have keycard.
  Both may be false.
  At issue is whether stop.
\end{note}
...
\begin{note}
  ...
  So, I am sensitive to whether or not reasoning is \sR{0}.
...
\begin{note}
  Though \requ{1} are not incompatible with \issueConstraint{} (see \autoref{prop:requ-not-n-ce} on \autopageref{prop:requ-not-n-ce}), \requ{1} have a key role in identifying counterexamples to \issueConstraint{}.
