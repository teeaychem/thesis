...
\begin{thesis}[First pass]
  A constraint on answers to: \emph{Why} an \eiw{0} an agent concludes happened

  By answers to: \emph{How} the \eiw{0} an agent concludes happens
...
\end{comment}
  ...
  \begin{itemize}[label = \squareBullet]
  \item
    An agent concludes a \prop{0} \(\phi\) has some \val{0} \(v\) \underline{\textbf{from}} some \pool{} \(\Phi\). \hfill (During some event.)
    \begin{itemize}
    \item
    ...
    \end{itemize}
  \item
    A \prop{0} \(\psi\) having some \val{0} \(v'\) \underline{\textbf{\fof{}}} some \pool{} \(\Psi\), from an \agpe{}. \hfill (Relative to some event.)
    \begin{itemize}
    \item
...
\begin{note}
  ...
  \begin{itemize}
  \item[\hand]
    \qWhy{} asks about `explanatory' \fingfr{1}. \hfill (Non-technical)
  \item
...
\begin{note}
  ...
  \item
    ...
    \begin{itenum}
    \item[\emph{If}:]
      For the agent, it is not the case \(\phi\) having \val{} \(v\) \fof{} \pool{} \(\Phi\).
    \item[\emph{Then}:]
      There is nothing which ensures the agent concluding \(\phi\) has \val{} \(v\) from \pool{} \(\Phi\).
    \end{itenum}
  \end{itemize}
...
\begin{comment}
  Intuitively:
    ...
    \item
      \begin{itenum}
      \item[\emph{If}:]
        \prop{2} \(\phi\) having \val{} \(v\) is a \fc{} from \pool{} \(\Phi\).
      \item[\emph{Then}:]
        \prop{2} \(\psi\) having \val{} \(v'\) is a \fc{} from \(\Psi\).
      \end{itenum}
    \item
      \prop{2} \(\phi\) having \val{} \(v\) is a \fc{} from \pool{} \(\Phi\).
    \item
    ...
    What secures the relevant \fc{1} is that \(\ed{\flat}\) is an \eiw{} an agent concluding \prop{} \(\phi\) has \val{} \(v\) from \pool{} \(\Phi\).
\end{comment}
...
\begin{note}
    ...
    \begin{itemize}
    \item
      \prop{2} \(\phi\) having \val{} \(v\) is a \emph{\fc{0}} from \pool{} \(\Phi\) for agent \vAgent{} through an event \(\ed{}\).
    \end{itemize}
    ...
    \item
      ...
      \item
        \label{def:fc:act}
        The \eiw[\(\edn{a_{i}}\)]{0} \vAgent{} does \(a_{i}\) is an \eiw{0} \vAgent{} is concluding \prop{} \(\phi\) has \val{} \(v\) from \pool{} \(\Phi\).
        %
      \item
        \label{def:fc:result}
