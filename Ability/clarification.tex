...
\end{quote}
  ...
  \footnote{
    Consider the follow passage from \citeauthor{Hume:2011aa}'s \textcite{Hume:2011aa}:
    \begin{quote}
      ...
      \mbox{ }\hfill\mbox{(\hyperref[cite.Hume:2011aa]{T.2.3.3})}
    \end{quote}
  }
...
\begin{note}
  \begin{idea}[Support I]
    ...
    \item[\emph{If}]
      ...
      \item
        \vAgent{} has concluded some proposition \(\phi\) has some value \(v\) from some pool of premises \(\Phi\).
      \end{enumerate}
      \item[\emph{then}]
        \begin{enumerate}[label=\alph*., ref=(\alph*), resume]
        \item
          A \emph{\support{}} held between \(\pvp{\phi}{v}{\Phi}\) and \(\phi\), when concluding \(\pvp{\phi}{v}{\Phi}\) from \(\Phi\), from the \vAgent{}' perspective.
        \end{enumerate}
      \end{itemize}
      \vspace{-\baselineskip}
  \end{idea}
...
\begin{note}
  ...
  Forward looking.
  Return to this in \autoref{cha:clarification:support-wo-concluding}.
  Where, clarify how this possibility may arise.
\end{note}
...
\begin{note}
  Final piece of motivation for support.

  Clarifying something as a premise.
\end{note}

\begin{note}
  Highlight it's \support{}, rather than what is part of the relation.
\end{note}
...
\begin{note}
  ...
  Still, the \support{} gets embedded.%
  \footnote{
...
\subsection{Support without concluding}
\label{cha:clarification:support-wo-concluding}
...
\begin{note}[The variant]
  ...
  \begin{question}[\qWhyV{}]
    \label{q:why:v}
    \emph{Which} proposition-value-premises pairings are such that concluding depends on support between, from the agent's perspective?
  \end{question}
...
\begin{note}
 Following:
  ...
  \begin{restatable}[]{proposition}{propVariationsAndInclusion}
    ...
    \begin{itemize}
    \item
      \support{2} between \(\pv{\psi}{v'}\) and \(\Psi\) is, in part, an answer to why an agent concluded \(\pv{\phi}{v}\) from \(\Phi\).
    \end{itemize}
    \emph{Only if:}
    \begin{itemize}
    \item
      The agent has witnessed reasoning to \(\pv{\psi}{v'}\) from \(\Psi\).
    \end{itemize}
  \end{restatable}
