...
\begin{note}
  ...
  \begin{definition}[\sR{2}]
    The reasoning of \(e\) is \sR{0}, just in case, instance of a general type of reasoning.
  \end{definition}

  Basic idea, generality.

  Type, whether reasoning is a instance of that type.%
  \footnote{
  ...
  Classify some general kind of reasoning, and then say whether instance of reasoning is of that general kind.

  Addition.
  2 + 2 = 4.
  Though, is this an instance of addition?

  Or, is this multiplication instead of addition?
...
\begin{note}
  ...
  However, \gR{} does not require rule following.
  Rather, \gR{} only requires the rough phenomenon that \citeauthor{Boghossian:2008vf} argues rule following is the only plausible explanation of.
\end{note}
...
\begin{note}
  ...
  Whatever the generality of \gR{0} on our understanding, it may fail for arbitrary pairs.
...
\begin{note}
  Primary upshot is that \gR{} does not depend on resolutions to rule following paradox.

  To illustrate.
  Strictly, \gR{} does not depend on addition.
  Compatible that \gR{} is smaddition(?)
...
\begin{note}
  ...
  \gR{}.
  However, \gR{} does not entail \sR{}.
\end{note}

\begin{note}
  \gR{} is compatible with dispositionalism.

  \begin{quote}
    But a capacity to grasp inﬁnitely long propositions—the inputs in the rulefollowing case—does not follow from our nature as thinking beings, and certainly not from our nature as physical beings. In fact, it seems pretty clear that we do not have that capacity and could not have it, no matter how liberally we apply the notion of idealization.
  \end{quote}
\end{note}
...
\paragraph{\citeauthor{Carroll:1895uj}}

\begin{note}
  The difficulty here is similar to a difficulty found on a reading of \citeauthor{Carroll:1895uj}'s \citetitle{Carroll:1895uj}.

  At issue is not whether we need a belief, but the generality of modus ponens.
  The point is, asking the Tortoise to infer according to a general rule, without giving an account of what establishes the link between the antecedent and the consequent.

  No indication that the Tortoise is opposed to particular instance.
  Indeed, accepts each instance.
  However, does not accept the rule.

  If modus ponens, then generality.
  Hence, \gR{}.

  Whatever this amounts to, it extends beyond any specific instance.

  On this reading, what's strange is what \gR{0} amounts to.

  However, to the extent that the situation is puzzling, \gR{0}.
\end{note}

\begin{note}
  Here, the thing is, accepting an arbitrary number of instances of the conditional does not require one to accept all instances of the conditional.
\end{note}

\begin{note}
  Boghossian draws links to \citeauthor{Carroll:1895uj}.
  However, thinks its about justification.
  This is by no means clear.

  Rather, letter to the editor.

  Carrol seems to highlight the issue is general only if specific.

  Could read this as justification.
  However, point is, that these instances do not correspond to any move.

  If not sure about MP, then need to see that it works.
  However, no finite number of instances are sufficient to see that MP works.

  So, redo this, in which the Tortoise does a whole bunch of instances of MP.
  But, still does not do the one of interest.
\end{note}
