...
\begin{note}
  ...
  % In the former case, assumptions may be considered definitions which
  % However, assumptions often involve multiple terms, and hence it has been more straightforward to define the terms separately and patch the terms together via assumptions.
  In the latter case, we include some references to literature or a short summary of possible objections.

  In certain cases something useful (for the purposes of this document) follows from something already stated.
  As these things are useful we mark them as propositions and include an argument.

  In other cases something which may be helpful follows from something already stated.
  We term these observations and include motivation.
  Observations are not too important and may be ignored if you like.
\end{note}
...
\begin{note}
  ...
  The first section covers conclusions as \evalN{1} and the second covers \pool{1}.
\end{note}
...
\begin{note}
  Start with propositions and values.
  The \evalN{1}.
  with \evalN{1} in hand, conclusions.
\end{note}
...
\begin{note}
  ...
  \begin{notation}[Propositions and values]%
    \vspace{-\baselineskip}
    ...
    \item
      ...
      Likewise, \val{True} is the value `True'.
    \item
      ...
      \item
        ...
        (\(v, v'\), and \(v''\), etc., may all correspond to the same value.)
      \end{itemize}
    \end{itemize}
    \vspace{-\baselineskip}
  \end{notation}
  ...
  \agpe{Agent's}, the way in which things are for an agent.

  \begin{notation}[\evalN{3}]%
    \vspace{-\baselineskip}
    \begin{itemize}[leftmargin=*]
    \item
      \(\pv{\phi}{v}\) abbreviates an \evalN{0}.
...
\begin{note}
  ...
  By contrast, if the agent has been entertaining the idea that Rover is tired then the agent is entertaining the idea that the proposition \prop{Rover is tired} has the value \val{True}, but we do not consider the proposition and valid \evaled{0}.

  \phantlabel{mention:concluding-non-factive}
  Further, we allow for the possibility that some proposition is \evaled{0} with some value for the agent, though in reality things are otherwise.
  Hence, we do not require, for example, that an agent concludes \(\phi\) is \val{True} only if \(\phi\) is the case.
...
\begin{note}
  ...
  If you are inclined to the folk-psychological picture, then the benefit of speaking in terms of \evalN{1} is an easy way to talk about how things are for the agent, rather than recasting in terms of propositional attitudes the agent has.

  And, if you are not inclined to the folk-psychological picture then the sketched equivalence should suggest how to understand the way in which \evalN{1} are `fundamental'.
...
\begin{note}
  \begin{definition}[Conclusions as \evalN{1}]
    ...
    \begin{itemize}
    \item
      A \emph{conclusion} is an \evalN{} that has been made by an agent.
    \end{itemize}
    \vspace{-\baselineskip}
  \end{definition}

  For the purpose of this document conclusions are \evalN{} which have been made.
  Hence, a conclusion is the outcome of some event.

  However, before turning to events, we detail the role of \pool{1}.
\end{note}
...
\begin{note}
  ...
  \begin{definition}[\pool{3}]
    ...
    \item
      ...
      \begin{itemize}
      \item
        For every \(\pv{\phi}{v}\) in \pool{}, \vAgent{} \eval{1} \(\phi\) as having value \(v\).
      \end{itemize}
    \end{itemize}
    \vspace{-\baselineskip}
  \end{definition}
  ...
  \evalN{3}.
  \pool{} is a collection of \evalN{1}, and so captures the relevant stuff.
...
\begin{note}
  ...
  The event is not limited to the agent \evaling{} \(\phi\) by value \(v\), but includes how the agent comes to \eval{0} \(\phi\) by \(v\).

  \begin{definition}[Concludes is inclusive]
...
\begin{note}
  ...
  \ref{prog:abmig} captures something about an event, and as the verb is \textquote{studying}, it follows that studying is happening.
  For example, we may expand:
...
\begin{note}
  ...
  Some possible event:
  A agent is concluding \(\pv{\phi}{v}\) from \(\Phi\) only if there is some \progAdj{0} event in which the agent concludes \(\pv{\phi}{v}\) from \(\Phi\).
\end{note}
...
\begin{note}
  \eval{3} which have not yet been concluded.
  ...
  In this respect, an event of concluding need not involve the agent \evaling{} \(\phi\) with \(v\), it need only be the case that if the relevant event went on uninterrupted, then event would develop into an event in which the agent concludes \(\pv{\phi}{v}\) from \(\Phi\), and hence \eval{1} \(\phi\) by \(v\).
\end{note}
...
\begin{note}
  ...
  \begin{argument}{obs:cds-arb}
    ...
    \item
      ...
      For example, an agent looks outside, sees a bird in a tree and instantly concludes \prop{There is a bird in that tree} has value \val{True}.
    \item
